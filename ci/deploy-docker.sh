...
if [ -n "$DOCKER_PASSWORD" ]; then
    echo "$DOCKER_PASSWORD" | docker login -u ysureleaseteam --password-stdin
    ...
    if [[ "$TRAVIS_BRANCH" == "master" || "$TRAVIS_BRANCH" == "docker_cache" ]] && [[ "${TRAVIS_BUILD_STAGE_NAME}" =~ 'Build' ]]; then
        ci_image_name="ysucurrency/ysu-env:$TRAVIS_JOB_NAME"
    ...
    if [[ "$TRAVIS_BUILD_STAGE_NAME" == "Master_beta_docker" ]]; then
        ...
        "$scripts"/custom-timeout.sh 30 docker build --build-arg NETWORK=beta --build-arg CI_BUILD=true --build-arg TRAVIS_TAG="$TRAVIS_TAG" -f docker/node/Dockerfile -t ysucurrency/ysu-beta:master --cache-from ysucurrency/ysu-beta:master .
        "$scripts"/custom-timeout.sh 30 docker push ysucurrency/ysu-beta:master
        ...
        if [[ "$TRAVIS_JOB_NAME" =~ "live" ]]; then
            ...
            cached="--cache-from=ysucurrency/ysu-beta:master"
            docker pull ysucurrency/ysu-beta:master
        fi

        docker_image_name="ysucurrency/ysu${network_tag_suffix}"
