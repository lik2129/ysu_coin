...
if [[ "$OSTYPE" == "linux-gnu" ]]; then
    ...
    DISTRO_CFG="-DYSU_DISTRO_NAME=${distro}"
    ...
    CPACK_TYPE="TBZ2"
    DISTRO_CFG="-DYSU_DISTRO_NAME='freebsd'"
else
    CPACK_TYPE="TBZ2"
fi

if [[ ${SIMD} -eq 1 ]]; then
    SIMD_CFG="-DYSU_SIMD_OPTIMIZATIONS=ON"
...
if [[ "${BETA}" -eq 1 ]]; then
    NETWORK_CFG="-DACTIVE_NETWORK=ysu_beta_network"
    CONFIGURATION="RelWithDebInfo"
else
    NETWORK_CFG="-DACTIVE_NETWORK=ysu_live_network"
