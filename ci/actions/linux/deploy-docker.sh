...
if [ -n "$DOCKER_PASSWORD" ]; then
    ...
    if [[ "$GITHUB_WORKFLOW" = "Develop" ]]; then
        "$scripts"/custom-timeout.sh 30 docker push "ysucurrency/ysu-env:base"
        "$scripts"/custom-timeout.sh 30 docker push "ysucurrency/ysu-env:gcc"
        "$scripts"/custom-timeout.sh 30 docker push "ysucurrency/ysu-env:clang-6"
        echo "Deployed ysu-env"
        exit 0
    else
        tags=()
