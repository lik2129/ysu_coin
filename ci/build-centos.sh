...
run_build() {
    ...
    mv -f ~/ysu-${VERSIONS}.tar.gz ~/rpmbuild/SOURCES/.
    scl enable devtoolset-7 'rpmbuild -ba ysucurrency.spec'
    scl enable devtoolset-7 'rpmbuild -ba ysucurrency-beta.spec'
}

run_update() {
    for file in ./ysucurrency*.in; do
