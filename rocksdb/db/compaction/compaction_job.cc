...
Status CompactionJob::Install(const MutableCFOptions& mutable_cf_options) {
  ...
  if (measure_io_stats_ && compaction_job_stats_ != nullptr) {
    stream << "file_write_ysus" << compaction_job_stats_->file_write_ysus;
    stream << "file_range_sync_ysus"
           << compaction_job_stats_->file_range_sync_ysus;
    stream << "file_fsync_ysus" << compaction_job_stats_->file_fsync_ysus;
    stream << "file_prepare_write_ysus"
           << compaction_job_stats_->file_prepare_write_ysus;
  }
...
void CompactionJob::ProcessKeyValueCompaction(SubcompactionState* sub_compact) {
  ...
  uint64_t prev_cpu_micros = env_->NowCPUYsus() / 1000;
  ...
  uint64_t prev_write_ysus = 0;
  uint64_t prev_fsync_ysus = 0;
  uint64_t prev_range_sync_ysus = 0;
  uint64_t prev_prepare_write_ysus = 0;
  uint64_t prev_cpu_write_ysus = 0;
  uint64_t prev_cpu_read_ysus = 0;
  if (measure_io_stats_) {
    prev_perf_level = GetPerfLevel();
    SetPerfLevel(PerfLevel::kEnableTimeAndCPUTimeExceptForMutex);
    prev_write_ysus = IOSTATS(write_ysus);
    prev_fsync_ysus = IOSTATS(fsync_ysus);
    prev_range_sync_ysus = IOSTATS(range_sync_ysus);
    prev_prepare_write_ysus = IOSTATS(prepare_write_ysus);
    prev_cpu_write_ysus = IOSTATS(cpu_write_ysus);
    prev_cpu_read_ysus = IOSTATS(cpu_read_ysus);
  }
  ...
  sub_compact->compaction_job_stats.cpu_micros =
      env_->NowCPUYsus() / 1000 - prev_cpu_micros;

  if (measure_io_stats_) {
    sub_compact->compaction_job_stats.file_write_ysus +=
        IOSTATS(write_ysus) - prev_write_ysus;
    sub_compact->compaction_job_stats.file_fsync_ysus +=
        IOSTATS(fsync_ysus) - prev_fsync_ysus;
    sub_compact->compaction_job_stats.file_range_sync_ysus +=
        IOSTATS(range_sync_ysus) - prev_range_sync_ysus;
    sub_compact->compaction_job_stats.file_prepare_write_ysus +=
        IOSTATS(prepare_write_ysus) - prev_prepare_write_ysus;
    sub_compact->compaction_job_stats.cpu_micros -=
        (IOSTATS(cpu_write_ysus) - prev_cpu_write_ysus +
         IOSTATS(cpu_read_ysus) - prev_cpu_read_ysus) /
        1000;
