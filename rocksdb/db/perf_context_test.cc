...
TEST_F(PerfContextTest, SeekIntoDeletion) {
  ...
  for (int i = 0; i < FLAGS_total_keys - 1; ++i) {
    ...
    StopWatchYsu timer(Env::Default());
    timer.Start();
    auto status = db->Get(read_options, key, &value);
    auto elapsed_ysus = timer.ElapsedYsus();
    ASSERT_TRUE(status.IsNotFound());
    hist_get.Add(get_perf_context()->user_key_comparison_count);
    hist_get_time.Add(elapsed_ysus);
  }
...

  {
    ...
    get_perf_context()->Reset();
    StopWatchYsu timer(Env::Default(), true);
    iter->SeekToFirst();
  ...
  for (int i = 0; i < FLAGS_total_keys; ++i) {
    ...
    if (FLAGS_verbose) {
      std::cout << "seek cmp: " << get_perf_context()->user_key_comparison_count
                ...
                << " idelete skipped "
                << get_perf_context()->internal_delete_skipped_count
                << " elapsed: " << elapsed_ysus << "ns\n";
    }

    get_perf_context()->Reset();
    ASSERT_TRUE(iter->Valid());
    StopWatchYsu timer2(Env::Default(), true);
    iter->Next();
    auto elapsed_ysus2 = timer2.ElapsedYsus();
    if (FLAGS_verbose) {
      std::cout << "next cmp: " << get_perf_context()->user_key_comparison_count
                << "elapsed: " << elapsed_ysus2 << "ns\n";
    }
  }
...
TEST_F(PerfContextTest, StopWatchYsuOverhead) {
  ...
  for (auto& timing : timings) {
    timing = timer.ElapsedYsus(true /* reset */);
  }
...
void ProfileQueries(bool enabled_time = false) {
  ...
  HistogramImpl hist_write_thread_wait_ysus;
  HistogramImpl hist_write_scheduling_time;

  uint64_t total_db_mutex_ysus = 0;
  ...
  for (const int i : keys) {
    ...
    hist_write_thread_wait_ysus.Add(
        get_perf_context()->write_thread_wait_ysus);
    ...
    total_db_mutex_ysus += get_perf_context()->db_mutex_lock_ysus;
  }
#ifndef NDEBUG
  ThreadStatusUtil::TEST_SetStateDelay(ThreadStatus::STATE_MUTEX_WAIT, 0U);
#endif
  ...
  if (FLAGS_verbose) {
    ...
    std::cout << "Put(): Pre and Post Process Time: \n"
              ...
              << hist_write_thread_wait_ysus.ToString() << "\n"
  ...
  if (enabled_time) {
    ...
    EXPECT_EQ(hist_write_thread_wait_ysus.Average(), 0);
    EXPECT_GT(hist_write_scheduling_time.Average(), 0);

#ifndef NDEBUG
    ASSERT_GT(total_db_mutex_ysus, 2000U);
#endif
  }
...
TEST_F(PerfContextTest, SeekKeyComparison) {
  ...
  for (const int i : keys) {
    ...
    timer.Start();
    db->Put(write_options, key, value);
    auto put_time = timer.ElapsedYsus();
...
TEST_F(PerfContextTest, DBMutexLockCounter) {
  ...
  for (PerfLevel perf_level_test :
    ...
    ROCKSDB_NAMESPACE::port::Thread child_thread([&] {
      ...
      get_perf_context()->Reset();
      ASSERT_EQ(get_perf_context()->db_mutex_lock_ysus, 0);
      mutex.Lock();
      mutex.Unlock();
      if (perf_level_test == PerfLevel::kEnableTimeExceptForMutex ||
        ...
        ASSERT_GT(get_perf_context()->db_mutex_lock_ysus, 0);
      }
    });
...
TEST_F(PerfContextTest, FalseDBMutexWait) {
  ...
  for (int c = 0; c < 2; ++c) {
    ...
    if (stats_code[c] == static_cast<int>(DB_MUTEX_WAIT_MICROS)) {
      // increment the counter only when it's a DB Mutex
      ASSERT_GT(get_perf_context()->db_condition_wait_ysus, 0);
    } else {
      ASSERT_EQ(get_perf_context()->db_condition_wait_ysus, 0);
    }
  }
}
...
#endif
  EXPECT_GT(get_perf_context()->merge_operator_time_ysus, 0);
...
TEST_F(PerfContextTest, CPUTimer) {
...

  {
    ...
    if (FLAGS_verbose) {
      std::cout << "Get CPU time ysus: " << get_perf_context()->get_cpu_ysus
                << "ns\n";
    }
    ...
    if (FLAGS_verbose) {
      std::cout << "Iter Seek CPU time ysus: "
                << get_perf_context()->iter_seek_cpu_ysus << "ns\n";
    }
    ...
    if (FLAGS_verbose) {
      std::cout << "Iter SeekForPrev CPU time ysus: "
                << get_perf_context()->iter_seek_cpu_ysus << "ns\n";
    }
    ...
    if (FLAGS_verbose) {
      std::cout << "Iter SeekToLast CPU time ysus: "
                << get_perf_context()->iter_seek_cpu_ysus << "ns\n";
    }
    ...
    if (FLAGS_verbose) {
      std::cout << "Iter SeekToFirst CPU time ysus: "
                << get_perf_context()->iter_seek_cpu_ysus << "ns\n";
    }
    ...
    if (FLAGS_verbose) {
      std::cout << "Iter Next CPU time ysus: "
                << get_perf_context()->iter_next_cpu_ysus << "ns\n";
    }
    ...
    if (FLAGS_verbose) {
      std::cout << "Iter Prev CPU time ysus: "
                << get_perf_context()->iter_prev_cpu_ysus << "ns\n";
    }

    // monotonically increasing
    get_perf_context()->Reset();
    auto count = get_perf_context()->iter_seek_cpu_ysus;
    for (int i = 0; i < FLAGS_total_keys; ++i) {
      iter->Seek("k" + ToString(i));
      ASSERT_TRUE(iter->Valid());
      ASSERT_EQ("v" + ToString(i), iter->value().ToString());
      auto next_count = get_perf_context()->iter_seek_cpu_ysus;
    ...
    // iterator creation/destruction; multiple iterators
    {
      ...
      ASSERT_EQ(count, get_perf_context()->iter_seek_cpu_ysus);
      iter2->Seek(last_key);
      ASSERT_TRUE(iter2->Valid());
      ASSERT_EQ(last_value, iter2->value().ToString());
      ASSERT_GT(get_perf_context()->iter_seek_cpu_ysus, count);
      count = get_perf_context()->iter_seek_cpu_ysus;
    }
    ASSERT_EQ(count, get_perf_context()->iter_seek_cpu_ysus);
  }
}
