...
#ifdef OS_LINUX
// Make sure that in CPU time perf context counters, Env::NowCPUYsus()
// is used, rather than Env::CPUYsus();
TEST_F(DBTest2, TestPerfContextGetCpuTime) {
  // force resizing table cache so table handle is not preloaded so that
  // we can measure find_table_ysus during Get().
  ...
  ASSERT_LT(get_perf_context()->get_cpu_ysus, kDummyAddonTime);
  ASSERT_GT(get_perf_context()->find_table_ysus, kDummyAddonTime);
...
TEST_F(DBTest2, TestPerfContextIterCpuTime) {
  ...
  // force resizing table cache so table handle is not preloaded so that
  // we can measure find_table_ysus during iteration
  dbfull()->TEST_table_cache()->SetCapacity(0);
  ...
  ASSERT_TRUE(iter->Valid());
  ASSERT_EQ("v0", iter->value().ToString());
  ASSERT_EQ(0, get_perf_context()->iter_seek_cpu_ysus);
  iter->Next();
  ASSERT_TRUE(iter->Valid());
  ASSERT_EQ("v1", iter->value().ToString());
  ASSERT_EQ(0, get_perf_context()->iter_next_cpu_ysus);
  iter->Prev();
  ASSERT_TRUE(iter->Valid());
  ASSERT_EQ("v0", iter->value().ToString());
  ASSERT_EQ(0, get_perf_context()->iter_prev_cpu_ysus);
  ASSERT_EQ(0, env_->now_cpu_count_.load());
  delete iter;
  ...
  ASSERT_TRUE(iter->Valid());
  ASSERT_EQ("v0", iter->value().ToString());
  ASSERT_GT(get_perf_context()->iter_seek_cpu_ysus, 0);
  ASSERT_LT(get_perf_context()->iter_seek_cpu_ysus, kDummyAddonTime);
  iter->Next();
  ASSERT_TRUE(iter->Valid());
  ASSERT_EQ("v1", iter->value().ToString());
  ASSERT_GT(get_perf_context()->iter_next_cpu_ysus, 0);
  ASSERT_LT(get_perf_context()->iter_next_cpu_ysus, kDummyAddonTime);
  iter->Prev();
  ASSERT_TRUE(iter->Valid());
  ASSERT_EQ("v0", iter->value().ToString());
  ASSERT_GT(get_perf_context()->iter_prev_cpu_ysus, 0);
  ASSERT_LT(get_perf_context()->iter_prev_cpu_ysus, kDummyAddonTime);
