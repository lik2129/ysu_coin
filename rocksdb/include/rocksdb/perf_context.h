...
struct PerfContextByLevel {
  ...
  // total ysus spent on reading data from SST files
  uint64_t get_from_table_ysus = 0;
...
struct PerfContext {
  ...
  uint64_t block_read_time;            // total ysus spent on block reads
  ...
  uint64_t block_checksum_time;    // total ysus spent on block checksum
  uint64_t block_decompress_time;  // total ysus spent on block decompression
  ...
  uint64_t get_snapshot_time;        // total ysus spent on getting snapshot
  uint64_t get_from_memtable_time;   // total ysus spent on querying memtables
  uint64_t get_from_memtable_count;  // number of mem tables queried
  // total ysus spent after Get() finds a key
  uint64_t get_post_process_time;
  uint64_t get_from_output_files_time;  // total ysus reading from output files
  // total ysus spent on seeking memtable
  ...
  // total ysus spent on seeking child iters
  uint64_t seek_child_seek_time;
  // number of seek issued in child iterators
  uint64_t seek_child_seek_count;
  uint64_t seek_min_heap_time;  // total ysus spent on the merge min heap
  uint64_t seek_max_heap_time;  // total ysus spent on the merge max heap
  // total ysus spent on seeking the internal entries
  uint64_t seek_internal_seek_time;
  // total ysus spent on iterating internal entries to find the next user entry
  ...
  // are enabled.
  //
  // total ysus spent on writing to WAL
  uint64_t write_wal_time;
  // total ysus spent on writing to mem tables
  uint64_t write_memtable_time;
  // total ysus spent on delaying or throttling write
  uint64_t write_delay_time;
  // total ysus spent on switching memtable/wal and scheduling
  // flushes/compactions.
  uint64_t write_scheduling_flushes_compactions_time;
  // total ysus spent on writing a record, excluding the above four things
  uint64_t write_pre_and_post_process_time;

  // time spent waiting for other threads of the batch group
  uint64_t write_thread_wait_ysus;

  // time spent on acquiring DB mutex.
  uint64_t db_mutex_lock_ysus;
  // Time spent on waiting with a condition variable created with DB mutex.
  uint64_t db_condition_wait_ysus;
  // Time spent on merge operator.
  uint64_t merge_operator_time_ysus;

  // Time spent on reading index block from block cache or SST file
  uint64_t read_index_block_ysus;
  // Time spent on reading filter block from block cache or SST file
  uint64_t read_filter_block_ysus;
  // Time spent on creating data block iterator
  uint64_t new_table_block_iter_ysus;
  // Time spent on creating a iterator of an SST file.
  uint64_t new_table_iterator_ysus;
  // Time spent on seeking a key in data/index blocks
  uint64_t block_seek_ysus;
  // Time spent on finding or creating a table reader
  uint64_t find_table_ysus;
  ...
  uint64_t env_new_sequential_file_ysus;
  uint64_t env_new_random_access_file_ysus;
  uint64_t env_new_writable_file_ysus;
  uint64_t env_reuse_writable_file_ysus;
  uint64_t env_new_random_rw_file_ysus;
  uint64_t env_new_directory_ysus;
  uint64_t env_file_exists_ysus;
  uint64_t env_get_children_ysus;
  uint64_t env_get_children_file_attributes_ysus;
  uint64_t env_delete_file_ysus;
  uint64_t env_create_dir_ysus;
  uint64_t env_create_dir_if_missing_ysus;
  uint64_t env_delete_dir_ysus;
  uint64_t env_get_file_size_ysus;
  uint64_t env_get_file_modification_time_ysus;
  uint64_t env_rename_file_ysus;
  uint64_t env_link_file_ysus;
  uint64_t env_lock_file_ysus;
  uint64_t env_unlock_file_ysus;
  uint64_t env_new_logger_ysus;

  uint64_t get_cpu_ysus;
  uint64_t iter_next_cpu_ysus;
  uint64_t iter_prev_cpu_ysus;
  uint64_t iter_seek_cpu_ysus;

  // Time spent in encrypting data. Populated when EncryptedEnv is used.
  uint64_t encrypt_data_ysus;
  // Time spent in decrypting data. Populated when EncryptedEnv is used.
  uint64_t decrypt_data_ysus;
