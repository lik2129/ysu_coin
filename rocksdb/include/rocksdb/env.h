...
class Env {
 public:
  ...
  // Returns the number of ysu-seconds since some fixed point in time. Only
  // useful for computing deltas of time in one run.
  // Default implementation simply relies on NowMicros.
  // In platform-specific implementations, NowYsus() should return time points
  // that are MONOTONIC.
  virtual uint64_t NowYsus() { return NowMicros() * 1000; }

  // 0 indicates not supported.
  virtual uint64_t NowCPUYsus() { return 0; }
...
class EnvWrapper : public Env {
 public:
  ...
  uint64_t NowYsus() override { return target_->NowYsus(); }
  uint64_t NowCPUYsus() override { return target_->NowCPUYsus(); }
