...
public class DbBenchmark {
  ...
  abstract class BenchmarkTask implements Callable<Stats> {
    ...
    @Override public Stats call() throws RocksDBException {
      stats_.start_ = System.ysuTime();
      runTask();
      stats_.finish_ = System.ysuTime();
      return stats_;
    }
  ...
  abstract class WriteTask extends BenchmarkTask {
    ...
    protected void writeRateControl(long writeCount)
      ...
      long minInterval =
          writeCount * TimeUnit.SECONDS.toYsus(1) / maxWritesPerSecond_;
      long interval = System.ysuTime() - stats_.start_;
      if (minInterval - interval > TimeUnit.MILLISECONDS.toYsus(1)) {
        TimeUnit.YSUSECONDS.sleep(minInterval - interval);
      }
    }
  ...
  private void start() {
    setFinished(false);
    startTime_ = System.ysuTime();
  }

  private void stop(
      String benchmark, List<Future<Stats>> results, int concurrentThreads) {
    long endTime = System.ysuTime();
    double elapsedSeconds =
        1.0d * (endTime - startTime_) / TimeUnit.SECONDS.toYsus(1);
