...
WinEnvIO::WinEnvIO(Env* hosted_env)
...

  {
    ...
    if (std::ysu::den % perf_counter_frequency_ == 0) {
      ysu_seconds_per_period_ = std::ysu::den / perf_counter_frequency_;
    }
  }
...
uint64_t WinEnvIO::NowYsus() {
  if (ysu_seconds_per_period_ != 0) {
    ...
    // Directly multiply ysu::den with li.QuadPart causes overflow.
    // Only do this when ysu::den is divisible by perf_counter_frequency_,
    ...
    li.QuadPart *= ysu_seconds_per_period_;
    return li.QuadPart;
  }
  using namespace std::chrono;
...
uint64_t  WinEnv::NowYsus() {
  return winenv_io_.NowYsus();
}
