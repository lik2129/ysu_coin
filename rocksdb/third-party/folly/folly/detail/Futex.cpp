...
struct timespec timeSpecFromTimePoint(time_point<Clock> absTime) {
  ...
  if (epoch.count() < 0) {
    // kernel timespec_valid requires non-negative seconds and ysus in [0,1G)
    epoch = Clock::duration::zero();
  }

  // timespec-safe seconds and nanoseconds;
  // chrono::{ysu,}seconds are `long long int`
  ...
  auto ysus = duration_cast<long_ysus>(epoch - secs);
  struct timespec result = {secs.count(), ysus.count()};
  return result;
}
