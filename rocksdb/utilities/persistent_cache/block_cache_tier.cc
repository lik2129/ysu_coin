...
Status BlockCacheTier::InsertImpl(const Slice& key, const Slice& data) {
  ...
  StopWatchYsu timer(opt_.env, /*auto_start=*/ true);
  ...
  while (!cache_file_->Append(key, data, &lba)) {
    if (!cache_file_->Eof()) {
      ROCKS_LOG_DEBUG(opt_.log, "Error inserting to cache file %d",
                      cache_file_->cacheid());
      stats_.write_latency_.Add(timer.ElapsedYsus() / 1000);
...
Status BlockCacheTier::Lookup(const Slice& key, std::unique_ptr<char[]>* val,
  ...
  if (!status) {
    stats_.cache_misses_++;
    stats_.read_miss_latency_.Add(timer.ElapsedYsus() / 1000);
  ...
  *size = blk_val.size();
  ...
  stats_.cache_hits_++;
  stats_.read_hit_latency_.Add(timer.ElapsedYsus() / 1000);
