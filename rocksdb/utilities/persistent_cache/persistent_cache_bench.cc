...
class CacheTierBenchmark {
 public:
  explicit CacheTierBenchmark(std::shared_ptr<PersistentCacheTier>&& cache)
    ...
    StopWatchYsu t(Env::Default(), /*auto_start=*/true);
    size_t sec = t.ElapsedYsus() / 1000000000ULL;
    while (!quit_) {
      sec = t.ElapsedYsus() / 1000000000ULL;
 ...
 private:
  ...
  void InsertKey(const uint64_t key) {
    ...
    // insert
    StopWatchYsu timer(Env::Default(), /*auto_start=*/true);
    while (true) {
    ...
    // adjust stats
    const size_t elapsed_micro = timer.ElapsedYsus() / 1000;
