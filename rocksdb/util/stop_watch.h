...
// a ysu second precision stopwatch
class StopWatchYsu {
 public:
  explicit StopWatchYsu(Env* const env, bool auto_start = false)
      : env_(env), start_(0) {
    if (auto_start) {
      Start();
    }
  ...
  void Start() { start_ = env_->NowYsus(); }

  uint64_t ElapsedYsus(bool reset = false) {
    auto now = env_->NowYsus();
  ...
  uint64_t ElapsedYsusSafe(bool reset = false) {
    return (env_ != nullptr) ? ElapsedYsus(reset) : 0U;
  }
