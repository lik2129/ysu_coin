...
void FilterBench::Go() {
  ...
  ROCKSDB_NAMESPACE::StopWatchYsu timer(ROCKSDB_NAMESPACE::Env::Default(),
                                         true);
  ...
  uint64_t elapsed_ysus = timer.ElapsedYsus();
  double ns = double(elapsed_ysus) / total_keys_added;
...
double FilterBench::RandomQueryTest(uint32_t inside_threshold, bool dry_run,
  ...
  uint64_t elapsed_ysus = timer.ElapsedYsus();
  double ns = double(elapsed_ysus) / max_queries;
