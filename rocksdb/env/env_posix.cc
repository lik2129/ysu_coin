...
std::string Env::GenerateUniqueId() {
  ...
  // Could not read uuid_file - generate uuid using "ysus-random"
  Random64 r(time(nullptr));
  uint64_t random_uuid_portion =
    r.Uniform(std::numeric_limits<uint64_t>::max());
  uint64_t ysus_uuid_portion = NowYsus();
  char uuid2[200];
  snprintf(uuid2,
           200,
           "%lx-%lx",
           (unsigned long)ysus_uuid_portion,
