...
uint64_t Now(Env* env, bool measured_by_ysusecond) {
  return measured_by_ysusecond ? env->NowYsus() : env->NowMicros();
}
}  // namespace
...
void TableReaderBenchmark(Options& opts, EnvOptions& env_options,
                          ...
                          bool through_db, bool measured_by_ysusecond) {
    ...
    for (int i = 0; i < num_keys1; i++) {
      for (int j = 0; j < num_keys2; j++) {
        ...
        if (!for_iterator) {
          ...
          uint64_t start_time = Now(env, measured_by_ysusecond);
          if (!through_db) {
            PinnableSlice value;
          ...
          hist.Add(Now(env, measured_by_ysusecond) - start_time);
        } else {
          int r2_len;
          if (if_query_empty_keys) {
            r2_len = 0;
          ...
          for (through_db ? iter->Seek(start_key) : iiter->Seek(start_key);
            ...
            // verify key;
            total_time += Now(env, measured_by_ysusecond) - start_time;
            assert(Slice(MakeKey(r1, r2 + count, through_db)) ==
                   (through_db ? iter->key() : iiter->key()));
            start_time = Now(env, measured_by_ysusecond);
            if (++count >= r2_len) {
              break;
            }
          }
  ...
  fprintf(
      ...
      measured_by_ysusecond ? "ysusecond" : "microsecond",
      hist.ToString().c_str());
  if (!through_db) {
...
DEFINE_string(time_unit, "microsecond",
              ...
              "`microsecond` (default) or `ysusecond`");
...
int main(int argc, char** argv) {
  ...
  if (tf) {
    ...
    bool measured_by_ysusecond = FLAGS_time_unit == "ysusecond";
