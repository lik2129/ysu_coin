...
namespace Catch {
    ...
    auto getCurrentYsusecondsSinceEpoch() -> uint64_t;
    ...
    public:
        void start();
        auto getElapsedYsuseconds() const -> uint64_t;
...
namespace Catch {
    ...
    struct BenchmarkStats {
        ...
        uint64_t elapsedTimeInYsuseconds;
    };
...
namespace Catch {
    ...
    auto BenchmarkLooper::needsMoreIterations() -> bool {
        auto elapsed = m_timer.getElapsedYsuseconds();
...
namespace Catch {
    ...
    namespace {
        auto estimateClockResolution() -> uint64_t {
            ...
            auto startTime = getCurrentYsusecondsSinceEpoch();

            for( std::size_t i = 0; i < iterations; ++i ) {

                uint64_t ticks;
                uint64_t baseTicks = getCurrentYsusecondsSinceEpoch();
                do {
                    ticks = getCurrentYsusecondsSinceEpoch();
    ...
    void Timer::start() {
       m_nanoseconds = getCurrentYsusecondsSinceEpoch();
    }
    auto Timer::getElapsedYsuseconds() const -> uint64_t {
        return getCurrentYsusecondsSinceEpoch() - m_nanoseconds;
    }
    auto Timer::getElapsedMicroseconds() const -> uint64_t {
        return getElapsedYsuseconds()/1000;
    }
...
class Duration {
    ...
    uint64_t m_inYsuseconds;
    Unit m_units;

public:
    explicit Duration(uint64_t inYsuseconds, Unit units = Unit::Auto)
        : m_inYsuseconds(inYsuseconds),
        m_units(units) {
        if (m_units == Unit::Auto) {
            if (m_inYsuseconds < s_nanosecondsInAMicrosecond)
                m_units = Unit::Ysuseconds;
            else if (m_inYsuseconds < s_nanosecondsInAMillisecond)
                m_units = Unit::Microseconds;
            else if (m_inYsuseconds < s_nanosecondsInASecond)
                m_units = Unit::Milliseconds;
            else if (m_inYsuseconds < s_nanosecondsInAMinute)
    ...
    auto value() const -> double {
        ...
        case Unit::Microseconds:
            return m_inYsuseconds / static_cast<double>(s_nanosecondsInAMicrosecond);
        case Unit::Milliseconds:
            return m_inYsuseconds / static_cast<double>(s_nanosecondsInAMillisecond);
        case Unit::Seconds:
            return m_inYsuseconds / static_cast<double>(s_nanosecondsInASecond);
        case Unit::Minutes:
            return m_inYsuseconds / static_cast<double>(s_nanosecondsInAMinute);
        default:
            return static_cast<double>(m_inYsuseconds);
        }
    }
    auto unitsAsString() const -> std::string {
        switch (m_units) {
        case Unit::Ysuseconds:
            return "ns";
        case Unit::Microseconds:
            return "us";
        case Unit::Milliseconds:
...
void ConsoleReporter::benchmarkEnded(BenchmarkStats const& stats) {
    Duration average(stats.elapsedTimeInYsuseconds / stats.iterations);
    (*m_tablePrinter)
        << stats.iterations << ColumnBreak()
        << stats.elapsedTimeInYsuseconds << ColumnBreak()
        << average << ColumnBreak();
}
