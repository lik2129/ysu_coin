...
REGISTER_ERROR_CODES (ysu, error_common);
REGISTER_ERROR_CODES (ysu, error_blocks);
REGISTER_ERROR_CODES (ysu, error_rpc);
REGISTER_ERROR_CODES (ysu, error_process);
REGISTER_ERROR_CODES (ysu, error_config);
...
public:
	error () = default;
	error (ysu::error const & error_a) = default;
	error (ysu::error && error_a) = default;
	...
	error & operator= (ysu::error const & err_a);
	error & operator= (ysu::error && err_a);
	...
	error & then (std::function<ysu::error &()> next);
	...
	error & set (std::string message_a, std::error_code code_a = ysu::error_common::generic);
...
/**
 * A type that manages a ysu::error.
 * The default return type is ysu::error&, though shared_ptr<ysu::error> is a good option in cases
 * where shared error state is desirable.
 */
template <typename RET_TYPE = ysu::error &>
class error_aware
{
	static_assert (std::is_same<RET_TYPE, ysu::error &>::value || std::is_same<RET_TYPE, std::shared_ptr<ysu::error>>::value, "Must be ysu::error& or shared_ptr<ysu::error>");
