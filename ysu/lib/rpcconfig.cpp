...
ysu::error ysu::rpc_secure_config::serialize_json (ysu::jsonconfig & json) const
{
...
ysu::error ysu::rpc_secure_config::deserialize_json (ysu::jsonconfig & json)
{
...
ysu::error ysu::rpc_secure_config::serialize_toml (ysu::tomlconfig & toml) const
{
...
ysu::error ysu::rpc_secure_config::deserialize_toml (ysu::tomlconfig & toml)
{
...
ysu::rpc_config::rpc_config (uint16_t port_a, bool enable_control_a) :
address (boost::asio::ip::address_v6::loopback ().to_string ()),
port (port_a),
...
ysu::error ysu::rpc_config::serialize_json (ysu::jsonconfig & json) const
{
...
ysu::error ysu::rpc_config::deserialize_json (bool & upgraded_a, ysu::jsonconfig & json)
{
	if (!json.empty ())
	{
...
ysu::error ysu::rpc_config::serialize_toml (ysu::tomlconfig & toml) const
{
...
ysu::rpc_process_config::rpc_process_config () :
...
namespace ysu
{
ysu::error read_rpc_config_toml (boost::filesystem::path const & data_path_a, ysu::rpc_config & config_a, std::vector<std::string> const & config_overrides)
{
	ysu::error error;
	auto json_config_path = ysu::get_rpc_config_path (data_path_a);
	auto toml_config_path = ysu::get_rpc_toml_config_path (data_path_a);
	if (boost::filesystem::exists (json_config_path))
	{
		...
		else
		{
			...
			if (!error)
			{
				...
				ysu::rpc_config config_defaults_l;
...
ysu::error read_and_update_rpc_config (boost::filesystem::path const & data_path, ysu::rpc_config & config_a)
{
	boost::system::error_code error_chmod;
	ysu::jsonconfig json;
	auto config_path = ysu::get_rpc_config_path (data_path);
...
std::string get_default_rpc_filepath ()
{
	...
	// Construct the ysu_rpc executable file path based on where the currently running executable is found.
	auto rpc_filepath = running_executable_filepath.parent_path () / "ysu_rpc";
