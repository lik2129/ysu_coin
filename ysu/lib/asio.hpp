...
#include <ysu/boost/asio/write.hpp>
...
async_write (AsyncWriteStream & s, ysu::shared_const_buffer const & buffer, WriteHandler && handler)
{
	return boost::asio::async_write (s, buffer, std::move (handler));
}

/**
 * Alternative to ysu::async_write where scatter/gather is desired for best performance, and where
