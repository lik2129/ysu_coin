...
void output (const char * str, std::chrono::milliseconds time, Mutex & mutex)
{
	...
	auto stacktrace = ysu::generate_stacktrace ();
...
void output_if_held_long_enough (ysu::timer<std::chrono::milliseconds> & timer, Mutex & mutex)
{
	...
	if (time_held >= std::chrono::milliseconds (YSU_TIMED_LOCKS))
	{
		output ("held", time_held, mutex);
	}
	if (timer.current_state () != ysu::timer_state::stopped)
	{
		timer.stop ();
	}
}

#ifndef YSU_TIMED_LOCKS_IGNORE_BLOCKED
template <typename Mutex>
void output_if_blocked_long_enough (ysu::timer<std::chrono::milliseconds> & timer, Mutex & mutex)
{
	...
	if (time_blocked >= std::chrono::milliseconds (YSU_TIMED_LOCKS))
	{
...
template void output_if_held_long_enough (ysu::timer<std::chrono::milliseconds> & timer, std::mutex & mutex);
#ifndef YSU_TIMED_LOCKS_IGNORE_BLOCKED
template void output_if_blocked_long_enough (ysu::timer<std::chrono::milliseconds> & timer, std::mutex & mutex);
#endif
...
void condition_variable::wait (ysu::unique_lock<std::mutex> & lk)
{
	if (!lk.mut || !lk.owns)
	{
		throw (std::system_error (std::make_error_code (std::errc::operation_not_permitted)));
