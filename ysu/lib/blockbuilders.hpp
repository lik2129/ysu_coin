...
inline ysu::build_flags operator| (ysu::build_flags a, ysu::build_flags b)
{
	return static_cast<ysu::build_flags> (static_cast<uint8_t> (a) | static_cast<uint8_t> (b));
}
inline uint8_t operator| (uint8_t a, ysu::build_flags b)
{
...
public:
	...
	abstract_builder & sign (ysu::raw_key const & private_key, ysu::public_key const & public_key);
...
protected:
	...
	uint8_t base_fields = static_cast<uint8_t> (ysu::build_flags::work_present | ysu::build_flags::signature_present);
};

/** Builder for state blocks */
class state_block_builder : public abstract_builder<ysu::state_block, state_block_builder>
{
public:
	/** Creates a state block builder by calling make_block() */
	state_block_builder ();
	/** Initialize from an existing block */
	state_block_builder & from (ysu::state_block const & block);
	...
	/** Set link */
	state_block_builder & link (ysu::link const & link);
	/** Set link from hex representation */
	state_block_builder & link_hex (std::string const & link_hex);
	/** Set link from an xrb_ or ysu_ address */
	state_block_builder & link_address (std::string const & link_address);
	/** Provides validation for build() */
	void validate ();

private:
	uint8_t required_fields = base_fields | static_cast<uint8_t> (ysu::build_flags::account_present | ysu::build_flags::balance_present | ysu::build_flags::link_present | ysu::build_flags::previous_present | ysu::build_flags::representative_present);
};

/** Builder for open blocks */
class open_block_builder : public abstract_builder<ysu::open_block, open_block_builder>
{
public:
	...
	open_block_builder & source (ysu::block_hash source);
...
private:
	uint8_t required_fields = base_fields | static_cast<uint8_t> (ysu::build_flags::account_present | ysu::build_flags::representative_present | ysu::build_flags::link_present);
};

/** Builder for change blocks */
class change_block_builder : public abstract_builder<ysu::change_block, change_block_builder>
{
public:
	...
	/** Set representative */
	change_block_builder & representative (ysu::account account);
	/** Set representative from hex representation of public key */
	...
	/** Set previous block hash */
	change_block_builder & previous (ysu::block_hash previous);
	/** Set previous block hash from hex representation */
...
private:
	uint8_t required_fields = base_fields | static_cast<uint8_t> (ysu::build_flags::previous_present | ysu::build_flags::representative_present);
};

/** Builder for send blocks */
class send_block_builder : public abstract_builder<ysu::send_block, send_block_builder>
{
public:
...
class receive_block_builder : public abstract_builder<ysu::receive_block, receive_block_builder>
{
public:
	...
	/** Set previous block hash */
	receive_block_builder & previous (ysu::block_hash previous);
	/** Set previous block hash from hex representation */
	receive_block_builder & previous_hex (std::string previous_hex);
	/** Set source block hash */
	receive_block_builder & source (ysu::block_hash source);
	/** Set source block hash from hex representation */
...
public:
	...
	ysu::change_block_builder & change ()
	{
	...
	ysu::receive_block_builder & receive ()
	{
...
private:
	ysu::state_block_builder state_builder;
	ysu::open_block_builder open_builder;
	ysu::change_block_builder change_builder;
	ysu::send_block_builder send_builder;
	ysu::receive_block_builder receive_builder;
};
}
