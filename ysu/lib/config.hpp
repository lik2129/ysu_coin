...
*/
const char * const YSU_VERSION_STRING = xstr (TAG_VERSION_STRING);
const char * const YSU_MAJOR_VERSION_STRING = xstr (MAJOR_VERSION_STRING);
const char * const YSU_MINOR_VERSION_STRING = xstr (MINOR_VERSION_STRING);
const char * const YSU_PATCH_VERSION_STRING = xstr (PATCH_VERSION_STRING);
const char * const YSU_PRE_RELEASE_VERSION_STRING = xstr (PRE_RELEASE_VERSION_STRING);
...
struct work_thresholds
{
	...
	work_thresholds operator= (ysu::work_thresholds const & other_a)
	{
		return other_a;
	}
};
...
public:
	...
	network_constants (ysu_networks network_a) :
	...
	static const ysu::work_thresholds publish_full;
	static const ysu::work_thresholds publish_beta;
	static const ysu::work_thresholds publish_dev;
	...
	ysu_networks current_network{ ysu::network_constants::active_network };
	ysu::work_thresholds publish_thresholds;
	 ...
	 */
	static void set_active_network (ysu_networks network_a)
	{
	...
	static bool set_active_network (std::string network_a)
	{
		...
		if (network_a == "live")
		{
			active_network = ysu::ysu_networks::ysu_live_network;
		}
		else if (network_a == "beta")
		{
			active_network = ysu::ysu_networks::ysu_beta_network;
		}
		else if (network_a == "dev")
		{
			active_network = ysu::ysu_networks::ysu_dev_network;
		}
		else if (network_a == "test")
		{
			active_network = ysu::ysu_networks::ysu_test_network;
		}
		else
		{
			error = true;
	...
	{
		return current_network == ysu_networks::ysu_live_network;
	}
	bool is_beta_network () const
	{
		return current_network == ysu_networks::ysu_beta_network;
	}
	bool is_dev_network () const
	{
		return current_network == ysu_networks::ysu_dev_network;
	}
	bool is_test_network () const
	{
		return current_network == ysu_networks::ysu_test_network;
	}

	/** Initial value is ACTIVE_NETWORK compile flag, but can be overridden by a CLI flag */
	static ysu::ysu_networks active_network;
};
