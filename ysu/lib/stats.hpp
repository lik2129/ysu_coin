...
public:
	...
	ysu::error deserialize_json (ysu::jsonconfig & json);
...
public:
	...
	ysu::observer_set<boost::circular_buffer<stat_datapoint> &> sample_observers;

	/** Observers for count. Called on each update. */
	ysu::observer_set<uint64_t, uint64_t> count_observers;
};
...
private:
	...
	std::shared_ptr<ysu::stat_entry> get_entry (uint32_t key);

	/** Get entry for key, creating a new entry if necessary */
	std::shared_ptr<ysu::stat_entry> get_entry (uint32_t key, size_t sample_interval, size_t max_samples);

	/** Unlocked implementation of get_entry() */
	std::shared_ptr<ysu::stat_entry> get_entry_impl (uint32_t key, size_t sample_interval, size_t max_samples);
	...
	ysu::stat_config config;

	/** Stat entries are sorted by key to simplify processing of log output */
	std::map<uint32_t, std::shared_ptr<ysu::stat_entry>> entries;
