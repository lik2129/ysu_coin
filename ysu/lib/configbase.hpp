...
class configbase : public ysu::error_aware<>
{
public:
	configbase () = default;
	configbase (std::shared_ptr<ysu::error> const & error_a) :
	error (error_a)
	{
	}

	/** Returns the current error */
	ysu::error & get_error () override
	{
		return *error;
	}
...
protected:
	...
	/** We're a ysu::error_aware type. Child nodes share the error state. */
	std::shared_ptr<ysu::error> error;
