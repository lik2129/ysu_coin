...
std::string ysu::to_string (ysu::work_version const version_a)
{
	...
	switch (version_a)
	{
		case ysu::work_version::work_1:
			result = "work_1";
			break;
		case ysu::work_version::unspecified:
			result = "unspecified";
			break;
	}
	return result;
...
bool ysu::work_validate_entry (ysu::block const & block_a)
{
	return block_a.difficulty () < ysu::work_threshold_entry (block_a.work_version (), block_a.type ());
}

bool ysu::work_validate_entry (ysu::work_version const version_a, ysu::root const & root_a, uint64_t const work_a)
{
	return ysu::work_difficulty (version_a, root_a, work_a) < ysu::work_threshold_entry (version_a, ysu::block_type::state);
}

uint64_t ysu::work_difficulty (ysu::work_version const version_a, ysu::root const & root_a, uint64_t const work_a)
{
	uint64_t result{ 0 };
	switch (version_a)
	{
		case ysu::work_version::work_1:
			result = ysu::work_v1::value (root_a, work_a);
			break;
		default:
...
uint64_t ysu::work_threshold_base (ysu::work_version const version_a)
{
	uint64_t result{ std::numeric_limits<uint64_t>::max () };
	switch (version_a)
	{
		case ysu::work_version::work_1:
			result = ysu::work_v1::threshold_base ();
			break;
		default:
...
uint64_t ysu::work_threshold_entry (ysu::work_version const version_a, ysu::block_type const type_a)
{
	uint64_t result{ std::numeric_limits<uint64_t>::max () };
	if (type_a == ysu::block_type::state)
	{
		...
		{
			case ysu::work_version::work_1:
				result = ysu::work_v1::threshold_entry ();
				break;
			default:
...
uint64_t ysu::work_threshold (ysu::work_version const version_a, ysu::block_details const details_a)
{
	uint64_t result{ std::numeric_limits<uint64_t>::max () };
	switch (version_a)
	{
		case ysu::work_version::work_1:
			result = ysu::work_v1::threshold (details_a);
			break;
		default:
...
uint64_t ysu::work_v1::threshold_base ()
{
	static ysu::network_constants network_constants;
	return network_constants.publish_thresholds.base;
}

uint64_t ysu::work_v1::threshold_entry ()
{
	static ysu::network_constants network_constants;
	return network_constants.publish_thresholds.entry;
}

uint64_t ysu::work_v1::threshold (ysu::block_details const details_a)
{
	static_assert (ysu::epoch::max == ysu::epoch::epoch_2, "work_v1::threshold is ill-defined");
	static ysu::network_constants network_constants;
...
uint64_t ysu::work_v1::value (ysu::root const & root_a, uint64_t work_a)
{
	uint64_t result;
	blake2b_state hash;
...
double ysu::normalized_multiplier (double const multiplier_a, uint64_t const threshold_a)
{
	static ysu::network_constants network_constants;
	...
	*/
	...
	{
		auto ratio (ysu::difficulty::to_multiplier (network_constants.publish_thresholds.epoch_2, threshold_a));
...
double ysu::denormalized_multiplier (double const multiplier_a, uint64_t const threshold_a)
{
	static ysu::network_constants network_constants;
	debug_assert (multiplier_a >= 1);
	auto multiplier (multiplier_a);
...
ysu::work_pool::work_pool (unsigned max_threads_a, std::chrono::nanoseconds pow_rate_limiter_a, std::function<boost::optional<uint64_t> (ysu::work_version const, ysu::root const &, uint64_t, std::atomic<int> &)> opencl_a) :
ticket (0),
done (false),
...
opencl (opencl_a)
{
	...
	for (auto i (0u); i < count; ++i)
	{
		threads.emplace_back (attrs, [this, i]() {
			ysu::thread_role::set (ysu::thread_role::name::work);
			ysu::work_thread_reprioritize ();
			loop (i);
		});
	}
}

ysu::work_pool::~work_pool ()
{
	stop ();
	for (auto & i : threads)
	{
...
void ysu::work_pool::loop (uint64_t thread)
{
	// Quick RNG for work attempts.
	xorshift1024star rng;
	ysu::random_pool::generate_block (reinterpret_cast<uint8_t *> (rng.s.data ()), rng.s.size () * sizeof (decltype (rng.s)::value_type));
	uint64_t work;
	uint64_t output;
	blake2b_state hash;
	...
	while (!done)
	{
		...
		if (!empty)
		{
			...
			{
				work = *opt_work;
				output = ysu::work_v1::value (current_l.item, work);
			}
			else
			{
			...
			if (ticket == ticket_l)
			{
				...
				debug_assert (current_l.difficulty == 0 || ysu::work_v1::value (current_l.item, work) == output);
...
void ysu::work_pool::cancel (ysu::root const & root_a)
{
...
void ysu::work_pool::generate (ysu::work_version const version_a, ysu::root const & root_a, uint64_t difficulty_a, std::function<void(boost::optional<uint64_t> const &)> callback_a)
{
...
boost::optional<uint64_t> ysu::work_pool::generate (ysu::root const & root_a)
{
	static ysu::network_constants network_constants;
	debug_assert (network_constants.is_dev_network ());
	return generate (ysu::work_version::work_1, root_a, network_constants.publish_thresholds.base);
}

boost::optional<uint64_t> ysu::work_pool::generate (ysu::root const & root_a, uint64_t difficulty_a)
{
	static ysu::network_constants network_constants;
	debug_assert (network_constants.is_dev_network ());
	return generate (ysu::work_version::work_1, root_a, difficulty_a);
}

boost::optional<uint64_t> ysu::work_pool::generate (ysu::work_version const version_a, ysu::root const & root_a, uint64_t difficulty_a)
{
...
size_t ysu::work_pool::size ()
{
	ysu::lock_guard<std::mutex> lock (mutex);
	return pending.size ();
}

std::unique_ptr<ysu::container_info_component> ysu::collect_container_info (work_pool & work_pool, const std::string & name)
{
	size_t count;
	{
		ysu::lock_guard<std::mutex> guard (work_pool.mutex);
