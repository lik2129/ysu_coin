...
#include <ysu/node/rocksdb/rocksdb_txn.hpp>
...
ysu::rocksdb_store::rocksdb_store (ysu::logger_mt & logger_a, boost::filesystem::path const & path_a, ysu::rocksdb_config const & rocksdb_config_a, bool open_read_only_a) :
logger{ logger_a },
rocksdb_config{ rocksdb_config_a },
max_block_write_batch_num_m{ ysu::narrow_cast<unsigned> (blocks_memtable_size_bytes () / (2 * (sizeof (ysu::block_type) + ysu::state_block::size + ysu::block_sideband::size (ysu::block_type::state)))) },
...
std::unordered_map<const char *, ysu::tables> ysu::rocksdb_store::create_cf_name_table_map () const
{
	std::unordered_map<const char *, ysu::tables> map{ { rocksdb::kDefaultColumnFamilyName.c_str (), tables::default_unused },
...
void ysu::rocksdb_store::open (bool & error_a, boost::filesystem::path const & path_a, bool open_read_only_a)
{
...
void ysu::rocksdb_store::generate_tombstone_map ()
{
	tombstone_map.emplace (std::piecewise_construct, std::forward_as_tuple (ysu::tables::unchecked), std::forward_as_tuple (0, 50000));
	tombstone_map.emplace (std::piecewise_construct, std::forward_as_tuple (ysu::tables::blocks), std::forward_as_tuple (0, 25000));
	tombstone_map.emplace (std::piecewise_construct, std::forward_as_tuple (ysu::tables::accounts), std::forward_as_tuple (0, 25000));
	tombstone_map.emplace (std::piecewise_construct, std::forward_as_tuple (ysu::tables::pending), std::forward_as_tuple (0, 25000));
}

rocksdb::ColumnFamilyOptions ysu::rocksdb_store::get_common_cf_options (std::shared_ptr<rocksdb::TableFactory> const & table_factory_a, unsigned long long memtable_size_bytes_a) const
{
...
rocksdb::ColumnFamilyOptions ysu::rocksdb_store::get_cf_options (std::string const & cf_name_a) const
{
	rocksdb::ColumnFamilyOptions cf_options;
	...
	if (cf_name_a == "unchecked")
	{
		...
		cf_options.prefix_extractor.reset (rocksdb::NewFixedPrefixTransform (sizeof (ysu::root)));
...
std::vector<rocksdb::ColumnFamilyDescriptor> ysu::rocksdb_store::create_column_families ()
{
...
ysu::write_transaction ysu::rocksdb_store::tx_begin_write (std::vector<ysu::tables> const & tables_requiring_locks_a, std::vector<ysu::tables> const & tables_no_locks_a)
{
	std::unique_ptr<ysu::write_rocksdb_txn> txn;
	...
	if (tables_requiring_locks_a.empty () && tables_no_locks_a.empty ())
	{
		...
		txn = std::make_unique<ysu::write_rocksdb_txn> (optimistic_db, all_tables (), tables_no_locks_a, write_lock_mutexes);
	}
	else
	{
		txn = std::make_unique<ysu::write_rocksdb_txn> (optimistic_db, tables_requiring_locks_a, tables_no_locks_a, write_lock_mutexes);
	}
	...
	return ysu::write_transaction{ std::move (txn) };
}

ysu::read_transaction ysu::rocksdb_store::tx_begin_read ()
{
	return ysu::read_transaction{ std::make_unique<ysu::read_rocksdb_txn> (db.get ()) };
}

std::string ysu::rocksdb_store::vendor_get () const
{
	return boost::str (boost::format ("RocksDB %1%.%2%.%3%") % ROCKSDB_MAJOR % ROCKSDB_MINOR % ROCKSDB_PATCH);
}

rocksdb::ColumnFamilyHandle * ysu::rocksdb_store::table_to_column_family (tables table_a) const
{
...
bool ysu::rocksdb_store::exists (ysu::transaction const & transaction_a, tables table_a, ysu::rocksdb_val const & key_a) const
{
...
int ysu::rocksdb_store::del (ysu::write_transaction const & transaction_a, tables table_a, ysu::rocksdb_val const & key_a)
{
...
void ysu::rocksdb_store::flush_tombstones_check (tables table_a)
{
...
void ysu::rocksdb_store::flush_table (ysu::tables table_a)
{
	db->Flush (rocksdb::FlushOptions{}, table_to_column_family (table_a));
}

void ysu::rocksdb_store::version_put (ysu::write_transaction const & transaction_a, int version_a)
{
	debug_assert (transaction_a.contains (tables::meta));
	ysu::uint256_union version_key (1);
	ysu::uint256_union version_value (version_a);
	auto status (put (transaction_a, tables::meta, version_key, ysu::rocksdb_val (version_value)));
	release_assert (success (status));
}

rocksdb::Transaction * ysu::rocksdb_store::tx (ysu::transaction const & transaction_a) const
{
	debug_assert (!is_read (transaction_a));
	return static_cast<rocksdb::Transaction *> (transaction_a.get_handle ());
}

int ysu::rocksdb_store::get (ysu::transaction const & transaction_a, tables table_a, ysu::rocksdb_val const & key_a, ysu::rocksdb_val & value_a) const
{
	rocksdb::ReadOptions options;
	rocksdb::PinnableSlice slice;
...
int ysu::rocksdb_store::put (ysu::write_transaction const & transaction_a, tables table_a, ysu::rocksdb_val const & key_a, ysu::rocksdb_val const & value_a)
{
	debug_assert (transaction_a.contains (table_a));
...
bool ysu::rocksdb_store::not_found (int status) const
{
	return (status_code_not_found () == status);
}

bool ysu::rocksdb_store::success (int status) const
{
	return (static_cast<int> (rocksdb::Status::Code::kOk) == status);
}

int ysu::rocksdb_store::status_code_not_found () const
{
	return static_cast<int> (rocksdb::Status::Code::kNotFound);
}

uint64_t ysu::rocksdb_store::count (ysu::transaction const & transaction_a, tables table_a) const
{
	uint64_t sum = 0;
...
int ysu::rocksdb_store::drop (ysu::write_transaction const & transaction_a, tables table_a)
{
	debug_assert (transaction_a.contains (table_a));
	...
	if (success (status))
	{
		...
		if (table_a == tables::peers)
		{
			...
			for (auto i = peers_begin (transaction_a), n = peers_end (); i != n; ++i)
			{
				status = del (transaction_a, tables::peers, ysu::rocksdb_val (i->first));
				release_assert (success (status));
			}
			return status;
		}
...
int ysu::rocksdb_store::clear (rocksdb::ColumnFamilyHandle * column_family)
{
...
std::vector<ysu::unchecked_info> ysu::rocksdb_store::unchecked_get (ysu::transaction const & transaction_a, ysu::block_hash const & hash_a)
{
	auto cf = table_to_column_family (tables::unchecked);

	std::unique_ptr<rocksdb::Iterator> iter;
	ysu::qualified_root upper (hash_a, ysu::block_hash (std::numeric_limits<ysu::uint256_t>::max ()));
	ysu::rocksdb_val upper_bound (sizeof (upper), (void *)&upper);
	if (is_read (transaction_a))
	{
	...
	auto prefix = ysu::rocksdb_val (hash_a);
	for (iter->Seek (prefix); iter->Valid () && iter->key ().starts_with (prefix); iter->Next ())
	{
		auto unchecked_info = static_cast<ysu::unchecked_info> (ysu::rocksdb_val (iter->value ()));
		result.push_back (unchecked_info);
	}
	return result;
}

void ysu::rocksdb_store::construct_column_family_mutexes ()
{
...
rocksdb::Options ysu::rocksdb_store::get_db_options ()
{
...
rocksdb::BlockBasedTableOptions ysu::rocksdb_store::get_active_table_options (size_t lru_size) const
{
...
rocksdb::BlockBasedTableOptions ysu::rocksdb_store::get_small_table_options () const
{
	rocksdb::BlockBasedTableOptions table_options;
	// Improve point lookup performance be using the data block hash index (uses about 5% more space).
	table_options.data_block_index_type = rocksdb::BlockBasedTableOptions::DataBlockIndexType::kDataBlockBinaryAndHash;
...
rocksdb::ColumnFamilyOptions ysu::rocksdb_store::get_small_cf_options (std::shared_ptr<rocksdb::TableFactory> const & table_factory_a) const
{
...
rocksdb::ColumnFamilyOptions ysu::rocksdb_store::get_active_cf_options (std::shared_ptr<rocksdb::TableFactory> const & table_factory_a, unsigned long long memtable_size_bytes_a) const
{
...
void ysu::rocksdb_store::on_flush (rocksdb::FlushJobInfo const & flush_job_info_a)
{
...
std::vector<ysu::tables> ysu::rocksdb_store::all_tables () const
{
	return std::vector<ysu::tables>{ tables::accounts, tables::blocks, tables::confirmation_height, tables::frontiers, tables::meta, tables::online_weight, tables::peers, tables::pending, tables::pruned, tables::unchecked, tables::vote };
}

bool ysu::rocksdb_store::copy_db (boost::filesystem::path const & destination_path)
{
	...
	if (status.ok ())
	{
		ysu::rocksdb_store rocksdb_store (logger, destination_path.string (), rocksdb_config, false);
		return !rocksdb_store.init_error ();
	}
	return false;
}

void ysu::rocksdb_store::rebuild_db (ysu::write_transaction const & transaction_a)
{
	// Not available for RocksDB
}

bool ysu::rocksdb_store::init_error () const
{
	return error;
}

void ysu::rocksdb_store::serialize_memory_stats (boost::property_tree::ptree & json)
{
	uint64_t val;
...
unsigned long long ysu::rocksdb_store::blocks_memtable_size_bytes () const
{
	return base_memtable_size_bytes ();
}

unsigned long long ysu::rocksdb_store::base_memtable_size_bytes () const
{
...
unsigned ysu::rocksdb_store::max_block_write_batch_num () const
{
	return max_block_write_batch_num_m;
}

ysu::rocksdb_store::tombstone_info::tombstone_info (uint64_t num_since_last_flush_a, uint64_t const max_a) :
...
template class ysu::block_store_partial<rocksdb::Slice, ysu::rocksdb_store>;
