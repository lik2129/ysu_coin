...
namespace
{
inline bool is_read (ysu::transaction const & transaction_a)
{
	return (dynamic_cast<const ysu::read_transaction *> (&transaction_a) != nullptr);
}

inline rocksdb::ReadOptions & snapshot_options (ysu::transaction const & transaction_a)
{
...
public:
	...
	rocksdb_iterator (rocksdb::DB * db, ysu::transaction const & transaction_a, rocksdb::ColumnFamilyHandle * handle_a, rocksdb_val const * val_a)
	{
	...
	rocksdb_iterator (rocksdb::DB * db, ysu::transaction const & transaction_a, rocksdb::ColumnFamilyHandle * handle_a) :
	rocksdb_iterator (db, transaction_a, handle_a, nullptr)
	{
	}

	rocksdb_iterator (ysu::rocksdb_iterator<T, U> && other_a)
	{
	...
	rocksdb_iterator (ysu::rocksdb_iterator<T, U> const &) = delete;
	...
	std::pair<ysu::rocksdb_val, ysu::rocksdb_val> * operator-> ()
	{
		return &current;
	}
	...
	{
		auto const other_a (boost::polymorphic_downcast<ysu::rocksdb_iterator<T, U> const *> (&base_a));
	...
	void clear ()
	{
		current.first = ysu::rocksdb_val{};
		current.second = ysu::rocksdb_val{};
		debug_assert (is_end_sentinal ());
	}
	ysu::rocksdb_iterator<T, U> & operator= (ysu::rocksdb_iterator<T, U> && other_a)
	{
	...
	std::pair<ysu::rocksdb_val, ysu::rocksdb_val> current;
