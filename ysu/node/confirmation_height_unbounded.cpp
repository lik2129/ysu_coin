...
#include <ysu/node/confirmation_height_unbounded.hpp>
...
void ysu::confirmation_height_unbounded::process ()
{
	if (pending_empty ())
	{
		clear_process_vars ();
	...
	std::vector<ysu::block_hash> orig_block_callback_data;
	...
	do
	{
		...
		std::vector<ysu::block_hash> block_callback_datas_required;
...
void ysu::confirmation_height_unbounded::collect_unconfirmed_receive_and_sources_for_account (uint64_t block_height_a, uint64_t confirmation_height_a, ysu::block_hash const & hash_a, ysu::account const & account_a, ysu::read_transaction const & transaction_a, std::vector<receive_source_pair> & receive_source_pairs_a, std::vector<ysu::block_hash> & block_callback_data_a, std::vector<ysu::block_hash> & orig_block_callback_data_a)
{
	auto hash (hash_a);
	...
	while ((num_to_confirm > 0) && !hash.is_zero () && !stopped)
	{
		...
		if (block)
		{
			...
			if (!source.is_zero () && !ledger.is_epoch_link (source) && ledger.store.block_exists (transaction_a, source))
			{
				...
				receive_source_pairs_a.emplace_back (std::make_shared<conf_height_details> (account_a, hash, block_height, 1, std::vector<ysu::block_hash>{ hash }), source);
			}
...
void ysu::confirmation_height_unbounded::prepare_iterated_blocks_for_cementing (preparation_data & preparation_data_a)
{
 ...
 */
void ysu::confirmation_height_unbounded::cement_blocks (ysu::write_guard & scoped_write_guard_a)
{
	ysu::timer<std::chrono::milliseconds> cemented_batch_timer;
	...
	auto error = false;
	{
		...
		while (!pending_writes.empty ())
		{
			...
			if (!error && pending.height > confirmation_height)
			{
				...
				ledger.stats.add (ysu::stat::type::confirmation_height, ysu::stat::detail::blocks_confirmed, ysu::stat::dir::in, pending.height - confirmation_height);
				ledger.stats.add (ysu::stat::type::confirmation_height, ysu::stat::detail::blocks_confirmed_unbounded, ysu::stat::dir::in, pending.height - confirmation_height);
				...
				ysu::lock_guard<std::mutex> guard (block_cache_mutex);
...
std::shared_ptr<ysu::block> ysu::confirmation_height_unbounded::get_block_and_sideband (ysu::block_hash const & hash_a, ysu::transaction const & transaction_a)
{
	ysu::lock_guard<std::mutex> guard (block_cache_mutex);
...
bool ysu::confirmation_height_unbounded::pending_empty () const
{
	return pending_writes.empty ();
}

void ysu::confirmation_height_unbounded::clear_process_vars ()
{
...
bool ysu::confirmation_height_unbounded::has_iterated_over_block (ysu::block_hash const & hash_a) const
{
	ysu::lock_guard<std::mutex> guard (block_cache_mutex);
	return block_cache.count (hash_a) == 1;
}

uint64_t ysu::confirmation_height_unbounded::block_cache_size () const
{
	ysu::lock_guard<std::mutex> guard (block_cache_mutex);
	return block_cache.size ();
}

ysu::confirmation_height_unbounded::conf_height_details::conf_height_details (ysu::account const & account_a, ysu::block_hash const & hash_a, uint64_t height_a, uint64_t num_blocks_confirmed_a, std::vector<ysu::block_hash> const & block_callback_data_a) :
account (account_a),
hash (hash_a),
height (height_a),
...
ysu::confirmation_height_unbounded::receive_source_pair::receive_source_pair (std::shared_ptr<conf_height_details> const & receive_details_a, const block_hash & source_a) :
receive_details (receive_details_a),
source_hash (source_a)
{
}

ysu::confirmation_height_unbounded::confirmed_iterated_pair::confirmed_iterated_pair (uint64_t confirmed_height_a, uint64_t iterated_height_a) :
confirmed_height (confirmed_height_a),
iterated_height (iterated_height_a)
{
}

std::unique_ptr<ysu::container_info_component> ysu::collect_container_info (confirmation_height_unbounded & confirmation_height_unbounded, const std::string & name_a)
{
