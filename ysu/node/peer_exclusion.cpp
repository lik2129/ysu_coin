...
constexpr std::chrono::hours ysu::peer_exclusion::exclude_time_hours;
constexpr std::chrono::hours ysu::peer_exclusion::exclude_remove_hours;
constexpr size_t ysu::peer_exclusion::size_max;
constexpr double ysu::peer_exclusion::peers_percentage_limit;

uint64_t ysu::peer_exclusion::add (ysu::tcp_endpoint const & endpoint_a, size_t const network_peers_count_a)
{
	uint64_t result (0);
...
bool ysu::peer_exclusion::check (ysu::tcp_endpoint const & endpoint_a)
{
	bool excluded (false);
	ysu::lock_guard<std::mutex> guard (mutex);
	auto & peers_by_endpoint (peers.get<tag_endpoint> ());
...
void ysu::peer_exclusion::remove (ysu::tcp_endpoint const & endpoint_a)
{
	ysu::lock_guard<std::mutex> guard (mutex);
	peers.get<tag_endpoint> ().erase (endpoint_a.address ());
}

size_t ysu::peer_exclusion::limited_size (size_t const network_peers_count_a) const
{
	return std::min (size_max, static_cast<size_t> (network_peers_count_a * peers_percentage_limit));
}

size_t ysu::peer_exclusion::size () const
{
	ysu::lock_guard<std::mutex> guard (mutex);
	return peers.size ();
}

std::unique_ptr<ysu::container_info_component> ysu::collect_container_info (ysu::peer_exclusion const & excluded_peers, const std::string & name)
{
	auto composite = std::make_unique<container_info_composite> (name);

	size_t excluded_peers_count = excluded_peers.size ();
	auto sizeof_excluded_peers_element = sizeof (ysu::peer_exclusion::ordered_endpoints::value_type);
