...
class socket : public std::enable_shared_from_this<ysu::socket>
{
...
public:
	 ...
	 */
	explicit socket (ysu::node & node, boost::optional<std::chrono::seconds> io_timeout = boost::none);
	virtual ~socket ();
	...
	void async_write (ysu::shared_const_buffer const &, std::function<void(boost::system::error_code const &, size_t)> const & = nullptr);
...
public:
	 ...
	 */
	explicit server_socket (ysu::node & node_a, boost::asio::ip::tcp::endpoint local_a, size_t max_connections_a);
	...
	void on_connection (std::function<bool(std::shared_ptr<ysu::socket> new_connection, boost::system::error_code const &)>);
...
private:
	std::vector<std::weak_ptr<ysu::socket>> connections;
