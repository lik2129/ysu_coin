...
{
public:
	confirmation_height_processor (ysu::ledger &, ysu::write_database_queue &, std::chrono::milliseconds, ysu::logging const &, ysu::logger_mt &, boost::latch & initialized_latch, confirmation_height_mode = confirmation_height_mode::automatic);
	~confirmation_height_processor ();
	void pause ();
	void unpause ();
	void stop ();
	void add (ysu::block_hash const & hash_a);
	void run (confirmation_height_mode);
	size_t awaiting_processing_size () const;
	bool is_processing_added_block (ysu::block_hash const & hash_a) const;
	bool is_processing_block (ysu::block_hash const &) const;
	ysu::block_hash current () const;

	void add_cemented_observer (std::function<void(std::shared_ptr<ysu::block>)> const &);
	void add_block_already_cemented_observer (std::function<void(ysu::block_hash const &)> const &);
...
private:
	...
	std::vector<std::function<void(std::shared_ptr<ysu::block>)>> cemented_observers;
	std::vector<std::function<void(ysu::block_hash const &)>> block_already_cemented_observers;
	...
	void set_next_hash ();
	void notify_observers (std::vector<std::shared_ptr<ysu::block>> const &);
	void notify_observers (ysu::block_hash const &);
