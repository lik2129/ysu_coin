...
{
public:
	explicit matches_txn (const ysu::transaction_impl * transaction_impl_a) :
	transaction_impl (transaction_impl_a)
	{
	}

	bool operator() (ysu::mdb_txn_stats const & mdb_txn_stats)
	{
...
ysu::read_mdb_txn::read_mdb_txn (ysu::mdb_env const & environment_a, ysu::mdb_txn_callbacks txn_callbacks_a) :
...
void ysu::read_mdb_txn::reset ()
{
	mdb_txn_reset (handle);
	txn_callbacks.txn_end (this);
}

void ysu::read_mdb_txn::renew ()
{
...
void * ysu::read_mdb_txn::get_handle () const
{
	return handle;
}

ysu::write_mdb_txn::write_mdb_txn (ysu::mdb_env const & environment_a, ysu::mdb_txn_callbacks txn_callbacks_a) :
env (environment_a),
txn_callbacks (txn_callbacks_a)
{
	renew ();
...
ysu::write_mdb_txn::~write_mdb_txn ()
{
	commit ();
}

void ysu::write_mdb_txn::commit () const
{
	auto status (mdb_txn_commit (handle));
	release_assert (status == MDB_SUCCESS);
	txn_callbacks.txn_end (this);
...
void * ysu::write_mdb_txn::get_handle () const
{
	return handle;
}

bool ysu::write_mdb_txn::contains (ysu::tables table_a) const
{
	// LMDB locks on every write
	return true;
}

ysu::mdb_txn_tracker::mdb_txn_tracker (ysu::logger_mt & logger_a, ysu::txn_tracking_config const & txn_tracking_config_a, std::chrono::milliseconds block_processor_batch_max_time_a) :
logger (logger_a),
...
void ysu::mdb_txn_tracker::serialize_json (boost::property_tree::ptree & json, std::chrono::milliseconds min_read_time, std::chrono::milliseconds min_write_time)
{
	...
	for (size_t i = 0; i < times_since_start.size (); ++i)
	{
		...
		{
			ysu::jsonconfig mdb_lock_config;
			...
			ysu::jsonconfig stack (stacktrace_config);
...
void ysu::mdb_txn_tracker::log_if_held_long_enough (ysu::mdb_txn_stats const & mdb_txn_stats) const
{
	...
	bool is_blk_processing_thread = mdb_txn_stats.thread_name == ysu::thread_role::get_string (ysu::thread_role::name::block_processing);
...
void ysu::mdb_txn_tracker::add (const ysu::transaction_impl * transaction_impl)
{
	ysu::lock_guard<std::mutex> guard (mutex);
...
void ysu::mdb_txn_tracker::erase (const ysu::transaction_impl * transaction_impl)
{
...
ysu::mdb_txn_stats::mdb_txn_stats (const ysu::transaction_impl * transaction_impl) :
transaction_impl (transaction_impl),
thread_name (ysu::thread_role::get_string ()),
stacktrace (std::make_shared<boost::stacktrace::stacktrace> ())
{
	timer.start ();
}

bool ysu::mdb_txn_stats::is_write () const
{
	return (dynamic_cast<const ysu::write_transaction_impl *> (transaction_impl) != nullptr);
}
