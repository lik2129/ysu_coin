...
#include <ysu/node/bootstrap/bootstrap_bulk_pull.hpp>
#include <ysu/node/bootstrap/bootstrap_connections.hpp>
...
ysu::pull_info::pull_info (ysu::hash_or_account const & account_or_head_a, ysu::block_hash const & head_a, ysu::block_hash const & end_a, uint64_t bootstrap_id_a, count_t count_a, unsigned retry_limit_a) :
...
ysu::bulk_pull_client::bulk_pull_client (std::shared_ptr<ysu::bootstrap_client> connection_a, std::shared_ptr<ysu::bootstrap_attempt> attempt_a, ysu::pull_info const & pull_a) :
...
ysu::bulk_pull_client::~bulk_pull_client ()
{
	// If received end block is not expected end block
	if (expected != pull.end)
	{
		pull.head = expected;
		if (attempt->mode != ysu::bootstrap_mode::legacy)
		{
...
void ysu::bulk_pull_client::request ()
{
	...
	req, [this_l](boost::system::error_code const & ec, size_t size_a) {
		...
		else
		{
			...
			this_l->connection->node->stats.inc (ysu::stat::type::bootstrap, ysu::stat::detail::bulk_pull_request_failure, ysu::stat::dir::in);
		}
	},
	ysu::buffer_drop_policy::no_limiter_drop);
}

void ysu::bulk_pull_client::throttled_receive_block ()
{
...
void ysu::bulk_pull_client::receive_block ()
{
	auto this_l (shared_from_this ());
	connection->socket->async_read (connection->receive_buffer, 1, [this_l](boost::system::error_code const & ec, size_t size_a) {
		...
		else
		{
			...
			this_l->connection->node->stats.inc (ysu::stat::type::bootstrap, ysu::stat::detail::bulk_pull_receive_block_failure, ysu::stat::dir::in);
			this_l->network_error = true;
		}
	});
}

void ysu::bulk_pull_client::received_type ()
{
	auto this_l (shared_from_this ());
	ysu::block_type type (static_cast<ysu::block_type> (connection->receive_buffer->data ()[0]));
	...
	switch (type)
	{
		...
		{
			socket_l->async_read (connection->receive_buffer, ysu::send_block::size, [this_l, type](boost::system::error_code const & ec, size_t size_a) {
		...
		{
			socket_l->async_read (connection->receive_buffer, ysu::receive_block::size, [this_l, type](boost::system::error_code const & ec, size_t size_a) {
				this_l->received_block (ec, size_a, type);
			});
			break;
		}
		...
		{
			socket_l->async_read (connection->receive_buffer, ysu::open_block::size, [this_l, type](boost::system::error_code const & ec, size_t size_a) {
				this_l->received_block (ec, size_a, type);
			});
			break;
		}
		...
		{
			socket_l->async_read (connection->receive_buffer, ysu::change_block::size, [this_l, type](boost::system::error_code const & ec, size_t size_a) {
				this_l->received_block (ec, size_a, type);
			});
			break;
		}
		...
		{
			socket_l->async_read (connection->receive_buffer, ysu::state_block::size, [this_l, type](boost::system::error_code const & ec, size_t size_a) {
				this_l->received_block (ec, size_a, type);
			});
			break;
		}
...
void ysu::bulk_pull_client::received_block (boost::system::error_code const & ec, size_t size_a, ysu::block_type type_a)
{
	if (!ec)
	{
		ysu::bufferstream stream (connection->receive_buffer->data (), size_a);
		std::shared_ptr<ysu::block> block (ysu::deserialize_block (stream, type_a));
		if (block != nullptr && !ysu::work_validate_entry (*block))
		{
			...
			// Unconfirmed head is used only for lazy destinations if legacy bootstrap is not available, see ysu::bootstrap_attempt::lazy_destinations_increment (...)
			...
			if (!stop_pull && !connection->hard_stop.load ())
			{
				...
				to prevent spam */
				if (attempt->mode != ysu::bootstrap_mode::legacy || unexpected_count < 16384)
				{
		...
		else if (block == nullptr)
		{
			...
			connection->node->stats.inc (ysu::stat::type::bootstrap, ysu::stat::detail::bulk_pull_deserialize_receive_block, ysu::stat::dir::in);
		}
		else // Work invalid
		{
			if (connection->node->config.logging.bulk_pull_logging ())
	...
	else
	{
		...
		connection->node->stats.inc (ysu::stat::type::bootstrap, ysu::stat::detail::bulk_pull_receive_block_failure, ysu::stat::dir::in);
		network_error = true;
	}
}

ysu::bulk_pull_account_client::bulk_pull_account_client (std::shared_ptr<ysu::bootstrap_client> connection_a, std::shared_ptr<ysu::bootstrap_attempt> attempt_a, ysu::account const & account_a) :
connection (connection_a),
attempt (attempt_a),
account (account_a),
pull_blocks (0)
...
ysu::bulk_pull_account_client::~bulk_pull_account_client ()
{
	attempt->pull_finished ();
}

void ysu::bulk_pull_account_client::request ()
{
	ysu::bulk_pull_account req;
	req.account = account;
	req.minimum_amount = connection->node->config.receive_minimum;
	req.flags = ysu::bulk_pull_account_flags::pending_hash_and_amount;
	if (connection->node->config.logging.bulk_pull_logging ())
	{
	...
	req, [this_l](boost::system::error_code const & ec, size_t size_a) {
		...
		else
		{
			...
			this_l->connection->node->stats.inc (ysu::stat::type::bootstrap, ysu::stat::detail::bulk_pull_error_starting_request, ysu::stat::dir::in);
		}
	},
	ysu::buffer_drop_policy::no_limiter_drop);
}

void ysu::bulk_pull_account_client::receive_pending ()
{
	auto this_l (shared_from_this ());
	size_t size_l (sizeof (ysu::uint256_union) + sizeof (ysu::uint128_union));
	connection->socket->async_read (connection->receive_buffer, size_l, [this_l, size_l](boost::system::error_code const & ec, size_t size_a) {
		...
		if (size_a == size_l)
		{
			if (!ec)
			{
				ysu::block_hash pending;
				ysu::bufferstream frontier_stream (this_l->connection->receive_buffer->data (), sizeof (ysu::uint256_union));
				auto error1 (ysu::try_read (frontier_stream, pending));
				(void)error1;
				debug_assert (!error1);
				ysu::amount balance;
				ysu::bufferstream balance_stream (this_l->connection->receive_buffer->data () + sizeof (ysu::uint256_union), sizeof (ysu::uint128_union));
				auto error2 (ysu::try_read (balance_stream, balance));
				(void)error2;
				debug_assert (!error2);
 ...
 */
void ysu::bulk_pull_server::set_current_end ()
{
	include_start = false;
...
void ysu::bulk_pull_server::send_next ()
{
	auto block (get_next ());
	if (block != nullptr)
	{
		...
		{
			ysu::vectorstream stream (send_buffer);
			ysu::serialize_block (stream, *block);
		}
		auto this_l (shared_from_this ());
		if (connection->node->config.logging.bulk_pull_logging ())
		{
		...
		connection->socket->async_write (ysu::shared_const_buffer (std::move (send_buffer)), [this_l](boost::system::error_code const & ec, size_t size_a) {
...
std::shared_ptr<ysu::block> ysu::bulk_pull_server::get_next ()
{
...
void ysu::bulk_pull_server::sent_action (boost::system::error_code const & ec, size_t size_a)
{
	if (!ec)
	{
		send_next ();
...
void ysu::bulk_pull_server::send_finished ()
{
	ysu::shared_const_buffer send_buffer (static_cast<uint8_t> (ysu::block_type::not_a_block));
	auto this_l (shared_from_this ());
	if (connection->node->config.logging.bulk_pull_logging ())
	{
...
void ysu::bulk_pull_server::no_block_sent (boost::system::error_code const & ec, size_t size_a)
{
	if (!ec)
	{
...
ysu::bulk_pull_server::bulk_pull_server (std::shared_ptr<ysu::bootstrap_server> const & connection_a, std::unique_ptr<ysu::bulk_pull> request_a) :
connection (connection_a),
 ...
 */
void ysu::bulk_pull_account_server::set_params ()
{
	debug_assert (request != nullptr);
	...
	if (request->flags == ysu::bulk_pull_account_flags::pending_address_only)
	{
		pending_address_only = true;
	}
	else if (request->flags == ysu::bulk_pull_account_flags::pending_hash_amount_and_address)
	{
		/**
	...
	else if (request->flags == ysu::bulk_pull_account_flags::pending_hash_and_amount)
	{
...
void ysu::bulk_pull_account_server::send_frontier ()
{
	/*
	...
	if (!invalid_request)
	{
		...
		ysu::uint128_union account_frontier_balance (account_frontier_balance_int);

		// Write the frontier block hash and balance into a buffer
		std::vector<uint8_t> send_buffer;
		{
			ysu::vectorstream output_stream (send_buffer);
...
void ysu::bulk_pull_account_server::send_next_block ()
{
	/*
...
std::pair<std::unique_ptr<ysu::pending_key>, std::unique_ptr<ysu::pending_info>> ysu::bulk_pull_account_server::get_next ()
{
	std::pair<std::unique_ptr<ysu::pending_key>, std::unique_ptr<ysu::pending_info>> result;
	...
	while (true)
	{
		...
		if (stream == ysu::store_iterator<ysu::pending_key, ysu::pending_info> (nullptr))
		{
			break;
		}

		ysu::pending_key key (stream->first);
		ysu::pending_info info (stream->second);
		...
		result.first = std::make_unique<ysu::pending_key> (key);
		result.second = std::make_unique<ysu::pending_info> (info);
...
void ysu::bulk_pull_account_server::sent_action (boost::system::error_code const & ec, size_t size_a)
{
	if (!ec)
	{
		send_next_block ();
...
void ysu::bulk_pull_account_server::send_finished ()
{
	/*
	...
	{
		ysu::vectorstream output_stream (send_buffer);
		ysu::uint256_union account_zero (0);
		ysu::uint128_union balance_zero (0);
...
void ysu::bulk_pull_account_server::complete (boost::system::error_code const & ec, size_t size_a)
{
	if (!ec)
	{
		if (pending_address_only)
...
ysu::bulk_pull_account_server::bulk_pull_account_server (std::shared_ptr<ysu::bootstrap_server> const & connection_a, std::unique_ptr<ysu::bulk_pull_account> request_a) :
connection (connection_a),
request (std::move (request_a)),
current_key (0, 0)
{
