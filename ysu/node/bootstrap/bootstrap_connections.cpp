...
constexpr double ysu::bootstrap_limits::bootstrap_connection_scale_target_blocks;
constexpr double ysu::bootstrap_limits::bootstrap_minimum_blocks_per_sec;
constexpr double ysu::bootstrap_limits::bootstrap_minimum_termination_time_sec;
constexpr unsigned ysu::bootstrap_limits::bootstrap_max_new_connections;
constexpr unsigned ysu::bootstrap_limits::requeued_pulls_processed_blocks_factor;

ysu::bootstrap_client::bootstrap_client (std::shared_ptr<ysu::node> node_a, std::shared_ptr<ysu::bootstrap_connections> connections_a, std::shared_ptr<ysu::transport::channel_tcp> channel_a, std::shared_ptr<ysu::socket> socket_a) :
node (node_a),
...
ysu::bootstrap_client::~bootstrap_client ()
{
	--connections->connections_count;
}

double ysu::bootstrap_client::sample_block_rate ()
{
	auto elapsed = std::max (elapsed_seconds (), ysu::bootstrap_limits::bootstrap_minimum_elapsed_seconds_blockrate);
	block_rate = static_cast<double> (block_count.load ()) / elapsed;
	return block_rate;
}

void ysu::bootstrap_client::set_start_time (std::chrono::steady_clock::time_point start_time_a)
{
	ysu::lock_guard<std::mutex> guard (start_time_mutex);
	start_time_m = start_time_a;
}

double ysu::bootstrap_client::elapsed_seconds () const
{
	ysu::lock_guard<std::mutex> guard (start_time_mutex);
	return std::chrono::duration_cast<std::chrono::duration<double>> (std::chrono::steady_clock::now () - start_time_m).count ();
}

void ysu::bootstrap_client::stop (bool force)
{
	pending_stop = true;
	if (force)
	{
...
ysu::bootstrap_connections::bootstrap_connections (ysu::node & node_a) :
node (node_a)
{
}

std::shared_ptr<ysu::bootstrap_client> ysu::bootstrap_connections::connection (std::shared_ptr<ysu::bootstrap_attempt> attempt_a, bool use_front_connection)
{
	...
	std::shared_ptr<ysu::bootstrap_client> result;
...
void ysu::bootstrap_connections::pool_connection (std::shared_ptr<ysu::bootstrap_client> client_a, bool new_client, bool push_front)
{
	ysu::unique_lock<std::mutex> lock (mutex);
...
void ysu::bootstrap_connections::add_connection (ysu::endpoint const & endpoint_a)
{
	connect_client (ysu::tcp_endpoint (endpoint_a.address (), endpoint_a.port ()), true);
}

std::shared_ptr<ysu::bootstrap_client> ysu::bootstrap_connections::find_connection (ysu::tcp_endpoint const & endpoint_a)
{
...
void ysu::bootstrap_connections::connect_client (ysu::tcp_endpoint const & endpoint_a, bool push_front)
{
	++connections_count;
	auto socket (std::make_shared<ysu::socket> (node));
	...
	[this_l, socket, endpoint_a, push_front](boost::system::error_code const & ec) {
		if (!ec)
		{
			...
			auto client (std::make_shared<ysu::bootstrap_client> (this_l->node.shared (), this_l, std::make_shared<ysu::transport::channel_tcp> (*this_l->node.shared (), socket), socket));
...
unsigned ysu::bootstrap_connections::target_connections (size_t pulls_remaining, size_t attempts_count)
{
	auto const attempts_factor = ysu::narrow_cast<unsigned> (node.config.bootstrap_connections * attempts_count);
	...
	double step_scale = std::min (1.0, std::max (0.0, (double)pulls_remaining / ysu::bootstrap_limits::bootstrap_connection_scale_target_blocks));
...
{
	bool operator() (const std::shared_ptr<ysu::bootstrap_client> & lhs, const std::shared_ptr<ysu::bootstrap_client> & rhs) const
	{
		return lhs->block_rate > rhs->block_rate;
	}
};

void ysu::bootstrap_connections::populate_connections (bool repeat)
{
	double rate_sum = 0.0;
	size_t num_pulls = 0;
	size_t attempts_count = node.bootstrap_initiator.attempts.size ();
	std::priority_queue<std::shared_ptr<ysu::bootstrap_client>, std::vector<std::shared_ptr<ysu::bootstrap_client>>, block_rate_cmp> sorted_connections;
	std::unordered_set<ysu::tcp_endpoint> endpoints;
	{
		ysu::unique_lock<std::mutex> lock (mutex);
		num_pulls = pulls.size ();
		std::deque<std::weak_ptr<ysu::bootstrap_client>> new_clients;
		for (auto & c : clients)
		{
			if (auto client = c.lock ())
			{
				...
				if (client->elapsed_seconds () > ysu::bootstrap_limits::bootstrap_connection_warmup_time_sec && client->block_count > 0)
				{
				...
				if (elapsed_sec > ysu::bootstrap_limits::bootstrap_minimum_termination_time_sec && blocks_per_sec < ysu::bootstrap_limits::bootstrap_minimum_blocks_per_sec)
				{
					if (node.config.logging.bulk_pull_logging ())
					{
						node.logger.try_log (boost::str (boost::format ("Stopping slow peer %1% (elapsed sec %2%s > %3%s and %4% blocks per second < %5%)") % client->channel->to_string () % elapsed_sec % ysu::bootstrap_limits::bootstrap_minimum_termination_time_sec % blocks_per_sec % ysu::bootstrap_limits::bootstrap_minimum_blocks_per_sec));
					}
	...
	{
		auto delta = std::min ((target - connections_count) * 2, ysu::bootstrap_limits::bootstrap_max_new_connections);
		// TODO - tune this better
		// Not many peers respond, need to try to make more connections than we need.
		for (auto i = 0u; i < delta; i++)
		{
			auto endpoint (node.network.bootstrap_peer (true));
			if (endpoint != ysu::tcp_endpoint (boost::asio::ip::address_v6::any (), 0) && (node.flags.allow_bootstrap_peers_duplicates || endpoints.find (endpoint) == endpoints.end ()) && !node.network.excluded_peers.check (endpoint))
			{
	...
	if (!stopped && repeat)
	{
		std::weak_ptr<ysu::bootstrap_connections> this_w (shared_from_this ());
...
void ysu::bootstrap_connections::start_populate_connections ()
{
...
void ysu::bootstrap_connections::add_pull (ysu::pull_info const & pull_a)
{
...
void ysu::bootstrap_connections::request_pull (ysu::unique_lock<std::mutex> & lock_a)
{
	lock_a.unlock ();
	...
	{
		std::shared_ptr<ysu::bootstrap_attempt> attempt_l;
		ysu::pull_info pull;
		...
		while (attempt_l == nullptr && !pulls.empty ())
		{
			...
			if (attempt_l != nullptr && attempt_l->mode == ysu::bootstrap_mode::lazy && !pull.head.is_zero () && attempt_l->lazy_processed_or_exists (pull.head))
			{
		...
		{
			if (attempt_l->mode == ysu::bootstrap_mode::legacy)
			{
			...
			node.background ([connection_l, attempt_l, pull]() {
				auto client (std::make_shared<ysu::bulk_pull_client> (connection_l, attempt_l, pull));
				client->request ();
			});
		}
	}
...
void ysu::bootstrap_connections::requeue_pull (ysu::pull_info const & pull_a, bool network_error)
{
	auto pull (pull_a);
	if (!network_error)
	{
	...
	if (attempt_l != nullptr)
	{
		...
		else if (attempt_l->mode == ysu::bootstrap_mode::lazy)
		{
			pull.count = attempt_l->lazy_batch_size ();
		}
		if (attempt_l->mode == ysu::bootstrap_mode::legacy && (pull.attempts < pull.retry_limit + (pull.processed / ysu::bootstrap_limits::requeued_pulls_processed_blocks_factor)))
		{
			{
				ysu::lock_guard<std::mutex> lock (mutex);
		...
		else if (attempt_l->mode == ysu::bootstrap_mode::lazy && (pull.retry_limit == std::numeric_limits<unsigned>::max () || pull.attempts <= pull.retry_limit + (pull.processed / node.network_params.bootstrap.lazy_max_pull_blocks)))
		{
		...
		else
		{
			...
			node.stats.inc (ysu::stat::type::bootstrap, ysu::stat::detail::bulk_pull_failed_account, ysu::stat::dir::in);

			if (attempt_l->mode == ysu::bootstrap_mode::lazy && pull.processed > 0)
			{
				attempt_l->lazy_add (pull);
			}
			else if (attempt_l->mode == ysu::bootstrap_mode::legacy)
			{
...
void ysu::bootstrap_connections::clear_pulls (uint64_t bootstrap_id_a)
{
	{
		ysu::lock_guard<std::mutex> lock (mutex);
		auto i (pulls.begin ());
...
void ysu::bootstrap_connections::run ()
{
...
void ysu::bootstrap_connections::stop ()
{
	ysu::unique_lock<std::mutex> lock (mutex);
	stopped = true;
	lock.unlock ();
