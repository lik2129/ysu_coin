...
public:
	...
	ysu::uint512_union account_head;
	ysu::block_hash new_head;
};
class pulls_cache final
{
public:
	void add (ysu::pull_info const &);
	void update_pull (ysu::pull_info &);
	void remove (ysu::pull_info const &);
	std::mutex pulls_cache_mutex;
	class account_head_tag
	{
	};
	// clang-format off
	boost::multi_index_container<ysu::cached_pulls,
	mi::indexed_by<
		mi::ordered_non_unique<
			mi::member<ysu::cached_pulls, std::chrono::steady_clock::time_point, &ysu::cached_pulls::time>>,
		mi::hashed_unique<mi::tag<account_head_tag>,
			mi::member<ysu::cached_pulls, ysu::uint512_union, &ysu::cached_pulls::account_head>>>>
	cache;
	// clang-format on
...
{
public:
	void add (std::shared_ptr<ysu::bootstrap_attempt>);
	void remove (uint64_t);
	void clear ();
	std::shared_ptr<ysu::bootstrap_attempt> find (uint64_t);
	size_t size ();
	std::atomic<uint64_t> incremental{ 0 };
	std::mutex bootstrap_attempts_mutex;
	std::map<uint64_t, std::shared_ptr<ysu::bootstrap_attempt>> attempts;
};

class bootstrap_initiator final
{
public:
	explicit bootstrap_initiator (ysu::node &);
	~bootstrap_initiator ();
	void bootstrap (ysu::endpoint const &, bool add_to_peers = true, bool frontiers_confirmed = false, std::string id_a = "");
	void bootstrap (bool force = false, std::string id_a = "");
	void bootstrap_lazy (ysu::hash_or_account const &, bool force = false, bool confirmed = true, std::string id_a = "");
	void bootstrap_wallet (std::deque<ysu::account> &);
	void run_bootstrap ();
	void lazy_requeue (ysu::block_hash const &, ysu::block_hash const &, bool);
	void notify_listeners (bool);
	void add_observer (std::function<void(bool)> const &);
	bool in_progress ();
	std::shared_ptr<ysu::bootstrap_connections> connections;
	std::shared_ptr<ysu::bootstrap_attempt> new_attempt ();
	bool has_new_attempts ();
	std::shared_ptr<ysu::bootstrap_attempt> current_attempt ();
	std::shared_ptr<ysu::bootstrap_attempt> current_lazy_attempt ();
	std::shared_ptr<ysu::bootstrap_attempt> current_wallet_attempt ();
	ysu::pulls_cache cache;
	ysu::bootstrap_attempts attempts;
	void stop ();

private:
	ysu::node & node;
	std::shared_ptr<ysu::bootstrap_attempt> find_attempt (ysu::bootstrap_mode);
	void remove_attempt (std::shared_ptr<ysu::bootstrap_attempt>);
	void stop_attempts ();
	std::vector<std::shared_ptr<ysu::bootstrap_attempt>> attempts_list;
