...
const char * default_beta_peer_network = "peering-beta.ysu.org";
const char * default_live_peer_network = "peering.ysu.org";
const char * default_test_peer_network = "peering-test.ysu.org";
}

ysu::node_config::node_config () :
node_config (0, ysu::logging ())
{
}

ysu::node_config::node_config (uint16_t peering_port_a, ysu::logging const & logging_a) :
...
external_address (boost::asio::ip::address_v6{}.to_string ())
{
	...
	switch (network_params.network.network ())
	{
		...
		case ysu::ysu_networks::ysu_beta_network:
		{
			...
			ysu::account offline_representative;
			release_assert (!offline_representative.decode_account ("ysu_1defau1t9off1ine9rep99999999999999999999999999999999wgmuzxxy"));
...
ysu::error ysu::node_config::serialize_toml (ysu::tomlconfig & toml) const
{
...
ysu::error ysu::node_config::deserialize_toml (ysu::tomlconfig & toml)
{
	try
	{
		...
		if (frontiers_confirmation == ysu::frontiers_confirmation_mode::invalid)
		{
...
ysu::error ysu::node_config::serialize_json (ysu::jsonconfig & json) const
{
...
bool ysu::node_config::upgrade_json (unsigned version_a, ysu::jsonconfig & json)
{
	json.put ("version", json_version ());
	switch (version_a)
	{
...
ysu::error ysu::node_config::deserialize_json (bool & upgraded_a, ysu::jsonconfig & json)
{
	try
	{
...
std::string ysu::node_config::serialize_frontiers_confirmation (ysu::frontiers_confirmation_mode mode_a) const
{
	switch (mode_a)
	{
		case ysu::frontiers_confirmation_mode::always:
			return "always";
		case ysu::frontiers_confirmation_mode::automatic:
			return "auto";
		case ysu::frontiers_confirmation_mode::disabled:
			return "disabled";
		default:
			return "auto";
	}
...
ysu::frontiers_confirmation_mode ysu::node_config::deserialize_frontiers_confirmation (std::string const & string_a)
{
	if (string_a == "always")
	{
		return ysu::frontiers_confirmation_mode::always;
	}
	else if (string_a == "auto")
	{
		return ysu::frontiers_confirmation_mode::automatic;
	}
	else if (string_a == "disabled")
	{
		return ysu::frontiers_confirmation_mode::disabled;
	}
	else
	{
		return ysu::frontiers_confirmation_mode::invalid;
	}
}

void ysu::node_config::deserialize_address (std::string const & entry_a, std::vector<std::pair<std::string, uint16_t>> & container_a) const
{
...
ysu::account ysu::node_config::random_representative () const
{
	debug_assert (!preconfigured_representatives.empty ());
	size_t index (ysu::random_pool::generate_word32 (0, static_cast<CryptoPP::word32> (preconfigured_representatives.size () - 1)));
