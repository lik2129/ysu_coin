...
ysu::vote_processor::vote_processor (ysu::signature_checker & checker_a, ysu::active_transactions & active_a, ysu::node_observers & observers_a, ysu::stat & stats_a, ysu::node_config & config_a, ysu::node_flags & flags_a, ysu::logger_mt & logger_a, ysu::online_reps & online_reps_a, ysu::ledger & ledger_a, ysu::network_params & network_params_a) :
checker (checker_a),
active (active_a),
...
is_active (false),
thread ([this]() {
	ysu::thread_role::set (ysu::thread_role::name::vote_processing);
	process_loop ();
})
{
...
void ysu::vote_processor::process_loop ()
{
	ysu::timer<std::chrono::milliseconds> elapsed;
...
bool ysu::vote_processor::vote (std::shared_ptr<ysu::vote> vote_a, std::shared_ptr<ysu::transport::channel> channel_a)
{
	...
	if (!stopped)
	{
		...
		{
			stats.inc (ysu::stat::type::vote, ysu::stat::detail::vote_overflow);
		}
	}
	return !process;
}

void ysu::vote_processor::verify_votes (decltype (votes) const & votes_a)
{
	...
	hashes.reserve (size);
	std::vector<size_t> lengths (size, sizeof (ysu::block_hash));
...
ysu::vote_code ysu::vote_processor::vote_blocking (std::shared_ptr<ysu::vote> vote_a, std::shared_ptr<ysu::transport::channel> channel_a, bool validated)
{
	auto result (ysu::vote_code::invalid);
	...
	switch (result)
	{
		case ysu::vote_code::invalid:
			status = "Invalid";
			stats.inc (ysu::stat::type::vote, ysu::stat::detail::vote_invalid);
			break;
		case ysu::vote_code::replay:
			status = "Replay";
			stats.inc (ysu::stat::type::vote, ysu::stat::detail::vote_replay);
			break;
		case ysu::vote_code::vote:
			status = "Vote";
			stats.inc (ysu::stat::type::vote, ysu::stat::detail::vote_valid);
			break;
		case ysu::vote_code::indeterminate:
			status = "Indeterminate";
			stats.inc (ysu::stat::type::vote, ysu::stat::detail::vote_indeterminate);
			break;
	}
...
void ysu::vote_processor::flush_active ()
{
	ysu::unique_lock<std::mutex> lock (mutex);
	while (is_active)
	{
...
size_t ysu::vote_processor::size ()
{
	ysu::lock_guard<std::mutex> guard (mutex);
	return votes.size ();
}

bool ysu::vote_processor::empty ()
{
	ysu::lock_guard<std::mutex> guard (mutex);
	return votes.empty ();
}

bool ysu::vote_processor::half_full ()
{
	return size () >= max_votes / 2;
}

void ysu::vote_processor::calculate_weights ()
{
	ysu::unique_lock<std::mutex> lock (mutex);
	if (!stopped)
	{
		...
		{
			ysu::account const & representative (rep_amount.first);
...
std::unique_ptr<ysu::container_info_component> ysu::collect_container_info (vote_processor & vote_processor, const std::string & name)
{
	size_t votes_count;
	...
	{
		ysu::lock_guard<std::mutex> guard (vote_processor.mutex);
