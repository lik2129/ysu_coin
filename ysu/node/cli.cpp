...
namespace
{
void reset_confirmation_heights (ysu::block_store & store);
bool is_using_rocksdb (boost::filesystem::path const & data_path, std::error_code & ec);
}

std::string ysu::error_cli_messages::message (int ev) const
{
	switch (static_cast<ysu::error_cli> (ev))
	{
		case ysu::error_cli::generic:
			return "Unknown error";
		case ysu::error_cli::parse_error:
			return "Coud not parse command line";
		case ysu::error_cli::invalid_arguments:
			return "Invalid arguments";
		case ysu::error_cli::unknown_command:
			return "Unknown command";
		case ysu::error_cli::database_write_error:
			return "Database write error";
		case ysu::error_cli::reading_config:
			return "Config file read error";
		case ysu::error_cli::disable_all_network:
			return "Flags --disable_tcp_realtime and --disable_udp cannot be used together";
		case ysu::error_cli::ambiguous_udp_options:
			return "Flags --disable_udp and --enable_udp cannot be used together";
		case ysu::error_cli::ambiguous_pruning_voting_options:
...
void ysu::add_node_options (boost::program_options::options_description & description_a)
{
	// clang-format off
...
void ysu::add_node_flag_options (boost::program_options::options_description & description_a)
{
	// clang-format off
	description_a.add_options()
...
std::error_code ysu::update_flags (ysu::node_flags & flags_a, boost::program_options::variables_map const & vm)
{
	std::error_code ec;
	...
	if (config != vm.end ())
	{
		flags_a.config_overrides = ysu::config_overrides (config->second.as<std::vector<ysu::config_key_value_pair>> ());
	}
	return ec;
}

std::error_code ysu::flags_config_conflicts (ysu::node_flags const & flags_a, ysu::node_config const & config_a)
{
	std::error_code ec;
	if (flags_a.enable_pruning && config_a.enable_voting)
	{
		ec = ysu::error_cli::ambiguous_pruning_voting_options;
	}
	return ec;
}
...
void database_write_lock_error (std::error_code & ec)
{
	...
	ec = ysu::error_cli::database_write_error;
}
...
std::error_code ysu::handle_node_options (boost::program_options::variables_map const & vm)
{
	std::error_code ec;
	boost::filesystem::path data_path = vm.count ("data_path") ? boost::filesystem::path (vm["data_path"].as<std::string> ()) : ysu::working_path ();
			...
			if (!wallet_id.decode_hex (vm["wallet"].as<std::string> ()))
			{
				...
				if (wallet != nullptr)
				{
					...
					else
					{
						...
						ec = ysu::error_cli::invalid_arguments;
					}
				}
				else
				{
		...
		try
		{
			...
			if (!ec)
			{
				...
				if (success)
				{
					...
					if (using_rocksdb)
					{
						ysu::remove_all_files_in_dir (backup_path);
						ysu::move_all_files_to_dir (source_path, backup_path);
						ysu::move_all_files_to_dir (vacuum_path, source_path);
	...
	else if (vm.count ("generate_config"))
	{
		...
		if (valid_type)
		{
			std::cout << "# This is an example configuration file for Ysu. Visit https://docs.ysu.org/running-a-node/configuration/ for more information.\n#\n"
			          ...
			          << "# Additional information for notable configuration options is available in https://docs.ysu.org/running-a-node/configuration/#notable-configuration-options\n";
	...
	else if (vm.count ("diagnostics"))
	{
		...
		ysu::raw_key key;
		key.data.clear ();
		ysu::send_block send (0, 0, 0, key, 0, 0);
		std::cout << "Testing key derivation function" << std::endl;
		ysu::raw_key junk1;
		junk1.data.clear ();
		ysu::uint256_union junk2 (0);
		ysu::kdf kdf;
		kdf.phs (junk1, "", junk2);
		std::cout << "Testing time retrieval latency... " << std::flush;
		ysu::timer<std::chrono::nanoseconds> timer (ysu::timer_state::started);
	...
	else if (vm.count ("wallet_create"))
	{
		...
		{
			auto inactive_node = ysu::default_inactive_node (data_path, vm);
			auto wallet_key = ysu::random_wallet_id ();
				...
				if (existing != inactive_node->node->wallets.items.end ())
				{
					...
					if (!existing->second->enter_password (transaction, password))
					{
						...
						for (auto i (existing->second->store.begin (transaction)), m (existing->second->store.end ()); i != m; ++i)
						{
							...
							if (ysu::pub_key (key.as_private_key ()) != account)
							{
						...
						else
						{
							...
							else
							{
								bool error (true);
								{
									...
									auto transaction (node->wallets.tx_begin_write ());
									ysu::wallet wallet (error, transaction, node->wallets, wallet_id.to_string (), contents.str ());
								}
								if (error)
								{
									std::cerr << "Unable to import wallet\n";
	...
	else if (vm.count ("wallet_list"))
	{
		...
		for (auto i (node->wallets.items.begin ()), n (node->wallets.items.end ()); i != n; ++i)
		{
			...
			{
				std::cout << ysu::account (j->first).to_account () << '\n';
			}
		}
	}
	...
	{
		ec = ysu::error_cli::unknown_command;
	}

	return ec;
}

std::unique_ptr<ysu::inactive_node> ysu::default_inactive_node (boost::filesystem::path const & path_a, boost::program_options::variables_map const & vm_a)
{
	auto node_flags = ysu::inactive_node_flag_defaults ();
	ysu::update_flags (node_flags, vm_a);
	return std::make_unique<ysu::inactive_node> (path_a, node_flags);
}
...
{
	ysu::daemon_config config (data_path);
	auto error = ysu::read_node_config_toml (data_path, config);
	if (!error)
	{
	...
	{
		ec = ysu::error_cli::reading_config;
	}
