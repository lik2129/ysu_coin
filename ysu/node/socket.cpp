...
ysu::socket::socket (ysu::node & node_a, boost::optional<std::chrono::seconds> io_timeout_a) :
...
void ysu::socket::async_connect (ysu::tcp_endpoint const & endpoint_a, std::function<void(boost::system::error_code const &)> callback_a)
{
	checkup ();
...
void ysu::socket::async_read (std::shared_ptr<std::vector<uint8_t>> buffer_a, size_t size_a, std::function<void(boost::system::error_code const &, size_t)> callback_a)
{
		...
		if (!closed)
		{
			...
			boost::asio::post (strand, boost::asio::bind_executor (strand, [buffer_a, callback_a, size_a, this_l]() {
				...
				[this_l, buffer_a, callback_a](boost::system::error_code const & ec, size_t size_a) {
					this_l->node.stats.add (ysu::stat::type::traffic_tcp, ysu::stat::dir::in, size_a);
					this_l->stop_timer ();
	...
	{
		debug_assert (false && "ysu::socket::async_read called with incorrect buffer size");
...
void ysu::socket::async_write (ysu::shared_const_buffer const & buffer_a, std::function<void(boost::system::error_code const &, size_t)> const & callback_a)
{
	if (!closed)
	{
		++queue_size;
		boost::asio::post (strand, boost::asio::bind_executor (strand, [buffer_a, callback_a, this_l = shared_from_this ()]() {
			...
			{
				this_l->start_timer ();
				ysu::async_write (this_l->tcp_socket, buffer_a,
				boost::asio::bind_executor (this_l->strand,
				[buffer_a, callback_a, this_l](boost::system::error_code ec, std::size_t size_a) {
					--this_l->queue_size;
					this_l->node.stats.add (ysu::stat::type::traffic_tcp, ysu::stat::dir::out, size_a);
					this_l->stop_timer ();
					if (callback_a)
					{
						callback_a (ec, size_a);
...
void ysu::socket::start_timer ()
{
	start_timer (io_timeout.get ());
}

void ysu::socket::start_timer (std::chrono::seconds deadline_a)
{
	next_deadline = deadline_a.count ();
}

void ysu::socket::stop_timer ()
{
	last_completion_time = ysu::seconds_since_epoch ();
}

void ysu::socket::checkup ()
{
	std::weak_ptr<ysu::socket> this_w (shared_from_this ());
	node.alarm.add (std::chrono::steady_clock::now () + std::chrono::seconds (node.network_params.network.is_dev_network () ? 1 : 2), [this_w]() {
		if (auto this_l = this_w.lock ())
		{
			uint64_t now (ysu::seconds_since_epoch ());
...
bool ysu::socket::has_timed_out () const
{
	return timed_out;
}

void ysu::socket::set_timeout (std::chrono::seconds io_timeout_a)
{
	auto this_l (shared_from_this ());
...
void ysu::socket::close_internal ()
{
	if (!closed.exchange (true))
	{
		...
		if (ec)
		{
			...
			node.stats.inc (ysu::stat::type::bootstrap, ysu::stat::detail::error_socket_close);
		}
	}
}

ysu::tcp_endpoint ysu::socket::remote_endpoint () const
{
	return remote;
}

ysu::server_socket::server_socket (ysu::node & node_a, boost::asio::ip::tcp::endpoint local_a, size_t max_connections_a) :
...
void ysu::server_socket::start (boost::system::error_code & ec_a)
{
...
void ysu::server_socket::close ()
{
	auto this_l (std::static_pointer_cast<ysu::server_socket> (shared_from_this ()));
...
void ysu::server_socket::on_connection (std::function<bool(std::shared_ptr<ysu::socket>, boost::system::error_code const &)> callback_a)
{
	auto this_l (std::static_pointer_cast<ysu::server_socket> (shared_from_this ()));
	...
	boost::asio::post (strand, boost::asio::bind_executor (strand, [this_l, callback_a]() {
		if (this_l->acceptor.is_open ())
		{
			if (this_l->connections.size () < this_l->max_inbound_connections)
			{
				...
				auto new_connection = std::make_shared<ysu::socket> (this_l->node, boost::none);
				...
				[this_l, new_connection, callback_a](boost::system::error_code const & ec_a) {
					...
					if (this_l->connections.size () < this_l->max_inbound_connections)
					{
						if (!ec_a)
						{
							...
							this_l->node.stats.inc (ysu::stat::type::tcp, ysu::stat::detail::tcp_accept_success, ysu::stat::dir::in);
					...
					{
						this_l->node.stats.inc (ysu::stat::type::tcp, ysu::stat::detail::tcp_accept_failure, ysu::stat::dir::in);
...
void ysu::server_socket::evict_dead_connections ()
{
