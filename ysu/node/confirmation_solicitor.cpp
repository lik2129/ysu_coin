...
ysu::confirmation_solicitor::confirmation_solicitor (ysu::network & network_a, ysu::network_constants const & params_a) :
...
void ysu::confirmation_solicitor::prepare (std::vector<ysu::representative> const & representatives_a)
{
...
bool ysu::confirmation_solicitor::broadcast (ysu::election const & election_a)
{
	debug_assert (prepared);
	bool error (true);
	if (rebroadcasted++ < max_block_broadcasts)
	{
		...
		ysu::publish winner (election_a.status.winner);
		unsigned count = 0;
		...
		network.flood_message (winner, ysu::buffer_drop_policy::limiter, 0.5f);
		error = false;
	}
	return error;
}

bool ysu::confirmation_solicitor::add (ysu::election const & election_a)
{
	debug_assert (prepared);
	bool error (true);
	unsigned count = 0;
	auto const max_channel_requests (max_confirm_req_batches * ysu::network::confirm_req_hashes_max);
	auto const & hash (election_a.status.winner->hash ());
...
void ysu::confirmation_solicitor::flush ()
{
	debug_assert (prepared);
	for (auto const & request_queue : requests)
	{
		...
		for (auto const & root_hash : request_queue.second)
		{
			...
			if (roots_hashes_l.size () == ysu::network::confirm_req_hashes_max)
			{
