...
namespace
{
void construct_json (ysu::container_info_component * component, boost::property_tree::ptree & parent);
using ipc_json_handler_no_arg_func_map = std::unordered_map<std::string, std::function<void(ysu::json_handler *)>>;
...
bool block_confirmed (ysu::node & node, ysu::transaction & transaction, ysu::block_hash const & hash, bool include_active, bool include_only_confirmed);
const char * epoch_as_string (ysu::epoch);
}

ysu::json_handler::json_handler (ysu::node & node_a, ysu::node_rpc_config const & node_rpc_config_a, std::string const & body_a, std::function<void(std::string const &)> const & response_a, std::function<void()> stop_callback_a) :
body (body_a),
node (node_a),
response (response_a),
...
std::function<void()> ysu::json_handler::create_worker_task (std::function<void(std::shared_ptr<ysu::json_handler> const &)> const & action_a)
{
...
void ysu::json_handler::process_request (bool unsafe_a)
{
	try
	{
		...
		else
		{
			...
			else if (action == "kysu_from_raw" || action == "krai_from_raw")
			{
				mysu_from_raw (ysu::kxrb_ratio);
			}
			else if (action == "kysu_to_raw" || action == "krai_to_raw")
			{
				mysu_to_raw (ysu::kxrb_ratio);
			}
			else if (action == "ysu_from_raw" || action == "rai_from_raw")
			{
				mysu_from_raw (ysu::xrb_ratio);
			}
			else if (action == "ysu_to_raw" || action == "rai_to_raw")
			{
				mysu_to_raw (ysu::xrb_ratio);
			}
			else if (action == "mysu_from_raw" || action == "mrai_from_raw")
			{
				mysu_from_raw ();
			}
			else if (action == "mysu_to_raw" || action == "mrai_to_raw")
			{
				mysu_to_raw ();
			}
...
std::shared_ptr<ysu::wallet> ysu::json_handler::wallet_impl ()
{
	if (!ec)
	{
		...
		if (!wallet.decode_hex (wallet_text))
		{
			...
			{
				ec = ysu::error_common::wallet_not_found;
			}
		}
		else
		{
			ec = ysu::error_common::bad_wallet_number;
		}
	}
	return nullptr;
}

bool ysu::json_handler::wallet_locked_impl (ysu::transaction const & transaction_a, std::shared_ptr<ysu::wallet> wallet_a)
{
	bool result (false);
	if (!ec)
	{
		...
		{
			ec = ysu::error_common::wallet_locked;
			result = true;
		}
	}
	return result;
...
bool ysu::json_handler::wallet_account_impl (ysu::transaction const & transaction_a, std::shared_ptr<ysu::wallet> wallet_a, ysu::account const & account_a)
{
	bool result (false);
	if (!ec)
	{
		...
		{
			ec = ysu::error_common::account_not_found_wallet;
		}
	}
	return result;
}

ysu::account ysu::json_handler::account_impl (std::string account_text, std::error_code ec_a)
{
	...
	if (!ec)
	{
		...
		{
			// ysu- and xrb- prefixes are deprecated
...
ysu::account_info ysu::json_handler::account_info_impl (ysu::transaction const & transaction_a, ysu::account const & account_a)
{
	...
	if (!ec)
	{
		...
		{
			ec = ysu::error_common::account_not_found;
...
ysu::amount ysu::json_handler::amount_impl ()
{
	ysu::amount result (0);
	if (!ec)
	{
		...
		{
			ec = ysu::error_common::invalid_amount;
		}
	}
	return result;
}

std::shared_ptr<ysu::block> ysu::json_handler::block_impl (bool signature_work_required)
{
	const bool json_block_l = request.get<bool> ("json_block", false);
	std::shared_ptr<ysu::block> result{ nullptr };
	if (!ec)
	{
		...
		else
		{
			...
			{
				ec = ysu::error_blocks::invalid_block;
			}
		}
		if (!ec)
		{
			...
			result = ysu::deserialize_block_json (block_l);
			if (result == nullptr)
			{
				ec = ysu::error_blocks::invalid_block;
			}
...
ysu::block_hash ysu::json_handler::hash_impl (std::string search_text)
{
	...
	if (!ec)
	{
		...
		{
			ec = ysu::error_blocks::invalid_block_hash;
		}
	}
	return result;
}

ysu::amount ysu::json_handler::threshold_optional_impl ()
{
	ysu::amount result (0);
	...
	if (!ec && threshold_text.is_initialized ())
	{
		...
		{
			ec = ysu::error_common::bad_threshold;
		}
	}
	return result;
}

uint64_t ysu::json_handler::work_optional_impl ()
{
	uint64_t result (0);
	boost::optional<std::string> work_text (request.get_optional<std::string> ("work"));
	if (!ec && work_text.is_initialized ())
	{
		if (ysu::from_string_hex (work_text.get (), result))
		{
			ec = ysu::error_common::bad_work_format;
		}
	}
	return result;
}

uint64_t ysu::json_handler::difficulty_optional_impl (ysu::work_version const version_a)
{
	...
	{
		if (ysu::from_string_hex (difficulty_text.get (), difficulty))
		{
			ec = ysu::error_rpc::bad_difficulty_format;
		}
	}
	return difficulty;
}

uint64_t ysu::json_handler::difficulty_ledger (ysu::block const & block_a)
{
	ysu::block_details details (ysu::epoch::epoch_0, false, false, false);
	bool details_found (false);
	auto transaction (node.store.tx_begin_read ());
	// Previous block find
	std::shared_ptr<ysu::block> block_previous (nullptr);
	...
	if (!link.is_zero () && !details.is_send)
	{
		...
		if (block_link != nullptr && node.store.pending_exists (transaction, ysu::pending_key (block_a.account (), link.as_block_hash ())))
		{
	...
	return details_found ? ysu::work_threshold (block_a.work_version (), details) : node.default_difficulty (block_a.work_version ());
}

double ysu::json_handler::multiplier_optional_impl (ysu::work_version const version_a, uint64_t & difficulty)
{
	double multiplier (1.);
	...
	if (!ec && multiplier_text.is_initialized ())
	{
		...
		{
			difficulty = ysu::difficulty::from_multiplier (multiplier, node.default_difficulty (version_a));
		}
		else
		{
			ec = ysu::error_rpc::bad_multiplier_format;
		}
	}
	return multiplier;
}

ysu::work_version ysu::json_handler::work_version_optional_impl (ysu::work_version const default_a)
{
	ysu::work_version result = default_a;
	boost::optional<std::string> version_text (request.get_optional<std::string> ("version"));
	if (!ec && version_text.is_initialized ())
	{
		if (*version_text == ysu::to_string (ysu::work_version::work_1))
		{
			result = ysu::work_version::work_1;
		}
		else
		{
			ec = ysu::error_rpc::bad_work_version;
		}
	}
	return result;
}
...
uint64_t ysu::json_handler::count_impl ()
{
	uint64_t result (0);
	if (!ec)
	{
		...
		{
			ec = ysu::error_common::invalid_count;
		}
	}
	return result;
}

uint64_t ysu::json_handler::count_optional_impl (uint64_t result)
{
...
uint64_t ysu::json_handler::offset_optional_impl (uint64_t result)
{
	...
	if (!ec && offset_text.is_initialized ())
	{
		...
		{
			ec = ysu::error_rpc::invalid_offset;
		}
	}
	return result;
}

void ysu::json_handler::account_balance ()
{
...
void ysu::json_handler::account_block_count ()
{
	auto account (account_impl ());
	if (!ec)
	{
...
void ysu::json_handler::account_create ()
{
	node.worker.push_task (create_worker_task ([](std::shared_ptr<ysu::json_handler> const & rpc_l) {
		...
		if (!rpc_l->ec)
		{
			...
			if (index_text.is_initialized ())
			{
				...
				{
					rpc_l->ec = ysu::error_common::invalid_index;
				}
				else
				{
			...
			if (!rpc_l->ec)
			{
				...
				{
					rpc_l->ec = ysu::error_common::wallet_locked;
				}
			}
		}
...
void ysu::json_handler::account_get ()
{
	...
	{
		ec = ysu::error_common::bad_public_key;
	}
	response_errors ();
}

void ysu::json_handler::account_info ()
{
	auto account (account_impl ());
	if (!ec)
	{
		...
		if (!ec)
		{
			...
			std::string balance;
			ysu::uint128_union (info.balance).encode_dec (balance);
...
void ysu::json_handler::account_key ()
{
	auto account (account_impl ());
	if (!ec)
	{
...
void ysu::json_handler::account_list ()
{
	...
	if (!ec)
	{
		...
		for (auto i (wallet->store.begin (transaction)), j (wallet->store.end ()); i != j; ++i)
		{
			...
			entry.put ("", ysu::account (i->first).to_account ());
...
void ysu::json_handler::account_move ()
{
	node.worker.push_task (create_worker_task ([](std::shared_ptr<ysu::json_handler> const & rpc_l) {
		auto wallet (rpc_l->wallet_impl ());
		if (!rpc_l->ec)
		{
			...
			if (!source.decode_hex (source_text))
			{
				...
				if (existing != rpc_l->node.wallets.items.end ())
				{
					...
					std::vector<ysu::public_key> accounts;
				...
				{
					rpc_l->ec = ysu::error_rpc::source_not_found;
				}
			}
			else
			{
				rpc_l->ec = ysu::error_rpc::bad_source;
			}
		}
		rpc_l->response_errors ();
	}));
...
void ysu::json_handler::account_remove ()
{
	node.worker.push_task (create_worker_task ([](std::shared_ptr<ysu::json_handler> const & rpc_l) {
		auto wallet (rpc_l->wallet_impl ());
...
void ysu::json_handler::account_representative ()
{
	auto account (account_impl ());
	if (!ec)
	{
...
void ysu::json_handler::account_representative_set ()
{
	node.worker.push_task (create_worker_task ([work_generation_enabled = node.work_generation_enabled ()](std::shared_ptr<ysu::json_handler> const & rpc_l) {
		auto wallet (rpc_l->wallet_impl ());
		auto account (rpc_l->account_impl ());
		std::string representative_text (rpc_l->request.get<std::string> ("representative"));
		auto representative (rpc_l->account_impl (representative_text, ysu::error_rpc::bad_representative_number));
		if (!rpc_l->ec)
		{
			...
			if (!rpc_l->ec && work)
			{
				...
				if (!rpc_l->ec)
				{
					...
					if (!rpc_l->ec)
					{
						ysu::block_details details (info.epoch (), false, false, false);
						if (ysu::work_difficulty (ysu::work_version::work_1, info.head, work) < ysu::work_threshold (ysu::work_version::work_1, details))
						{
							rpc_l->ec = ysu::error_common::invalid_work;
						}
					}
				}
			}
			else if (!rpc_l->ec) // work == 0
			{
				...
				{
					rpc_l->ec = ysu::error_common::disabled_work_generation;
				}
			}
			if (!rpc_l->ec)
			{
				...
				wallet->change_async (
				account, representative, [response_a, response_data](std::shared_ptr<ysu::block> block) {
					if (block != nullptr)
					{
...
void ysu::json_handler::account_weight ()
{
	auto account (account_impl ());
	if (!ec)
	{
...
void ysu::json_handler::accounts_balances ()
{
...
void ysu::json_handler::accounts_create ()
{
	node.worker.push_task (create_worker_task ([](std::shared_ptr<ysu::json_handler> const & rpc_l) {
		auto wallet (rpc_l->wallet_impl ());
		...
		if (!rpc_l->ec)
		{
			...
			{
				ysu::account new_key (wallet->deterministic_insert (generate_work));
				if (!new_key.is_zero ())
				{
					boost::property_tree::ptree entry;
...
void ysu::json_handler::accounts_frontiers ()
{
...
void ysu::json_handler::accounts_pending ()
{
	...
	for (auto & accounts : request.get_child ("accounts"))
	{
		...
		if (!ec)
		{
			...
			for (auto i (node.store.pending_begin (transaction, ysu::pending_key (account, 0))), n (node.store.pending_end ()); i != n && ysu::pending_key (i->first).account == account && peers_l.size () < count; ++i)
			{
			...
			if (sorting && !simple)
			{
				if (source)
				{
					peers_l.sort ([](const auto & child1, const auto & child2) -> bool {
						return child1.second.template get<ysu::uint128_t> ("amount") > child2.second.template get<ysu::uint128_t> ("amount");
					});
				}
				else
				{
					peers_l.sort ([](const auto & child1, const auto & child2) -> bool {
						return child1.second.template get<ysu::uint128_t> ("") > child2.second.template get<ysu::uint128_t> ("");
					});
				}
			}
...
void ysu::json_handler::active_difficulty ()
{
	auto include_trend (request.get<bool> ("include_trend", false));
	auto const multiplier_active = node.active.active_multiplier ();
	auto const default_difficulty (node.default_difficulty (ysu::work_version::work_1));
	auto const default_receive_difficulty (node.default_receive_difficulty (ysu::work_version::work_1));
	auto const receive_current_denormalized (ysu::denormalized_multiplier (multiplier_active, node.network_params.network.publish_thresholds.epoch_2_receive));
	response_l.put ("network_minimum", ysu::to_string_hex (default_difficulty));
	response_l.put ("network_receive_minimum", ysu::to_string_hex (default_receive_difficulty));
	response_l.put ("network_current", ysu::to_string_hex (ysu::difficulty::from_multiplier (multiplier_active, default_difficulty)));
	response_l.put ("network_receive_current", ysu::to_string_hex (ysu::difficulty::from_multiplier (receive_current_denormalized, default_receive_difficulty)));
	...
	if (include_trend)
	{
		...
		{
			boost::property_tree::ptree entry;
			entry.put ("", ysu::to_string (multiplier_l));
...
void ysu::json_handler::available_supply ()
{
	auto genesis_balance (node.balance (node.network_params.ledger.genesis_account)); // Cold storage genesis
	auto landing_balance (node.balance (ysu::account ("059F68AAB29DE0D3A27443625C7EA9CDDB6517A8B76FE37727EF6A4D76832AD5"))); // Active unavailable account
	auto faucet_balance (node.balance (ysu::account ("8E319CE6F3025E5B2DF66DA7AB1467FE48F1679C13DD43BFDB29FA2E9FC40D3B"))); // Faucet account
	auto burned_balance ((node.balance_pending (ysu::account (0))).second); // Burning 0 account
...
void ysu::json_handler::block_info ()
{
	auto hash (hash_impl ());
	if (!ec)
	{
		...
		if (block != nullptr)
		{
			ysu::account account (block->account ().is_zero () ? block->sideband ().account : block->account ());
			...
			if (block->type () == ysu::block_type::state)
			{
				auto subtype (ysu::state_subtype (block->sideband ().details));
				response_l.put ("subtype", subtype);
			}
		}
		else
		{
			ec = ysu::error_blocks::not_found;
		}
	}
	response_errors ();
}

void ysu::json_handler::block_confirm ()
{
	auto hash (hash_impl ());
	if (!ec)
	{
		...
		if (block_l != nullptr)
		{
			...
			else
			{
				...
				ysu::election_status status{ block_l, 0, std::chrono::duration_cast<std::chrono::milliseconds> (std::chrono::system_clock::now ().time_since_epoch ()), std::chrono::duration_values<std::chrono::milliseconds>::zero (), 0, 1, 0, ysu::election_status_type::active_confirmation_height };
				{
					ysu::lock_guard<std::mutex> lock (node.active.mutex);
				...
				if (auto state = dynamic_cast<ysu::state_block *> (block_l.get ()))
				{
...
void ysu::json_handler::blocks ()
{
	...
	for (boost::property_tree::ptree::value_type & hashes : request.get_child ("hashes"))
	{
		if (!ec)
		{
			...
			{
				ec = ysu::error_blocks::bad_hash_number;
			}
		}
	}
...
void ysu::json_handler::blocks_info ()
{
	const bool pending = request.get<bool> ("pending", false);
	const bool source = request.get<bool> ("source", false);
	const bool json_block_l = request.get<bool> ("json_block", false);
			...
			if (!hash.decode_hex (hash_text))
			{
				...
				if (block != nullptr)
				{
					...
					if (pending)
					{
						...
						{
							exists = node.store.pending_exists (transaction, ysu::pending_key (destination, hash));
						}
						entry.put ("pending", exists ? "1" : "0");
					}
					if (source)
					{
						ysu::block_hash source_hash (node.ledger.block_source (transaction, *block));
...
void ysu::json_handler::block_account ()
{
	auto hash (hash_impl ());
	if (!ec)
	{
...
void ysu::json_handler::block_count ()
{
...
void ysu::json_handler::block_create ()
{
	std::string type (request.get<std::string> ("type"));
	ysu::wallet_id wallet (0);
	// Default to work_1 if not specified
	auto work_version (work_version_optional_impl (ysu::work_version::work_1));
	...
	{
		representative = account_impl (representative_text.get (), ysu::error_rpc::bad_representative_number);
	}
	ysu::account destination (0);
	...
	{
		destination = account_impl (destination_text.get (), ysu::error_rpc::bad_destination);
	}
	ysu::block_hash source (0);
	...
	ysu::raw_key prv;
	prv.data.clear ();
	ysu::block_hash previous (0);
	ysu::amount balance (0);
	if (work == 0 && !node.work_generation_enabled ())
	{
		ec = ysu::error_common::disabled_work_generation;
	}
	...
	if (!ec && key_text.is_initialized ())
	{
		...
		{
			ec = ysu::error_common::bad_private_key;
		}
	}
	...
	if (!ec && balance_text.is_initialized ())
	{
		...
		{
			ec = ysu::error_rpc::invalid_balance;
		}
	}
	ysu::link link (0);
	...
	if (!ec)
	{
		...
		auto block_response_put_l = [rpc_l, this](ysu::block const & block_a) {
			boost::property_tree::ptree response_l;
			response_l.put ("hash", block_a.hash ().to_string ());
			response_l.put ("difficulty", ysu::to_string_hex (block_a.difficulty ()));
			bool json_block_l = request.get<bool> ("json_block", false);
			if (json_block_l)
			{
				boost::property_tree::ptree block_node_l;
		...
		auto get_callback_l = [rpc_l, block_response_put_l](std::shared_ptr<ysu::block> block_a) {
			...
			return [block_a, rpc_l, block_response_put_l](boost::optional<uint64_t> const & work_a) {
				if (block_a != nullptr)
				{
					...
					{
						rpc_l->ec = ysu::error_common::failure_work_generation;
					}
				}
				else
				{
					rpc_l->ec = ysu::error_common::generic;
				}
				if (rpc_l->ec)
				{
					rpc_l->response_errors ();
		...
		if (prv.data != 0)
		{
			ysu::account pub (ysu::pub_key (prv.as_private_key ()));
			...
			else if (previous_text.is_initialized () && balance_text.is_initialized () && type == "send")
			{
				...
				{
					ec = ysu::error_rpc::block_create_balance_mismatch;
				}
			}
			...
			if (!ec && account_text.is_initialized ())
			{
				if (account != pub)
				{
					ec = ysu::error_rpc::block_create_public_key_mismatch;
				}
			}
			ysu::block_builder builder_l;
			std::shared_ptr<ysu::block> block_l{ nullptr };
			ysu::root root_l;
			...
			if (type == "state")
			{
				...
				{
					ec = ysu::error_rpc::block_create_requirements_state;
				}
			}
			else if (type == "open")
			{
				...
				{
					ec = ysu::error_rpc::block_create_requirements_open;
				}
			}
			else if (type == "receive")
			{
				...
				{
					ec = ysu::error_rpc::block_create_requirements_receive;
				}
			}
			else if (type == "change")
			{
				...
				{
					ec = ysu::error_rpc::block_create_requirements_change;
				}
			}
			else if (type == "send")
			{
				if (destination != 0 && previous != 0 && balance != 0 && amount != 0)
				{
					...
					{
						ec = ysu::error_common::insufficient_balance;
					}
				}
				else
				{
					ec = ysu::error_rpc::block_create_requirements_send;
				}
			}
			else
			{
				ec = ysu::error_blocks::invalid_type;
			}
			if (!ec && (!ec_build || ec_build == ysu::error_common::missing_work))
			{
				if (work == 0)
				{
					...
					node.work_generate (work_version, root_l, difficulty_l, get_callback_l (block_l), ysu::account (pub));
				}
				else
				{
		...
		{
			ec = ysu::error_rpc::block_create_key_required;
		}
	}
	// Because of callback
	if (ec)
...
void ysu::json_handler::block_hash ()
{
...
void ysu::json_handler::bootstrap ()
{
	...
	{
		uint16_t port;
		if (!ysu::parse_port (port_text, port))
		{
			if (!node.flags.disable_legacy_bootstrap)
			{
				std::string bootstrap_id (request.get<std::string> ("id", ""));
				node.bootstrap_initiator.bootstrap (ysu::endpoint (address, port), true, bypass_frontier_confirmation, bootstrap_id);
				response_l.put ("success", "");
			}
			else
			{
				ec = ysu::error_rpc::disabled_bootstrap_legacy;
			}
		}
		else
		{
			ec = ysu::error_common::invalid_port;
		}
	}
	else
	{
		ec = ysu::error_common::invalid_ip_address;
	}
	response_errors ();
}

void ysu::json_handler::bootstrap_any ()
{
...
void ysu::json_handler::bootstrap_lazy ()
{
	auto hash (hash_impl ());
	const bool force = request.get<bool> ("force", false);
	if (!ec)
	{
		...
		{
			ec = ysu::error_rpc::disabled_bootstrap_lazy;
		}
	}
	response_errors ();
}
 ...
 */
void ysu::json_handler::bootstrap_status ()
{
	...
	{
		ysu::lock_guard<std::mutex> connections_lock (node.bootstrap_initiator.connections->mutex);
	...
	{
		ysu::lock_guard<std::mutex> attempts_lock (node.bootstrap_initiator.attempts.bootstrap_attempts_mutex);
...
void ysu::json_handler::chain (bool successors)
{
...
void ysu::json_handler::confirmation_active ()
{
...
void ysu::json_handler::confirmation_height_currently_processing ()
{
	...
	{
		ec = ysu::error_rpc::confirmation_height_not_processing;
	}
	response_errors ();
}

void ysu::json_handler::confirmation_history ()
{
	boost::property_tree::ptree elections;
...
void ysu::json_handler::confirmation_info ()
{
	...
	if (!root.decode_hex (root_text))
	{
		...
		if (election != nullptr && !election->confirmed ())
		{
			...
			for (auto i (tally_l.begin ()), n (tally_l.end ()); i != n; ++i)
			{
				...
				if (representatives)
				{
					std::multimap<ysu::uint128_t, ysu::account, std::greater<ysu::uint128_t>> representatives;
					for (auto ii (election->last_votes.begin ()), nn (election->last_votes.end ()); ii != nn; ++ii)
					{
						if (i->second->hash () == ii->second.hash)
						{
							ysu::account const & representative (ii->first);
		...
		{
			ec = ysu::error_rpc::confirmation_not_found;
		}
	}
	else
	{
		ec = ysu::error_rpc::invalid_root;
	}
	response_errors ();
}

void ysu::json_handler::confirmation_quorum ()
{
...
void ysu::json_handler::database_txn_tracker ()
{
	...
	{
		ec = ysu::error_common::tracking_not_enabled;
	}

	response_errors ();
}

void ysu::json_handler::delegators ()
{
	auto account (account_impl ());
	if (!ec)
	{
...
void ysu::json_handler::delegators_count ()
{
	auto account (account_impl ());
	if (!ec)
	{
...
void ysu::json_handler::deterministic_key ()
{
	...
	if (!seed.data.decode_hex (seed_text))
	{
		try
		{
			...
			ysu::private_key prv = ysu::deterministic_key (seed, index);
			ysu::public_key pub (ysu::pub_key (prv));
		...
		{
			ec = ysu::error_common::invalid_index;
		}
	}
	else
	{
		ec = ysu::error_common::bad_seed;
	}
	response_errors ();
}
 ...
 * @warning This is an internal/diagnostic RPC, do not rely on its interface being stable
 */
void ysu::json_handler::epoch_upgrade ()
{
	ysu::epoch epoch (ysu::epoch::invalid);
	uint8_t epoch_int (request.get<uint8_t> ("epoch"));
	switch (epoch_int)
	{
		case 1:
			epoch = ysu::epoch::epoch_1;
			break;
		case 2:
			epoch = ysu::epoch::epoch_2;
			break;
		default:
			break;
	}
	if (epoch != ysu::epoch::invalid)
	{
		...
		if (!ec && threads_text.is_initialized ())
		{
			...
			{
				ec = ysu::error_rpc::invalid_threads_count;
			}
		}
		std::string key_text (request.get<std::string> ("key"));
		ysu::private_key prv;
		...
		{
			if (ysu::pub_key (prv) == node.ledger.epoch_signer (node.ledger.epoch_link (epoch)))
			{
			...
			{
				ec = ysu::error_rpc::invalid_epoch_signer;
			}
		}
		else
		{
	...
	{
		ec = ysu::error_rpc::invalid_epoch;
	}
	response_errors ();
}

void ysu::json_handler::frontiers ()
{
...
void ysu::json_handler::account_count ()
{
...
namespace
{
class history_visitor : public ysu::block_visitor
{
public:
	history_visitor (ysu::json_handler & handler_a, bool raw_a, ysu::transaction & transaction_a, boost::property_tree::ptree & tree_a, ysu::block_hash const & hash_a, std::vector<ysu::public_key> const & accounts_filter_a) :
	handler (handler_a),
	raw (raw_a),
	...
	void send_block (ysu::send_block const & block_a)
	{
	...
	void receive_block (ysu::receive_block const & block_a)
	{
	...
	void open_block (ysu::open_block const & block_a)
	{
		if (raw)
		{
			tree.put ("type", "open");
	...
	void change_block (ysu::change_block const & block_a)
	{
	...
	void state_block (ysu::state_block const & block_a)
	{
		if (raw)
		{
			tree.put ("type", "state");
	...
	bool should_ignore_account (ysu::public_key const & account)
	{
		bool ignore (false);
	...
	ysu::json_handler & handler;
	bool raw;
	ysu::transaction & transaction;
	boost::property_tree::ptree & tree;
	ysu::block_hash const & hash;
	ysu::network_params network_params;
	std::vector<ysu::public_key> const & accounts_filter;
};
}

void ysu::json_handler::account_history ()
{
	std::vector<ysu::public_key> accounts_to_filter;
		...
		while (block != nullptr && count > 0)
		{
			...
			else
			{
				...
				if (!entry.empty ())
				{
					...
					if (output_raw)
					{
						entry.put ("work", ysu::to_string_hex (block->block_work ()));
...
void ysu::json_handler::keepalive ()
{
	if (!ec)
	{
		std::string address_text (request.get<std::string> ("address"));
...
void ysu::json_handler::key_create ()
{
	ysu::keypair pair;
...
void ysu::json_handler::key_expand ()
{
	std::string key_text (request.get<std::string> ("key"));
	ysu::private_key prv;
	if (!prv.decode_hex (key_text))
...
void ysu::json_handler::ledger ()
{
	...
	if (!ec)
	{
		...
		if (modified_since_text.is_initialized ())
		{
			...
			{
				ec = ysu::error_rpc::invalid_timestamp;
			}
		}
		const bool sorting = request.get<bool> ("sorting", false);
		const bool representative = request.get<bool> ("representative", false);
		...
		{
			std::vector<std::pair<ysu::uint128_union, ysu::account>> ledger_l;
			for (auto i (node.store.accounts_begin (transaction, start)), n (node.store.accounts_end ()); i != n; ++i)
			{
				ysu::account_info const & info (i->second);
				ysu::uint128_union balance (info.balance);
			...
			for (auto i (ledger_l.begin ()), n (ledger_l.end ()); i != n && accounts.size () < count; ++i)
			{
				...
				{
					ysu::account const & account (i->second);
					boost::property_tree::ptree response_a;
					if (pending)
					{
						auto account_pending (node.ledger.account_pending (transaction, account));
...
void ysu::json_handler::mysu_from_raw (ysu::uint128_t ratio)
{
...
void ysu::json_handler::mysu_to_raw (ysu::uint128_t ratio)
{
	auto amount (amount_impl ());
	if (!ec)
	{
		...
		{
			ec = ysu::error_common::invalid_amount_big;
		}
	}
	response_errors ();
}
 ...
 * @warning This is an internal/diagnostic RPC, do not rely on its interface being stable
 */
void ysu::json_handler::node_id_delete ()
{
	response_l.put ("deprecated", "1");
	response_errors ();
}

void ysu::json_handler::password_change ()
{
	node.worker.push_task (create_worker_task ([](std::shared_ptr<ysu::json_handler> const & rpc_l) {
		auto wallet (rpc_l->wallet_impl ());
		if (!rpc_l->ec)
...
void ysu::json_handler::password_enter ()
{
	node.worker.push_task (create_worker_task ([](std::shared_ptr<ysu::json_handler> const & rpc_l) {
		auto wallet (rpc_l->wallet_impl ());
		if (!rpc_l->ec)
...
void ysu::json_handler::password_valid (bool wallet_locked)
{
	auto wallet (wallet_impl ());
	if (!ec)
	{
...
void ysu::json_handler::peers ()
{
	...
	for (auto i (peers_list.begin ()), n (peers_list.end ()); i != n; ++i)
	{
		...
		if (peer_details)
		{
			...
			pending_tree.put ("type", channel->get_type () == ysu::transport::transport_type::tcp ? "tcp" : "udp");
...
void ysu::json_handler::pending_exists ()
{
	auto hash (hash_impl ());
	const bool include_active = request.get<bool> ("include_active", false);
	const bool include_only_confirmed = request.get<bool> ("include_only_confirmed", false);
...
void ysu::json_handler::payment_begin ()
{
	node.worker.push_task (create_worker_task ([](std::shared_ptr<ysu::json_handler> const & rpc_l) {
		...
		if (!id.decode_hex (id_text))
		{
			...
			{
				auto transaction (rpc_l->node.wallets.tx_begin_write ());
				std::shared_ptr<ysu::wallet> wallet (existing->second);
				if (wallet->store.valid_password (transaction))
				{
					...
					{
						rpc_l->ec = ysu::error_rpc::payment_unable_create_account;
					}
				}
				else
				{
			...
			{
				rpc_l->ec = ysu::error_common::wallet_not_found;
			}
		}
		else
		{
			rpc_l->ec = ysu::error_common::bad_wallet_number;
		}
		rpc_l->response_errors ();
	}));
}

void ysu::json_handler::payment_init ()
{
	node.worker.push_task (create_worker_task ([](std::shared_ptr<ysu::json_handler> const & rpc_l) {
		auto wallet (rpc_l->wallet_impl ());
		if (!rpc_l->ec)
...
void ysu::json_handler::payment_end ()
{
	auto account (account_impl ());
	auto wallet (wallet_impl ());
	if (!ec)
	{
		...
		if (!ec)
		{
			...
			{
				ec = ysu::error_rpc::payment_account_balance;
			}
		}
	}
	response_errors ();
...
void ysu::json_handler::payment_wait ()
{
	...
	if (!ec)
	{
		...
		{
			{
				auto observer (std::make_shared<ysu::json_payment_observer> (node, response, account, amount));
...
void ysu::json_handler::process ()
{
	node.worker.push_task (create_worker_task ([](std::shared_ptr<ysu::json_handler> const & rpc_l) {
		...
		if (!rpc_l->ec && block->type () == ysu::block_type::state)
		{
			std::string subtype_text (rpc_l->request.get<std::string> ("subtype", ""));
			if (!subtype_text.empty ())
			{
				std::shared_ptr<ysu::state_block> block_state (std::static_pointer_cast<ysu::state_block> (block));
				...
				{
					rpc_l->ec = ysu::error_process::gap_previous;
				}
				else
				{
					...
					if (subtype_text == "send")
					{
						...
						{
							rpc_l->ec = ysu::error_rpc::invalid_subtype_balance;
						}
					...
					else if (subtype_text == "open")
					{
						...
						{
							rpc_l->ec = ysu::error_rpc::invalid_subtype_previous;
						}
					}
					...
					else if (subtype_text == "epoch")
					{
						...
						{
							rpc_l->ec = ysu::error_rpc::invalid_subtype_epoch_link;
						}
					}
					else
					{
						rpc_l->ec = ysu::error_rpc::invalid_subtype;
					}
				}
			}
		}
		if (!rpc_l->ec)
		{
			if (!ysu::work_validate_entry (*block))
			{
				...
				switch (result.code)
				{
					...
					case ysu::process_result::fork:
					{
						...
						{
							rpc_l->ec = ysu::error_process::fork;
						}
						break;
					}
					case ysu::process_result::insufficient_work:
					{
						rpc_l->ec = ysu::error_process::insufficient_work;
						break;
					}
					default:
					{
						rpc_l->ec = ysu::error_process::other;
						break;
					}
				}
			}
			...
			{
				rpc_l->ec = ysu::error_blocks::work_low;
			}
		}
		rpc_l->response_errors ();
	}));
...
void ysu::json_handler::receive ()
{
	...
	if (!ec)
	{
		...
		if (!ec)
		{
			...
			if (block != nullptr)
			{
				if (node.store.pending_exists (block_transaction, ysu::pending_key (account, hash)))
				{
					auto work (work_optional_impl ());
					if (!ec && work)
					{
						ysu::account_info info;
						ysu::root head;
						ysu::epoch epoch = block->sideband ().details.epoch;
						...
						ysu::block_details details (epoch, false, true, false);
						if (ysu::work_difficulty (ysu::work_version::work_1, head, work) < ysu::work_threshold (ysu::work_version::work_1, details))
						{
							ec = ysu::error_common::invalid_work;
						}
					}
					else if (!ec) // && work == 0
					{
					...
					if (!ec)
					{
						...
						ysu::account representative (wallet->store.representative (wallet_transaction));
						bool generate_work (work == 0); // Disable work generation if "work" option is provided
						auto response_a (response);
						wallet->receive_async (
						std::move (block), representative, node.network_params.ledger.genesis_amount, [response_a](std::shared_ptr<ysu::block> block_a) {
							if (block_a != nullptr)
							{
								boost::property_tree::ptree response_l;
				...
				{
					ec = ysu::error_process::unreceivable;
				}
			}
			else
			{
...
void ysu::json_handler::receive_minimum ()
{
	if (!ec)
	{
...
void ysu::json_handler::receive_minimum_set ()
{
	auto amount (amount_impl ());
	if (!ec)
	{
...
void ysu::json_handler::representatives ()
{
	auto count (count_optional_impl ());
	if (!ec)
	{
		...
		if (!sorting) // Simple
		{
			std::map<ysu::account, ysu::uint128_t> ordered (rep_amounts.begin (), rep_amounts.end ());
		...
		{
			std::vector<std::pair<ysu::uint128_t, std::string>> representation;
...
void ysu::json_handler::representatives_online ()
{
...
void ysu::json_handler::republish ()
{
	...
	if (!ec && sources_text.is_initialized ())
	{
		...
		{
			ec = ysu::error_rpc::invalid_sources;
		}
	}
	...
	if (!ec && destinations_text.is_initialized ())
	{
		...
		{
			ec = ysu::error_rpc::invalid_destinations;
		}
	}
	auto hash (hash_impl ());
	if (!ec)
	{
		...
		if (block != nullptr)
		{
			std::deque<std::shared_ptr<ysu::block>> republish_bundle;
			for (auto i (0); !hash.is_zero () && i < count; ++i)
			{
				block = node.store.block_get (transaction, hash);
				if (sources != 0) // Republish source chain
				{
					ysu::block_hash source (node.ledger.block_source (transaction, *block));
				...
				if (destinations != 0) // Republish destination chain
				{
					...
					if (!destination.is_zero ())
					{
						if (!node.store.pending_exists (transaction, ysu::pending_key (destination, hash)))
						{
							ysu::block_hash previous (node.ledger.latest (transaction, destination));
...
void ysu::json_handler::search_pending ()
{
	auto wallet (wallet_impl ());
	if (!ec)
	{
...
void ysu::json_handler::search_pending_all ()
{
	if (!ec)
	{
...
void ysu::json_handler::send ()
{
	...
	auto source (account_impl (source_text, ysu::error_rpc::bad_source));
	std::string destination_text (request.get<std::string> ("destination"));
	auto destination (account_impl (destination_text, ysu::error_rpc::bad_destination));
	if (!ec)
	{
		auto work (work_optional_impl ());
		ysu::uint128_t balance (0);
		...
		if (!ec)
		{
			...
			if (!ec && work)
			{
				ysu::block_details details (info.epoch (), true, false, false);
				if (ysu::work_difficulty (ysu::work_version::work_1, info.head, work) < ysu::work_threshold (ysu::work_version::work_1, details))
				{
					ec = ysu::error_common::invalid_work;
				}
		...
		if (!ec)
		{
			...
			wallet->send_async (
			source, destination, amount.number (), [balance, amount, response_a, response_data](std::shared_ptr<ysu::block> block_a) {
				if (block_a != nullptr)
				{
				...
				else
				{
					...
					{
						std::error_code ec (ysu::error_common::insufficient_balance);
...
void ysu::json_handler::sign ()
{
	...
	{
		ec = ysu::error_rpc::sign_hash_disabled;
	}
	if (!ec)
	{
		ysu::raw_key prv;
		...
		if (prv.data != 0)
		{
			...
			ysu::signature signature (ysu::sign_message (prv, pub, hash));
...
void ysu::json_handler::stats ()
{
	...
	{
		ec = ysu::error_rpc::invalid_missing_type;
	}
	if (!ec && use_sink)
	{
...
void ysu::json_handler::stats_clear ()
{
	node.stats.clear ();
	response_l.put ("success", "");
	std::stringstream ostream;
...
void ysu::json_handler::telemetry ()
{
	auto rpc_l (shared_from_this ());
	...
	if (address_text.is_initialized () || port_text.is_initialized ())
	{
		...
		if (address_text.is_initialized () && port_text.is_initialized ())
		{
			...
			if (!ysu::parse_port (*port_text, port))
			{
				...
				if (!ysu::parse_address (*address_text, address))
				{
					ysu::endpoint endpoint (address, port);
					if (address.is_loopback () && port == rpc_l->node.network.endpoint ().port ())
					{
						// Requesting telemetry metrics locally
						auto telemetry_data = ysu::local_telemetry_data (rpc_l->node.store, rpc_l->node.ledger.cache, rpc_l->node.network, rpc_l->node.config.bandwidth_limit, rpc_l->node.network_params, rpc_l->node.startup_time, rpc_l->node.active.active_difficulty (), rpc_l->node.node_id);

						ysu::jsonconfig config_l;
					...
					{
						channel = node.network.find_channel (ysu::transport::map_endpoint_to_v6 (endpoint));
						if (!channel)
						{
							ec = ysu::error_rpc::peer_not_found;
						}
					}
				}
				else
		...
		{
			ec = ysu::error_rpc::requires_port_and_address;
		}
			...
			if (node.telemetry)
			{
				node.telemetry->get_metrics_single_peer_async (channel, [rpc_l](auto const & telemetry_response_a) {
					if (!telemetry_response_a.error)
					{
						...
						{
							rpc_l->ec = ysu::error_rpc::generic;
						}
					}
					else
					{
	...
	else
	{
		...
		if (node.telemetry)
		{
			...
			{
				ysu::jsonconfig config_l;
				std::vector<ysu::telemetry_data> telemetry_datas;
				...
				auto average_telemetry_metrics = ysu::consolidate_telemetry_data (telemetry_datas);
...
void ysu::json_handler::unchecked ()
{
	const bool json_block_l = request.get<bool> ("json_block", false);
	auto count (count_optional_impl ());
	if (!ec)
...
void ysu::json_handler::unchecked_clear ()
{
	node.worker.push_task (create_worker_task ([](std::shared_ptr<ysu::json_handler> const & rpc_l) {
...
void ysu::json_handler::unchecked_get ()
{
	const bool json_block_l = request.get<bool> ("json_block", false);
	auto hash (hash_impl ());
	if (!ec)
...
void ysu::json_handler::unchecked_keys ()
{
	const bool json_block_l = request.get<bool> ("json_block", false);
	auto count (count_optional_impl ());
	ysu::block_hash key (0);
	...
	if (!ec)
	{
		boost::property_tree::ptree unchecked;
		auto transaction (node.store.tx_begin_read ());
		for (auto i (node.store.unchecked_begin (transaction, ysu::unchecked_key (key, 0))), n (node.store.unchecked_end ()); i != n && unchecked.size () < count; ++i)
		{
			boost::property_tree::ptree entry;
			ysu::unchecked_info const & info (i->second);
...
void ysu::json_handler::unopened ()
{
	auto count (count_optional_impl ());
	auto threshold (threshold_optional_impl ());
	ysu::account start (1); // exclude burn account by default
	boost::optional<std::string> account_text (request.get_optional<std::string> ("account"));
	if (account_text.is_initialized ())
	{
		start = account_impl (account_text.get ());
	...
	{
		auto transaction (node.store.tx_begin_read ());
		auto iterator (node.store.pending_begin (transaction, ysu::pending_key (start, 0)));
		auto end (node.store.pending_end ());
		ysu::account current_account (start);
		ysu::uint128_t current_account_sum{ 0 };
		boost::property_tree::ptree accounts;
		while (iterator != end && accounts.size () < count)
		{
			ysu::pending_key key (iterator->first);
			ysu::account account (key.account);
			ysu::pending_info info (iterator->second);
			if (node.store.account_exists (transaction, account))
			{
				if (account.number () == std::numeric_limits<ysu::uint256_t>::max ())
				{
					break;
				}
				// Skip existing accounts
				iterator = node.store.pending_begin (transaction, ysu::pending_key (account.number () + 1, 0));
			}
			else
			{
...
void ysu::json_handler::validate_account_number ()
{
	auto account (account_impl ());
	(void)account;
...
void ysu::json_handler::wallet_add ()
{
	node.worker.push_task (create_worker_task ([](std::shared_ptr<ysu::json_handler> const & rpc_l) {
		auto wallet (rpc_l->wallet_impl ());
		if (!rpc_l->ec)
		{
			...
			{
				rpc_l->ec = ysu::error_common::bad_private_key;
			}
		}
		rpc_l->response_errors ();
	}));
...
void ysu::json_handler::wallet_add_watch ()
{
	node.worker.push_task (create_worker_task ([](std::shared_ptr<ysu::json_handler> const & rpc_l) {
		auto wallet (rpc_l->wallet_impl ());
		if (!rpc_l->ec)
			...
			if (wallet->store.valid_password (transaction))
			{
				for (auto & accounts : rpc_l->request.get_child ("accounts"))
				{
					...
					if (!rpc_l->ec)
					{
						...
						{
							rpc_l->ec = ysu::error_common::bad_public_key;
						}
					}
				}
				if (!rpc_l->ec)
...
void ysu::json_handler::wallet_info ()
{
	auto wallet (wallet_impl ());
	if (!ec)
	{
		ysu::uint128_t balance (0);
		ysu::uint128_t pending (0);
		uint64_t count (0);
		...
		for (auto i (wallet->store.begin (transaction)), n (wallet->store.end ()); i != n; ++i)
		{
			...
			ysu::key_type key_type (wallet->store.key_type (i->second));
			if (key_type == ysu::key_type::deterministic)
			{
				deterministic_count++;
			}
			else if (key_type == ysu::key_type::adhoc)
			{
				adhoc_count++;
			}
			count++;
...
void ysu::json_handler::wallet_balances ()
{
	auto wallet (wallet_impl ());
	auto threshold (threshold_optional_impl ());
	if (!ec)
	{
		...
		{
			ysu::account const & account (i->first);
			ysu::uint128_t balance = node.ledger.account_balance (block_transaction, account);
			if (balance >= threshold.number ())
			{
				boost::property_tree::ptree entry;
				ysu::uint128_t pending = node.ledger.account_pending (block_transaction, account);
				entry.put ("balance", balance.convert_to<std::string> ());
...
void ysu::json_handler::wallet_change_seed ()
{
	node.worker.push_task (create_worker_task ([](std::shared_ptr<ysu::json_handler> const & rpc_l) {
		auto wallet (rpc_l->wallet_impl ());
		if (!rpc_l->ec)
		{
			...
			if (!seed.data.decode_hex (seed_text))
			{
				...
				if (wallet->store.valid_password (transaction))
				{
					ysu::public_key account (wallet->change_seed (transaction, seed, count));
					rpc_l->response_l.put ("success", "");
			...
			{
				rpc_l->ec = ysu::error_common::bad_seed;
			}
		}
		rpc_l->response_errors ();
	}));
...
void ysu::json_handler::wallet_contains ()
{
	auto account (account_impl ());
	auto wallet (wallet_impl ());
	if (!ec)
...
void ysu::json_handler::wallet_create ()
{
	node.worker.push_task (create_worker_task ([](std::shared_ptr<ysu::json_handler> const & rpc_l) {
		ysu::raw_key seed;
		...
		if (!rpc_l->ec)
		{
			...
			{
				rpc_l->ec = ysu::error_common::wallet_lmdb_max_dbs;
			}
			if (!rpc_l->ec && seed_text.is_initialized ())
			{
				auto transaction (rpc_l->node.wallets.tx_begin_write ());
				ysu::public_key account (wallet->change_seed (transaction, seed));
				rpc_l->response_l.put ("last_restored_account", account.to_account ());
				auto index (wallet->store.deterministic_index_get (transaction));
				debug_assert (index > 0);
				rpc_l->response_l.put ("restored_count", std::to_string (index));
...
void ysu::json_handler::wallet_destroy ()
{
	node.worker.push_task (create_worker_task ([](std::shared_ptr<ysu::json_handler> const & rpc_l) {
...
void ysu::json_handler::wallet_export ()
{
	auto wallet (wallet_impl ());
	if (!ec)
	{
...
void ysu::json_handler::wallet_frontiers ()
{
	auto wallet (wallet_impl ());
	if (!ec)
	{
...
void ysu::json_handler::wallet_history ()
{
		...
		for (auto i (wallet->store.begin (transaction)), n (wallet->store.end ()); i != n; ++i)
		{
			...
			if (!node.store.account_get (block_transaction, account, info))
			{
				...
				while (timestamp >= modified_since && !hash.is_zero ())
				{
					...
					{
						boost::property_tree::ptree entry;
						std::vector<ysu::public_key> no_filter;
...
void ysu::json_handler::wallet_key_valid ()
{
	auto wallet (wallet_impl ());
	if (!ec)
	{
...
void ysu::json_handler::wallet_ledger ()
{
	const bool representative = request.get<bool> ("representative", false);
	const bool weight = request.get<bool> ("weight", false);
	const bool pending = request.get<bool> ("pending", false);
...
void ysu::json_handler::wallet_lock ()
{
	auto wallet (wallet_impl ());
	if (!ec)
	{
...
void ysu::json_handler::wallet_pending ()
{
	auto wallet (wallet_impl ());
	auto count (count_optional_impl ());
	auto threshold (threshold_optional_impl ());
	...
	if (!ec)
	{
		...
		for (auto i (wallet->store.begin (transaction)), n (wallet->store.end ()); i != n; ++i)
		{
			ysu::account const & account (i->first);
			boost::property_tree::ptree peers_l;
			for (auto ii (node.store.pending_begin (block_transaction, ysu::pending_key (account, 0))), nn (node.store.pending_end ()); ii != nn && ysu::pending_key (ii->first).account == account && peers_l.size () < count; ++ii)
			{
				ysu::pending_key key (ii->first);
				if (block_confirmed (node, block_transaction, key.hash, include_active, include_only_confirmed))
				{
					...
					{
						ysu::pending_info info (ii->second);
						if (info.amount.number () >= threshold.number ())
						{
							if (source || min_version)
							{
...
void ysu::json_handler::wallet_representative ()
{
	auto wallet (wallet_impl ());
	if (!ec)
	{
...
void ysu::json_handler::wallet_representative_set ()
{
	node.worker.push_task (create_worker_task ([](std::shared_ptr<ysu::json_handler> const & rpc_l) {
		auto wallet (rpc_l->wallet_impl ());
		std::string representative_text (rpc_l->request.get<std::string> ("representative"));
		...
		if (!rpc_l->ec)
		{
			...
			if (!rpc_l->ec && update_existing_accounts)
			{
				...
				{
					wallet->change_async (
					account, representative, [](std::shared_ptr<ysu::block>) {}, 0, false);
				}
			}
		}
		rpc_l->response_errors ();
...
void ysu::json_handler::wallet_republish ()
{
	auto wallet (wallet_impl ());
	auto count (count_impl ());
	if (!ec)
...
void ysu::json_handler::wallet_seed ()
{
	auto wallet (wallet_impl ());
	if (!ec)
	{
...
void ysu::json_handler::wallet_work_get ()
{
	auto wallet (wallet_impl ());
	if (!ec)
	{
		...
		for (auto i (wallet->store.begin (transaction)), n (wallet->store.end ()); i != n; ++i)
		{
			...
			(void)error_work;
			works.put (account.to_account (), ysu::to_string_hex (work));
		}
...
void ysu::json_handler::work_generate ()
{
	boost::optional<ysu::account> account;
	...
	if (!ec)
	{
		...
		if (!ec && (difficulty > node.max_work_generate_difficulty (work_version) || difficulty < ysu::work_threshold_entry (work_version, ysu::block_type::state)))
		{
			ec = ysu::error_rpc::difficulty_limit;
		}
		...
		if (!ec && request.count ("block"))
		{
			...
			if (block != nullptr)
			{
				if (hash != block->root ())
				{
					ec = ysu::error_rpc::block_root_mismatch;
				}
				...
				{
					ec = ysu::error_rpc::block_work_version_mismatch;
				}
				// Difficulty calculation
				...
				{
					ec = ysu::error_rpc::block_work_enough;
				}
			}
		}
		if (!ec && response_l.empty ())
		{
			...
			auto callback = [rpc_l, hash, work_version, this](boost::optional<uint64_t> const & work_a) {
				if (work_a)
				{
					...
					response_l.put ("work", ysu::to_string_hex (work));
					std::stringstream ostream;
					auto result_difficulty (ysu::work_difficulty (work_version, hash, work));
					response_l.put ("difficulty", ysu::to_string_hex (result_difficulty));
					auto result_multiplier = ysu::difficulty::to_multiplier (result_difficulty, node.default_difficulty (work_version));
					response_l.put ("multiplier", ysu::to_string (result_multiplier));
					boost::property_tree::write_json (ostream, response_l);
					rpc_l->response (ostream.str ());
				}
				else
			...
			if (!use_peers)
			{
				if (node.local_work_generation_enabled ())
				{
					...
					if (error)
					{
						ec = ysu::error_common::failure_work_generation;
					}
				}
				else
				{
					ec = ysu::error_common::disabled_local_work_generation;
				}
			}
			else
			{
...
void ysu::json_handler::work_cancel ()
{
	auto hash (hash_impl ());
	if (!ec)
	{
...
void ysu::json_handler::work_get ()
{
	auto wallet (wallet_impl ());
	auto account (account_impl ());
	if (!ec)
...
void ysu::json_handler::work_set ()
{
	node.worker.push_task (create_worker_task ([](std::shared_ptr<ysu::json_handler> const & rpc_l) {
		auto wallet (rpc_l->wallet_impl ());
		auto account (rpc_l->account_impl ());
...
void ysu::json_handler::work_validate ()
{
	auto hash (hash_impl ());
	auto work (work_optional_impl ());
	// Default to work_1 if not specified
	...
	if (!ec)
	{
		...
		response_l.put ("valid_receive", (result_difficulty >= ysu::work_threshold (work_version, ysu::block_details (ysu::epoch::epoch_2, false, true, false))) ? "1" : "0");
		response_l.put ("difficulty", ysu::to_string_hex (result_difficulty));
		auto result_multiplier = ysu::difficulty::to_multiplier (result_difficulty, node.default_difficulty (work_version));
		response_l.put ("multiplier", ysu::to_string (result_multiplier));
	}
...
void ysu::json_handler::work_peer_add ()
{
	std::string address_text = request.get<std::string> ("address");
	std::string port_text = request.get<std::string> ("port");
	uint16_t port;
...
void ysu::json_handler::work_peers ()
{
...
void ysu::json_handler::work_peers_clear ()
{
...
void ysu::inprocess_rpc_handler::process_request (std::string const &, std::string const & body_a, std::function<void(std::string const &)> response_a)
{
	// Note that if the rpc action is async, the shared_ptr<json_handler> lifetime will be extended by the action handler
	auto handler (std::make_shared<ysu::json_handler> (node, node_rpc_config, body_a, response_a, [this]() {
		this->stop_callback ();
		this->stop ();
	}));
...
void ysu::inprocess_rpc_handler::process_request_v2 (rpc_handler_request_params const & params_a, std::string const & body_a, std::function<void(std::shared_ptr<std::string>)> response_a)
{
	std::string body_l = params_a.json_envelope (body_a);
	auto handler (std::make_shared<ysu::ipc::flatbuffers_handler> (node, ipc_server, nullptr, node.config.ipc_config));
...
void construct_json (ysu::container_info_component * component, boost::property_tree::ptree & parent)
{
	...
	{
		auto & leaf_info = static_cast<ysu::container_info_leaf *> (component)->get_info ();
	...
	auto composite = static_cast<ysu::container_info_composite *> (component);
...
ipc_json_handler_no_arg_func_map create_ipc_json_handler_no_arg_func_map ()
{
	...
	no_arg_funcs.emplace ("account_balance", &ysu::json_handler::account_balance);
	no_arg_funcs.emplace ("account_block_count", &ysu::json_handler::account_block_count);
	no_arg_funcs.emplace ("account_count", &ysu::json_handler::account_count);
	no_arg_funcs.emplace ("account_create", &ysu::json_handler::account_create);
	no_arg_funcs.emplace ("account_get", &ysu::json_handler::account_get);
	no_arg_funcs.emplace ("account_history", &ysu::json_handler::account_history);
	no_arg_funcs.emplace ("account_info", &ysu::json_handler::account_info);
	no_arg_funcs.emplace ("account_key", &ysu::json_handler::account_key);
	no_arg_funcs.emplace ("account_list", &ysu::json_handler::account_list);
	no_arg_funcs.emplace ("account_move", &ysu::json_handler::account_move);
	no_arg_funcs.emplace ("account_remove", &ysu::json_handler::account_remove);
	no_arg_funcs.emplace ("account_representative", &ysu::json_handler::account_representative);
	no_arg_funcs.emplace ("account_representative_set", &ysu::json_handler::account_representative_set);
	no_arg_funcs.emplace ("account_weight", &ysu::json_handler::account_weight);
	no_arg_funcs.emplace ("accounts_balances", &ysu::json_handler::accounts_balances);
	no_arg_funcs.emplace ("accounts_create", &ysu::json_handler::accounts_create);
	no_arg_funcs.emplace ("accounts_frontiers", &ysu::json_handler::accounts_frontiers);
	no_arg_funcs.emplace ("accounts_pending", &ysu::json_handler::accounts_pending);
	no_arg_funcs.emplace ("active_difficulty", &ysu::json_handler::active_difficulty);
	no_arg_funcs.emplace ("available_supply", &ysu::json_handler::available_supply);
	no_arg_funcs.emplace ("block_info", &ysu::json_handler::block_info);
	no_arg_funcs.emplace ("block", &ysu::json_handler::block_info);
	no_arg_funcs.emplace ("block_confirm", &ysu::json_handler::block_confirm);
	no_arg_funcs.emplace ("blocks", &ysu::json_handler::blocks);
	no_arg_funcs.emplace ("blocks_info", &ysu::json_handler::blocks_info);
	no_arg_funcs.emplace ("block_account", &ysu::json_handler::block_account);
	no_arg_funcs.emplace ("block_count", &ysu::json_handler::block_count);
	no_arg_funcs.emplace ("block_create", &ysu::json_handler::block_create);
	no_arg_funcs.emplace ("block_hash", &ysu::json_handler::block_hash);
	no_arg_funcs.emplace ("bootstrap", &ysu::json_handler::bootstrap);
	no_arg_funcs.emplace ("bootstrap_any", &ysu::json_handler::bootstrap_any);
	no_arg_funcs.emplace ("bootstrap_lazy", &ysu::json_handler::bootstrap_lazy);
	no_arg_funcs.emplace ("bootstrap_status", &ysu::json_handler::bootstrap_status);
	no_arg_funcs.emplace ("confirmation_active", &ysu::json_handler::confirmation_active);
	no_arg_funcs.emplace ("confirmation_height_currently_processing", &ysu::json_handler::confirmation_height_currently_processing);
	no_arg_funcs.emplace ("confirmation_history", &ysu::json_handler::confirmation_history);
	no_arg_funcs.emplace ("confirmation_info", &ysu::json_handler::confirmation_info);
	no_arg_funcs.emplace ("confirmation_quorum", &ysu::json_handler::confirmation_quorum);
	no_arg_funcs.emplace ("database_txn_tracker", &ysu::json_handler::database_txn_tracker);
	no_arg_funcs.emplace ("delegators", &ysu::json_handler::delegators);
	no_arg_funcs.emplace ("delegators_count", &ysu::json_handler::delegators_count);
	no_arg_funcs.emplace ("deterministic_key", &ysu::json_handler::deterministic_key);
	no_arg_funcs.emplace ("epoch_upgrade", &ysu::json_handler::epoch_upgrade);
	no_arg_funcs.emplace ("frontiers", &ysu::json_handler::frontiers);
	no_arg_funcs.emplace ("frontier_count", &ysu::json_handler::account_count);
	no_arg_funcs.emplace ("keepalive", &ysu::json_handler::keepalive);
	no_arg_funcs.emplace ("key_create", &ysu::json_handler::key_create);
	no_arg_funcs.emplace ("key_expand", &ysu::json_handler::key_expand);
	no_arg_funcs.emplace ("ledger", &ysu::json_handler::ledger);
	no_arg_funcs.emplace ("node_id", &ysu::json_handler::node_id);
	no_arg_funcs.emplace ("node_id_delete", &ysu::json_handler::node_id_delete);
	no_arg_funcs.emplace ("password_change", &ysu::json_handler::password_change);
	no_arg_funcs.emplace ("password_enter", &ysu::json_handler::password_enter);
	no_arg_funcs.emplace ("wallet_unlock", &ysu::json_handler::password_enter);
	no_arg_funcs.emplace ("payment_begin", &ysu::json_handler::payment_begin);
	no_arg_funcs.emplace ("payment_init", &ysu::json_handler::payment_init);
	no_arg_funcs.emplace ("payment_end", &ysu::json_handler::payment_end);
	no_arg_funcs.emplace ("payment_wait", &ysu::json_handler::payment_wait);
	no_arg_funcs.emplace ("peers", &ysu::json_handler::peers);
	no_arg_funcs.emplace ("pending", &ysu::json_handler::pending);
	no_arg_funcs.emplace ("pending_exists", &ysu::json_handler::pending_exists);
	no_arg_funcs.emplace ("process", &ysu::json_handler::process);
	no_arg_funcs.emplace ("receive", &ysu::json_handler::receive);
	no_arg_funcs.emplace ("receive_minimum", &ysu::json_handler::receive_minimum);
	no_arg_funcs.emplace ("receive_minimum_set", &ysu::json_handler::receive_minimum_set);
	no_arg_funcs.emplace ("representatives", &ysu::json_handler::representatives);
	no_arg_funcs.emplace ("representatives_online", &ysu::json_handler::representatives_online);
	no_arg_funcs.emplace ("republish", &ysu::json_handler::republish);
	no_arg_funcs.emplace ("search_pending", &ysu::json_handler::search_pending);
	no_arg_funcs.emplace ("search_pending_all", &ysu::json_handler::search_pending_all);
	no_arg_funcs.emplace ("send", &ysu::json_handler::send);
	no_arg_funcs.emplace ("sign", &ysu::json_handler::sign);
	no_arg_funcs.emplace ("stats", &ysu::json_handler::stats);
	no_arg_funcs.emplace ("stats_clear", &ysu::json_handler::stats_clear);
	no_arg_funcs.emplace ("stop", &ysu::json_handler::stop);
	no_arg_funcs.emplace ("telemetry", &ysu::json_handler::telemetry);
	no_arg_funcs.emplace ("unchecked", &ysu::json_handler::unchecked);
	no_arg_funcs.emplace ("unchecked_clear", &ysu::json_handler::unchecked_clear);
	no_arg_funcs.emplace ("unchecked_get", &ysu::json_handler::unchecked_get);
	no_arg_funcs.emplace ("unchecked_keys", &ysu::json_handler::unchecked_keys);
	no_arg_funcs.emplace ("unopened", &ysu::json_handler::unopened);
	no_arg_funcs.emplace ("uptime", &ysu::json_handler::uptime);
	no_arg_funcs.emplace ("validate_account_number", &ysu::json_handler::validate_account_number);
	no_arg_funcs.emplace ("version", &ysu::json_handler::version);
	no_arg_funcs.emplace ("wallet_add", &ysu::json_handler::wallet_add);
	no_arg_funcs.emplace ("wallet_add_watch", &ysu::json_handler::wallet_add_watch);
	no_arg_funcs.emplace ("wallet_balances", &ysu::json_handler::wallet_balances);
	no_arg_funcs.emplace ("wallet_change_seed", &ysu::json_handler::wallet_change_seed);
	no_arg_funcs.emplace ("wallet_contains", &ysu::json_handler::wallet_contains);
	no_arg_funcs.emplace ("wallet_create", &ysu::json_handler::wallet_create);
	no_arg_funcs.emplace ("wallet_destroy", &ysu::json_handler::wallet_destroy);
	no_arg_funcs.emplace ("wallet_export", &ysu::json_handler::wallet_export);
	no_arg_funcs.emplace ("wallet_frontiers", &ysu::json_handler::wallet_frontiers);
	no_arg_funcs.emplace ("wallet_history", &ysu::json_handler::wallet_history);
	no_arg_funcs.emplace ("wallet_info", &ysu::json_handler::wallet_info);
	no_arg_funcs.emplace ("wallet_balance_total", &ysu::json_handler::wallet_info);
	no_arg_funcs.emplace ("wallet_key_valid", &ysu::json_handler::wallet_key_valid);
	no_arg_funcs.emplace ("wallet_ledger", &ysu::json_handler::wallet_ledger);
	no_arg_funcs.emplace ("wallet_lock", &ysu::json_handler::wallet_lock);
	no_arg_funcs.emplace ("wallet_pending", &ysu::json_handler::wallet_pending);
	no_arg_funcs.emplace ("wallet_representative", &ysu::json_handler::wallet_representative);
	no_arg_funcs.emplace ("wallet_representative_set", &ysu::json_handler::wallet_representative_set);
	no_arg_funcs.emplace ("wallet_republish", &ysu::json_handler::wallet_republish);
	no_arg_funcs.emplace ("wallet_work_get", &ysu::json_handler::wallet_work_get);
	no_arg_funcs.emplace ("work_generate", &ysu::json_handler::work_generate);
	no_arg_funcs.emplace ("work_cancel", &ysu::json_handler::work_cancel);
	no_arg_funcs.emplace ("work_get", &ysu::json_handler::work_get);
	no_arg_funcs.emplace ("work_set", &ysu::json_handler::work_set);
	no_arg_funcs.emplace ("work_validate", &ysu::json_handler::work_validate);
	no_arg_funcs.emplace ("work_peer_add", &ysu::json_handler::work_peer_add);
	no_arg_funcs.emplace ("work_peers", &ysu::json_handler::work_peers);
	no_arg_funcs.emplace ("work_peers_clear", &ysu::json_handler::work_peers_clear);
	return no_arg_funcs;
}
...
const char * epoch_as_string (ysu::epoch epoch)
{
	switch (epoch)
	{
		case ysu::epoch::epoch_2:
			return "2";
		case ysu::epoch::epoch_1:
			return "1";
		default:
			return "0";
	}
