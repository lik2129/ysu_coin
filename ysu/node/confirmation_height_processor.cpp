...
ysu::confirmation_height_processor::confirmation_height_processor (ysu::ledger & ledger_a, ysu::write_database_queue & write_database_queue_a, std::chrono::milliseconds batch_separate_pending_min_time_a, ysu::logging const & logging_a, ysu::logger_mt & logger_a, boost::latch & latch, confirmation_height_mode mode_a) :
ledger (ledger_a),
...
thread ([this, &latch, mode_a]() {
	ysu::thread_role::set (ysu::thread_role::name::confirmation_height_processing);
...
void ysu::confirmation_height_processor::stop ()
{
	{
...
void ysu::confirmation_height_processor::run (confirmation_height_mode mode_a)
{
...
void ysu::confirmation_height_processor::pause ()
{
	ysu::lock_guard<std::mutex> lk (mutex);
	paused = true;
}

void ysu::confirmation_height_processor::unpause ()
{
	{
		ysu::lock_guard<std::mutex> lk (mutex);
		paused = false;
...
void ysu::confirmation_height_processor::add (ysu::block_hash const & hash_a)
{
	{
		ysu::lock_guard<std::mutex> lk (mutex);
...
void ysu::confirmation_height_processor::set_next_hash ()
{
	ysu::lock_guard<std::mutex> guard (mutex);
...
void ysu::confirmation_height_processor::add_cemented_observer (std::function<void(std::shared_ptr<ysu::block>)> const & callback_a)
{
	cemented_observers.push_back (callback_a);
}

// Not thread-safe, only call before this processor has begun cementing
void ysu::confirmation_height_processor::add_block_already_cemented_observer (std::function<void(ysu::block_hash const &)> const & callback_a)
{
	block_already_cemented_observers.push_back (callback_a);
}

void ysu::confirmation_height_processor::notify_observers (std::vector<std::shared_ptr<ysu::block>> const & cemented_blocks)
{
...
void ysu::confirmation_height_processor::notify_observers (ysu::block_hash const & hash_already_cemented_a)
{
...
std::unique_ptr<ysu::container_info_component> ysu::collect_container_info (confirmation_height_processor & confirmation_height_processor_a, const std::string & name_a)
{
...
size_t ysu::confirmation_height_processor::awaiting_processing_size () const
{
	ysu::lock_guard<std::mutex> guard (mutex);
	return awaiting_processing.size ();
}

bool ysu::confirmation_height_processor::is_processing_added_block (ysu::block_hash const & hash_a) const
{
	ysu::lock_guard<std::mutex> guard (mutex);
	return original_hashes_pending.count (hash_a) > 0 || awaiting_processing.get<tag_hash> ().count (hash_a) > 0;
}

bool ysu::confirmation_height_processor::is_processing_block (ysu::block_hash const & hash_a) const
{
	return is_processing_added_block (hash_a) || unbounded_processor.has_iterated_over_block (hash_a);
}

ysu::block_hash ysu::confirmation_height_processor::current () const
{
	ysu::lock_guard<std::mutex> lk (mutex);
