...
ysu::transport::channel_udp::channel_udp (ysu::transport::udp_channels & channels_a, ysu::endpoint const & endpoint_a, uint8_t protocol_version_a) :
...
size_t ysu::transport::channel_udp::hash_code () const
{
	std::hash<::ysu::endpoint> hash;
	return hash (endpoint);
}

bool ysu::transport::channel_udp::operator== (ysu::transport::channel const & other_a) const
{
	bool result (false);
	auto other_l (dynamic_cast<ysu::transport::channel_udp const *> (&other_a));
	if (other_l != nullptr)
	{
		return *this == *other_l;
	}
...
void ysu::transport::channel_udp::send_buffer (ysu::shared_const_buffer const & buffer_a, std::function<void(boost::system::error_code const &, size_t)> const & callback_a, ysu::buffer_drop_policy drop_policy_a)
{
	set_last_packet_sent (std::chrono::steady_clock::now ());
	channels.send (buffer_a, endpoint, [node = std::weak_ptr<ysu::node> (channels.node.shared ()), callback_a](boost::system::error_code const & ec, size_t size_a) {
		if (auto node_l = node.lock ())
		{
			...
			if (size_a > 0)
			{
				node_l->stats.add (ysu::stat::type::traffic_udp, ysu::stat::dir::out, size_a);
			}
...
std::string ysu::transport::channel_udp::to_string () const
{
	return boost::str (boost::format ("%1%") % endpoint);
}

ysu::transport::udp_channels::udp_channels (ysu::node & node_a, uint16_t port_a) :
node (node_a),
strand (node_a.io_ctx.get_executor ())
{
	if (!node.flags.disable_udp)
	{
		socket = std::make_unique<boost::asio::ip::udp::socket> (node_a.io_ctx, ysu::endpoint (boost::asio::ip::address_v6::any (), port_a));
		...
		local_endpoint = ysu::endpoint (boost::asio::ip::address_v6::loopback (), port);
	}
	else
	{
		local_endpoint = ysu::endpoint (boost::asio::ip::address_v6::loopback (), 0);
		stopped = true;
	}
}

void ysu::transport::udp_channels::send (ysu::shared_const_buffer const & buffer_a, ysu::endpoint endpoint_a, std::function<void(boost::system::error_code const &, size_t)> const & callback_a)
{
...
std::shared_ptr<ysu::transport::channel_udp> ysu::transport::udp_channels::insert (ysu::endpoint const & endpoint_a, unsigned network_version_a)
{
	debug_assert (endpoint_a.address ().is_v6 ());
	std::shared_ptr<ysu::transport::channel_udp> result;
	if (!node.network.not_a_peer (endpoint_a, node.config.allow_local_peers) && (node.network_params.network.is_dev_network () || !max_ip_connections (endpoint_a)))
	{
		...
		{
			result = std::make_shared<ysu::transport::channel_udp> (*this, endpoint_a, network_version_a);
...
void ysu::transport::udp_channels::erase (ysu::endpoint const & endpoint_a)
{
	ysu::lock_guard<std::mutex> lock (mutex);
	channels.get<endpoint_tag> ().erase (endpoint_a);
}

size_t ysu::transport::udp_channels::size () const
{
	ysu::lock_guard<std::mutex> lock (mutex);
	return channels.size ();
}

std::shared_ptr<ysu::transport::channel_udp> ysu::transport::udp_channels::channel (ysu::endpoint const & endpoint_a) const
{
	ysu::lock_guard<std::mutex> lock (mutex);
	std::shared_ptr<ysu::transport::channel_udp> result;
	auto existing (channels.get<endpoint_tag> ().find (endpoint_a));
...
std::unordered_set<std::shared_ptr<ysu::transport::channel>> ysu::transport::udp_channels::random_set (size_t count_a, uint8_t min_version) const
{
...
void ysu::transport::udp_channels::random_fill (std::array<ysu::endpoint, 8> & target_a) const
{
...
bool ysu::transport::udp_channels::store_all (bool clear_peers)
{
...
std::shared_ptr<ysu::transport::channel_udp> ysu::transport::udp_channels::find_node_id (ysu::account const & node_id_a)
{
	std::shared_ptr<ysu::transport::channel_udp> result;
	ysu::lock_guard<std::mutex> lock (mutex);
...
void ysu::transport::udp_channels::clean_node_id (ysu::account const & node_id_a)
{
	ysu::lock_guard<std::mutex> lock (mutex);
	channels.get<node_id_tag> ().erase (node_id_a);
}

void ysu::transport::udp_channels::clean_node_id (ysu::endpoint const & endpoint_a, ysu::account const & node_id_a)
{
	ysu::lock_guard<std::mutex> lock (mutex);
...
ysu::tcp_endpoint ysu::transport::udp_channels::bootstrap_peer (uint8_t connection_protocol_version_min)
{
	...
	for (auto i (channels.get<last_bootstrap_attempt_tag> ().begin ()), n (channels.get<last_bootstrap_attempt_tag> ().end ()); i != n;)
	{
		...
		{
			result = ysu::transport::map_endpoint_to_tcp (i->endpoint ());
...
void ysu::transport::udp_channels::receive ()
{
	if (!stopped)
	{
		...
		socket->async_receive_from (boost::asio::buffer (data->buffer, ysu::network::buffer_size), data->endpoint,
...
void ysu::transport::udp_channels::start ()
{
...
void ysu::transport::udp_channels::stop ()
{
...
void ysu::transport::udp_channels::close_socket ()
{
...
ysu::endpoint ysu::transport::udp_channels::get_local_endpoint () const
{
	ysu::lock_guard<std::mutex> lock (mutex);
...
namespace
{
class udp_message_visitor : public ysu::message_visitor
{
public:
	udp_message_visitor (ysu::node & node_a, ysu::endpoint const & endpoint_a) :
	node (node_a),
	endpoint (endpoint_a)
	{
	}
	void keepalive (ysu::keepalive const & message_a) override
	{
		if (!node.network.udp_channels.max_ip_connections (endpoint))
		{
			...
			if (cookie)
			{
				...
				else if (!node.network.tcp_channels.find_channel (ysu::transport::map_endpoint_to_tcp (endpoint)))
				{
					// Don't start connection if TCP channel to same IP:port exists
					find_channel = std::make_shared<ysu::transport::channel_udp> (node.network.udp_channels, endpoint, node.network_params.protocol.protocol_version);
					node.network.send_node_id_handshake (find_channel, *cookie, boost::none);
				}
			}
			...
			{
				ysu::endpoint new_endpoint (endpoint.address (), peer0.port ());
	...
	void telemetry_req (ysu::telemetry_req const & message_a) override
	{
		...
		if (find_channel)
		{
			...
			auto cache_exceeded = std::chrono::steady_clock::now () >= find_channel->get_last_telemetry_req () + ysu::telemetry_cache_cutoffs::network_to_time (node.network_params.network);
			if (is_very_first_message || cache_exceeded)
			{
				node.network.udp_channels.modify (find_channel, [](std::shared_ptr<ysu::transport::channel_udp> channel_a) {
	...
	void node_id_handshake (ysu::node_id_handshake const & message_a) override
	{
		...
		boost::optional<ysu::uint256_union> out_query;
		boost::optional<ysu::uint256_union> out_respond_to;
		if (message_a.query)
		{
		...
		if (message_a.response)
		{
			if (!node.network.syn_cookies.validate (endpoint, message_a.response->first, message_a.response->second))
			{
				...
				if (message_a.response->first != node.node_id.pub && !node.network.tcp_channels.find_node_id (message_a.response->first))
				{
					...
					if (new_channel)
					{
						node.network.udp_channels.modify (new_channel, [&message_a](std::shared_ptr<ysu::transport::channel_udp> channel_a) {
	...
	void message (ysu::message const & message_a)
	{
		auto find_channel (node.network.udp_channels.channel (endpoint));
		if (find_channel)
		{
...
void ysu::transport::udp_channels::receive_action (ysu::message_buffer * data_a)
{
	...
	else if (ysu::transport::reserved_address (data_a->endpoint, node.config.allow_local_peers))
	{
		allowed_sender = false;
	}
	if (allowed_sender)
	{
		...
		ysu::message_parser parser (node.network.publish_filter, node.block_uniquer, node.vote_uniquer, visitor, node.work, node.ledger.cache.epoch_2_started);
		parser.deserialize_buffer (data_a->buffer, data_a->size);
		if (parser.status == ysu::message_parser::parse_status::success)
		{
			node.stats.add (ysu::stat::type::traffic_udp, ysu::stat::dir::in, data_a->size);
		}
		else if (parser.status == ysu::message_parser::parse_status::duplicate_publish_message)
		{
			node.stats.inc (ysu::stat::type::filter, ysu::stat::detail::duplicate_publish);
		}
		else
		{
			node.stats.inc (ysu::stat::type::error);
			...
			switch (parser.status)
			{
				...
				case ysu::message_parser::parse_status::invalid_header:
					node.stats.inc (ysu::stat::type::udp, ysu::stat::detail::invalid_header);
					break;
				case ysu::message_parser::parse_status::invalid_message_type:
					node.stats.inc (ysu::stat::type::udp, ysu::stat::detail::invalid_message_type);
					break;
				case ysu::message_parser::parse_status::invalid_keepalive_message:
					node.stats.inc (ysu::stat::type::udp, ysu::stat::detail::invalid_keepalive_message);
					break;
				case ysu::message_parser::parse_status::invalid_publish_message:
					node.stats.inc (ysu::stat::type::udp, ysu::stat::detail::invalid_publish_message);
					break;
				case ysu::message_parser::parse_status::invalid_confirm_req_message:
					node.stats.inc (ysu::stat::type::udp, ysu::stat::detail::invalid_confirm_req_message);
					break;
				case ysu::message_parser::parse_status::invalid_confirm_ack_message:
					node.stats.inc (ysu::stat::type::udp, ysu::stat::detail::invalid_confirm_ack_message);
					break;
				case ysu::message_parser::parse_status::invalid_node_id_handshake_message:
					node.stats.inc (ysu::stat::type::udp, ysu::stat::detail::invalid_node_id_handshake_message);
					break;
				case ysu::message_parser::parse_status::invalid_telemetry_req_message:
					node.stats.inc (ysu::stat::type::udp, ysu::stat::detail::invalid_telemetry_req_message);
					break;
				case ysu::message_parser::parse_status::invalid_telemetry_ack_message:
					node.stats.inc (ysu::stat::type::udp, ysu::stat::detail::invalid_telemetry_ack_message);
					break;
				case ysu::message_parser::parse_status::outdated_version:
					node.stats.inc (ysu::stat::type::udp, ysu::stat::detail::outdated_version);
					break;
	...
	else
	{
		...
		node.stats.inc_detail_only (ysu::stat::type::error, ysu::stat::detail::bad_sender);
	}
}

void ysu::transport::udp_channels::process_packets ()
{
	while (!stopped)
	{
...
std::shared_ptr<ysu::transport::channel> ysu::transport::udp_channels::create (ysu::endpoint const & endpoint_a)
{
	return std::make_shared<ysu::transport::channel_udp> (*this, endpoint_a, node.network_params.protocol.protocol_version);
}

bool ysu::transport::udp_channels::max_ip_connections (ysu::endpoint const & endpoint_a)
{
	bool result (false);
...
bool ysu::transport::udp_channels::reachout (ysu::endpoint const & endpoint_a)
{
	// Don't overload single IP
	bool error = max_ip_connections (endpoint_a);
	if (!error && !node.flags.disable_udp)
	{
		auto endpoint_l (ysu::transport::map_endpoint_to_v6 (endpoint_a));
...
std::unique_ptr<ysu::container_info_component> ysu::transport::udp_channels::collect_container_info (std::string const & name)
{
...
void ysu::transport::udp_channels::purge (std::chrono::steady_clock::time_point const & cutoff_a)
{
	ysu::lock_guard<std::mutex> lock (mutex);
...
void ysu::transport::udp_channels::ongoing_keepalive ()
{
	ysu::keepalive message;
	node.network.random_fill (message.peers);
	std::vector<std::shared_ptr<ysu::transport::channel_udp>> send_list;
	ysu::unique_lock<std::mutex> lock (mutex);
...
void ysu::transport::udp_channels::list_below_version (std::vector<std::shared_ptr<ysu::transport::channel>> & channels_a, uint8_t cutoff_version_a)
{
	ysu::lock_guard<std::mutex> lock (mutex);
	// clang-format off
...
void ysu::transport::udp_channels::list (std::deque<std::shared_ptr<ysu::transport::channel>> & deque_a, uint8_t minimum_version_a)
{
	ysu::lock_guard<std::mutex> lock (mutex);
	// clang-format off
...
void ysu::transport::udp_channels::modify (std::shared_ptr<ysu::transport::channel_udp> channel_a, std::function<void(std::shared_ptr<ysu::transport::channel_udp>)> modify_callback_a)
{
	ysu::lock_guard<std::mutex> lock (mutex);
