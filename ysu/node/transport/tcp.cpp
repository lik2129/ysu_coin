...
ysu::transport::channel_tcp::channel_tcp (ysu::node & node_a, std::weak_ptr<ysu::socket> socket_a) :
channel (node_a),
socket (socket_a)
{
}

ysu::transport::channel_tcp::~channel_tcp ()
{
...
size_t ysu::transport::channel_tcp::hash_code () const
{
...
bool ysu::transport::channel_tcp::operator== (ysu::transport::channel const & other_a) const
{
	bool result (false);
	auto other_l (dynamic_cast<ysu::transport::channel_tcp const *> (&other_a));
	if (other_l != nullptr)
	{
		return *this == *other_l;
	}
...
void ysu::transport::channel_tcp::send_buffer (ysu::shared_const_buffer const & buffer_a, std::function<void(boost::system::error_code const &, size_t)> const & callback_a, ysu::buffer_drop_policy policy_a)
{
	if (auto socket_l = socket.lock ())
	{
		if (!socket_l->max () || (policy_a == ysu::buffer_drop_policy::no_socket_drop && !socket_l->full ()))
		{
			socket_l->async_write (
			buffer_a, [endpoint_a = socket_l->remote_endpoint (), node = std::weak_ptr<ysu::node> (node.shared ()), callback_a](boost::system::error_code const & ec, size_t size_a) {
				if (auto node_l = node.lock ())
				{
					...
					{
						node_l->stats.inc (ysu::stat::type::error, ysu::stat::detail::unreachable_host, ysu::stat::dir::out);
					}
					if (callback_a)
					{
		...
		{
			if (policy_a == ysu::buffer_drop_policy::no_socket_drop)
			{
				node.stats.inc (ysu::stat::type::tcp, ysu::stat::detail::tcp_write_no_socket_drop, ysu::stat::dir::out);
			}
			else
			{
				node.stats.inc (ysu::stat::type::tcp, ysu::stat::detail::tcp_write_drop, ysu::stat::dir::out);
			}
			if (callback_a)
			{
...
std::string ysu::transport::channel_tcp::to_string () const
{
	if (auto socket_l = socket.lock ())
	{
...
ysu::transport::tcp_channels::tcp_channels (ysu::node & node_a) :
node (node_a)
{
}

bool ysu::transport::tcp_channels::insert (std::shared_ptr<ysu::transport::channel_tcp> channel_a, std::shared_ptr<ysu::socket> socket_a, std::shared_ptr<ysu::bootstrap_server> bootstrap_server_a)
{
	auto endpoint (channel_a->get_tcp_endpoint ());
	debug_assert (endpoint.address ().is_v6 ());
	auto udp_endpoint (ysu::transport::map_tcp_to_endpoint (endpoint));
	bool error (true);
...
void ysu::transport::tcp_channels::erase (ysu::tcp_endpoint const & endpoint_a)
{
	ysu::lock_guard<std::mutex> lock (mutex);
	channels.get<endpoint_tag> ().erase (endpoint_a);
}

size_t ysu::transport::tcp_channels::size () const
{
	ysu::lock_guard<std::mutex> lock (mutex);
	return channels.size ();
}

std::shared_ptr<ysu::transport::channel_tcp> ysu::transport::tcp_channels::find_channel (ysu::tcp_endpoint const & endpoint_a) const
{
	ysu::lock_guard<std::mutex> lock (mutex);
	std::shared_ptr<ysu::transport::channel_tcp> result;
...
std::unordered_set<std::shared_ptr<ysu::transport::channel>> ysu::transport::tcp_channels::random_set (size_t count_a, uint8_t min_version, bool include_temporary_channels_a) const
{
	std::unordered_set<std::shared_ptr<ysu::transport::channel>> result;
	...
	if (!channels.empty ())
	{
		...
		{
			auto index (ysu::random_pool::generate_word32 (0, static_cast<CryptoPP::word32> (peers_size - 1)));
...
void ysu::transport::tcp_channels::random_fill (std::array<ysu::endpoint, 8> & target_a) const
{
...
bool ysu::transport::tcp_channels::store_all (bool clear_peers)
{
	// We can't hold the mutex while starting a write transaction, so
	// we collect endpoints to be saved and then relase the lock.
	std::vector<ysu::endpoint> endpoints;
	{
		ysu::lock_guard<std::mutex> lock (mutex);
		endpoints.reserve (channels.size ());
		std::transform (channels.begin (), channels.end (),
		std::back_inserter (endpoints), [](const auto & channel) { return ysu::transport::map_tcp_to_endpoint (channel.endpoint ()); });
	}
	bool result (false);
	if (!endpoints.empty ())
	{
...
std::shared_ptr<ysu::transport::channel_tcp> ysu::transport::tcp_channels::find_node_id (ysu::account const & node_id_a)
{
	std::shared_ptr<ysu::transport::channel_tcp> result;
	ysu::lock_guard<std::mutex> lock (mutex);
...
ysu::tcp_endpoint ysu::transport::tcp_channels::bootstrap_peer (uint8_t connection_protocol_version_min)
{
...
void ysu::transport::tcp_channels::process_messages ()
{
	while (!stopped)
	{
...
void ysu::transport::tcp_channels::process_message (ysu::message const & message_a, ysu::tcp_endpoint const & endpoint_a, ysu::account const & node_id_a, std::shared_ptr<ysu::socket> socket_a, ysu::bootstrap_server_type type_a)
{
	if (!stopped && message_a.header.version_using >= protocol_constants ().protocol_version_min (node.ledger.cache.epoch_2_started))
	{
		auto channel (node.network.find_channel (ysu::transport::map_tcp_to_endpoint (endpoint_a)));
		if (channel)
		{
		...
		else
		{
			...
			else if (!node.network.excluded_peers.check (endpoint_a))
			{
				if (!node_id_a.is_zero ())
				{
					...
					auto temporary_channel (std::make_shared<ysu::transport::channel_tcp> (node, socket_a));
					...
					debug_assert (type_a == ysu::bootstrap_server_type::realtime || type_a == ysu::bootstrap_server_type::realtime_response_server);
					// Don't insert temporary channels for response_server
					if (type_a == ysu::bootstrap_server_type::realtime)
					{
				...
				else
				{
					...
					debug_assert (message_a.header.type == ysu::message_type::node_id_handshake);
					debug_assert (type_a == ysu::bootstrap_server_type::undefined);
...
void ysu::transport::tcp_channels::start ()
{
	ongoing_keepalive ();
}

void ysu::transport::tcp_channels::stop ()
{
	stopped = true;
	ysu::unique_lock<std::mutex> lock (mutex);
...
bool ysu::transport::tcp_channels::max_ip_connections (ysu::tcp_endpoint const & endpoint_a)
{
	bool result (false);
...
bool ysu::transport::tcp_channels::reachout (ysu::endpoint const & endpoint_a)
{
	auto tcp_endpoint (ysu::transport::map_endpoint_to_tcp (endpoint_a));
...
std::unique_ptr<ysu::container_info_component> ysu::transport::tcp_channels::collect_container_info (std::string const & name)
{
...
void ysu::transport::tcp_channels::purge (std::chrono::steady_clock::time_point const & cutoff_a)
{
	ysu::lock_guard<std::mutex> lock (mutex);
...
void ysu::transport::tcp_channels::ongoing_keepalive ()
{
	ysu::keepalive message;
	node.network.random_fill (message.peers);
	ysu::unique_lock<std::mutex> lock (mutex);
	// Wake up channels
	std::vector<std::shared_ptr<ysu::transport::channel_tcp>> send_list;
	...
	ysu::tcp_endpoint invalid_endpoint (boost::asio::ip::address_v6::any (), 0);
	if (!node.network_params.network.is_dev_network () && !node.flags.disable_udp)
	{
		...
		for (auto i (0); i <= random_count; ++i)
		{
			...
			{
				start_tcp (ysu::transport::map_tcp_to_endpoint (tcp_endpoint));
			}
		}
	}
...
void ysu::transport::tcp_channels::list_below_version (std::vector<std::shared_ptr<ysu::transport::channel>> & channels_a, uint8_t cutoff_version_a)
{
	ysu::lock_guard<std::mutex> lock (mutex);
	// clang-format off
	ysu::transform_if (channels.get<random_access_tag> ().begin (), channels.get<random_access_tag> ().end (), std::back_inserter (channels_a),
...
void ysu::transport::tcp_channels::list (std::deque<std::shared_ptr<ysu::transport::channel>> & deque_a, uint8_t minimum_version_a, bool include_temporary_channels_a)
{
	ysu::lock_guard<std::mutex> lock (mutex);
	// clang-format off
	ysu::transform_if (channels.get<random_access_tag> ().begin (), channels.get<random_access_tag> ().end (), std::back_inserter (deque_a),
...
void ysu::transport::tcp_channels::modify (std::shared_ptr<ysu::transport::channel_tcp> channel_a, std::function<void(std::shared_ptr<ysu::transport::channel_tcp>)> modify_callback_a)
{
	ysu::lock_guard<std::mutex> lock (mutex);
...
void ysu::transport::tcp_channels::update (ysu::tcp_endpoint const & endpoint_a)
{
	ysu::lock_guard<std::mutex> lock (mutex);
	auto existing (channels.get<endpoint_tag> ().find (endpoint_a));
	if (existing != channels.get<endpoint_tag> ().end ())
...
bool ysu::transport::tcp_channels::node_id_handhake_sockets_empty () const
{
	ysu::lock_guard<std::mutex> guard (mutex);
	return node_id_handshake_sockets.empty ();
}

void ysu::transport::tcp_channels::push_node_id_handshake_socket (std::shared_ptr<ysu::socket> const & socket_a)
{
	ysu::lock_guard<std::mutex> guard (mutex);
	node_id_handshake_sockets.push_back (socket_a);
}

void ysu::transport::tcp_channels::remove_node_id_handshake_socket (std::shared_ptr<ysu::socket> const & socket_a)
{
	std::weak_ptr<ysu::node> node_w (node.shared ());
	if (auto node_l = node_w.lock ())
	{
...
void ysu::transport::tcp_channels::start_tcp (ysu::endpoint const & endpoint_a, std::function<void(std::shared_ptr<ysu::transport::channel>)> const & callback_a)
{
	...
	auto socket = std::make_shared<ysu::socket> (node, boost::none);
	std::weak_ptr<ysu::socket> socket_w (socket);
	auto channel (std::make_shared<ysu::transport::channel_tcp> (node, socket_w));
	std::weak_ptr<ysu::node> node_w (node.shared ());
	socket->async_connect (ysu::transport::map_endpoint_to_tcp (endpoint_a),
	[node_w, channel, socket, endpoint_a, callback_a](boost::system::error_code const & ec) {
		if (auto node_l = node_w.lock ())
		{
			if (!ec && channel)
			{
				...
				ysu::node_id_handshake message (cookie, boost::none);
...
void ysu::transport::tcp_channels::start_tcp_receive_node_id (std::shared_ptr<ysu::transport::channel_tcp> channel_a, ysu::endpoint const & endpoint_a, std::shared_ptr<std::vector<uint8_t>> receive_buffer_a, std::function<void(std::shared_ptr<ysu::transport::channel>)> const & callback_a)
{
	std::weak_ptr<ysu::node> node_w (node.shared ());
	if (auto socket_l = channel_a->socket.lock ())
	{
		auto cleanup_node_id_handshake_socket = [socket_w = channel_a->socket, node_w](ysu::endpoint const & endpoint_a, std::function<void(std::shared_ptr<ysu::transport::channel>)> const & callback_a) {
			if (auto node_l = node_w.lock ())
			{
		...
		auto cleanup_and_udp_fallback = [socket_w = channel_a->socket, node_w, cleanup_node_id_handshake_socket](ysu::endpoint const & endpoint_a, std::function<void(std::shared_ptr<ysu::transport::channel>)> const & callback_a) {
			if (auto node_l = node_w.lock ())
			{
				node_l->network.tcp_channels.udp_fallback (endpoint_a, callback_a);
		...
		socket_l->async_read (receive_buffer_a, 8 + sizeof (ysu::account) + sizeof (ysu::account) + sizeof (ysu::signature), [node_w, channel_a, endpoint_a, receive_buffer_a, callback_a, cleanup_and_udp_fallback, cleanup_node_id_handshake_socket](boost::system::error_code const & ec, size_t size_a) {
			if (auto node_l = node_w.lock ())
			{
				if (!ec && channel_a)
				{
					node_l->stats.inc (ysu::stat::type::message, ysu::stat::detail::node_id_handshake, ysu::stat::dir::in);
					auto error (false);
					ysu::bufferstream stream (receive_buffer_a->data (), size_a);
					ysu::message_header header (error, stream);
					if (!error && header.type == ysu::message_type::node_id_handshake)
					{
						if (header.version_using >= node_l->network_params.protocol.protocol_version_min (node_l->ledger.cache.epoch_2_started))
						{
							ysu::node_id_handshake message (error, stream, header);
							if (!error && message.response && message.query)
							{
								...
								if (process)
								{
									...
									boost::optional<std::pair<ysu::account, ysu::signature>> response (std::make_pair (node_l->node_id.pub, ysu::sign_message (node_l->node_id.prv, node_l->node_id.pub, *message.query)));
									ysu::node_id_handshake response_message (boost::none, response);
										...
										if (auto node_l = node_w.lock ())
										{
											if (!ec && channel_a)
											{
												...
												if (auto socket_l = channel_a->socket.lock ())
												{
													...
													auto response_server = std::make_shared<ysu::bootstrap_server> (socket_l, node_l);
													...
													response_server->type = ysu::bootstrap_server_type::realtime_response_server;
													...
													{
														node_l->telemetry->get_metrics_single_peer_async (channel_a, [](ysu::telemetry_data_response /* unused */) {
						...
						else
						{
							...
							cleanup_node_id_handshake_socket (endpoint_a, callback_a);
							{
								ysu::lock_guard<std::mutex> lock (node_l->network.tcp_channels.mutex);
								node_l->network.tcp_channels.attempts.get<endpoint_tag> ().erase (ysu::transport::map_endpoint_to_tcp (endpoint_a));
							}
						}
					}
					else
...
void ysu::transport::tcp_channels::udp_fallback (ysu::endpoint const & endpoint_a, std::function<void(std::shared_ptr<ysu::transport::channel>)> const & callback_a)
{
	{
		ysu::lock_guard<std::mutex> lock (mutex);
		attempts.get<endpoint_tag> ().erase (ysu::transport::map_endpoint_to_tcp (endpoint_a));
	}
