...
{
public:
	std::shared_ptr<ysu::message> message;
	ysu::tcp_endpoint endpoint;
	ysu::account node_id;
	std::shared_ptr<ysu::socket> socket;
	ysu::bootstrap_server_type type;
};
namespace transport
{
	class tcp_channels;
	class channel_tcp : public ysu::transport::channel
	{
		friend class ysu::transport::tcp_channels;

	public:
		channel_tcp (ysu::node &, std::weak_ptr<ysu::socket>);
		~channel_tcp ();
		...
		bool operator== (ysu::transport::channel_tcp const & other_a) const
		{
			return &node == &other_a.node && socket.lock () == other_a.socket.lock ();
		}
		std::weak_ptr<ysu::socket> socket;
		std::weak_ptr<ysu::bootstrap_server> response_server;
		...
		ysu::endpoint get_endpoint () const override
		{
			...
			{
				return ysu::transport::map_tcp_to_endpoint (socket_l->remote_endpoint ());
			}
			else
			{
				return ysu::endpoint (boost::asio::ip::address_v6::any (), 0);
			}
		}
		...
		ysu::tcp_endpoint get_tcp_endpoint () const override
		{
			...
			{
				return ysu::tcp_endpoint (boost::asio::ip::address_v6::any (), 0);
			}
		}

		ysu::transport::transport_type get_type () const override
		{
			return ysu::transport::transport_type::tcp;
		}
	};
	class tcp_channels final
	{
		friend class ysu::transport::channel_tcp;
		friend class telemetry_simultaneous_requests_Test;

	public:
		tcp_channels (ysu::node &);
		bool insert (std::shared_ptr<ysu::transport::channel_tcp>, std::shared_ptr<ysu::socket>, std::shared_ptr<ysu::bootstrap_server>);
		void erase (ysu::tcp_endpoint const &);
		size_t size () const;
		std::shared_ptr<ysu::transport::channel_tcp> find_channel (ysu::tcp_endpoint const &) const;
		...
		std::shared_ptr<ysu::transport::channel_tcp> find_node_id (ysu::account const &);
		// Get the next peer for attempting a tcp connection
		ysu::tcp_endpoint bootstrap_peer (uint8_t connection_protocol_version_min);
		void receive ();
		void start ();
		void stop ();
		void process_messages ();
		void process_message (ysu::message const &, ysu::tcp_endpoint const &, ysu::account const &, std::shared_ptr<ysu::socket>, ysu::bootstrap_server_type);
		bool max_ip_connections (ysu::tcp_endpoint const &);
		// Should we reach out to this endpoint with a keepalive message
		bool reachout (ysu::endpoint const &);
		std::unique_ptr<container_info_component> collect_container_info (std::string const &);
		void purge (std::chrono::steady_clock::time_point const &);
		void ongoing_keepalive ();
		void list_below_version (std::vector<std::shared_ptr<ysu::transport::channel>> &, uint8_t);
		void list (std::deque<std::shared_ptr<ysu::transport::channel>> &, uint8_t = 0, bool = true);
		void modify (std::shared_ptr<ysu::transport::channel_tcp>, std::function<void(std::shared_ptr<ysu::transport::channel_tcp>)>);
		void update (ysu::tcp_endpoint const &);
		// Connection start
		void start_tcp (ysu::endpoint const &, std::function<void(std::shared_ptr<ysu::transport::channel>)> const & = nullptr);
		void start_tcp_receive_node_id (std::shared_ptr<ysu::transport::channel_tcp>, ysu::endpoint const &, std::shared_ptr<std::vector<uint8_t>>, std::function<void(std::shared_ptr<ysu::transport::channel>)> const &);
		void udp_fallback (ysu::endpoint const &, std::function<void(std::shared_ptr<ysu::transport::channel>)> const &);
		void push_node_id_handshake_socket (std::shared_ptr<ysu::socket> const & socket_a);
		void remove_node_id_handshake_socket (std::shared_ptr<ysu::socket> const & socket_a);
	...
	private:
		...
		{
		public:
			std::shared_ptr<ysu::transport::channel_tcp> channel;
			std::shared_ptr<ysu::socket> socket;
			std::shared_ptr<ysu::bootstrap_server> response_server;
			channel_tcp_wrapper (std::shared_ptr<ysu::transport::channel_tcp> const & channel_a, std::shared_ptr<ysu::socket> const & socket_a, std::shared_ptr<ysu::bootstrap_server> const & server_a) :
			channel (channel_a), socket (socket_a), response_server (server_a)
			{
			}
			ysu::tcp_endpoint endpoint () const
			{
		...
		public:
			...
			explicit tcp_endpoint_attempt (ysu::tcp_endpoint const & endpoint_a) :
			endpoint (endpoint_a),
		...
		mi::indexed_by<
			...
			mi::hashed_unique<mi::tag<endpoint_tag>,
				mi::const_mem_fun<channel_tcp_wrapper, ysu::tcp_endpoint, &channel_tcp_wrapper::endpoint>>,
			mi::hashed_non_unique<mi::tag<node_id_tag>,
				mi::const_mem_fun<channel_tcp_wrapper, ysu::account, &channel_tcp_wrapper::node_id>>,
		...
		mi::indexed_by<
			mi::hashed_unique<mi::tag<endpoint_tag>,
				mi::member<tcp_endpoint_attempt, ysu::tcp_endpoint, &tcp_endpoint_attempt::endpoint>>,
			mi::hashed_non_unique<mi::tag<ip_address_tag>,
		...
		std::vector<std::shared_ptr<ysu::socket>> node_id_handshake_sockets;
