...
{
	class udp_channels;
	class channel_udp final : public ysu::transport::channel
	{
		friend class ysu::transport::udp_channels;

	public:
		channel_udp (ysu::transport::udp_channels &, ysu::endpoint const &, uint8_t protocol_version);
		...
		bool operator== (ysu::transport::channel_udp const & other_a) const
		{
		...
		{
			return ysu::transport::transport_type::udp;
		}
	...
	private:
		...
		ysu::transport::udp_channels & channels;
		std::chrono::steady_clock::time_point last_telemetry_req{ std::chrono::steady_clock::time_point () };
	};
	class udp_channels final
	{
		friend class ysu::transport::channel_udp;

	public:
		udp_channels (ysu::node &, uint16_t);
		std::shared_ptr<ysu::transport::channel_udp> insert (ysu::endpoint const &, unsigned);
		void erase (ysu::endpoint const &);
		size_t size () const;
		std::shared_ptr<ysu::transport::channel_udp> channel (ysu::endpoint const &) const;
		void random_fill (std::array<ysu::endpoint, 8> &) const;
		std::unordered_set<std::shared_ptr<ysu::transport::channel>> random_set (size_t, uint8_t = 0) const;
		bool store_all (bool = true);
		std::shared_ptr<ysu::transport::channel_udp> find_node_id (ysu::account const &);
		void clean_node_id (ysu::account const &);
		void clean_node_id (ysu::endpoint const &, ysu::account const &);
		// Get the next peer for attempting a tcp bootstrap connection
		ysu::tcp_endpoint bootstrap_peer (uint8_t connection_protocol_version_min);
		void receive ();
		void start ();
		void stop ();
		void send (ysu::shared_const_buffer const & buffer_a, ysu::endpoint endpoint_a, std::function<void(boost::system::error_code const &, size_t)> const & callback_a);
		ysu::endpoint get_local_endpoint () const;
		void receive_action (ysu::message_buffer *);
		void process_packets ();
		std::shared_ptr<ysu::transport::channel> create (ysu::endpoint const &);
		bool max_ip_connections (ysu::endpoint const &);
		...
		void list (std::deque<std::shared_ptr<ysu::transport::channel>> &, uint8_t = 0);
		void modify (std::shared_ptr<ysu::transport::channel_udp>, std::function<void(std::shared_ptr<ysu::transport::channel_udp>)>);
		ysu::node & node;
	...
	private:
		...
		{
		public:
			std::shared_ptr<ysu::transport::channel_udp> channel;
			channel_udp_wrapper (std::shared_ptr<ysu::transport::channel_udp> const & channel_a) :
			channel (channel_a)
			{
			}
		...
		public:
			...
			explicit endpoint_attempt (ysu::endpoint const & endpoint_a) :
			endpoint (endpoint_a)
			{
			}
		};
		...
		mi::indexed_by<
			...
			mi::hashed_unique<mi::tag<endpoint_tag>,
				mi::const_mem_fun<channel_udp_wrapper, ysu::endpoint, &channel_udp_wrapper::endpoint>>,
			mi::hashed_non_unique<mi::tag<node_id_tag>,
				mi::const_mem_fun<channel_udp_wrapper, ysu::account, &channel_udp_wrapper::node_id>>,
		...
		mi::indexed_by<
			mi::hashed_unique<mi::tag<endpoint_tag>,
				mi::member<endpoint_attempt, ysu::endpoint, &endpoint_attempt::endpoint>>,
