#include <ysu/crypto_lib/random_pool_shuffle.hpp>
...
ysu::network::network (ysu::node & node_a, uint16_t port_a) :
syn_cookies (node_a.network_params.node.max_peers_per_ip),
buffer_container (node_a.stats, ysu::network::buffer_size, 4096), // 2Mb receive buffer
...
void ysu::network::send_keepalive (std::shared_ptr<ysu::transport::channel> channel_a)
{
...
void ysu::network::send_keepalive_self (std::shared_ptr<ysu::transport::channel> channel_a)
{
	ysu::keepalive message;
...
void ysu::network::send_node_id_handshake (std::shared_ptr<ysu::transport::channel> channel_a, boost::optional<ysu::uint256_union> const & query, boost::optional<ysu::uint256_union> const & respond_to)
{
	boost::optional<std::pair<ysu::account, ysu::signature>> response (boost::none);
	if (respond_to)
	{
		response = std::make_pair (node.node_id.pub, ysu::sign_message (node.node_id.prv, node.node_id.pub, *respond_to));
		debug_assert (!ysu::validate_message (response->first, *respond_to, response->second));
	}
	ysu::node_id_handshake message (query, response);
...
void ysu::network::flood_message (ysu::message const & message_a, ysu::buffer_drop_policy const drop_policy_a, float const scale_a)
{
...
void ysu::network::flood_block (std::shared_ptr<ysu::block> const & block_a, ysu::buffer_drop_policy const drop_policy_a)
{
	ysu::publish message (block_a);
	flood_message (message, drop_policy_a);
}

void ysu::network::flood_block_initial (std::shared_ptr<ysu::block> const & block_a)
{
	ysu::publish message (block_a);
	for (auto const & i : node.rep_crawler.principal_representatives ())
	{
		i.channel->send (message, nullptr, ysu::buffer_drop_policy::no_limiter_drop);
	}
	for (auto & i : list_non_pr (fanout (1.0)))
	{
		i->send (message, nullptr, ysu::buffer_drop_policy::no_limiter_drop);
	}
}

void ysu::network::flood_vote (std::shared_ptr<ysu::vote> const & vote_a, float scale)
{
	ysu::confirm_ack message (vote_a);
...
void ysu::network::flood_vote_pr (std::shared_ptr<ysu::vote> const & vote_a)
{
	ysu::confirm_ack message (vote_a);
	for (auto const & i : node.rep_crawler.principal_representatives ())
	{
...
void ysu::network::flood_block_many (std::deque<std::shared_ptr<ysu::block>> blocks_a, std::function<void()> callback_a, unsigned delay_a)
{
...
void ysu::network::send_confirm_req (std::shared_ptr<ysu::transport::channel> channel_a, std::pair<ysu::block_hash, ysu::block_hash> const & hash_root_a)
{
	// Confirmation request with hash + root
	ysu::confirm_req req (hash_root_a.first, hash_root_a.second);
	channel_a->send (req);
}

void ysu::network::broadcast_confirm_req (std::shared_ptr<ysu::block> block_a)
{
	auto list (std::make_shared<std::vector<std::shared_ptr<ysu::transport::channel>>> (node.rep_crawler.representative_endpoints (std::numeric_limits<size_t>::max ())));
	...
	ysu::random_pool_shuffle (list->begin (), list->end ());
...
void ysu::network::broadcast_confirm_req_base (std::shared_ptr<ysu::block> block_a, std::shared_ptr<std::vector<std::shared_ptr<ysu::transport::channel>>> endpoints_a, unsigned delay_a, bool resumption)
{
...
void ysu::network::broadcast_confirm_req_batched_many (std::unordered_map<std::shared_ptr<ysu::transport::channel>, std::deque<std::pair<ysu::block_hash, ysu::root>>> request_bundle_a, std::function<void()> callback_a, unsigned delay_a, bool resumption_a)
{
	...
	{
		std::vector<std::pair<ysu::block_hash, ysu::root>> roots_hashes_l;
		...
		ysu::confirm_req req (roots_hashes_l);
		i->first->send (req);
		if (i->second.empty ())
		{
...
void ysu::network::broadcast_confirm_req_many (std::deque<std::pair<std::shared_ptr<ysu::block>, std::shared_ptr<std::vector<std::shared_ptr<ysu::transport::channel>>>>> requests_a, std::function<void()> callback_a, unsigned delay_a)
{
...
namespace
{
class network_message_visitor : public ysu::message_visitor
{
public:
	network_message_visitor (ysu::node & node_a, std::shared_ptr<ysu::transport::channel> const & channel_a) :
	node (node_a),
	channel (channel_a)
	{
	}
	void keepalive (ysu::keepalive const & message_a) override
	{
		...
		node.stats.inc (ysu::stat::type::message, ysu::stat::detail::keepalive, ysu::stat::dir::in);
		node.network.merge_peers (message_a.peers);
		// Check for special node port data
		auto peer0 (message_a.peers[0]);
		if (peer0.address () == boost::asio::ip::address_v6{} && peer0.port () != 0)
		{
			ysu::endpoint new_endpoint (channel->get_tcp_endpoint ().address (), peer0.port ());
	...
	void publish (ysu::publish const & message_a) override
	{
		...
		node.stats.inc (ysu::stat::type::message, ysu::stat::detail::publish, ysu::stat::dir::in);
		...
		else
		{
			...
			node.stats.inc (ysu::stat::type::drop, ysu::stat::detail::publish, ysu::stat::dir::in);
		}
	}
	void confirm_req (ysu::confirm_req const & message_a) override
	{
		...
		node.stats.inc (ysu::stat::type::message, ysu::stat::detail::confirm_req, ysu::stat::dir::in);
	...
	void confirm_ack (ysu::confirm_ack const & message_a) override
	{
		...
		node.stats.inc (ysu::stat::type::message, ysu::stat::detail::confirm_ack, ysu::stat::dir::in);
		if (!message_a.vote->account.is_zero ())
		{
			if (message_a.header.block_type () != ysu::block_type::not_a_block)
			{
				for (auto & vote_block : message_a.vote->blocks)
				{
					if (!vote_block.which ())
					{
						...
						{
							node.stats.inc (ysu::stat::type::drop, ysu::stat::detail::confirm_ack, ysu::stat::dir::in);
						}
					}
				}
			}
	...
	{
		node.stats.inc (ysu::stat::type::message, ysu::stat::detail::node_id_handshake, ysu::stat::dir::in);
	}
	void telemetry_req (ysu::telemetry_req const & message_a) override
	{
		...
		node.stats.inc (ysu::stat::type::message, ysu::stat::detail::telemetry_req, ysu::stat::dir::in);
		...
		{
			auto telemetry_data = ysu::local_telemetry_data (node.store, node.ledger.cache, node.network, node.config.bandwidth_limit, node.network_params, node.startup_time, node.active.active_difficulty (), node.node_id);
			telemetry_ack = ysu::telemetry_ack (telemetry_data);
		}
		channel->send (telemetry_ack, nullptr, ysu::buffer_drop_policy::no_socket_drop);
	}
	void telemetry_ack (ysu::telemetry_ack const & message_a) override
	{
		...
		node.stats.inc (ysu::stat::type::message, ysu::stat::detail::telemetry_ack, ysu::stat::dir::in);
		if (node.telemetry)
		{
...
void ysu::network::process_message (ysu::message const & message_a, std::shared_ptr<ysu::transport::channel> channel_a)
{
...
void ysu::network::merge_peers (std::array<ysu::endpoint, 8> const & peers_a)
{
...
void ysu::network::merge_peer (ysu::endpoint const & peer_a)
{
	if (!reachout (peer_a, node.config.allow_local_peers))
	{
		std::weak_ptr<ysu::node> node_w (node.shared ());
		node.network.tcp_channels.start_tcp (peer_a, [node_w](std::shared_ptr<ysu::transport::channel> channel_a) {
			if (auto node_l = node_w.lock ())
			{
...
bool ysu::network::not_a_peer (ysu::endpoint const & endpoint_a, bool allow_local_peers)
{
	bool result (false);
	...
	else if (ysu::transport::reserved_address (endpoint_a, allow_local_peers))
	{
		result = true;
	}
...
bool ysu::network::reachout (ysu::endpoint const & endpoint_a, bool allow_local_peers)
{
...
std::deque<std::shared_ptr<ysu::transport::channel>> ysu::network::list (size_t count_a, uint8_t minimum_version_a, bool include_tcp_temporary_channels_a)
{
	std::deque<std::shared_ptr<ysu::transport::channel>> result;
	tcp_channels.list (result, minimum_version_a, include_tcp_temporary_channels_a);
	udp_channels.list (result, minimum_version_a);
	ysu::random_pool_shuffle (result.begin (), result.end ());
...
std::deque<std::shared_ptr<ysu::transport::channel>> ysu::network::list_non_pr (size_t count_a)
{
	std::deque<std::shared_ptr<ysu::transport::channel>> result;
	tcp_channels.list (result);
	udp_channels.list (result);
	ysu::random_pool_shuffle (result.begin (), result.end ());
	result.erase (std::remove_if (result.begin (), result.end (), [this](std::shared_ptr<ysu::transport::channel> const & channel) {
...
size_t ysu::network::fanout (float scale) const
{
	return static_cast<size_t> (std::ceil (scale * size_sqrt ()));
}

std::unordered_set<std::shared_ptr<ysu::transport::channel>> ysu::network::random_set (size_t count_a, uint8_t min_version_a, bool include_temporary_channels_a) const
{
	std::unordered_set<std::shared_ptr<ysu::transport::channel>> result (tcp_channels.random_set (count_a, min_version_a, include_temporary_channels_a));
	std::unordered_set<std::shared_ptr<ysu::transport::channel>> udp_random (udp_channels.random_set (count_a, min_version_a));
...
void ysu::network::random_fill (std::array<ysu::endpoint, 8> & target_a) const
{
	...
	auto endpoint (ysu::endpoint (boost::asio::ip::address_v6{}, 0));
...
void ysu::network::fill_keepalive_self (std::array<ysu::endpoint, 8> & target_a) const
{
	random_fill (target_a);
	// Replace part of message with node external address or listening port
	target_a[1] = ysu::endpoint (boost::asio::ip::address_v6{}, 0); // For node v19 (response channels)
	if (node.config.external_address != boost::asio::ip::address_v6{}.to_string () && node.config.external_port != 0)
	{
		target_a[0] = ysu::endpoint (boost::asio::ip::make_address_v6 (node.config.external_address), node.config.external_port);
	}
	else
	{
		...
		{
			target_a[0] = ysu::endpoint (boost::asio::ip::address_v6{}, port);
			boost::system::error_code ec;
			auto external_v6 = boost::asio::ip::make_address_v6 (external_address.address ().to_string (), ec);
			target_a[1] = ysu::endpoint (external_v6, external_address.port ());
		}
		else
		{
			target_a[0] = ysu::endpoint (boost::asio::ip::address_v6{}, port);
...
ysu::tcp_endpoint ysu::network::bootstrap_peer (bool lazy_bootstrap)
{
	ysu::tcp_endpoint result (boost::asio::ip::address_v6::any (), 0);
	bool use_udp_peer (ysu::random_pool::generate_word32 (0, 1));
	...
	if (result == ysu::tcp_endpoint (boost::asio::ip::address_v6::any (), 0))
	{
...
std::shared_ptr<ysu::transport::channel> ysu::network::find_channel (ysu::endpoint const & endpoint_a)
{
	std::shared_ptr<ysu::transport::channel> result (tcp_channels.find_channel (ysu::transport::map_endpoint_to_tcp (endpoint_a)));
	if (!result)
	{
...
std::shared_ptr<ysu::transport::channel> ysu::network::find_node_id (ysu::account const & node_id_a)
{
	std::shared_ptr<ysu::transport::channel> result (tcp_channels.find_node_id (node_id_a));
	if (!result)
	{
...
ysu::endpoint ysu::network::endpoint ()
{
	return ysu::endpoint (boost::asio::ip::address_v6::loopback (), port);
}

void ysu::network::cleanup (std::chrono::steady_clock::time_point const & cutoff_a)
{
...
void ysu::network::ongoing_cleanup ()
{
	node.spawn (
...
void ysu::network::ongoing_syn_cookie_cleanup ()
{
	node.spawn (
	[this](boost::asio::yield_context yield) {
		boost::system::error_code ec;
		while (!stopped && !ec)
		{
			this->syn_cookies.purge (std::chrono::steady_clock::now () - ysu::transport::syn_cookie_cutoff);
			cookie_timer.expires_from_now (ysu::transport::syn_cookie_cutoff * 2);
...
void ysu::network::ongoing_keepalive ()
{
	node.spawn (
	[this](boost::asio::yield_context yield) {
		boost::system::error_code ec;
...
size_t ysu::network::size () const
{
	return tcp_channels.size () + udp_channels.size ();
}

float ysu::network::size_sqrt () const
{
	return static_cast<float> (std::sqrt (size ()));
}

bool ysu::network::empty () const
{
	return size () == 0;
}

void ysu::network::erase_below_version (uint8_t cutoff_version_a)
{
	std::vector<std::shared_ptr<ysu::transport::channel>> channels_to_remove;
...
void ysu::network::erase (ysu::transport::channel const & channel_a)
{
	auto const channel_type = channel_a.get_type ();
	if (channel_type == ysu::transport::transport_type::tcp)
	{
		tcp_channels.erase (channel_a.get_tcp_endpoint ());
	}
	else if (channel_type != ysu::transport::transport_type::loopback)
	{
...
ysu::message_buffer_manager::message_buffer_manager (ysu::stat & stats_a, size_t size, size_t count) :
stats (stats_a),
free (count),
full (count),
slab (size * count),
...
stopped (false)
{
	...
	{
		*entry_data = { slab_data + i * size, 0, ysu::endpoint () };
		free.push_back (entry_data);
	}
}

ysu::message_buffer * ysu::message_buffer_manager::allocate ()
{
	ysu::unique_lock<std::mutex> lock (mutex);
	if (!stopped && free.empty () && full.empty ())
	{
		stats.inc (ysu::stat::type::udp, ysu::stat::detail::blocking, ysu::stat::dir::in);
		condition.wait (lock, [& stopped = stopped, &free = free, &full = full] { return stopped || !free.empty () || !full.empty (); });
	}
	ysu::message_buffer * result (nullptr);
	if (!free.empty ())
	{
		result = free.front ();
		free.pop_front ();
	...
	if (result == nullptr && !full.empty ())
	{
		result = full.front ();
		full.pop_front ();
		stats.inc (ysu::stat::type::udp, ysu::stat::detail::overflow, ysu::stat::dir::in);
	}
	release_assert (result || stopped);
	return result;
}

void ysu::message_buffer_manager::enqueue (ysu::message_buffer * data_a)
{
...
ysu::message_buffer * ysu::message_buffer_manager::dequeue ()
{
	ysu::unique_lock<std::mutex> lock (mutex);
...
void ysu::message_buffer_manager::release (ysu::message_buffer * data_a)
{
	debug_assert (data_a != nullptr);
	{
		ysu::lock_guard<std::mutex> lock (mutex);
...
void ysu::message_buffer_manager::stop ()
{
	{
		ysu::lock_guard<std::mutex> lock (mutex);
		stopped = true;
...
ysu::tcp_message_manager::tcp_message_manager (unsigned incoming_connections_max_a) :
max_entries (incoming_connections_max_a * ysu::tcp_message_manager::max_entries_per_connection + 1)
{
	debug_assert (max_entries > 0);
}

void ysu::tcp_message_manager::put_message (ysu::tcp_message_item const & item_a)
{
	{
		ysu::unique_lock<std::mutex> lock (mutex);
...
ysu::tcp_message_item ysu::tcp_message_manager::get_message ()
{
	ysu::tcp_message_item result;
	ysu::unique_lock<std::mutex> lock (mutex);
	...
	{
		result = ysu::tcp_message_item{ std::make_shared<ysu::keepalive> (), ysu::tcp_endpoint (boost::asio::ip::address_v6::any (), 0), 0, nullptr, ysu::bootstrap_server_type::undefined };
	}
	lock.unlock ();
...
void ysu::tcp_message_manager::stop ()
{
	{
		ysu::lock_guard<std::mutex> lock (mutex);
		stopped = true;
...
ysu::syn_cookies::syn_cookies (size_t max_cookies_per_ip_a) :
max_cookies_per_ip (max_cookies_per_ip_a)
{
}

boost::optional<ysu::uint256_union> ysu::syn_cookies::assign (ysu::endpoint const & endpoint_a)
{
	auto ip_addr (endpoint_a.address ());
	debug_assert (ip_addr.is_v6 ());
	ysu::lock_guard<std::mutex> lock (syn_cookie_mutex);
	unsigned & ip_cookies = cookies_per_ip[ip_addr];
	boost::optional<ysu::uint256_union> result;
	if (ip_cookies < max_cookies_per_ip)
	{
		if (cookies.find (endpoint_a) == cookies.end ())
		{
			ysu::uint256_union query;
...
bool ysu::syn_cookies::validate (ysu::endpoint const & endpoint_a, ysu::account const & node_id, ysu::signature const & sig)
{
	auto ip_addr (endpoint_a.address ());
	debug_assert (ip_addr.is_v6 ());
	ysu::lock_guard<std::mutex> lock (syn_cookie_mutex);
	...
	if (cookie_it != cookies.end () && !ysu::validate_message (node_id, cookie_it->second.cookie, sig))
	{
		result = false;
...
void ysu::syn_cookies::purge (std::chrono::steady_clock::time_point const & cutoff_a)
{
	ysu::lock_guard<std::mutex> lock (syn_cookie_mutex);
...
size_t ysu::syn_cookies::cookies_size ()
{
	ysu::lock_guard<std::mutex> lock (syn_cookie_mutex);
	return cookies.size ();
}

std::unique_ptr<ysu::container_info_component> ysu::collect_container_info (network & network, const std::string & name)
{
...
std::unique_ptr<ysu::container_info_component> ysu::syn_cookies::collect_container_info (std::string const & name)
{
	size_t syn_cookies_count;
	size_t syn_cookies_per_ip_count;
	{
		ysu::lock_guard<std::mutex> syn_cookie_guard (syn_cookie_mutex);
