...
ysu::telemetry::telemetry (ysu::network & network_a, ysu::alarm & alarm_a, ysu::worker & worker_a, ysu::observer_set<ysu::telemetry_data const &, ysu::endpoint const &> & observers_a, ysu::stat & stats_a, ysu::network_params & network_params_a, bool disable_ongoing_requests_a) :
network (network_a),
alarm (alarm_a),
worker (worker_a),
...
void ysu::telemetry::stop ()
{
	stopped = true;
}

void ysu::telemetry::set (ysu::telemetry_ack const & message_a, ysu::transport::channel const & channel_a)
{
	if (!stopped)
	{
		ysu::unique_lock<std::mutex> lk (mutex);
		ysu::endpoint endpoint = channel_a.get_endpoint ();
		...
		if (it == recent_or_initial_request_telemetry_data.cend () || !it->undergoing_request)
		{
			...
			stats.inc (ysu::stat::type::telemetry, ysu::stat::detail::unsolicited_telemetry_ack);
			return;
		}

		recent_or_initial_request_telemetry_data.modify (it, [&message_a](ysu::telemetry_info & telemetry_info_a) {
...
bool ysu::telemetry::verify_message (ysu::telemetry_ack const & message_a, ysu::transport::channel const & channel_a)
{
	...
	if (!node_id_mismatch)
	{
		...
		if (!remove_channel)
		{
			...
			if (remove_channel)
			{
				stats.inc (ysu::stat::type::telemetry, ysu::stat::detail::different_genesis_hash);
			}
		}
		else
		{
			stats.inc (ysu::stat::type::telemetry, ysu::stat::detail::invalid_signature);
		}
	}
	else
	{
		stats.inc (ysu::stat::type::telemetry, ysu::stat::detail::node_id_mismatch);
	}
...
std::chrono::milliseconds ysu::telemetry::cache_plus_buffer_cutoff_time () const
{
...
bool ysu::telemetry::within_cache_plus_buffer_cutoff (telemetry_info const & telemetry_info) const
{
...
bool ysu::telemetry::within_cache_cutoff (telemetry_info const & telemetry_info) const
{
	auto is_within = (telemetry_info.last_response + cache_cutoff) >= std::chrono::steady_clock::now ();
	return !telemetry_info.awaiting_first_response () && is_within;
}

void ysu::telemetry::ongoing_req_all_peers (std::chrono::milliseconds next_request_interval)
{
		...
		if (auto this_l = this_w.lock ())
		{
			...
			if (!this_l->stopped)
			{
				...
				struct channel_wrapper
				{
					...
					channel_wrapper (std::shared_ptr<ysu::transport::channel> const & channel_a) :
					channel (channel_a)
					{
					}
					ysu::endpoint endpoint () const
					{
				...
				mi::indexed_by<
					mi::hashed_unique<mi::tag<tag_endpoint>,
						mi::const_mem_fun<channel_wrapper, ysu::endpoint, &channel_wrapper::endpoint>>,
					mi::hashed_unique<mi::tag<tag_channel>,
						mi::member<channel_wrapper, std::shared_ptr<ysu::transport::channel>, &channel_wrapper::channel>>>> peers;
				// clang-format on
...

				{
					...
					ysu::lock_guard<std::mutex> guard (this_l->mutex);
...
std::unordered_map<ysu::endpoint, ysu::telemetry_data> ysu::telemetry::get_metrics ()
{
	std::unordered_map<ysu::endpoint, ysu::telemetry_data> telemetry_data;

	ysu::lock_guard<std::mutex> guard (mutex);
	auto range = boost::make_iterator_range (recent_or_initial_request_telemetry_data);
	// clang-format off
	ysu::transform_if (range.begin (), range.end (), std::inserter (telemetry_data, telemetry_data.end ()),
		[this](auto const & telemetry_info) { return this->within_cache_plus_buffer_cutoff (telemetry_info); },
		[](auto const & telemetry_info) { return std::pair<const ysu::endpoint, ysu::telemetry_data>{ telemetry_info.endpoint, telemetry_info.data }; });
	// clang-format on

	return telemetry_data;
}

void ysu::telemetry::get_metrics_single_peer_async (std::shared_ptr<ysu::transport::channel> const & channel_a, std::function<void(telemetry_data_response const &)> const & callback_a)
{
	auto invoke_callback_with_error = [&callback_a, &worker = this->worker, channel_a]() {
		...
		worker.push_task ([callback_a, endpoint]() {
			...
			callback_a ({ ysu::telemetry_data{}, endpoint, error });
		});
	};
	...
	if (!stopped)
	{
		...
		{
			auto add_callback_async = [& worker = this->worker, &callback_a](telemetry_data const & telemetry_data_a, ysu::endpoint const & endpoint_a) {
			...
			else
			{
				...
				else
				{
					if (it == recent_or_initial_request_telemetry_data.cend ())
					{
						...
						recent_or_initial_request_telemetry_data.emplace (channel_a->get_endpoint (), ysu::telemetry_data (), std::chrono::steady_clock::now (), true);
						it = recent_or_initial_request_telemetry_data.find (channel_a->get_endpoint ());
					}
					else
					{
						recent_or_initial_request_telemetry_data.modify (it, [](ysu::telemetry_info & telemetry_info_a) {
...
ysu::telemetry_data_response ysu::telemetry::get_metrics_single_peer (std::shared_ptr<ysu::transport::channel> const & channel_a)
{
...
void ysu::telemetry::fire_request_message (std::shared_ptr<ysu::transport::channel> const & channel_a)
{
	...
	std::weak_ptr<ysu::telemetry> this_w (shared_from_this ());
	...
	channel_a->send (message, [this_w, endpoint = channel_a->get_endpoint (), round_l](boost::system::error_code const & ec, size_t size_a) {
		if (auto this_l = this_w.lock ())
		{
			if (ec)
			{
				...
				this_l->stats.inc (ysu::stat::type::telemetry, ysu::stat::detail::failed_send_telemetry_req);
				ysu::lock_guard<std::mutex> guard (this_l->mutex);
			...
			else
			{
				...
				this_l->alarm.add (std::chrono::steady_clock::now () + this_l->response_time_cutoff, [round_l, this_w, endpoint]() {
					if (auto this_l = this_w.lock ())
					{
						...
						{
							this_l->stats.inc (ysu::stat::type::telemetry, ysu::stat::detail::no_response_received);
							this_l->channel_processed (endpoint, true);
						}
					}
				});			
	...
	ysu::buffer_drop_policy::no_socket_drop);
	// clang-format on
}

void ysu::telemetry::channel_processed (ysu::endpoint const & endpoint_a, bool error_a)
{
...
void ysu::telemetry::flush_callbacks_async (ysu::endpoint const & endpoint_a, bool error_a)
{
	// Post to worker so that it's truly async and not on the calling thread (same problem as std::async otherwise)
	worker.push_task ([endpoint_a, error_a, this_w = std::weak_ptr<ysu::telemetry> (shared_from_this ())]() {
		if (auto this_l = this_w.lock ())
		{
			ysu::unique_lock<std::mutex> lk (this_l->mutex);
...
void ysu::telemetry::invoke_callbacks (ysu::endpoint const & endpoint_a, bool error_a)
{
	std::vector<std::function<void(telemetry_data_response const &)>> callbacks_l;
	telemetry_data_response response_data{ ysu::telemetry_data (), endpoint_a, error_a };
	{
...
size_t ysu::telemetry::telemetry_data_size ()
{
	ysu::lock_guard<std::mutex> guard (mutex);
	return recent_or_initial_request_telemetry_data.size ();
}

ysu::telemetry_info::telemetry_info (ysu::endpoint const & endpoint_a, ysu::telemetry_data const & data_a, std::chrono::steady_clock::time_point last_response_a, bool undergoing_request_a) :
endpoint (endpoint_a),
data (data_a),
...
bool ysu::telemetry_info::awaiting_first_response () const
{
	return data == ysu::telemetry_data ();
}

std::unique_ptr<ysu::container_info_component> ysu::collect_container_info (telemetry & telemetry, const std::string & name)
{
	auto composite = std::make_unique<container_info_composite> (name);
	size_t callbacks_count;
	{
		ysu::lock_guard<std::mutex> guard (telemetry.mutex);
		std::unordered_map<ysu::endpoint, std::vector<std::function<void(telemetry_data_response const &)>>> callbacks;
...
ysu::telemetry_data ysu::consolidate_telemetry_data (std::vector<ysu::telemetry_data> const & telemetry_datas)
{
	...
	std::unordered_map<ysu::block_hash, int> genesis_blocks;
	...
	auto strip_outliers_and_sum = [num_either_side_to_remove](auto & counts) {
		...
		return std::accumulate (counts.begin (), counts.end (), ysu::uint128_t (0), [](ysu::uint128_t total, auto count) {
			return total += count;
		});
	};
	...
	ysu::telemetry_data consolidated_data;
...
ysu::telemetry_data ysu::local_telemetry_data (ysu::block_store & store_a, ysu::ledger_cache const & ledger_cache_a, ysu::network & network_a, uint64_t bandwidth_limit_a, ysu::network_params const & network_params_a, std::chrono::steady_clock::time_point statup_time_a, uint64_t active_difficulty_a, ysu::keypair const & node_id_a)
{
	...
	telemetry_data.peer_count = ysu::narrow_cast<decltype (telemetry_data.peer_count)> (network_a.size ());
	telemetry_data.account_count = ledger_cache_a.account_count;
	telemetry_data.major_version = ysu::get_major_node_version ();
	telemetry_data.minor_version = ysu::get_minor_node_version ();
	telemetry_data.patch_version = ysu::get_patch_node_version ();
	telemetry_data.pre_release_version = ysu::get_pre_release_node_version ();
