...
ysu::error ysu::ipc::ipc_config::serialize_toml (ysu::tomlconfig & toml) const
{
	ysu::tomlconfig tcp_l;
...
ysu::error ysu::ipc::ipc_config::deserialize_toml (ysu::tomlconfig & toml)
{
...
ysu::error ysu::ipc::ipc_config::serialize_json (ysu::jsonconfig & json) const
{
	ysu::jsonconfig tcp_l;
	// Only write out experimental config values if they're previously set explicitly in the config file
	if (transport_tcp.io_threads >= 0)
	{
...
ysu::error ysu::ipc::ipc_config::deserialize_json (bool & upgraded_a, ysu::jsonconfig & json)
{
