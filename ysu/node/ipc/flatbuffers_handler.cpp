...
namespace
{
auto handler_map = ysu::ipc::action_handler::handler_map ();
...
ysu::ipc::flatbuffers_handler::flatbuffers_handler (ysu::node & node_a, ysu::ipc::ipc_server & ipc_server_a, std::shared_ptr<ysu::ipc::subscriber> const & subscriber_a, ysu::ipc::ipc_config const & ipc_config_a) :
node (node_a),
...
std::shared_ptr<flatbuffers::Parser> ysu::ipc::flatbuffers_handler::make_flatbuffers_parser (ysu::ipc::ipc_config const & ipc_config_a)
{
	...
	if (!api_path)
	{
		throw ysu::error ("Internal IPC error: unable to find api path");
	}

	const char * include_directories[] = { api_path->string ().c_str (), nullptr };
	std::string schemafile;
	if (!flatbuffers::LoadFile ((*api_path / "ysuapi.fbs").string ().c_str (), false, &schemafile))
	{
		throw ysu::error ("Internal IPC error: unable to load schema file");
	}
	...
	if (!parse_success)
	{
		...
		throw ysu::error (parser_error);
	}
	return parser;
}

void ysu::ipc::flatbuffers_handler::process_json (const uint8_t * message_buffer_a, size_t buffer_size_a,
...
void ysu::ipc::flatbuffers_handler::process (const uint8_t * message_buffer_a, size_t buffer_size_a,
std::function<void(std::shared_ptr<flatbuffers::FlatBufferBuilder>)> response_handler)
{
	...
	try
	{
		...
		if (ipc_config.flatbuffers.verify_buffers)
		{
			...
			{
				throw ysu::error ("Envelope buffer did not pass verifier");
			}
		}

		auto incoming = ysuapi::GetEnvelope (message_buffer_a);
		if (incoming == nullptr)
		{
			ysu::error err ("Invalid message");
		...
		{
			ysu::error err ("Unknown message type");
			actionhandler->make_error (err.error_code_as_int (), err.get_message ());
		}
	}
	catch (ysu::error const & err)
