...
ysu::ipc::access_permission from_string (std::string permission)
{
	if (permission == "unrestricted")
		return ysu::ipc::access_permission::unrestricted;
	if (permission == "api_account_weight")
		return ysu::ipc::access_permission::api_account_weight;
	if (permission == "api_service_register")
		return ysu::ipc::access_permission::api_service_register;
	if (permission == "api_service_stop")
		return ysu::ipc::access_permission::api_service_stop;
	if (permission == "api_topic_service_stop")
		return ysu::ipc::access_permission::api_topic_service_stop;
	if (permission == "api_topic_confirmation")
		return ysu::ipc::access_permission::api_topic_confirmation;
	if (permission == "account_query")
		return ysu::ipc::access_permission::account_query;
	if (permission == "epoch_upgrade")
		return ysu::ipc::access_permission::epoch_upgrade;
	if (permission == "service")
		return ysu::ipc::access_permission::service;
	if (permission == "wallet")
		return ysu::ipc::access_permission::wallet;
	if (permission == "wallet_read")
		return ysu::ipc::access_permission::wallet_read;
	if (permission == "wallet_write")
		return ysu::ipc::access_permission::wallet_write;
	if (permission == "wallet_seed_change")
		return ysu::ipc::access_permission::wallet_seed_change;

	return ysu::ipc::access_permission::invalid;
}
}

void ysu::ipc::access::set_effective_permissions (ysu::ipc::access_subject & subject_a, std::shared_ptr<cpptoml::table> const & config_subject_a)
{
	...
	for (auto const & permission : allow_strings_l)
	{
		if (!permission.empty ())
		{
			...
			if (permission_enum != ysu::ipc::access_permission::invalid)
			{
...
void ysu::ipc::access::clear ()
{
	...
	default_user.permissions.insert (ysu::ipc::access_permission::api_account_weight);
}

ysu::error ysu::ipc::access::deserialize_toml (ysu::tomlconfig & toml)
{
...
bool ysu::ipc::access::has_access (std::string const & credentials_a, ysu::ipc::access_permission permssion_a) const
{
	ysu::unique_lock<std::mutex> lock (mutex);
	...
	if (user != users.end ())
	{
		...
		if (!permitted)
		{
			permitted = user->second.permissions.find (ysu::ipc::access_permission::unrestricted) != user->second.permissions.end ();
		}
	}
	return permitted;
}

bool ysu::ipc::access::has_access_to_all (std::string const & credentials_a, std::initializer_list<ysu::ipc::access_permission> permissions_a) const
{
	ysu::unique_lock<std::mutex> lock (mutex);
	bool permitted = false;
	auto user = users.find (credentials_a);
...
bool ysu::ipc::access::has_access_to_oneof (std::string const & credentials_a, std::initializer_list<ysu::ipc::access_permission> permissions_a) const
{
	ysu::unique_lock<std::mutex> lock (mutex);
	bool permitted = false;
	auto user = users.find (credentials_a);
...
void ysu::ipc::access_subject::clear ()
{
	permissions.clear ();
}

void ysu::ipc::access_user::clear ()
{
...
namespace ipc
{
	ysu::error read_access_config_toml (boost::filesystem::path const & data_path_a, ysu::ipc::access & config_a)
	{
		ysu::error error;
		auto toml_config_path = ysu::get_access_toml_config_path (data_path_a);
