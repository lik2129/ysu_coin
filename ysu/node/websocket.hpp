...
namespace websocket
{
	...
	{
	public:
		message (ysu::websocket::topic topic_a) :
		topic (topic_a)
		{
		}
		message (ysu::websocket::topic topic_a, boost::property_tree::ptree & tree_a) :
		topic (topic_a), contents (tree_a)
		{
		}

		std::string to_string () const;
		ysu::websocket::topic topic;
	...
	{
	public:
		message block_confirmed (std::shared_ptr<ysu::block> block_a, ysu::account const & account_a, ysu::amount const & amount_a, std::string subtype, bool include_block, ysu::election_status const & election_status_a, ysu::websocket::confirmation_options const & options_a);
		message stopped_election (ysu::block_hash const & hash_a);
		message vote_received (std::shared_ptr<ysu::vote> vote_a, ysu::vote_code code_a);
		message difficulty_changed (uint64_t publish_threshold_a, uint64_t receive_threshold_a, uint64_t difficulty_active_a);
		message work_generation (ysu::work_version const version_a, ysu::block_hash const & root_a, uint64_t const work_a, uint64_t const difficulty_a, uint64_t const publish_threshold_a, std::chrono::milliseconds const & duration_a, std::string const & peer_a, std::vector<std::string> const & bad_peers_a, bool const completed_a = true, bool const cancelled_a = false);
		message work_cancelled (ysu::work_version const version_a, ysu::block_hash const & root_a, uint64_t const difficulty_a, uint64_t const publish_threshold_a, std::chrono::milliseconds const & duration_a, std::vector<std::string> const & bad_peers_a);
		message work_failed (ysu::work_version const version_a, ysu::block_hash const & root_a, uint64_t const difficulty_a, uint64_t const publish_threshold_a, std::chrono::milliseconds const & duration_a, std::vector<std::string> const & bad_peers_a);
		...
		message telemetry_received (ysu::telemetry_data const &, ysu::endpoint const &);
		message new_block_arrived (ysu::block const & block_a);
	...
	{
	public:
		confirmation_options (ysu::wallets & wallets_a);
		confirmation_options (boost::property_tree::ptree const & options_a, ysu::wallets & wallets_a, ysu::logger_mt & logger_a);
	...
	private:
		...
		ysu::wallets & wallets;
		boost::optional<ysu::logger_mt &> logger;
	...
	{
	public:
		vote_options (boost::property_tree::ptree const & options_a, ysu::logger_mt & logger_a);
	...
	public:
		...
		explicit session (ysu::websocket::listener & listener_a, socket_type socket_a);
		~session ();
		...
		void write (ysu::websocket::message message_a);

	private:
		/** The owning listener */
		ysu::websocket::listener & ws_listener;
		/** Websocket */
	...
	{
	public:
		listener (ysu::logger_mt & logger_a, ysu::wallets & wallets_a, boost::asio::io_context & io_ctx_a, boost::asio::ip::tcp::endpoint endpoint_a);
		...
		void broadcast_confirmation (std::shared_ptr<ysu::block> block_a, ysu::account const & account_a, ysu::amount const & amount_a, std::string subtype, ysu::election_status const & election_status_a);

		/** Broadcast \p message to all session subscribing to the message topic. */
		void broadcast (ysu::websocket::message message_a);

		ysu::logger_mt & get_logger () const
		{
			return logger;
		}

		ysu::wallets & get_wallets () const
		{
			return wallets;
		}
		 ...
		 */
		bool any_subscriber (ysu::websocket::topic const & topic_a) const
		{
			return subscriber_count (topic_a) > 0;
		}
		/** Getter for subscriber count of a specific topic*/
		size_t subscriber_count (ysu::websocket::topic const & topic_a) const
		{
	...
	private:
		...
		friend ysu::websocket::session;

		/** Adds to subscription count of a specific topic*/
		void increase_subscriber_count (ysu::websocket::topic const & topic_a);
		/** Removes from subscription count of a specific topic*/
		void decrease_subscriber_count (ysu::websocket::topic const & topic_a);
