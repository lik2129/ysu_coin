...
#include <ysu/boost/asio/dispatch.hpp>
...
ysu::websocket::confirmation_options::confirmation_options (ysu::wallets & wallets_a) :
wallets (wallets_a)
{
}

ysu::websocket::confirmation_options::confirmation_options (boost::property_tree::ptree const & options_a, ysu::wallets & wallets_a, ysu::logger_mt & logger_a) :
wallets (wallets_a),
logger (logger_a)
{
...
bool ysu::websocket::confirmation_options::should_filter (ysu::websocket::message const & message_a) const
{
	...
	if (destination_opt_l)
	{
		...
		if (all_local_accounts)
		{
			...
			ysu::account source_l (0), destination_l (0);
...
bool ysu::websocket::confirmation_options::update (boost::property_tree::ptree const & options_a)
{
...
void ysu::websocket::confirmation_options::check_filter_empty () const
{
...
ysu::websocket::vote_options::vote_options (boost::property_tree::ptree const & options_a, ysu::logger_mt & logger_a)
{
...
bool ysu::websocket::vote_options::should_filter (ysu::websocket::message const & message_a) const
{
...
ysu::websocket::session::session (ysu::websocket::listener & listener_a, socket_type socket_a) :
...
ysu::websocket::session::~session ()
{
	{
		ysu::unique_lock<std::mutex> lk (subscriptions_mutex);
...
void ysu::websocket::session::handshake ()
{
...
void ysu::websocket::session::close ()
{
...
void ysu::websocket::session::write (ysu::websocket::message message_a)
{
	ysu::unique_lock<std::mutex> lk (subscriptions_mutex);
	auto subscription (subscriptions.find (message_a.topic));
	if (message_a.topic == ysu::websocket::topic::ack || (subscription != subscriptions.end () && !subscription->second->should_filter (message_a)))
	{
		lk.unlock ();
		auto this_l (shared_from_this ());
...
void ysu::websocket::session::write_queued_messages ()
{
	auto msg (send_queue.front ().to_string ());
	auto this_l (shared_from_this ());

	ws.async_write (ysu::shared_const_buffer (msg),
...
void ysu::websocket::session::read ()
{
	auto this_l (shared_from_this ());
...
namespace
{
ysu::websocket::topic to_topic (std::string const & topic_a)
{
	ysu::websocket::topic topic = ysu::websocket::topic::invalid;
	if (topic_a == "confirmation")
	{
		topic = ysu::websocket::topic::confirmation;
	}
	else if (topic_a == "stopped_election")
	{
		topic = ysu::websocket::topic::stopped_election;
	}
	else if (topic_a == "vote")
	{
		topic = ysu::websocket::topic::vote;
	}
	else if (topic_a == "ack")
	{
		topic = ysu::websocket::topic::ack;
	}
	else if (topic_a == "active_difficulty")
	{
		topic = ysu::websocket::topic::active_difficulty;
	}
	else if (topic_a == "work")
	{
		topic = ysu::websocket::topic::work;
	}
	else if (topic_a == "bootstrap")
	{
		topic = ysu::websocket::topic::bootstrap;
	}
	else if (topic_a == "telemetry")
	{
		topic = ysu::websocket::topic::telemetry;
	}
	else if (topic_a == "new_unconfirmed_block")
	{
		topic = ysu::websocket::topic::new_unconfirmed_block;
	}

	return topic;
}

std::string from_topic (ysu::websocket::topic topic_a)
{
	std::string topic = "invalid";
	if (topic_a == ysu::websocket::topic::confirmation)
	{
		topic = "confirmation";
	}
	else if (topic_a == ysu::websocket::topic::stopped_election)
	{
		topic = "stopped_election";
	}
	else if (topic_a == ysu::websocket::topic::vote)
	{
		topic = "vote";
	}
	else if (topic_a == ysu::websocket::topic::ack)
	{
		topic = "ack";
	}
	else if (topic_a == ysu::websocket::topic::active_difficulty)
	{
		topic = "active_difficulty";
	}
	else if (topic_a == ysu::websocket::topic::work)
	{
		topic = "work";
	}
	else if (topic_a == ysu::websocket::topic::bootstrap)
	{
		topic = "bootstrap";
	}
	else if (topic_a == ysu::websocket::topic::telemetry)
	{
		topic = "telemetry";
	}
	else if (topic_a == ysu::websocket::topic::new_unconfirmed_block)
	{
...
void ysu::websocket::session::send_ack (std::string action_a, std::string id_a)
{
	...
	ysu::websocket::message msg (ysu::websocket::topic::ack);
...
void ysu::websocket::session::handle_message (boost::property_tree::ptree const & message_a)
{
	...
	if (action == "subscribe" && topic_l != ysu::websocket::topic::invalid)
	{
		auto options_text_l (message_a.get_child_optional ("options"));
		ysu::lock_guard<std::mutex> lk (subscriptions_mutex);
		std::unique_ptr<ysu::websocket::options> options_l{ nullptr };
		if (options_text_l && topic_l == ysu::websocket::topic::confirmation)
		{
			options_l = std::make_unique<ysu::websocket::confirmation_options> (options_text_l.get (), ws_listener.get_wallets (), ws_listener.get_logger ());
		}
		else if (options_text_l && topic_l == ysu::websocket::topic::vote)
		{
			options_l = std::make_unique<ysu::websocket::vote_options> (options_text_l.get (), ws_listener.get_logger ());
		}
		else
		{
			options_l = std::make_unique<ysu::websocket::options> ();
		}
	...
	else if (action == "unsubscribe" && topic_l != ysu::websocket::topic::invalid)
	{
		ysu::lock_guard<std::mutex> lk (subscriptions_mutex);
...
void ysu::websocket::listener::stop ()
{
	stopped = true;
	acceptor.close ();

	ysu::lock_guard<std::mutex> lk (sessions_mutex);
...
ysu::websocket::listener::listener (ysu::logger_mt & logger_a, ysu::wallets & wallets_a, boost::asio::io_context & io_ctx_a, boost::asio::ip::tcp::endpoint endpoint_a) :
logger (logger_a),
wallets (wallets_a),
acceptor (io_ctx_a),
socket (io_ctx_a)
...
void ysu::websocket::listener::run ()
{
	if (acceptor.is_open ())
	{
		accept ();
...
void ysu::websocket::listener::accept ()
{
	auto this_l (shared_from_this ());
...
void ysu::websocket::listener::on_accept (boost::system::error_code ec)
{
	if (ec)
	{
	...
	else
	{
		...
		auto session (std::make_shared<ysu::websocket::session> (*this, std::move (socket)));
		sessions_mutex.lock ();
...
void ysu::websocket::listener::broadcast_confirmation (std::shared_ptr<ysu::block> block_a, ysu::account const & account_a, ysu::amount const & amount_a, std::string subtype, ysu::election_status const & election_status_a)
{
	ysu::websocket::message_builder builder;

	ysu::lock_guard<std::mutex> lk (sessions_mutex);
	boost::optional<ysu::websocket::message> msg_with_block;
	boost::optional<ysu::websocket::message> msg_without_block;
	for (auto & weak_session : sessions)
	{
		auto session_ptr (weak_session.lock ());
		if (session_ptr)
		{
			auto subscription (session_ptr->subscriptions.find (ysu::websocket::topic::confirmation));
			if (subscription != session_ptr->subscriptions.end ())
			{
				ysu::websocket::confirmation_options default_options (wallets);
				auto conf_options (dynamic_cast<ysu::websocket::confirmation_options *> (subscription->second.get ()));
...
void ysu::websocket::listener::broadcast (ysu::websocket::message message_a)
{
	ysu::lock_guard<std::mutex> lk (sessions_mutex);
	for (auto & weak_session : sessions)
	{
...
void ysu::websocket::listener::increase_subscriber_count (ysu::websocket::topic const & topic_a)
{
	topic_subscriber_count[static_cast<std::size_t> (topic_a)] += 1;
}

void ysu::websocket::listener::decrease_subscriber_count (ysu::websocket::topic const & topic_a)
{
...
ysu::websocket::message ysu::websocket::message_builder::stopped_election (ysu::block_hash const & hash_a)
{
	ysu::websocket::message message_l (ysu::websocket::topic::stopped_election);
...
ysu::websocket::message ysu::websocket::message_builder::block_confirmed (std::shared_ptr<ysu::block> block_a, ysu::account const & account_a, ysu::amount const & amount_a, std::string subtype, bool include_block_a, ysu::election_status const & election_status_a, ysu::websocket::confirmation_options const & options_a)
{
	ysu::websocket::message message_l (ysu::websocket::topic::confirmation);
	set_common_fields (message_l);
...
ysu::websocket::message ysu::websocket::message_builder::vote_received (std::shared_ptr<ysu::vote> vote_a, ysu::vote_code code_a)
{
	ysu::websocket::message message_l (ysu::websocket::topic::vote);
	set_common_fields (message_l);
	...
	switch (code_a)
	{
		case ysu::vote_code::vote:
			vote_type = "vote";
			break;
		case ysu::vote_code::replay:
			vote_type = "replay";
			break;
		case ysu::vote_code::indeterminate:
			vote_type = "indeterminate";
			break;
		case ysu::vote_code::invalid:
			debug_assert (false);
			break;
	}
...
ysu::websocket::message ysu::websocket::message_builder::difficulty_changed (uint64_t publish_threshold_a, uint64_t receive_threshold_a, uint64_t difficulty_active_a)
{
	ysu::websocket::message message_l (ysu::websocket::topic::active_difficulty);
	set_common_fields (message_l);

	// Active difficulty information
	boost::property_tree::ptree difficulty_l;
	difficulty_l.put ("network_minimum", ysu::to_string_hex (publish_threshold_a));
	difficulty_l.put ("network_receive_minimum", ysu::to_string_hex (receive_threshold_a));
	difficulty_l.put ("network_current", ysu::to_string_hex (difficulty_active_a));
	auto multiplier = ysu::difficulty::to_multiplier (difficulty_active_a, publish_threshold_a);
	difficulty_l.put ("multiplier", ysu::to_string (multiplier));
	auto const receive_current_denormalized (ysu::denormalized_multiplier (multiplier, ysu::network_params{}.network.publish_thresholds.epoch_2_receive));
	difficulty_l.put ("network_receive_current", ysu::to_string_hex (ysu::difficulty::from_multiplier (receive_current_denormalized, receive_threshold_a)));

	message_l.contents.add_child ("message", difficulty_l);
	return message_l;
}

ysu::websocket::message ysu::websocket::message_builder::work_generation (ysu::work_version const version_a, ysu::block_hash const & root_a, uint64_t work_a, uint64_t difficulty_a, uint64_t publish_threshold_a, std::chrono::milliseconds const & duration_a, std::string const & peer_a, std::vector<std::string> const & bad_peers_a, bool completed_a, bool cancelled_a)
{
	ysu::websocket::message message_l (ysu::websocket::topic::work);
	set_common_fields (message_l);
	...
	request_l.put ("version", ysu::to_string (version_a));
	request_l.put ("hash", root_a.to_string ());
	request_l.put ("difficulty", ysu::to_string_hex (difficulty_a));
	auto request_multiplier_l (ysu::difficulty::to_multiplier (difficulty_a, publish_threshold_a));
	request_l.put ("multiplier", ysu::to_string (request_multiplier_l));
	...
	if (completed_a)
	{
		...
		result_l.put ("work", ysu::to_string_hex (work_a));
		auto result_difficulty_l (ysu::work_difficulty (version_a, root_a, work_a));
		result_l.put ("difficulty", ysu::to_string_hex (result_difficulty_l));
		auto result_multiplier_l (ysu::difficulty::to_multiplier (result_difficulty_l, publish_threshold_a));
		result_l.put ("multiplier", ysu::to_string (result_multiplier_l));
...
ysu::websocket::message ysu::websocket::message_builder::work_cancelled (ysu::work_version const version_a, ysu::block_hash const & root_a, uint64_t const difficulty_a, uint64_t const publish_threshold_a, std::chrono::milliseconds const & duration_a, std::vector<std::string> const & bad_peers_a)
{
	return work_generation (version_a, root_a, 0, difficulty_a, publish_threshold_a, duration_a, "", bad_peers_a, false, true);
}

ysu::websocket::message ysu::websocket::message_builder::work_failed (ysu::work_version const version_a, ysu::block_hash const & root_a, uint64_t const difficulty_a, uint64_t const publish_threshold_a, std::chrono::milliseconds const & duration_a, std::vector<std::string> const & bad_peers_a)
{
	return work_generation (version_a, root_a, 0, difficulty_a, publish_threshold_a, duration_a, "", bad_peers_a, false, false);
}

ysu::websocket::message ysu::websocket::message_builder::bootstrap_started (std::string const & id_a, std::string const & mode_a)
{
	ysu::websocket::message message_l (ysu::websocket::topic::bootstrap);
	set_common_fields (message_l);
...
ysu::websocket::message ysu::websocket::message_builder::bootstrap_exited (std::string const & id_a, std::string const & mode_a, std::chrono::steady_clock::time_point const start_time_a, uint64_t const total_blocks_a)
{
	ysu::websocket::message message_l (ysu::websocket::topic::bootstrap);
	set_common_fields (message_l);
...
ysu::websocket::message ysu::websocket::message_builder::telemetry_received (ysu::telemetry_data const & telemetry_data_a, ysu::endpoint const & endpoint_a)
{
	ysu::websocket::message message_l (ysu::websocket::topic::telemetry);
	set_common_fields (message_l);

	// Telemetry information
	ysu::jsonconfig telemetry_l;
...
ysu::websocket::message ysu::websocket::message_builder::new_block_arrived (ysu::block const & block_a)
{
	ysu::websocket::message message_l (ysu::websocket::topic::new_unconfirmed_block);
	set_common_fields (message_l);

	boost::property_tree::ptree block_l;
	block_a.serialize_json (block_l);
	auto subtype (ysu::state_subtype (block_a.sideband ().details));
...
void ysu::websocket::message_builder::set_common_fields (ysu::websocket::message & message_a)
{
...
std::string ysu::websocket::message::to_string () const
{
