...
class peer_exclusion final
{
	...
	public:
		...
		decltype (std::declval<ysu::tcp_endpoint> ().address ()) address;
		uint64_t score;
	};
...
public:
	...
	uint64_t add (ysu::tcp_endpoint const &, size_t const);
	bool check (ysu::tcp_endpoint const &);
	void remove (ysu::tcp_endpoint const &);
