...
std::string ysu::port_mapping::get_config_port (std::string const & node_port_a)
{
	return node.config.external_port != 0 ? std::to_string (node.config.external_port) : node_port_a;
}

void ysu::port_mapping::refresh_devices ()
{
...
ysu::endpoint ysu::port_mapping::external_address ()
{
	ysu::endpoint result_l (boost::asio::ip::address_v6{}, 0);
	ysu::lock_guard<std::mutex> guard_l (mutex);
	for (auto & protocol : protocols | boost::adaptors::filtered ([](auto const & p) { return p.enabled; }))
	{
		if (protocol.external_port != 0)
		{
			result_l = ysu::endpoint (protocol.external_address, protocol.external_port);
		}
	}
	return result_l;
}

void ysu::port_mapping::refresh_mapping ()
{
	...
	if (on)
	{
		...
		for (auto & protocol : protocols | boost::adaptors::filtered ([](auto const & p) { return p.enabled; }))
		{
			...
			auto upnp_description = std::string ("Ysu Node (") + network_params.network.get_current_network_as_string () + ")";
...
bool ysu::port_mapping::check_mapping ()
{
...
void ysu::port_mapping::check_mapping_loop ()
{
	refresh_devices ();
...
ysu::upnp_state & ysu::upnp_state::operator= (ysu::upnp_state && other_a)
{
	if (this == &other_a)
	{
		return *this;
