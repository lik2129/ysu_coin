...
ysu::rpc_handler::rpc_handler (ysu::rpc_config const & rpc_config, std::string const & body_a, std::string const & request_id_a, std::function<void(std::string const &)> const & response_a, ysu::rpc_handler_interface & rpc_handler_interface_a, ysu::logger_mt & logger) :
body (body_a),
...
void ysu::rpc_handler::process_request (ysu::rpc_handler_request_params const & request_params)
{
	try
	{
		...
		else
		{
			if (request_params.rpc_version == 1)
			{
				...
				std::error_code rpc_control_disabled_ec = ysu::error_rpc::rpc_control_disabled;
				...
				else
				{
					...
					{
						ysu::uint128_union random_id;
						ysu::random_pool::generate_block (random_id.bytes.data (), random_id.bytes.size ());
