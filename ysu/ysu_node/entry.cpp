...
#ifndef _WIN32
#ifdef YSU_STACKTRACE_BACKTRACE
...
int main (int argc, char * const * argv)
{
	...
	description.add_options ()
		...
		("config", boost::program_options::value<std::vector<ysu::config_key_value_pair>>()->multitoken(), "Pass node configuration values. This takes precedence over any values in the configuration file. This option can be repeated multiple times.")
		...
		("debug_output_last_backtrace_dump", "Displays the contents of the latest backtrace in the event of a ysu_node crash")
		("debug_generate_crash_report", "Consolidates the ysu_node_backtrace.dump file. Requires addr2line installed on Linux")
		...
		("debug_profile_process", "Profile active blocks processing (only for ysu_dev_network)")
		("debug_profile_votes", "Profile votes processing (only for ysu_dev_network)")
		("debug_profile_frontiers_confirmation", "Profile frontiers confirmation speed (only for ysu_dev_network)")
	...
	if (network != vm.end ())
	{
		...
		if (err)
		{
			std::cerr << ysu::network_constants::active_network_err_msg << std::endl;
			std::exit (1);
		}
	}
	...
	boost::filesystem::path data_path ((data_path_it != vm.end ()) ? data_path_it->second.as<std::string> () : ysu::working_path ());
	auto ec = ysu::handle_node_options (vm);
	if (ec == ysu::error_cli::unknown_command)
	{
		if (vm.count ("daemon") > 0)
		{
			ysu_daemon::daemon daemon;
		...
		{
			if (!ysu::network_constants ().is_dev_network ())
			{
				auto node_flags = ysu::inactive_node_flag_defaults ();
				ysu::update_flags (node_flags, vm);
				node_flags.generate_cache.reps = true;
				ysu::inactive_node inactive_node (data_path, node_flags);
				...
				auto get_total = [](decltype (bootstrap_weights.second) const & reps) -> ysu::uint128_union {
					return std::accumulate (reps.begin (), reps.end (), ysu::uint128_t{ 0 }, [](auto sum, auto const & rep) { return sum + rep.second; });
				};
				...
				{
					std::vector<std::pair<ysu::account, ysu::uint128_t>> sorted;
				...
				{
					ysu::account rep;
					ysu::uint128_union hardcoded;
					ysu::uint128_union ledger;
					ysu::uint128_union diff;
					std::string get_entry () const
					{
						return boost::str (boost::format ("representative %1% hardcoded %2% ledger %3% mismatch %4%")
						% rep.to_account () % hardcoded.format_balance (ysu::Mxrb_ratio, 0, true) % ledger.format_balance (ysu::Mxrb_ratio, 0, true) % diff.format_balance (ysu::Mxrb_ratio, 0, true));
					}
				};
				...
				std::transform (hardcoded.begin (), hardcoded.end (), std::back_inserter (mismatched), [&ledger](auto const & rep) {
					...
					ysu::uint128_t ledger_weight = (ledger_rep == ledger.end () ? 0 : ledger_rep->second);
				...
				ysu::uint128_union const mismatch_total = std::accumulate (mismatched.begin (), mismatched.end (), ysu::uint128_t{ 0 }, [](auto sum, mismatched_t const & sample) { return sum + sample.diff.number (); });
				ysu::uint128_union const mismatch_mean = mismatch_total.number () / mismatched.size ();

				ysu::uint512_union mismatch_variance = std::accumulate (mismatched.begin (), mismatched.end (), ysu::uint512_t (0), [M = mismatch_mean.number (), N = mismatched.size ()](ysu::uint512_t sum, mismatched_t const & sample) {
					auto x = sample.diff.number ();
					ysu::uint512_t const mean_diff = x > M ? x - M : M - x;
					ysu::uint512_t const sqr = mean_diff * mean_diff;
					return sum + sqr;
				})
				/ mismatched.size ();

				ysu::uint128_union const mismatch_stddev = ysu::narrow_cast<ysu::uint128_t> (boost::multiprecision::sqrt (mismatch_variance.number ()));

				auto const outlier_threshold = std::max (ysu::Gxrb_ratio, mismatch_mean.number () + 1 * mismatch_stddev.number ());
				...
				auto const newcomer_threshold = std::max (ysu::Gxrb_ratio, mismatch_mean.number ());
				std::vector<std::pair<ysu::account, ysu::uint128_t>> newcomers;
				...
				auto newcomer_entry = [](auto const & rep) {
					return boost::str (boost::format ("representative %1% hardcoded --- ledger %2%") % rep.first.to_account () % ysu::uint128_union (rep.second).format_balance (ysu::Mxrb_ratio, 0, true));
				};

				std::cout << boost::str (boost::format ("hardcoded weight %1% Mysu at %2% blocks\nledger weight %3% Mysu at %4% blocks\nmismatched\n\tsamples %5%\n\ttotal %6% Mysu\n\tmean %7% Mysu\n\tsigma %8% Mysu\n")
				% total_hardcoded.format_balance (ysu::Mxrb_ratio, 0, true)
				% hardcoded_height
				% total_ledger.format_balance (ysu::Mxrb_ratio, 0, true)
				% ledger_height
				% mismatched.size ()
				% mismatch_total.format_balance (ysu::Mxrb_ratio, 0, true)
				% mismatch_mean.format_balance (ysu::Mxrb_ratio, 0, true)
				% mismatch_stddev.format_balance (ysu::Mxrb_ratio, 0, true));
				...
				// Log more data
				auto const log_threshold = ysu::Gxrb_ratio;
		...
		else if (vm.count ("debug_bootstrap_generate"))
		{
			...
			if (key_it != vm.end ())
			{
				...
				{
					ysu::keypair genesis (key.to_string ());
					...
					ysu::uint128_t balance (std::numeric_limits<ysu::uint128_t>::max ());
					ysu::open_block genesis_block (reinterpret_cast<const ysu::block_hash &> (genesis.pub), genesis.pub, genesis.pub, genesis.prv, genesis.pub, *work.generate (ysu::work_version::work_1, genesis.pub, network_constants.publish_thresholds.epoch_1));
					std::cout << genesis_block.to_json ();
					std::cout.flush ();
					ysu::block_hash previous (genesis_block.hash ());
					for (auto i (0); i != 8; ++i)
					{
						ysu::uint128_t yearly_distribution (ysu::uint128_t (1) << (127 - (i == 7 ? 6 : i)));
						...
						for (auto j (0); j != 52; ++j)
						{
							...
							ysu::send_block send (previous, landing.pub, balance, genesis.prv, genesis.pub, *work.generate (ysu::work_version::work_1, previous, network_constants.publish_thresholds.epoch_1));
							previous = send.hash ();
		...
		{
			auto inactive_node = ysu::default_inactive_node (data_path, vm);
			auto node = inactive_node->node;
		...
		else if (vm.count ("debug_dump_representatives"))
		{
			...
			std::map<ysu::account, ysu::uint128_t> ordered_reps (rep_amounts.begin (), rep_amounts.end ());
		...
		else if (vm.count ("debug_dump_frontier_unchecked_dependents"))
		{
			...
			auto transaction (node->store.tx_begin_read ());
			std::unordered_set<ysu::block_hash> frontier_hashes;
		...
		else if (vm.count ("debug_profile_kdf"))
		{
			...
			ysu::uint256_union result;
			ysu::uint256_union salt (0);
			std::string password ("");
			while (true)
			{
		...
		else if (vm.count ("debug_profile_generate"))
		{
			...
			if (multiplier_it != vm.end ())
			{
				try
				{
					...
					difficulty = ysu::difficulty::from_multiplier (multiplier, difficulty);
				}
			...
			else
			{
				...
				{
					if (ysu::from_string_hex (difficulty_it->second.as<std::string> (), difficulty))
					{
			...
			ysu::work_pool work (std::numeric_limits<unsigned>::max (), pow_rate_limiter);
			ysu::change_block block (0, 0, ysu::keypair ().prv, 0, 0);
			if (!result)
			{
				std::cerr << boost::str (boost::format ("Starting generation profiling. Difficulty: %1$#x (%2%x from base difficulty %3$#x)\n") % difficulty % ysu::to_string (ysu::difficulty::to_multiplier (difficulty, network_constants.publish_full.base), 4) % network_constants.publish_full.base);
				while (!result)
				{
					block.hashables.previous.qwords[0] += 1;
					auto begin1 (std::chrono::high_resolution_clock::now ());
					block.block_work_set (*work.generate (ysu::work_version::work_1, block.root (), difficulty));
					auto end1 (std::chrono::high_resolution_clock::now ());
		...
		{
			uint64_t difficulty{ ysu::network_constants ().publish_full.base };
			...
			{
				valid = ysu::work_v1::value (hash, i) > difficulty;
			}
		...
		else if (vm.count ("debug_opencl"))
		{
			ysu::network_constants network_constants;
			bool error (false);
			ysu::opencl_environment environment (error);
			if (!error)
			{
				...
				if (!result)
				{
					...
					if (!error)
					{
						...
						if (!error)
						{
							...
							ysu::opencl_config config (platform, device, threads);
							auto opencl (ysu::opencl_work::create (true, config, logger));
							ysu::work_pool work_pool (0, std::chrono::nanoseconds (0), opencl ? [&opencl](ysu::work_version const version_a, ysu::root const & root_a, uint64_t difficulty_a, std::atomic<int> &) {
								return opencl->generate_work (version_a, root_a, difficulty_a);
							}
							                                                                   : std::function<boost::optional<uint64_t> (ysu::work_version const, ysu::root const &, uint64_t, std::atomic<int> &)> (nullptr));
							ysu::change_block block (0, 0, ysu::keypair ().prv, 0, 0);
							std::cerr << boost::str (boost::format ("Starting OpenCL generation profiling. Platform: %1%. Device: %2%. Threads: %3%. Difficulty: %4$#x (%5%x from base difficulty %6$#x)\n") % platform % device % threads % difficulty % ysu::to_string (ysu::difficulty::to_multiplier (difficulty, network_constants.publish_full.base), 4) % network_constants.publish_full.base);
							for (uint64_t i (0); true; ++i)
							{
								block.hashables.previous.qwords[0] += 1;
								auto begin1 (std::chrono::high_resolution_clock::now ());
								block.block_work_set (*work_pool.generate (ysu::work_version::work_1, block.root (), difficulty));
								auto end1 (std::chrono::high_resolution_clock::now ());
								std::cerr << boost::str (boost::format ("%|1$ 12d|\n") % std::chrono::duration_cast<std::chrono::microseconds> (end1 - begin1).count ());
							}
						}
		...
		{
			if (boost::filesystem::exists ("ysu_node_backtrace.dump"))
			{
				// There is a backtrace, so output the contents
				std::ifstream ifs ("ysu_node_backtrace.dump");
		...
		else if (vm.count ("debug_generate_crash_report"))
		{
			if (boost::filesystem::exists ("ysu_node_backtrace.dump"))
			{
				...
				std::string crash_report_filename = "ysu_node_crash_report.txt";
				...
				ofs << st;
#else
				// Read all the ysu node files
				boost::system::error_code err;
				auto running_executable_filepath = boost::dll::program_location (err);
				if (!err)
				{
					auto num = 0;
					auto format = boost::format ("ysu_node_crash_load_address_dump_%1%.txt");
					...
					{
						std::ofstream ofs (crash_report_filename);
						ysu::set_secure_perm_file (crash_report_filename);
					}
					...
					// Now run using relative addresses. This will give actual results for other dlls, the results from the ysu_node executable.
					run_addr2line (true);

					if (std::find (system_codes.begin (), system_codes.end (), 0) == system_codes.end ())
					{
						std::cerr << "Error: Check that addr2line is installed and that ysu_node_crash_load_address_dump_*.txt files exist." << std::endl;
						result = -1;
					}
				}
				else
			...
			{
				std::cerr << "Error: ysu_node_backtrace.dump could not be found";
				result = -1;
			}
		}
		...
		{
			ysu::keypair key;
			ysu::uint256_union message;
			auto signature = ysu::sign_message (key.prv, key.pub, message);
			auto begin (std::chrono::high_resolution_clock::now ());
			for (auto i (0u); i < 1000; ++i)
			{
				ysu::validate_message (key.pub, message, signature);
			}
		...
		else if (vm.count ("debug_verify_profile_batch"))
		{
			...
			ysu::uint256_union message;
			ysu::uint512_union signature (ysu::sign_message (key.prv, key.pub, message));
			...
			auto begin (std::chrono::high_resolution_clock::now ());
			ysu::validate_message_batch (messages.data (), lengths.data (), pub_keys.data (), signatures.data (), batch_count, verifications.data ());
			auto end (std::chrono::high_resolution_clock::now ());
		...
		else if (vm.count ("debug_profile_sign"))
		{
			...
			{
				ysu::keypair key;
				ysu::block_hash latest (0);
				auto begin1 (std::chrono::high_resolution_clock::now ());
				for (uint64_t balance (0); balance < 1000; ++balance)
				{
					ysu::send_block send (latest, key.pub, balance, key.prv, key.pub, 0);
					latest = send.hash ();
				}
				auto end1 (std::chrono::high_resolution_clock::now ());
				std::cerr << boost::str (boost::format ("%|1$ 12d|\n") % std::chrono::duration_cast<std::chrono::microseconds> (end1 - begin1).count ());
		...
		else if (vm.count ("debug_profile_process"))
		{
			...
			ysu::update_flags (node_flags, vm);
			auto node (std::make_shared<ysu::node> (io_ctx, 24001, path, alarm, logging, work, node_flags));
			ysu::block_hash genesis_latest (node->latest (dev_params.ledger.dev_genesis_key.pub));
			ysu::uint128_t genesis_balance (std::numeric_limits<ysu::uint128_t>::max ());
			// Generating keys
			std::vector<ysu::keypair> keys (num_accounts);
			std::vector<ysu::root> frontiers (num_accounts);
			std::vector<ysu::uint128_t> balances (num_accounts, 1000000000);
			// Generating blocks
			std::deque<std::shared_ptr<ysu::block>> blocks;
			for (auto i (0); i != num_accounts; ++i)
			{
				...
				auto send = builder.state ()
				            ...
				            .work (*work.generate (ysu::work_version::work_1, genesis_latest, node->network_params.network.publish_thresholds.epoch_1))
				            .build ();
				...
				auto open = builder.state ()
				            ...
				            .work (*work.generate (ysu::work_version::work_1, keys[i].pub, node->network_params.network.publish_thresholds.epoch_1))
				            .build ();
			...
			for (auto i (0); i != num_iterations; ++i)
			{
				for (auto j (0); j != num_accounts; ++j)
				{
					...
					auto send = builder.state ()
					            ...
					            .work (*work.generate (ysu::work_version::work_1, frontiers[j], node->network_params.network.publish_thresholds.epoch_1))
					            .build ();
					...
					auto receive = builder.state ()
					               ...
					               .work (*work.generate (ysu::work_version::work_1, frontiers[other], node->network_params.network.publish_thresholds.epoch_1))
					               .build ();
			...
			ysu::timer<std::chrono::seconds> timer_l (ysu::timer_state::started);
		...
		else if (vm.count ("debug_profile_votes"))
		{
			...
			auto path (ysu::unique_path ());
			logging.init (path);
			auto node (std::make_shared<ysu::node> (io_ctx, 24001, path, alarm, logging, work));
			ysu::block_hash genesis_latest (node->latest (dev_params.ledger.dev_genesis_key.pub));
			ysu::uint128_t genesis_balance (std::numeric_limits<ysu::uint128_t>::max ());
			// Generating keys
			std::vector<ysu::keypair> keys (num_representatives);
			ysu::uint128_t balance ((node->config.online_weight_minimum.number () / num_representatives) + 1);
			...
			// Generating votes
			std::deque<std::shared_ptr<ysu::vote>> votes;
			for (auto j (0); j != num_representatives; ++j)
			{
				uint64_t sequence (1);
				for (auto & i : blocks)
				{
					auto vote (std::make_shared<ysu::vote> (keys[j].pub, keys[j].prv, sequence, std::vector<ysu::block_hash> (1, i->hash ())));
		...
		else if (vm.count ("debug_profile_frontiers_confirmation"))
		{
			...
			ysu::alarm alarm1 (io_ctx1);
			ysu::alarm alarm2 (io_ctx2);
			ysu::work_pool work (std::numeric_limits<unsigned>::max ());
			ysu::logging logging;
			auto path1 (ysu::unique_path ());
			auto path2 (ysu::unique_path ());
			logging.init (path1);
			ysu::node_config config1 (24000, logging);
			ysu::node_flags flags;
			flags.disable_lazy_bootstrap = true;
			flags.disable_legacy_bootstrap = true;
			flags.disable_wallet_bootstrap = true;
			flags.disable_bootstrap_listener = true;
			auto node1 (std::make_shared<ysu::node> (io_ctx1, path1, alarm1, config1, work, flags, 0));
			ysu::block_hash genesis_latest (node1->latest (dev_params.ledger.dev_genesis_key.pub));
			ysu::uint128_t genesis_balance (std::numeric_limits<ysu::uint128_t>::max ());
			// Generating blocks
			std::deque<std::shared_ptr<ysu::block>> blocks;
			for (auto i (0); i != count; ++i)
			{
				...
				auto send = builder.state ()
				            ...
				            .link (key.pub)
				            .sign (dev_params.ledger.dev_genesis_key.prv, dev_params.ledger.dev_genesis_key.pub)
				            .work (*work.generate (ysu::work_version::work_1, genesis_latest, dev_params.network.publish_thresholds.epoch_1))
				            .build ();
				...
				auto open = builder.state ()
				            ...
				            .work (*work.generate (ysu::work_version::work_1, key.pub, dev_params.network.publish_thresholds.epoch_1))
				            .build ();
			...
			node1->start ();
			ysu::thread_runner runner1 (io_ctx1, node1->config.io_threads);
			...
			// Start new node
			ysu::node_config config2 (24001, logging);
			// Config override
			...
			{
				auto path (ysu::unique_path ());
				ysu::daemon_config daemon_config (path);
				auto error = ysu::read_node_config_toml (path, daemon_config, config_overrides);
				if (error)
				{
					std::cerr << "\n"
			...
			auto node2 (std::make_shared<ysu::node> (io_ctx2, path2, alarm2, config2, work, flags, 1));
			node2->start ();
			ysu::thread_runner runner2 (io_ctx2, node2->config.io_threads);
		...
		else if (vm.count ("debug_random_feed"))
		{
			 ...
			 *
			 *   ./ysu_node --debug_random_feed | dieharder -a -g 200
			 */
			ysu::raw_key seed;
			for (;;)
			{
		...
		else if (vm.count ("debug_rpc"))
		{
			...
			ysu::inactive_node inactive_node_l (data_path, node_flags);

			ysu::node_rpc_config config;
			ysu::ipc::ipc_server server (*inactive_node_l.node, config);
			auto handler_l (std::make_shared<ysu::json_handler> (*inactive_node_l.node, config, command_l.str (), response_handler_l));
			handler_l->process_request ();
		}
		else if (vm.count ("validate_blocks") || vm.count ("debug_validate_blocks"))
		{
			ysu::timer<std::chrono::seconds> timer;
			timer.start ();
			auto node_flags = ysu::inactive_node_flag_defaults ();
			ysu::update_flags (node_flags, vm);
			node_flags.generate_cache.block_count = true;
			...
			std::deque<std::pair<ysu::account, ysu::account_info>> accounts;
			...
			auto print_error_message = [&silent, &errors](std::string const & error_message_a) {
				if (!silent)
				{
					...
					ysu::lock_guard<std::mutex> lock (cerr_mutex);
			...
			auto check_account = [&print_error_message, &silent, &count, &block_count](std::shared_ptr<ysu::node> const & node, ysu::read_transaction const & transaction, ysu::account const & account, ysu::account_info const & info) {
				++count;
				...
				ysu::block_hash calculated_hash (0);
				...
				ysu::account calculated_representative (0);
				while (!hash.is_zero () && block != nullptr)
				{
					...
					if (block->type () == ysu::block_type::open || block->type () == ysu::block_type::state)
					{
					...
					if (height == 0 && block->type () != ysu::block_type::open && block->type () != ysu::block_type::state)
					{
					...
					if (validate_message (account, hash, block->block_signature ()))
					{
						...
						{
							auto & state_block (static_cast<ysu::state_block &> (*block.get ()));
					...
					if (block->difficulty () < ysu::work_threshold (block->work_version (), block->sideband ().details))
					{
						print_error_message (boost::str (boost::format ("Invalid work for block %1% value: %2%\n") % hash.to_string () % ysu::to_string_hex (block->block_work ())));
					}
					...
					if (block->type () == ysu::block_type::open || block->type () == ysu::block_type::change || block->type () == ysu::block_type::state)
					{
			...
			count = 0;
			finished = false;
			std::deque<std::pair<ysu::pending_key, ysu::pending_info>> pending;

			auto check_pending = [&print_error_message, &silent, &count](std::shared_ptr<ysu::node> const & node, ysu::read_transaction const & transaction, ysu::pending_key const & key, ysu::pending_info const & info) {
				++count;
				...
				else
				{
					...
					if (auto state = dynamic_cast<ysu::state_block *> (block.get ()))
					{
					...
					else if (auto send = dynamic_cast<ysu::send_block *> (block.get ()))
					{
		...
		else if (vm.count ("debug_profile_bootstrap"))
		{
			...
			ysu::update_flags (node_flags, vm);
			ysu::inactive_node node (ysu::unique_path (), node_flags);
			ysu::genesis genesis;
			auto begin (std::chrono::high_resolution_clock::now ());
			uint64_t block_count (0);
			size_t count (0);
			std::deque<ysu::unchecked_info> epoch_open_blocks;
			{
				auto node_flags = ysu::inactive_node_flag_defaults ();
				ysu::update_flags (node_flags, vm);
				node_flags.generate_cache.block_count = true;
				...
				for (auto i (source_node->store.accounts_begin (transaction)), n (source_node->store.accounts_end ()); i != n; ++i)
				{
					...
					while (!hash.is_zero ())
					{
						...
						if (block != nullptr)
						{
							...
							ysu::unchecked_info unchecked_info (block, account, 0, ysu::signature_verification::unknown);
							node.node->block_processor.add (unchecked_info);
							if (block->type () == ysu::block_type::state && block->previous ().is_zero () && source_node->ledger.is_epoch_link (block->link ()))
							{
		...
		else if (vm.count ("debug_peers"))
		{
			...
			{
				std::cout << boost::str (boost::format ("%1%\n") % ysu::endpoint (boost::asio::ip::address_v6 (i->first.address_bytes ()), i->first.port ()));
			}
		}
		else if (vm.count ("debug_cemented_block_count"))
		{
			...
			ysu::update_flags (node_flags, vm);
			ysu::inactive_node node (data_path, node_flags);
		...
		{
#ifdef BOOST_WINDOWS
			if (!ysu::event_log_reg_entry_exists () && !ysu::is_windows_elevated ())
			{
				std::cerr << "The event log requires the HKEY_LOCAL_MACHINE\\SYSTEM\\CurrentControlSet\\Services\\EventLog\\Ysu\\Ysu registry entry, run again as administator to create it.\n";
				return 1;
			}
#endif
			auto inactive_node = ysu::default_inactive_node (data_path, vm);
			inactive_node->node->logger.always_log (ysu::severity_level::error, "Testing system logger");
		}
		else if (vm.count ("debug_account_versions"))
		{
			auto inactive_node = ysu::default_inactive_node (data_path, vm);
			auto node = inactive_node->node;
			auto const epoch_count = ysu::normalized_epoch (ysu::epoch::max) + static_cast<std::underlying_type<ysu::epoch>::type> (1);
			// Cache the accounts in a collection to make searching quicker against unchecked keys. Group by epoch
			ysu::locked<std::vector<boost::unordered_set<ysu::account>>> opened_account_versions_shared (epoch_count);
			using opened_account_versions_t = decltype (opened_account_versions_shared)::value_type;
			node->store.accounts_for_each_par (
			[&opened_account_versions_shared, epoch_count](ysu::read_transaction const & /*unused*/, ysu::store_iterator<ysu::account, ysu::account_info> i, ysu::store_iterator<ysu::account, ysu::account_info> n) {
				// First cache locally
				...
				for (; i != n; ++i)
				{
					...
					auto epoch_idx = ysu::normalized_epoch (account_info.epoch ());
			...
			boost::unordered_set<ysu::account> opened_accounts;
			{
				auto opened_account_versions = opened_account_versions_shared.lock ();
			...
			ysu::locked<boost::unordered_map<ysu::account, std::underlying_type_t<ysu::epoch>>> unopened_highest_pending_shared;
			using unopened_highest_pending_t = decltype (unopened_highest_pending_shared)::value_type;
			node->store.pending_for_each_par (
			[&unopened_highest_pending_shared, &opened_accounts](ysu::read_transaction const & /*unused*/, ysu::store_iterator<ysu::pending_key, ysu::pending_info> i, ysu::store_iterator<ysu::pending_key, ysu::pending_info> n) {
				// First cache locally
				...
				for (; i != n; ++i)
				{
					...
					if (!exists)
					{
						...
						auto epoch = ysu::normalized_epoch (info.epoch);
		...
		{
			std::cout << "Version " << YSU_VERSION_STRING << "\n"
