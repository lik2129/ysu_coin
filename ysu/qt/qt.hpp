...
public:
	...
	QPushButton * back;
	ysu_qt::wallet & wallet;
};
class advanced_actions
{
public:
	advanced_actions (ysu_qt::wallet &);
	QWidget * window;
	QVBoxLayout * layout;
	...
	QRadioButton * mysu_unit;
	QRadioButton * kysu_unit;
	QRadioButton * ysu_unit;
...
{
public:
	self_pane (ysu_qt::wallet &, ysu::account const &);
	void set_balance_text (std::pair<ysu::uint128_t, ysu::uint128_t>);
	QWidget * window;
	QVBoxLayout * layout;
...
{
public:
	history (ysu::ledger &, ysu::account const &, ysu_qt::wallet &);
	void refresh ();
	QWidget * window;
	QVBoxLayout * layout;
	QStandardItemModel * model;
...
{
public:
	block_viewer (ysu_qt::wallet &);
	void rebroadcast_action (ysu::block_hash const &);
	QWidget * window;
	QVBoxLayout * layout;
	QLabel * hash_label;
	QLineEdit * hash;
...
{
public:
	status (ysu_qt::wallet &);
	void erase (ysu_qt::status_types);
	void insert (ysu_qt::status_types);
	void set_text ();
	std::string text ();
	std::string color ();
	std::set<ysu_qt::status_types> active;
	ysu_qt::wallet & wallet;
};
class wallet : public std::enable_shared_from_this<ysu_qt::wallet>
{
public:
	wallet (QApplication &, ysu_qt::eventloop_processor &, ysu::node &, std::shared_ptr<ysu::wallet>, ysu::account &);
	void start ();
	void refresh ();
	void update_connected ();
	void empty_password ();
	void change_rendering_ratio (ysu::uint128_t const &);
	std::string format_balance (ysu::uint128_t const &) const;
	ysu::uint128_t rendering_ratio;
	ysu::node & node;
	std::shared_ptr<ysu::wallet> wallet_m;
	ysu::account & account;
	ysu_qt::eventloop_processor & processor;
	ysu_qt::history history;
	ysu_qt::accounts accounts;
	ysu_qt::self_pane self;
	ysu_qt::settings settings;
	ysu_qt::advanced_actions advanced;
	ysu_qt::block_creation block_creation;
	ysu_qt::block_entry block_entry;
	ysu_qt::block_viewer block_viewer;
	ysu_qt::account_viewer account_viewer;
	ysu_qt::stats_viewer stats_viewer;
	ysu_qt::import import;
