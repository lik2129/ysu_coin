...
bool ysu_qt::eventloop_processor::event (QEvent * event_a)
{
	debug_assert (dynamic_cast<ysu_qt::eventloop_event *> (event_a) != nullptr);
	static_cast<ysu_qt::eventloop_event *> (event_a)->action ();
	return true;
}

ysu_qt::eventloop_event::eventloop_event (std::function<void()> const & action_a) :
QEvent (QEvent::Type::User),
action (action_a)
{
}

ysu_qt::self_pane::self_pane (ysu_qt::wallet & wallet_a, ysu::account const & account_a) :
window (new QWidget),
layout (new QVBoxLayout),
self_layout (new QHBoxLayout),
self_window (new QWidget),
your_account_label (new QLabel ("Your Ysu account:")),
...
wallet (wallet_a)
{
	...
	version = new QLabel (boost::str (boost::format ("%1% %2% network") % YSU_VERSION_STRING % network).c_str ());
...
void ysu_qt::self_pane::set_balance_text (std::pair<ysu::uint128_t, ysu::uint128_t> balance_a)
{
...
ysu_qt::accounts::accounts (ysu_qt::wallet & wallet_a) :
...
void ysu_qt::accounts::refresh_wallet_balance ()
{
	auto transaction (this->wallet.wallet_m->wallets.tx_begin_read ());
	...
	{
		ysu::public_key const & key (i->first);
...
ysu_qt::import::import (ysu_qt::wallet & wallet_a) :
window (new QWidget),
layout (new QVBoxLayout),
...
ysu_qt::history::history (ysu::ledger & ledger_a, ysu::account const & account_a, ysu_qt::wallet & wallet_a) :
window (new QWidget),
layout (new QVBoxLayout),
model (new QStandardItemModel),
view (new QTableView),
...
namespace
{
class short_text_visitor : public ysu::block_visitor
{
public:
	short_text_visitor (ysu::transaction const & transaction_a, ysu::ledger & ledger_a) :
...
ysu_qt::block_viewer::block_viewer (ysu_qt::wallet & wallet_a) :
window (new QWidget),
layout (new QVBoxLayout),
...
void ysu_qt::block_viewer::rebroadcast_action (ysu::block_hash const & hash_a)
{
	auto done (true);
...
ysu_qt::account_viewer::account_viewer (ysu_qt::wallet & wallet_a) :
window (new QWidget),
layout (new QVBoxLayout),
...
wallet (wallet_a)
{
	...
	QObject::connect (refresh, &QPushButton::released, [this]() {
		...
		if (!account.decode_account (account_line->text ().toStdString ()))
		{
			...
			auto final_text (std::string ("Balance (YSU): ") + wallet.format_balance (balance.first));
...
ysu_qt::stats_viewer::stats_viewer (ysu_qt::wallet & wallet_a) :
window (new QWidget),
layout (new QVBoxLayout),
refresh (new QPushButton ("Refresh")),
...
void ysu_qt::stats_viewer::refresh_stats ()
{
	model->removeRows (0, model->rowCount ());
...
ysu_qt::status::status (ysu_qt::wallet & wallet_a) :
wallet (wallet_a)
{
	wallet.status->setToolTip ("Wallet status, block count (blocks downloaded)");
	active.insert (ysu_qt::status_types::nominal);
	set_text ();
}

void ysu_qt::status::erase (ysu_qt::status_types status_a)
{
	debug_assert (status_a != ysu_qt::status_types::nominal);
	auto erased (active.erase (status_a));
	(void)erased;
	set_text ();
}

void ysu_qt::status::insert (ysu_qt::status_types status_a)
{
	debug_assert (status_a != ysu_qt::status_types::nominal);
...
std::string ysu_qt::status::text ()
{
	...
	switch (*active.begin ())
	{
		case ysu_qt::status_types::disconnected:
			result = "Status: Disconnected";
			break;
		case ysu_qt::status_types::working:
			result = "Status: Generating proof of work";
			break;
		case ysu_qt::status_types::synchronizing:
			result = "Status: Synchronizing";
			break;
		case ysu_qt::status_types::locked:
			result = "Status: Wallet locked";
			break;
		case ysu_qt::status_types::vulnerable:
			result = "Status: Wallet password empty";
			break;
		case ysu_qt::status_types::active:
			result = "Status: Wallet active";
			break;
		case ysu_qt::status_types::nominal:
...
std::string ysu_qt::status::color ()
{
	debug_assert (!active.empty ());
	std::string result;
	switch (*active.begin ())
...
ysu_qt::wallet::wallet (QApplication & application_a, ysu_qt::eventloop_processor & processor_a, ysu::node & node_a, std::shared_ptr<ysu::wallet> wallet_a, ysu::account & account_a) :
rendering_ratio (ysu::Mxrb_ratio),
node (node_a),
wallet_m (wallet_a),
account (account_a),
...
void ysu_qt::wallet::ongoing_refresh ()
{
	std::weak_ptr<ysu_qt::wallet> wallet_w (shared_from_this ());
...
void ysu_qt::wallet::start ()
{
	ongoing_refresh ();
	std::weak_ptr<ysu_qt::wallet> this_w (shared_from_this ());
					...
					if (actual <= balance)
					{
						...
						if (this_l->wallet_m->store.valid_password (transaction))
						{
							...
							this_l->node.background ([this_w, account_l, actual]() {
								if (auto this_l = this_w.lock ())
								{
									this_l->wallet_m->send_async (this_l->account, account_l, actual, [this_w](std::shared_ptr<ysu::block> block_a) {
										if (auto this_l = this_w.lock ())
										{
	...
	node.observers.blocks.add ([this_w](ysu::election_status const & status_a, ysu::account const & account_a, ysu::uint128_t const & amount_a, bool) {
		if (auto this_l = this_w.lock ())
		{
	...
	node.observers.account_balance.add ([this_w](ysu::account const & account_a, bool is_pending) {
		if (auto this_l = this_w.lock ())
		{
		...
		if (auto this_l = this_w.lock ())
		{
			this_l->application.postEvent (&this_l->processor, new eventloop_event ([this_w, active_a]() {
				if (auto this_l = this_w.lock ())
				{
					if (active_a)
					{
						this_l->active_status.insert (ysu_qt::status_types::active);
					}
					else
					{
						this_l->active_status.erase (ysu_qt::status_types::active);
					}
				}
			}));
		}
	});
	node.observers.endpoint.add ([this_w](std::shared_ptr<ysu::transport::channel>) {
		if (auto this_l = this_w.lock ())
		{
			this_l->application.postEvent (&this_l->processor, new eventloop_event ([this_w]() {
				if (auto this_l = this_w.lock ())
		...
		if (auto this_l = this_w.lock ())
		{
			this_l->application.postEvent (&this_l->processor, new eventloop_event ([this_w, active_a]() {
				if (auto this_l = this_w.lock ())
				{
					if (active_a)
					{
						this_l->active_status.insert (ysu_qt::status_types::synchronizing);
					}
					else
					{
						this_l->active_status.erase (ysu_qt::status_types::synchronizing);
		...
		if (auto this_l = this_w.lock ())
		{
			this_l->application.postEvent (&this_l->processor, new eventloop_event ([this_w, working]() {
				if (auto this_l = this_w.lock ())
				{
					if (working)
					{
						this_l->active_status.insert (ysu_qt::status_types::working);
					}
					else
					{
						this_l->active_status.erase (ysu_qt::status_types::working);
					}
				}
			}));
		}
...
void ysu_qt::wallet::update_connected ()
{
	if (node.network.empty ())
	{
		active_status.insert (ysu_qt::status_types::disconnected);
	}
	else
	{
		active_status.erase (ysu_qt::status_types::disconnected);
	}
}

void ysu_qt::wallet::empty_password ()
{
...
void ysu_qt::wallet::change_rendering_ratio (ysu::uint128_t const & rendering_ratio_a)
{
...
std::string ysu_qt::wallet::format_balance (ysu::uint128_t const & balance) const
{
	auto balance_str = ysu::amount (balance).format_balance (rendering_ratio, 3, false);
	auto unit = std::string ("YSU");
	if (rendering_ratio == ysu::kxrb_ratio)
	{
		unit = std::string ("kysu");
	}
	else if (rendering_ratio == ysu::xrb_ratio)
	{
		unit = std::string ("ysu");
	}
	else if (rendering_ratio == ysu::raw_ratio)
	{
		unit = std::string ("raw");
	}
...
void ysu_qt::wallet::push_main_stack (QWidget * widget_a)
{
	main_stack->addWidget (widget_a);
	main_stack->setCurrentIndex (main_stack->count () - 1);
}

void ysu_qt::wallet::pop_main_stack ()
{
	main_stack->removeWidget (main_stack->currentWidget ());
}

ysu_qt::settings::settings (ysu_qt::wallet & wallet_a) :
window (new QWidget),
layout (new QVBoxLayout),
password (new QLineEdit),
...
void ysu_qt::settings::refresh_representative ()
{
...
void ysu_qt::settings::activate ()
{
	this->wallet.push_main_stack (window);
}

void ysu_qt::settings::update_locked (bool invalid, bool vulnerable)
{
	if (invalid)
	{
		this->wallet.active_status.insert (ysu_qt::status_types::locked);
	}
	else
	{
		this->wallet.active_status.erase (ysu_qt::status_types::locked);
	}
	if (vulnerable)
	{
		this->wallet.active_status.insert (ysu_qt::status_types::vulnerable);
	}
	else
	{
		this->wallet.active_status.erase (ysu_qt::status_types::vulnerable);
	}
}

ysu_qt::advanced_actions::advanced_actions (ysu_qt::wallet & wallet_a) :
window (new QWidget),
layout (new QVBoxLayout),
...
mysu_unit (new QRadioButton ("Mysu")),
kysu_unit (new QRadioButton ("kysu")),
ysu_unit (new QRadioButton ("ysu")),
...
wallet (wallet_a)
{
	ratio_group->addButton (mysu_unit);
	ratio_group->addButton (kysu_unit);
	ratio_group->addButton (ysu_unit);
	ratio_group->addButton (raw_unit);
	ratio_group->setId (mysu_unit, 0);
	ratio_group->setId (kysu_unit, 1);
	ratio_group->setId (ysu_unit, 2);
	ratio_group->setId (raw_unit, 3);
	scale_layout->addWidget (scale_label);
	scale_layout->addWidget (mysu_unit);
	scale_layout->addWidget (kysu_unit);
	scale_layout->addWidget (ysu_unit);
	...
	QObject::connect (mysu_unit, &QRadioButton::toggled, [this]() {
		if (mysu_unit->isChecked ())
		{
			QSettings ().setValue (saved_ratio_key, ratio_group->id (mysu_unit));
			this->wallet.change_rendering_ratio (ysu::Mxrb_ratio);
		}
	});
	QObject::connect (kysu_unit, &QRadioButton::toggled, [this]() {
		if (kysu_unit->isChecked ())
		{
			QSettings ().setValue (saved_ratio_key, ratio_group->id (kysu_unit));
			this->wallet.change_rendering_ratio (ysu::kxrb_ratio);
		}
	});
	QObject::connect (ysu_unit, &QRadioButton::toggled, [this]() {
		if (ysu_unit->isChecked ())
		{
			QSettings ().setValue (saved_ratio_key, ratio_group->id (ysu_unit));
			this->wallet.change_rendering_ratio (ysu::xrb_ratio);
		}
	});
	QObject::connect (raw_unit, &QRadioButton::toggled, [this]() {
		if (raw_unit->isChecked ())
		{
			...
			this->wallet.change_rendering_ratio (ysu::raw_ratio);
		}
	});
	auto selected_ratio_id (QSettings ().value (saved_ratio_key, ratio_group->id (mysu_unit)).toInt ());
	...
	QObject::connect (peers_bootstrap, &QPushButton::released, [this]() {
		...
		auto error (ysu::parse_endpoint (bootstrap_line->text ().toStdString (), endpoint));
		if (!error)
		{
...
void ysu_qt::advanced_actions::refresh_peers ()
{
...
void ysu_qt::advanced_actions::refresh_ledger ()
{
	...
	for (auto i (wallet.node.ledger.store.accounts_begin (transaction)), j (wallet.node.ledger.store.accounts_end ()); i != j; ++i)
	{
		...
		std::string balance;
		ysu::amount (info.balance.number () / wallet.rendering_ratio).encode_dec (balance);
...
void ysu_qt::advanced_actions::refresh_stats ()
{
	wallet.stats_viewer.refresh_stats ();
}

ysu_qt::block_entry::block_entry (ysu_qt::wallet & wallet_a) :
window (new QWidget),
layout (new QVBoxLayout),
block (new QPlainTextEdit),
status (new QLabel),
...
wallet (wallet_a)
{
	...
	QObject::connect (process, &QPushButton::released, [this]() {
		...
		try
		{
			...
			auto block_l (ysu::deserialize_block_json (tree));
			if (block_l != nullptr)
			{
				show_label_ok (*status);
				this->status->setText ("");
				if (!ysu::work_validate_entry (*block_l))
				{
...
ysu_qt::block_creation::block_creation (ysu_qt::wallet & wallet_a) :
window (new QWidget),
layout (new QVBoxLayout),
...
void ysu_qt::block_creation::deactivate_all ()
{
	account_label->hide ();
	account->hide ();
	source_label->hide ();
...
void ysu_qt::block_creation::activate_send ()
{
	account_label->show ();
	account->show ();
	amount_label->show ();
...
void ysu_qt::block_creation::activate_receive ()
{
	source_label->show ();
	source->show ();
}

void ysu_qt::block_creation::activate_open ()
{
	source_label->show ();
	source->show ();
...
void ysu_qt::block_creation::activate_change ()
{
	account_label->show ();
	account->show ();
	representative_label->show ();
...
void ysu_qt::block_creation::create_send ()
{
	ysu::account account_l;
			...
			if (!error)
			{
				...
				if (!wallet.wallet_m->store.fetch (transaction, account_l, key))
				{
					...
					if (amount_l.number () <= balance)
					{
						...
						(void)error;
						debug_assert (!error);
						ysu::state_block send (account_l, info.head, info.representative, balance - amount_l.number (), destination_l, key, account_l, 0);
...
void ysu_qt::block_creation::create_receive ()
{
	ysu::block_hash source_l;
	...
	if (!error)
	{
		...
		if (block_l != nullptr)
		{
			...
			{
				ysu::pending_key pending_key (destination, source_l);
				...
				if (!wallet.node.store.pending_get (block_transaction, pending_key, pending))
				{
					...
					if (!error)
					{
						...
						if (!error)
						{
							ysu::state_block receive (pending_key.account, info.head, info.representative, info.balance.number () + pending.amount.number (), source_l, key, pending_key.account, 0);
...
void ysu_qt::block_creation::create_change ()
{
	ysu::account account_l;
	auto error (account_l.decode_account (account->text ().toStdString ()));
	if (!error)
	{
		...
		if (!error)
		{
			...
			if (!error)
			{
				...
				if (!error)
				{
					ysu::state_block change (account_l, info.head, representative_l, info.balance, 0, key, account_l, 0);
...
void ysu_qt::block_creation::create_open ()
{
	ysu::block_hash source_l;
	auto error (source_l.decode_hex (source->text ().toStdString ()));
	if (!error)
				...
				if (!destination.is_zero ())
				{
					...
					if (!wallet.node.store.pending_get (block_transaction, pending_key, pending))
					{
						...
						if (error)
						{
							...
							if (!error)
							{
								ysu::state_block open (pending_key.account, 0, representative_l, pending.amount, source_l, key, pending_key.account, 0);
