...
{
	size_t operator() (::ysu::uint256_union const & value_a) const
	{
		return std::hash<::ysu::uint256_union> () (value_a);
	}
};
...
{
	size_t operator() (::ysu::block_hash const & value_a) const
	{
		return std::hash<::ysu::block_hash> () (value_a);
	}
};

template <>
struct hash<::ysu::public_key>
{
	size_t operator() (::ysu::public_key const & value_a) const
	{
		return std::hash<::ysu::public_key> () (value_a);
	}
};
template <>
struct hash<::ysu::uint512_union>
{
	size_t operator() (::ysu::uint512_union const & value_a) const
	{
		return std::hash<::ysu::uint512_union> () (value_a);
	}
};
template <>
struct hash<::ysu::qualified_root>
{
	size_t operator() (::ysu::qualified_root const & value_a) const
	{
		return std::hash<::ysu::qualified_root> () (value_a);
	}
};
template <>
struct hash<::ysu::root>
{
	size_t operator() (::ysu::root const & value_a) const
	{
		return std::hash<::ysu::root> () (value_a);
	}
};
}
namespace ysu
...
public:
	...
	account_info (ysu::block_hash const &, ysu::account const &, ysu::block_hash const &, ysu::amount const &, uint64_t, uint64_t, epoch);
	bool deserialize (ysu::stream &);
	bool operator== (ysu::account_info const &) const;
	bool operator!= (ysu::account_info const &) const;
	size_t db_size () const;
	ysu::epoch epoch () const;
	...
	ysu::epoch epoch_m{ ysu::epoch::epoch_0 };
};
...
public:
	...
	pending_info (ysu::account const &, ysu::amount const &, ysu::epoch);
	size_t db_size () const;
	bool deserialize (ysu::stream &);
	bool operator== (ysu::pending_info const &) const;
...
public:
	pending_key () = default;
	pending_key (ysu::account const &, ysu::block_hash const &);
	bool deserialize (ysu::stream &);
	bool operator== (ysu::pending_key const &) const;
	ysu::account const & key () const;
...
public:
	...
	unchecked_key (ysu::block_hash const &, ysu::block_hash const &);
	bool deserialize (ysu::stream &);
	bool operator== (ysu::unchecked_key const &) const;
	ysu::block_hash const & key () const;
...
public:
	...
	unchecked_info (std::shared_ptr<ysu::block>, ysu::account const &, uint64_t, ysu::signature_verification = ysu::signature_verification::unknown, bool = false);
	...
	/** Seconds since posix epoch */
	uint64_t modified{ 0 };
	ysu::signature_verification verified{ ysu::signature_verification::unknown };
...
public:
	block_info () = default;
	block_info (ysu::account const &, ysu::amount const &);
	ysu::account account{ 0 };
	ysu::amount balance{ 0 };
};
...
public:
	...
	confirmation_height_info (uint64_t, ysu::block_hash const &);
	void serialize (ysu::stream &) const;
	bool deserialize (ysu::stream &);
	uint64_t height;
...
using vote_blocks_vec_iter = std::vector<boost::variant<std::shared_ptr<ysu::block>, ysu::block_hash>>::const_iterator;
class iterate_vote_blocks_as_hash final
{
public:
	iterate_vote_blocks_as_hash () = default;
	ysu::block_hash operator() (boost::variant<std::shared_ptr<ysu::block>, ysu::block_hash> const & item) const;
};
class vote final
{
public:
	vote () = default;
	vote (ysu::vote const &);
	vote (bool &, ysu::stream &, ysu::block_uniquer * = nullptr);
	vote (bool &, ysu::stream &, ysu::block_type, ysu::block_uniquer * = nullptr);
	vote (ysu::account const &, ysu::raw_key const &, uint64_t, std::shared_ptr<ysu::block>);
	vote (ysu::account const &, ysu::raw_key const &, uint64_t, std::vector<ysu::block_hash> const &);
	std::string hashes_string () const;
	ysu::block_hash hash () const;
	ysu::block_hash full_hash () const;
	bool operator== (ysu::vote const &) const;
	bool operator!= (ysu::vote const &) const;
	...
	bool validate () const;
	boost::transform_iterator<ysu::iterate_vote_blocks_as_hash, ysu::vote_blocks_vec_iter> begin () const;
	boost::transform_iterator<ysu::iterate_vote_blocks_as_hash, ysu::vote_blocks_vec_iter> end () const;
	std::string to_json () const;
	// Vote round sequence number
	uint64_t sequence;
	// The blocks, or block hashes, that this vote is for
	std::vector<boost::variant<std::shared_ptr<ysu::block>, ysu::block_hash>> blocks;
...
{
public:
	using value_type = std::pair<const ysu::block_hash, std::weak_ptr<ysu::vote>>;

	vote_uniquer (ysu::block_uniquer &);
	std::shared_ptr<ysu::vote> unique (std::shared_ptr<ysu::vote>);
	size_t size ();

private:
	ysu::block_uniquer & uniquer;
	std::mutex mutex;
...
{
public:
	ysu::process_result code;
	ysu::signature_verification verified;
	ysu::amount previous_balance;
};
...
{
public:
	ledger_constants (ysu::network_constants & network_constants);
	ledger_constants (ysu::ysu_networks network_a);
	ysu::keypair zero_key;
	ysu::keypair dev_genesis_key;
	ysu::account ysu_dev_account;
	ysu::account ysu_beta_account;
	ysu::account ysu_live_account;
	ysu::account ysu_test_account;
	std::string ysu_dev_genesis;
	std::string ysu_beta_genesis;
	std::string ysu_live_genesis;
	std::string ysu_test_genesis;
	ysu::account genesis_account;
	std::string genesis_block;
	ysu::block_hash genesis_hash;
	ysu::uint128_t genesis_amount;
	ysu::account burn_account;
	ysu::epochs epochs;
};
...
public:
	random_constants ();
	ysu::account not_an_account;
	ysu::uint128_union random_128;
};
...
{
public:
	node_constants (ysu::network_constants & network_constants);
...
{
public:
	voting_constants (ysu::network_constants & network_constants);
	size_t max_cache;
};
...
{
public:
	portmapping_constants (ysu::network_constants & network_constants);
...
{
public:
	bootstrap_constants (ysu::network_constants & network_constants);
...
public:
	...
	network_params (ysu::ysu_networks network_a);
...
ysu::wallet_id random_wallet_id ();
}
