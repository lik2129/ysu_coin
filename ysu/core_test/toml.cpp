...
TEST (toml, daemon_config_deserialize_defaults)
{
	...
	ysu::tomlconfig t;
	t.read (ss);
	ysu::daemon_config conf;
	ysu::daemon_config defaults;
	conf.deserialize_toml (t);
...
TEST (toml, base_override)
{
	...
	ASSERT_TRUE (t.get_error () == ysu::error_config::missing_value);
	t.get_error ().clear ();

	// Query uint16 that's too big, make sure we have an error
	t.get_required<uint16_t> ("node.too_big", port);
	ASSERT_TRUE (t.get_error ());
	ASSERT_TRUE (t.get_error () == ysu::error_config::invalid_value);
}

TEST (toml, put)
{
	ysu::tomlconfig config;
	ysu::tomlconfig config_node;
...
TEST (toml, daemon_config_deserialize_no_defaults)
{
	...
	preconfigured_representatives = ["ysu_3arg3asgtigae3xckabaaewkx3bzsh7nwz7jkmjos79ihyaxwphhm6qgjps4"]
	receive_minimum = "999"
...
TEST (toml, daemon_config_deserialize_errors)
{
...

	{
		...
		ASSERT_EQ (conf.node.frontiers_confirmation, ysu::frontiers_confirmation_mode::invalid);
	}
...
TEST (toml, daemon_read_config)
{
	...
	ASSERT_FALSE (boost::filesystem::exists (ysu::get_node_toml_config_path (path)));
	ASSERT_FALSE (ysu::read_node_config_toml (path, config));
	{
		auto error = ysu::read_node_config_toml (path, config, invalid_overrides1);
		ASSERT_TRUE (error);
		ASSERT_EQ (error.get_message (), expected_message1);
	}
	{
		auto error = ysu::read_node_config_toml (path, config, invalid_overrides2);
		ASSERT_TRUE (error);
	...
	ysu::tomlconfig toml;
	toml.write (ysu::get_node_toml_config_path (path));

	// Reading when there is a config file
	ASSERT_TRUE (boost::filesystem::exists (ysu::get_node_toml_config_path (path)));
	ASSERT_FALSE (ysu::read_node_config_toml (path, config));
	{
		auto error = ysu::read_node_config_toml (path, config, invalid_overrides1);
		ASSERT_TRUE (error);
