...
TEST (socket, drop_policy)
{
	...
	ysu::inactive_node inactivenode (ysu::unique_path (), node_flags);
	auto node = inactivenode.node;

	ysu::thread_runner runner (node->io_ctx, 1);

	std::vector<std::shared_ptr<ysu::socket>> connections;

	auto func = [&](size_t total_message_count, ysu::buffer_drop_policy drop_policy) {
		auto server_port (ysu::get_available_port ());
		boost::asio::ip::tcp::endpoint endpoint (boost::asio::ip::address_v6::any (), server_port);

		auto server_socket = std::make_shared<ysu::server_socket> (*node, endpoint, 1);
		boost::system::error_code ec;
		server_socket->start (ec);
		ASSERT_FALSE (ec);

		// Accept connection, but don't read so the writer will drop.
		server_socket->on_connection ([&connections](std::shared_ptr<ysu::socket> new_connection, boost::system::error_code const & ec_a) {
			connections.push_back (new_connection);
			return true;
		});

		auto client = std::make_shared<ysu::socket> (*node, boost::none);
		ysu::transport::channel_tcp channel{ *node, client };
		ysu::util::counted_completion write_completion (static_cast<unsigned> (total_message_count));
		...
		[&channel, total_message_count, node, &write_completion, &drop_policy, client](boost::system::error_code const & ec_a) mutable {
			for (int i = 0; i < total_message_count; i++)
			{
				...
				channel.send_buffer (
				ysu::shared_const_buffer (std::move (buff)), [&write_completion, client](boost::system::error_code const & ec, size_t size_a) mutable {
					client.reset ();
	...
	func (ysu::socket::queue_size_max * 2 + 1, ysu::buffer_drop_policy::no_socket_drop);
	ASSERT_EQ (1, node->stats.count (ysu::stat::type::tcp, ysu::stat::detail::tcp_write_no_socket_drop, ysu::stat::dir::out));
	ASSERT_EQ (0, node->stats.count (ysu::stat::type::tcp, ysu::stat::detail::tcp_write_drop, ysu::stat::dir::out));

	func (ysu::socket::queue_size_max + 1, ysu::buffer_drop_policy::limiter);
	// The stats are accumulated from before
	ASSERT_EQ (1, node->stats.count (ysu::stat::type::tcp, ysu::stat::detail::tcp_write_no_socket_drop, ysu::stat::dir::out));
	ASSERT_EQ (1, node->stats.count (ysu::stat::type::tcp, ysu::stat::detail::tcp_write_drop, ysu::stat::dir::out));
...
TEST (socket, concurrent_writes)
{
	...
	ysu::util::counted_completion read_count_completion (total_message_count);
	std::function<void(std::shared_ptr<ysu::socket>)> reader = [&read_count_completion, &total_message_count, &reader](std::shared_ptr<ysu::socket> socket_a) {
...
#pragma GCC diagnostic pop
	...
	auto server_socket = std::make_shared<ysu::server_socket> (*node, endpoint, max_connections);
	boost::system::error_code ec;
	server_socket->start (ec);
	ASSERT_FALSE (ec);
	std::vector<std::shared_ptr<ysu::socket>> connections;
	...
	server_socket->on_connection ([&connections, &reader](std::shared_ptr<ysu::socket> new_connection, boost::system::error_code const & ec_a) {
		if (ec_a)
		{
	...
	ysu::util::counted_completion connection_count_completion (client_count);
	std::vector<std::shared_ptr<ysu::socket>> clients;
...
#pragma GCC diagnostic ignored "-Wunused-lambda-capture"
		...
		client_threads.emplace_back ([&client, &message_count]() {
			for (int i = 0; i < message_count; i++)
			{
				...
				buff.push_back ('A' + i);
				client->async_write (ysu::shared_const_buffer (std::move (buff)));
			}
		});
#ifndef _WIN32
#pragma GCC diagnostic pop
	...
	ASSERT_EQ (node->stats.count (ysu::stat::type::tcp, ysu::stat::detail::tcp_accept_success, ysu::stat::dir::in), client_count);
	// We may exhaust max connections and have some tcp accept failures, but no more than the client count
	ASSERT_LT (node->stats.count (ysu::stat::type::tcp, ysu::stat::detail::tcp_accept_failure, ysu::stat::dir::in), client_count);
