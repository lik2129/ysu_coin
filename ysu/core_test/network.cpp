...
TEST (network, self_discard)
{
	...
	ysu::system system (1, ysu::transport::transport_type::tcp, node_flags);
	ysu::message_buffer data;
	data.endpoint = system.nodes[0]->network.endpoint ();
	ASSERT_EQ (0, system.nodes[0]->stats.count (ysu::stat::type::error, ysu::stat::detail::bad_sender));
	system.nodes[0]->network.udp_channels.receive_action (&data);
	ASSERT_EQ (1, system.nodes[0]->stats.count (ysu::stat::type::error, ysu::stat::detail::bad_sender));
}
...
TEST (network, send_node_id_handshake)
{
	...
	auto initial (node0->stats.count (ysu::stat::type::message, ysu::stat::detail::node_id_handshake, ysu::stat::dir::in));
	auto initial_node1 (node1->stats.count (ysu::stat::type::message, ysu::stat::detail::node_id_handshake, ysu::stat::dir::in));
	auto channel (std::make_shared<ysu::transport::channel_udp> (node0->network.udp_channels, node1->network.endpoint (), node1->network_params.protocol.protocol_version));
	node0->network.send_keepalive (channel);
	ASSERT_EQ (0, node0->network.size ());
	ASSERT_EQ (0, node1->network.size ());
	ASSERT_TIMELY (10s, node1->stats.count (ysu::stat::type::message, ysu::stat::detail::node_id_handshake, ysu::stat::dir::in) != initial_node1);
	ASSERT_TIMELY (10s, node0->network.size () == 0 || node1->network.size () == 1);
	ASSERT_TIMELY (10s, node0->stats.count (ysu::stat::type::message, ysu::stat::detail::node_id_handshake, ysu::stat::dir::in) == initial + 2);
...
TEST (network, send_node_id_handshake_tcp)
{
	...
	auto initial (node0->stats.count (ysu::stat::type::message, ysu::stat::detail::node_id_handshake, ysu::stat::dir::in));
	auto initial_node1 (node1->stats.count (ysu::stat::type::message, ysu::stat::detail::node_id_handshake, ysu::stat::dir::in));
	auto initial_keepalive (node0->stats.count (ysu::stat::type::message, ysu::stat::detail::keepalive, ysu::stat::dir::in));
	std::weak_ptr<ysu::node> node_w (node0);
	node0->network.tcp_channels.start_tcp (node1->network.endpoint (), [node_w](std::shared_ptr<ysu::transport::channel> channel_a) {
	...
	ASSERT_EQ (0, node1->network.size ());
	ASSERT_TIMELY (10s, node0->stats.count (ysu::stat::type::message, ysu::stat::detail::node_id_handshake, ysu::stat::dir::in) >= initial + 2);
	ASSERT_TIMELY (5s, node1->stats.count (ysu::stat::type::message, ysu::stat::detail::node_id_handshake, ysu::stat::dir::in) >= initial_node1 + 2);
	ASSERT_TIMELY (5s, node0->stats.count (ysu::stat::type::message, ysu::stat::detail::keepalive, ysu::stat::dir::in) >= initial_keepalive + 2);
	ASSERT_TIMELY (5s, node1->stats.count (ysu::stat::type::message, ysu::stat::detail::keepalive, ysu::stat::dir::in) >= initial_keepalive + 2);
...
TEST (network, last_contacted)
{
	...
	node1->start ();
	system.nodes.push_back (node1);
	auto channel1 (std::make_shared<ysu::transport::channel_udp> (node1->network.udp_channels, ysu::endpoint (boost::asio::ip::address_v6::loopback (), system.nodes.front ()->network.endpoint ().port ()), node1->network_params.protocol.protocol_version));
	...
	auto channel2 (node0->network.udp_channels.channel (ysu::endpoint (boost::asio::ip::address_v6::loopback (), node1->network.endpoint ().port ())));
	...
	node1->network.send_keepalive (channel1);
	ASSERT_TIMELY (10s, node0->stats.count (ysu::stat::type::message, ysu::stat::detail::keepalive, ysu::stat::dir::in) >= 2);
	ASSERT_EQ (node0->network.size (), 1);
...
TEST (network, multi_keepalive)
{
	...
	auto channel2 (std::make_shared<ysu::transport::channel_udp> (node2->network.udp_channels, node0->network.endpoint (), node2->network_params.protocol.protocol_version));
...
TEST (network, send_discarded_publish)
{
	...
	ysu::genesis genesis;
	{
		...
		ASSERT_EQ (genesis.hash (), node1.ledger.latest (transaction, ysu::dev_genesis_key.pub));
		ASSERT_EQ (genesis.hash (), node2.latest (ysu::dev_genesis_key.pub));
	}
	ASSERT_TIMELY (10s, node2.stats.count (ysu::stat::type::message, ysu::stat::detail::publish, ysu::stat::dir::in) != 0);
	auto transaction (node1.store.tx_begin_read ());
	ASSERT_EQ (genesis.hash (), node1.ledger.latest (transaction, ysu::dev_genesis_key.pub));
	ASSERT_EQ (genesis.hash (), node2.latest (ysu::dev_genesis_key.pub));
}
...
TEST (network, send_invalid_publish)
{
	...
	auto & node2 (*system.nodes[1]);
	ysu::genesis genesis;
	auto block (std::make_shared<ysu::send_block> (1, 1, 20, ysu::dev_genesis_key.prv, ysu::dev_genesis_key.pub, *system.work.generate (ysu::root (1))));
	{
		auto transaction (node1.store.tx_begin_read ());
		node1.network.flood_block (block);
		ASSERT_EQ (genesis.hash (), node1.ledger.latest (transaction, ysu::dev_genesis_key.pub));
...
TEST (network, send_valid_confirm_ack)
{
	...
	for (auto & type : types)
	{
		...
		ysu::send_block block2 (latest1, key2.pub, 50, ysu::dev_genesis_key.prv, ysu::dev_genesis_key.pub, *system.work.generate (latest1));
		ysu::block_hash latest2 (node2.latest (ysu::dev_genesis_key.pub));
		node1.process_active (std::make_shared<ysu::send_block> (block2));
		// Keep polling until latest block changes
		ASSERT_TIMELY (10s, node2.latest (ysu::dev_genesis_key.pub) != latest2);
		// Make sure the balance has decreased after processing the block.
		ASSERT_EQ (50, node2.balance (ysu::dev_genesis_key.pub));
	}
}
...
TEST (network, send_valid_publish)
{
	...
	for (auto & type : types)
	{
		...
		ysu::block_hash latest2 (node2.latest (ysu::dev_genesis_key.pub));
		node2.process_active (std::make_shared<ysu::send_block> (block2));
		ASSERT_TIMELY (10s, node1.stats.count (ysu::stat::type::message, ysu::stat::detail::publish, ysu::stat::dir::in) != 0);
...
TEST (network, send_insufficient_work_udp)
{
	...
	auto & node2 = *system.add_node (node_flags);
	auto block (std::make_shared<ysu::send_block> (0, 1, 20, ysu::dev_genesis_key.prv, ysu::dev_genesis_key.pub, 0));
	ysu::publish publish (block);
	ysu::transport::channel_udp channel (node1.network.udp_channels, node2.network.endpoint (), node1.network_params.protocol.protocol_version);
	channel.send (publish, [](boost::system::error_code const & ec, size_t size) {});
	ASSERT_EQ (0, node1.stats.count (ysu::stat::type::error, ysu::stat::detail::insufficient_work));
	ASSERT_TIMELY (10s, node2.stats.count (ysu::stat::type::error, ysu::stat::detail::insufficient_work) != 0);
	ASSERT_EQ (1, node2.stats.count (ysu::stat::type::error, ysu::stat::detail::insufficient_work));
}
...
TEST (network, send_insufficient_work)
{
	...
	auto & node2 = *system.nodes[1];
	// Block zero work
	auto block1 (std::make_shared<ysu::send_block> (0, 1, 20, ysu::dev_genesis_key.prv, ysu::dev_genesis_key.pub, 0));
	ysu::publish publish1 (block1);
	auto tcp_channel (node1.network.tcp_channels.find_channel (ysu::transport::map_endpoint_to_tcp (node2.network.endpoint ())));
	tcp_channel->send (publish1, [](boost::system::error_code const & ec, size_t size) {});
	ASSERT_EQ (0, node1.stats.count (ysu::stat::type::error, ysu::stat::detail::insufficient_work));
	ASSERT_TIMELY (10s, node2.stats.count (ysu::stat::type::error, ysu::stat::detail::insufficient_work) != 0);
	ASSERT_EQ (1, node2.stats.count (ysu::stat::type::error, ysu::stat::detail::insufficient_work));
	// Legacy block work between epoch_2_recieve & epoch_1
	auto block2 (std::make_shared<ysu::send_block> (block1->hash (), 1, 20, ysu::dev_genesis_key.prv, ysu::dev_genesis_key.pub, system.work_generate_limited (block1->hash (), node1.network_params.network.publish_thresholds.epoch_2_receive, node1.network_params.network.publish_thresholds.epoch_1 - 1)));
	ysu::publish publish2 (block2);
	tcp_channel->send (publish2, [](boost::system::error_code const & ec, size_t size) {});
	ASSERT_TIMELY (10s, node2.stats.count (ysu::stat::type::error, ysu::stat::detail::insufficient_work) != 1);
	ASSERT_EQ (2, node2.stats.count (ysu::stat::type::error, ysu::stat::detail::insufficient_work));
	// Legacy block work epoch_1
	auto block3 (std::make_shared<ysu::send_block> (block2->hash (), 1, 20, ysu::dev_genesis_key.prv, ysu::dev_genesis_key.pub, *system.work.generate (block2->hash (), node1.network_params.network.publish_thresholds.epoch_2)));
	ysu::publish publish3 (block3);
	tcp_channel->send (publish3, [](boost::system::error_code const & ec, size_t size) {});
	ASSERT_EQ (0, node2.stats.count (ysu::stat::type::message, ysu::stat::detail::publish, ysu::stat::dir::in));
	ASSERT_TIMELY (10s, node2.stats.count (ysu::stat::type::message, ysu::stat::detail::publish, ysu::stat::dir::in) != 0);
	ASSERT_EQ (1, node2.stats.count (ysu::stat::type::message, ysu::stat::detail::publish, ysu::stat::dir::in));
	// State block work epoch_2_recieve
	auto block4 (std::make_shared<ysu::state_block> (ysu::dev_genesis_key.pub, block1->hash (), ysu::dev_genesis_key.pub, 20, 1, ysu::dev_genesis_key.prv, ysu::dev_genesis_key.pub, system.work_generate_limited (block1->hash (), node1.network_params.network.publish_thresholds.epoch_2_receive, node1.network_params.network.publish_thresholds.epoch_1 - 1)));
	ysu::publish publish4 (block4);
...
TEST (receivable_processor, confirm_insufficient_pos)
{
	...
	auto & node1 (*system.nodes[0]);
	ysu::genesis genesis;
	auto block1 (std::make_shared<ysu::send_block> (genesis.hash (), 0, 0, ysu::dev_genesis_key.prv, ysu::dev_genesis_key.pub, 0));
	node1.work_generate_blocking (*block1);
	ASSERT_EQ (ysu::process_result::progress, node1.process (*block1).code);
	node1.active.insert (block1);
	ysu::keypair key1;
	auto vote (std::make_shared<ysu::vote> (key1.pub, key1.prv, 0, block1));
...
TEST (receivable_processor, confirm_sufficient_pos)
{
	...
	ASSERT_EQ (ysu::process_result::progress, node1.process (*block1).code);
	node1.active.insert (block1);
	auto vote (std::make_shared<ysu::vote> (ysu::dev_genesis_key.pub, ysu::dev_genesis_key.prv, 0, block1));
	ysu::confirm_ack con1 (vote);
	node1.network.process_message (con1, node1.network.udp_channels.create (node1.network.endpoint ()));
}
...
TEST (receivable_processor, send_with_receive)
{
	...
	for (auto & type : types)
	{
		...
		ysu::block_hash latest1 (node1.latest (ysu::dev_genesis_key.pub));
		system.wallet (1)->insert_adhoc (key2.prv);
		auto block1 (std::make_shared<ysu::send_block> (latest1, key2.pub, amount - node1.config.receive_minimum.number (), ysu::dev_genesis_key.prv, ysu::dev_genesis_key.pub, *system.work.generate (latest1)));
		ASSERT_EQ (amount, node1.balance (ysu::dev_genesis_key.pub));
		ASSERT_EQ (0, node1.balance (key2.pub));
		ASSERT_EQ (amount, node2.balance (ysu::dev_genesis_key.pub));
		ASSERT_EQ (0, node2.balance (key2.pub));
		node1.process_active (block1);
		node1.block_processor.flush ();
		node2.process_active (block1);
		node2.block_processor.flush ();
		ASSERT_EQ (amount - node1.config.receive_minimum.number (), node1.balance (ysu::dev_genesis_key.pub));
		ASSERT_EQ (0, node1.balance (key2.pub));
		ASSERT_EQ (amount - node1.config.receive_minimum.number (), node2.balance (ysu::dev_genesis_key.pub));
		ASSERT_EQ (0, node2.balance (key2.pub));
...
TEST (network, receive_weight_change)
{
	...
	ASSERT_TIMELY (10s, std::all_of (system.nodes.begin (), system.nodes.end (), [&](std::shared_ptr<ysu::node> const & node_a) { return node_a->weight (key2.pub) == system.nodes[0]->config.receive_minimum.number (); }));
}
...
TEST (parse_endpoint, valid)
{
	...
	ASSERT_FALSE (ysu::parse_endpoint (string, endpoint));
...
TEST (parse_endpoint, invalid_port)
{
	...
	ysu::endpoint endpoint;
	ASSERT_TRUE (ysu::parse_endpoint (string, endpoint));
}
...
TEST (network, ipv6)
{
	...
	ysu::endpoint endpoint1 (address, 16384);
	std::vector<uint8_t> bytes1;
	{
		ysu::vectorstream stream (bytes1);
		ysu::write (stream, address.to_bytes ());
	}
	...
	ysu::bufferstream stream (bytes1.data (), bytes1.size ());
	auto error (ysu::try_read (stream, bytes2));
	ASSERT_FALSE (error);
	ysu::endpoint endpoint2 (boost::asio::ip::address_v6 (bytes2), 16384);
	ASSERT_EQ (endpoint1, endpoint2);
}

TEST (network, ipv6_from_ipv4)
{
	ysu::endpoint endpoint1 (boost::asio::ip::address_v4::loopback (), 16000);
	ASSERT_TRUE (endpoint1.address ().is_v4 ());
	ysu::endpoint endpoint2 (boost::asio::ip::address_v6::v4_mapped (endpoint1.address ().to_v4 ()), 16000);
...
{
	boost::asio::io_context io_ctx;
	auto port1 = ysu::get_available_port ();
	auto port2 = ysu::get_available_port ();
	ysu::endpoint endpoint1 (boost::asio::ip::address_v6::any (), port1);
	ysu::endpoint endpoint2 (boost::asio::ip::address_v4::any (), port2);
	std::array<uint8_t, 16> bytes1;
	auto finish1 (false);
	ysu::endpoint endpoint3;
	...
	ysu::endpoint endpoint5 (boost::asio::ip::address_v4::loopback (), port1);
	ysu::endpoint endpoint6 (boost::asio::ip::address_v6::v4_mapped (boost::asio::ip::address_v4::loopback ()), port2);
...
TEST (network, reserved_address)
{
	ysu::system system (1);
	// 0 port test
	ASSERT_TRUE (ysu::transport::reserved_address (ysu::endpoint (boost::asio::ip::make_address_v6 ("2001::"), 0)));
	// Valid address test
	ASSERT_FALSE (ysu::transport::reserved_address (ysu::endpoint (boost::asio::ip::make_address_v6 ("2001::"), 1)));
	ysu::endpoint loopback (boost::asio::ip::make_address_v6 ("::1"), 1);
	ASSERT_FALSE (ysu::transport::reserved_address (loopback));
	ysu::endpoint private_network_peer (boost::asio::ip::make_address_v6 ("::ffff:10.0.0.0"), 1);
	ASSERT_TRUE (ysu::transport::reserved_address (private_network_peer, false));
	ASSERT_FALSE (ysu::transport::reserved_address (private_network_peer, true));
}
...
{
	ysu::stat stats;
	ysu::message_buffer_manager buffer (stats, 512, 1);
...
{
	ysu::stat stats;
	ysu::message_buffer_manager buffer (stats, 512, 2);
	auto buffer1 (buffer.allocate ());
	ASSERT_NE (nullptr, buffer1);
...
{
	ysu::stat stats;
	ysu::message_buffer_manager buffer (stats, 512, 16);
...
TEST (message_buffer_manager, stats)
{
	...
	buffer.enqueue (buffer1);
	buffer.allocate ();
	ASSERT_EQ (1, stats.count (ysu::stat::type::udp, ysu::stat::detail::overflow));
}

TEST (tcp_listener, tcp_node_id_handshake)
{
	ysu::system system (1);
	auto socket (std::make_shared<ysu::socket> (*system.nodes[0]));
	auto bootstrap_endpoint (system.nodes[0]->bootstrap.endpoint ());
	auto cookie (system.nodes[0]->network.syn_cookies.assign (ysu::transport::map_tcp_to_endpoint (bootstrap_endpoint)));
	ysu::node_id_handshake node_id_handshake (cookie, boost::none);
	...
	boost::optional<std::pair<ysu::account, ysu::signature>> response_zero (std::make_pair (ysu::account (0), ysu::signature (0)));
	ysu::node_id_handshake node_id_handshake_response (boost::none, response_zero);
...
TEST (tcp_listener, tcp_listener_timeout_empty)
{
	ysu::system system (1);
	auto node0 (system.nodes[0]);
	auto socket (std::make_shared<ysu::socket> (*node0));
	...
	{
		{
			ysu::lock_guard<std::mutex> guard (node0->bootstrap.mutex);
...
TEST (tcp_listener, tcp_listener_timeout_node_id_handshake)
{
	...
	auto node0 (system.nodes[0]);
	auto socket (std::make_shared<ysu::socket> (*node0));
	auto cookie (node0->network.syn_cookies.assign (ysu::transport::map_tcp_to_endpoint (node0->bootstrap.endpoint ())));
	ysu::node_id_handshake node_id_handshake (cookie, boost::none);
	auto input (node_id_handshake.to_shared_const_buffer (false));
	...
	ASSERT_TIMELY (5s, node0->stats.count (ysu::stat::type::message, ysu::stat::detail::node_id_handshake) != 0);
	{
		ysu::lock_guard<std::mutex> guard (node0->bootstrap.mutex);
...
TEST (network, replace_port)
{
	...
	ysu::node_config node0_config (ysu::get_available_port (), system.logging);
	node0_config.io_threads = 8;
	auto node0 = system.add_node (node0_config, node_flags);
	ASSERT_EQ (0, node0->network.size ());
	ysu::node_config node1_config (ysu::get_available_port (), system.logging);
	node1_config.io_threads = 1;
	auto node1 (std::make_shared<ysu::node> (system.io_ctx, ysu::unique_path (), system.alarm, node1_config, system.work, node_flags));
	node1->start ();
	system.nodes.push_back (node1);
	auto wrong_endpoint = ysu::endpoint (node1->network.endpoint ().address (), ysu::get_available_port ());
	...
	node0->network.udp_channels.modify (channel0, [&node1](std::shared_ptr<ysu::transport::channel> channel_a) {
	...
	auto channel1 (std::make_shared<ysu::transport::channel_udp> (node0->network.udp_channels, node1->network.endpoint (), node1->network_params.protocol.protocol_version));
...
TEST (network, peer_max_tcp_attempts)
{
	...
	{
		auto node2 (std::make_shared<ysu::node> (system.io_ctx, ysu::get_available_port (), ysu::unique_path (), system.alarm, system.logging, system.work, node_flags));
		node2->start ();
	...
	ASSERT_TRUE (node->network.tcp_channels.reachout (ysu::endpoint (node->network.endpoint ().address (), ysu::get_available_port ())));
}
...
TEST (network, duplicate_detection)
{
	...
	auto & node0 (*system.add_node (node_flags));
	auto & node1 (*system.add_node (node_flags));
	auto udp_channel (std::make_shared<ysu::transport::channel_udp> (node0.network.udp_channels, node1.network.endpoint (), node1.network_params.protocol.protocol_version));
	ysu::genesis genesis;
	ysu::publish publish (genesis.open);

	// Publish duplicate detection through UDP
	ASSERT_EQ (0, node1.stats.count (ysu::stat::type::filter, ysu::stat::detail::duplicate_publish));
	udp_channel->send (publish);
	udp_channel->send (publish);
	ASSERT_TIMELY (2s, node1.stats.count (ysu::stat::type::filter, ysu::stat::detail::duplicate_publish) == 1);

	// Publish duplicate detection through TCP
	auto tcp_channel (node0.network.tcp_channels.find_channel (ysu::transport::map_endpoint_to_tcp (node1.network.endpoint ())));
	ASSERT_EQ (1, node1.stats.count (ysu::stat::type::filter, ysu::stat::detail::duplicate_publish));
	tcp_channel->send (publish);
	ASSERT_TIMELY (2s, node1.stats.count (ysu::stat::type::filter, ysu::stat::detail::duplicate_publish) == 2);
}
...
TEST (network, duplicate_revert_publish)
{
	...
	auto channel (std::make_shared<ysu::transport::channel_udp> (node.network.udp_channels, node.network.endpoint (), node.network_params.protocol.protocol_version));
...
TEST (network, bandwidth_limiter)
{
	ysu::system system;
	ysu::genesis genesis;
	ysu::publish message (genesis.open);
	...
	ASSERT_TIMELY (1s, 0 == node.stats.count (ysu::stat::type::drop, ysu::stat::detail::publish, ysu::stat::dir::out));

	// Send droppable message; drop stats should increase by one now
	channel1->send (message);
	ASSERT_TIMELY (1s, 1 == node.stats.count (ysu::stat::type::drop, ysu::stat::detail::publish, ysu::stat::dir::out));

	// Send non-droppable message, i.e. drop stats should not increase
	channel2->send (message, nullptr, ysu::buffer_drop_policy::no_limiter_drop);
	ASSERT_TIMELY (1s, 1 == node.stats.count (ysu::stat::type::drop, ysu::stat::detail::publish, ysu::stat::dir::out));
...
TEST (peer_exclusion, validate)
{
	...
	{
		ysu::tcp_endpoint endpoint (boost::asio::ip::address_v6::v4_mapped (boost::asio::ip::address_v4 (i)), 0);
	...
	auto & peers_by_endpoint (excluded_peers.peers.get<ysu::peer_exclusion::tag_endpoint> ());
	ysu::tcp_endpoint oldest (boost::asio::ip::address_v6::v4_mapped (boost::asio::ip::address_v4 (0x0)), 0);
	...
	ysu::tcp_endpoint first (boost::asio::ip::address_v6::v4_mapped (boost::asio::ip::address_v4 (0x1)), 0);
	ASSERT_NE (peers_by_endpoint.end (), peers_by_endpoint.find (first.address ()));
	ysu::tcp_endpoint second (boost::asio::ip::address_v6::v4_mapped (boost::asio::ip::address_v4 (0x2)), 0);
	...
	auto component (ysu::collect_container_info (excluded_peers, ""));
	auto composite (dynamic_cast<ysu::container_info_composite *> (component.get ()));
	ASSERT_NE (nullptr, component);
	auto & children (composite->get_children ());
	ASSERT_EQ (1, children.size ());
	auto child_leaf (dynamic_cast<ysu::container_info_leaf *> (children.front ().get ()));
...
TEST (network, tcp_no_connect_excluded_peers)
{
	...
	auto endpoint1_tcp (ysu::transport::map_endpoint_to_tcp (endpoint1));
	while (!node0->network.excluded_peers.check (endpoint1_tcp))
	{
		node0->network.excluded_peers.add (endpoint1_tcp, 1);
	}
	ASSERT_EQ (0, node0->stats.count (ysu::stat::type::tcp, ysu::stat::detail::tcp_excluded));
	node1->network.merge_peer (node0->network.endpoint ());
	ASSERT_TIMELY (5s, node0->stats.count (ysu::stat::type::tcp, ysu::stat::detail::tcp_excluded) >= 1);
...
{
	ysu::tcp_message_manager manager (1);
	ysu::tcp_message_item item;
	item.node_id = ysu::account (100);
	...
	ysu::tcp_message_manager manager2 (2);
...
TEST (network, cleanup_purge)
{
	...
	std::weak_ptr<ysu::node> node_w = node1.shared ();
	node1.network.tcp_channels.start_tcp (node2->network.endpoint (), [node_w](std::shared_ptr<ysu::transport::channel> channel_a) {
		if (auto node_l = node_w.lock ())
		{
			node_l->network.send_keepalive (channel_a);
		}
...
TEST (network, loopback_channel)
{
	...
	auto & node1 = *system.nodes[0];
	auto & node2 = *system.nodes[1];
	ysu::transport::channel_loopback channel1 (node1);
	ASSERT_EQ (channel1.get_type (), ysu::transport::transport_type::loopback);
	ASSERT_EQ (channel1.get_endpoint (), node1.network.endpoint ());
	ASSERT_EQ (channel1.get_tcp_endpoint (), ysu::transport::map_endpoint_to_tcp (node1.network.endpoint ()));
	...
	ysu::transport::channel_loopback channel2 (node2);
