...
TEST (timer, states)
{
	ysu::timer<std::chrono::milliseconds> t1;
	ASSERT_EQ (t1.current_state (), ysu::timer_state::stopped);
	t1.start ();
	ASSERT_EQ (t1.current_state (), ysu::timer_state::started);
	t1.restart ();
	ASSERT_EQ (t1.current_state (), ysu::timer_state::started);
	t1.pause ();
	ASSERT_EQ (t1.current_state (), ysu::timer_state::stopped);
	...
	ysu::timer<std::chrono::milliseconds> t2 (ysu::timer_state::started);
	ASSERT_EQ (t2.current_state (), ysu::timer_state::started);
	t2.stop ();
	ASSERT_EQ (t2.current_state (), ysu::timer_state::stopped);
}

TEST (timer, measure_and_compare)
{
	using namespace std::chrono_literals;
	ysu::timer<std::chrono::milliseconds> t1 (ysu::timer_state::started);
	ASSERT_EQ (t1.current_state (), ysu::timer_state::started);
