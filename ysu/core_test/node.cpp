...
namespace
{
void add_required_children_node_config_tree (ysu::jsonconfig & tree);
}
...
TEST (node, work_generate)
{
	...
	ysu::block_hash root{ 1 };
	ysu::work_version version{ ysu::work_version::work_1 };
	{
		auto difficulty = ysu::difficulty::from_multiplier (1.5, node.network_params.network.publish_thresholds.base);
		auto work = node.work_generate_blocking (version, root, difficulty);
		ASSERT_TRUE (work.is_initialized ());
		ASSERT_TRUE (ysu::work_difficulty (version, root, *work) >= difficulty);
	}
	{
		auto difficulty = ysu::difficulty::from_multiplier (0.5, node.network_params.network.publish_thresholds.base);
		boost::optional<uint64_t> work;
		do
		{
			work = node.work_generate_blocking (version, root, difficulty);
		} while (ysu::work_difficulty (version, root, *work) >= node.network_params.network.publish_thresholds.base);
		ASSERT_TRUE (work.is_initialized ());
		ASSERT_TRUE (ysu::work_difficulty (version, root, *work) >= difficulty);
		ASSERT_FALSE (ysu::work_difficulty (version, root, *work) >= node.network_params.network.publish_thresholds.base);
	}
}
...
TEST (node, block_store_path_failure)
{
	...
	ysu::alarm alarm (*service);
	auto path (ysu::unique_path ());
	ysu::logging logging;
	logging.init (path);
	ysu::work_pool work (std::numeric_limits<unsigned>::max ());
	auto node (std::make_shared<ysu::node> (*service, ysu::get_available_port (), path, alarm, logging, work));
...
#endif
{
	...
	{
		ysu::inactive_node node (ysu::unique_path (), ysu::inactive_node_flag_defaults ());
		ASSERT_TRUE (node.node->init_error ());
	}
	else
	{
		ASSERT_EXIT (ysu::inactive_node node (ysu::unique_path (), ysu::inactive_node_flag_defaults ()), ::testing::ExitedWithCode (1), "");
	}
}
...
TEST (node, password_fanout)
{
	...
	auto node (std::make_shared<ysu::node> (*service, path, alarm, config, work));
...
TEST (node, balance)
{
	...
	ASSERT_EQ (std::numeric_limits<ysu::uint128_t>::max (), system.nodes[0]->ledger.account_balance (transaction, ysu::dev_genesis_key.pub));
}
...
TEST (node, send_unkeyed)
{
	...
	ysu::keypair key2;
	system.wallet (0)->insert_adhoc (ysu::dev_genesis_key.prv);
	system.wallet (0)->store.password.value_set (ysu::keypair ().prv);
	ASSERT_EQ (nullptr, system.wallet (0)->send_action (ysu::dev_genesis_key.pub, key2.pub, system.nodes[0]->config.receive_minimum.number ()));
}
...
TEST (node, send_self)
{
	...
	ASSERT_NE (nullptr, system.wallet (0)->send_action (ysu::dev_genesis_key.pub, key2.pub, system.nodes[0]->config.receive_minimum.number ()));
	ASSERT_TIMELY (10s, !system.nodes[0]->balance (key2.pub).is_zero ());
	ASSERT_EQ (std::numeric_limits<ysu::uint128_t>::max () - system.nodes[0]->config.receive_minimum.number (), system.nodes[0]->balance (ysu::dev_genesis_key.pub));
}
...
TEST (node, send_single_observing_peer)
{
	...
	ASSERT_EQ (std::numeric_limits<ysu::uint128_t>::max () - system.nodes[0]->config.receive_minimum.number (), system.nodes[0]->balance (ysu::dev_genesis_key.pub));
	ASSERT_TRUE (system.nodes[0]->balance (key2.pub).is_zero ());
	ASSERT_TIMELY (10s, std::all_of (system.nodes.begin (), system.nodes.end (), [&](std::shared_ptr<ysu::node> const & node_a) { return !node_a->balance (key2.pub).is_zero (); }));
}

TEST (node, send_single_many_peers)
{
	ysu::system system (10);
	ysu::keypair key2;
	system.wallet (0)->insert_adhoc (ysu::dev_genesis_key.prv);
	system.wallet (1)->insert_adhoc (key2.prv);
	ASSERT_NE (nullptr, system.wallet (0)->send_action (ysu::dev_genesis_key.pub, key2.pub, system.nodes[0]->config.receive_minimum.number ()));
	ASSERT_EQ (std::numeric_limits<ysu::uint128_t>::max () - system.nodes[0]->config.receive_minimum.number (), system.nodes[0]->balance (ysu::dev_genesis_key.pub));
	ASSERT_TRUE (system.nodes[0]->balance (key2.pub).is_zero ());
	ASSERT_TIMELY (3.5min, std::all_of (system.nodes.begin (), system.nodes.end (), [&](std::shared_ptr<ysu::node> const & node_a) { return !node_a->balance (key2.pub).is_zero (); }));
	system.stop ();
...
TEST (node, send_out_of_order)
{
	...
	auto send1 = builder.make_block ()
	             ...
	             .balance (std::numeric_limits<ysu::uint128_t>::max () - node1.config.receive_minimum.number ())
	...
	auto send2 = builder.make_block ()
	             .previous (send1->hash ())
	             .destination (key2.pub)
	             .balance (std::numeric_limits<ysu::uint128_t>::max () - 2 * node1.config.receive_minimum.number ())
	             .sign (ysu::dev_genesis_key.prv, ysu::dev_genesis_key.pub)
	...
	auto send3 = builder.make_block ()
	             .previous (send2->hash ())
	             .destination (key2.pub)
	             .balance (std::numeric_limits<ysu::uint128_t>::max () - 3 * node1.config.receive_minimum.number ())
	             .sign (ysu::dev_genesis_key.prv, ysu::dev_genesis_key.pub)
	...
	ASSERT_TIMELY (10s, std::all_of (system.nodes.begin (), system.nodes.end (), [&](std::shared_ptr<ysu::node> const & node_a) { return node_a->balance (ysu::dev_genesis_key.pub) == ysu::genesis_amount - node1.config.receive_minimum.number () * 3; }));
}
...
TEST (node, quick_confirm)
{
	...
	auto genesis_start_balance (node1.balance (ysu::dev_genesis_key.pub));
	...
	ASSERT_EQ (node1.balance (ysu::dev_genesis_key.pub), node1.config.online_weight_minimum.number () + 1);
...
TEST (node, node_receive_quorum)
{
	...
	ASSERT_TIMELY (10s, node1.active.election (ysu::qualified_root (previous, previous)) != nullptr);
	auto election (node1.active.election (ysu::qualified_root (previous, previous)));
	...
	system2.wallet (0)->insert_adhoc (ysu::dev_genesis_key.prv);
	ASSERT_TRUE (node1.balance (key.pub).is_zero ());
	auto channel (std::make_shared<ysu::transport::channel_udp> (node1.network.udp_channels, system2.nodes[0]->network.endpoint (), node1.network_params.protocol.protocol_version));
...
TEST (node, auto_bootstrap)
{
	...
	system.wallet (0)->insert_adhoc (ysu::dev_genesis_key.prv);
	system.wallet (0)->insert_adhoc (key2.prv);
	auto send1 (system.wallet (0)->send_action (ysu::dev_genesis_key.pub, key2.pub, node0->config.receive_minimum.number ()));
	ASSERT_NE (nullptr, send1);
	ASSERT_TIMELY (10s, node0->balance (key2.pub) == node0->config.receive_minimum.number ());
	auto node1 (std::make_shared<ysu::node> (system.io_ctx, ysu::get_available_port (), ysu::unique_path (), system.alarm, system.logging, system.work, node_flags));
	ASSERT_FALSE (node1->init_error ());
	auto channel (std::make_shared<ysu::transport::channel_udp> (node1->network.udp_channels, node0->network.endpoint (), node1->network_params.protocol.protocol_version));
	node1->network.send_keepalive (channel);
	node1->start ();
...
TEST (node, auto_bootstrap_reverse)
{
	...
	auto node1 (std::make_shared<ysu::node> (system.io_ctx, ysu::get_available_port (), ysu::unique_path (), system.alarm, system.logging, system.work, node_flags));
	ASSERT_FALSE (node1->init_error ());
	ASSERT_NE (nullptr, system.wallet (0)->send_action (ysu::dev_genesis_key.pub, key2.pub, node0->config.receive_minimum.number ()));
	auto channel (std::make_shared<ysu::transport::channel_udp> (node0->network.udp_channels, node1->network.endpoint (), node0->network_params.protocol.protocol_version));
...
TEST (node, receive_gap)
{
	...
	auto block = ysu::send_block_builder ()
	             .previous (5)
	             .destination (1)
	             .balance (2)
	             .sign (ysu::keypair ().prv, 4)
	             .work (0)
	             .build_shared ();
...
{
	ysu::system system (1);
	std::array<ysu::endpoint, 8> endpoints;
	endpoints.fill (ysu::endpoint (boost::asio::ip::address_v6::loopback (), ysu::get_available_port ()));
	endpoints[0] = ysu::endpoint (boost::asio::ip::address_v6::loopback (), ysu::get_available_port ());
...
TEST (node, search_pending)
{
	...
	ysu::keypair key2;
	system.wallet (0)->insert_adhoc (ysu::dev_genesis_key.prv);
	ASSERT_NE (nullptr, system.wallet (0)->send_action (ysu::dev_genesis_key.pub, key2.pub, node->config.receive_minimum.number ()));
	system.wallet (0)->insert_adhoc (key2.prv);
...
TEST (node, search_pending_multiple)
{
	...
	ASSERT_NE (nullptr, system.wallet (0)->send_action (ysu::dev_genesis_key.pub, key3.pub, node->config.receive_minimum.number ()));
...
TEST (node, search_pending_confirmed)
{
	...
	ysu::keypair key2;
	system.wallet (0)->insert_adhoc (ysu::dev_genesis_key.prv);
	auto send1 (system.wallet (0)->send_action (ysu::dev_genesis_key.pub, key2.pub, node->config.receive_minimum.number ()));
	ASSERT_NE (nullptr, send1);
	auto send2 (system.wallet (0)->send_action (ysu::dev_genesis_key.pub, key2.pub, node->config.receive_minimum.number ()));
	...
	}
	{
		...
		system.wallet (0)->store.erase (transaction, ysu::dev_genesis_key.pub);
	}
	system.wallet (0)->insert_adhoc (key2.prv);
	ASSERT_FALSE (system.wallet (0)->search_pending ());
	{
...
TEST (node, unlock_search)
{
	...
	auto node (system.nodes[0]);
	ysu::keypair key2;
	ysu::uint128_t balance (node->balance (ysu::dev_genesis_key.pub));
	{
	...
	system.wallet (0)->insert_adhoc (ysu::dev_genesis_key.prv);
	ASSERT_NE (nullptr, system.wallet (0)->send_action (ysu::dev_genesis_key.pub, key2.pub, node->config.receive_minimum.number ()));
	ASSERT_TIMELY (10s, node->balance (ysu::dev_genesis_key.pub) != balance);
	ASSERT_TIMELY (10s, node->active.empty ());
	system.wallet (0)->insert_adhoc (key2.prv);
	{
		ysu::lock_guard<std::recursive_mutex> lock (system.wallet (0)->store.mutex);
		system.wallet (0)->store.password.value_set (ysu::keypair ().prv);
	}
	{
		auto transaction (system.wallet (0)->wallets.tx_begin_write ());
...
TEST (node, connect_after_junk)
{
	...
	auto channel1 (std::make_shared<ysu::transport::channel_udp> (node1->network.udp_channels, node0->network.endpoint (), node1->network_params.protocol.protocol_version));
	channel1->send_buffer (ysu::shared_const_buffer (std::move (junk_buffer)), [](boost::system::error_code const &, size_t) {});
	ASSERT_TIMELY (10s, node0->stats.count (ysu::stat::type::error) != 0);
	node1->start ();
	system.nodes.push_back (node1);
	auto channel2 (std::make_shared<ysu::transport::channel_udp> (node1->network.udp_channels, node0->network.endpoint (), node1->network_params.protocol.protocol_version));
...
{
	ysu::system system (1);
	auto price1 (system.nodes[0]->price (ysu::Gxrb_ratio, 1));
	ASSERT_EQ (ysu::node::price_max * 100.0, price1);
	auto price2 (system.nodes[0]->price (ysu::Gxrb_ratio * int(ysu::node::free_cutoff + 1), 1));
	ASSERT_EQ (0, price2);
	auto price3 (system.nodes[0]->price (ysu::Gxrb_ratio * int(ysu::node::free_cutoff + 2) / 2, 1));
	ASSERT_EQ (ysu::node::price_max * 100.0 / 2, price3);
	auto price4 (system.nodes[0]->price (ysu::Gxrb_ratio * int(ysu::node::free_cutoff) * 2, 1));
	ASSERT_EQ (0, price4);
}
...
{
	auto path (ysu::unique_path ());
	ysu::logging logging1;
	logging1.init (path);
	ysu::node_config config1 (100, logging1);
...
public:
	...
	ysu::error serialize_json (ysu::jsonconfig & json)
	{
		json.put ("thing", text);
...
{
public:
	ysu::error deserialize_json (bool & upgraded, ysu::jsonconfig & json)
	{
		if (!json.empty ())
		{
			...
			if (text_l == "error")
			{
				json.get_error () = ysu::error_common::generic;
			}
			text = text_l;
		}
		else
...
TEST (node_flags, disable_tcp_realtime)
{
	...
	ASSERT_EQ (ysu::transport::transport_type::udp, list1[0]->get_type ());
	ASSERT_EQ (1, node2->network.size ());
	auto list2 (node2->network.list (2));
	ASSERT_EQ (node1->network.endpoint (), list2[0]->get_endpoint ());
	ASSERT_EQ (ysu::transport::transport_type::udp, list2[0]->get_type ());
}
...
TEST (node_flags, disable_tcp_realtime_and_bootstrap_listener)
{
	...
	auto node2 = system.add_node (node_flags);
	ASSERT_EQ (ysu::tcp_endpoint (boost::asio::ip::address_v6::loopback (), 0), node2->bootstrap.endpoint ());
	ASSERT_NE (ysu::endpoint (boost::asio::ip::address_v6::loopback (), 0), node2->network.endpoint ());
	ASSERT_EQ (1, node1->network.size ());
	auto list1 (node1->network.list (2));
	ASSERT_EQ (node2->network.endpoint (), list1[0]->get_endpoint ());
	ASSERT_EQ (ysu::transport::transport_type::udp, list1[0]->get_type ());
...
TEST (node_flags, disable_udp)
{
	...
	node_flags.disable_udp = false;
	auto node1 = system.add_node (node_flags);
	auto node2 (std::make_shared<ysu::node> (system.io_ctx, ysu::unique_path (), system.alarm, ysu::node_config (ysu::get_available_port (), system.logging), system.work));
	system.nodes.push_back (node2);
	node2->start ();
	ASSERT_EQ (ysu::endpoint (boost::asio::ip::address_v6::loopback (), 0), node2->network.udp_channels.get_local_endpoint ());
	ASSERT_NE (ysu::endpoint (boost::asio::ip::address_v6::loopback (), 0), node2->network.endpoint ());
	// Send UDP message
	auto channel (std::make_shared<ysu::transport::channel_udp> (node1->network.udp_channels, node2->network.endpoint (), node2->network_params.protocol.protocol_version));
	node1->network.send_keepalive (channel);
	...
	auto list1 (node1->network.list (2));
	ASSERT_EQ (node2->network.endpoint (), list1[0]->get_endpoint ());
	ASSERT_EQ (ysu::transport::transport_type::tcp, list1[0]->get_type ());
	ASSERT_EQ (1, node2->network.size ());
	auto list2 (node2->network.list (2));
	ASSERT_EQ (node1->network.endpoint (), list2[0]->get_endpoint ());
	ASSERT_EQ (ysu::transport::transport_type::tcp, list2[0]->get_type ());
	node2->stop ();
}

TEST (node, fork_publish)
{
	std::weak_ptr<ysu::node> node0;
	{
		ysu::system system (1);
...
TEST (node, fork_publish_inactive)
{
	...
	ASSERT_EQ (ysu::process_result::fork, node.process_local (send2).code);
...
TEST (node, fork_keep)
{
	...
	node2.block_processor.flush ();
	auto election1 (node2.active.election (ysu::qualified_root (genesis.hash (), genesis.hash ())));
...
{
	std::vector<ysu::transport::transport_type> types{ ysu::transport::transport_type::tcp, ysu::transport::transport_type::udp };
	for (auto & type : types)
	{
		ysu::system system;
		ysu::node_flags node_flags;
		if (type == ysu::transport::transport_type::udp)
		{
			node_flags.disable_tcp_realtime = true;
			node_flags.disable_bootstrap_listener = true;
			node_flags.disable_udp = false;
...
TEST (node, fork_bootstrap_flip)
{
	ysu::system system0;
	ysu::system system1;
	ysu::node_config config0 (ysu::get_available_port (), system0.logging);
	config0.frontiers_confirmation = ysu::frontiers_confirmation_mode::disabled;
	ysu::node_flags node_flags;
	node_flags.disable_bootstrap_bulk_push_client = true;
	node_flags.disable_lazy_bootstrap = true;
	node_flags.disable_udp = false;
	auto & node1 (*system0.add_node (config0, node_flags));
	ysu::node_config config1 (ysu::get_available_port (), system1.logging);
	config1.frontiers_confirmation = ysu::frontiers_confirmation_mode::disabled;
	...
	node1.block_processor.flush ();
	node2.block_processor.add (send2, ysu::seconds_since_epoch ());
	node2.block_processor.flush ();
	{
...
TEST (node, fork_open)
{
	...
	ysu::keypair key1;
	ysu::genesis genesis;
	auto send1 = ysu::send_block_builder ()
	             .previous (genesis.hash ())
	             .destination (key1.pub)
	             .balance (0)
	             .sign (ysu::dev_genesis_key.prv, ysu::dev_genesis_key.pub)
	...
	ysu::open_block_builder builder;
...
TEST (node, fork_open_flip)
{
	...
	auto send1_copy (std::make_shared<ysu::send_block> (*send1));
	node1.process_active (send1);
	...
	ASSERT_EQ (ysu::genesis_amount - 1, winner.first);
...
TEST (node, coherent_observer)
{
	ysu::system system (1);
	auto & node1 (*system.nodes[0]);
	node1.observers.blocks.add ([&node1](ysu::election_status const & status_a, ysu::account const &, ysu::uint128_t const &, bool) {
		auto transaction (node1.store.tx_begin_read ());
		ASSERT_TRUE (node1.store.block_exists (transaction, status_a.winner->hash ()));
	});
	system.wallet (0)->insert_adhoc (ysu::dev_genesis_key.prv);
	ysu::keypair key;
	system.wallet (0)->send_action (ysu::dev_genesis_key.pub, key.pub, 1);
}
...
TEST (node, fork_no_vote_quorum)
{
	...
	system.wallet (0)->send_action (ysu::dev_genesis_key.pub, key4, ysu::genesis_amount / 4);
	...
	auto block (system.wallet (0)->send_action (ysu::dev_genesis_key.pub, key1, node1.config.receive_minimum.number ()));
	...
	ysu::state_block send1 (ysu::dev_genesis_key.pub, block->hash (), ysu::dev_genesis_key.pub, (ysu::genesis_amount / 4) - (node1.config.receive_minimum.number () * 2), key1, ysu::dev_genesis_key.prv, ysu::dev_genesis_key.pub, *system.work.generate (block->hash ()));
	ASSERT_EQ (ysu::process_result::progress, node1.process (send1).code);
	ASSERT_EQ (ysu::process_result::progress, node2.process (send1).code);
	ASSERT_EQ (ysu::process_result::progress, node3.process (send1).code);
	auto key2 (system.wallet (2)->deterministic_insert ());
	auto send2 = ysu::send_block_builder ()
	             .previous (block->hash ())
	             .destination (key2)
	             .balance ((ysu::genesis_amount / 4) - (node1.config.receive_minimum.number () * 2))
	             .sign (ysu::dev_genesis_key.prv, ysu::dev_genesis_key.pub)
	...
	auto vote (std::make_shared<ysu::vote> (key1, key3, 0, send2));
	ysu::confirm_ack confirm (vote);
	std::vector<uint8_t> buffer;
	{
		ysu::vectorstream stream (buffer);
		confirm.serialize (stream, false);
	}
	ysu::transport::channel_udp channel (node2.network.udp_channels, node3.network.endpoint (), node1.network_params.protocol.protocol_version);
	channel.send_buffer (ysu::shared_const_buffer (std::move (buffer)));
	ASSERT_TIMELY (10s, node3.stats.count (ysu::stat::type::message, ysu::stat::detail::confirm_ack, ysu::stat::dir::in) >= 3);
	ASSERT_TRUE (node1.latest (ysu::dev_genesis_key.pub) == send1.hash ());
	ASSERT_TRUE (node2.latest (ysu::dev_genesis_key.pub) == send1.hash ());
	ASSERT_TRUE (node3.latest (ysu::dev_genesis_key.pub) == send1.hash ());
}
...
TEST (node, DISABLED_fork_pre_confirm)
{
	...
	auto block0 (system.wallet (0)->send_action (ysu::dev_genesis_key.pub, key1.pub, ysu::genesis_amount / 3));
	ASSERT_NE (nullptr, block0);
	ASSERT_TIMELY (30s, node0.balance (key1.pub) != 0);
	auto block1 (system.wallet (0)->send_action (ysu::dev_genesis_key.pub, key2.pub, ysu::genesis_amount / 3));
	...
	auto block2 = builder.make_block ()
	              ...
	              .previous (node0.latest (ysu::dev_genesis_key.pub))
	              .representative (key3.pub)
	              .balance (node0.balance (ysu::dev_genesis_key.pub))
	              .link (0)
	              .sign (ysu::dev_genesis_key.prv, ysu::dev_genesis_key.pub)
	              .work (0)
	              .build_shared ();
	...
	while (!done)
	{
		done |= node0.latest (ysu::dev_genesis_key.pub) == block2->hash () && node1.latest (ysu::dev_genesis_key.pub) == block2->hash () && node2.latest (ysu::dev_genesis_key.pub) == block2->hash ();
		done |= node0.latest (ysu::dev_genesis_key.pub) == block3->hash () && node1.latest (ysu::dev_genesis_key.pub) == block3->hash () && node2.latest (ysu::dev_genesis_key.pub) == block3->hash ();
		ASSERT_NO_ERROR (system.poll ());
	}
}
...
TEST (node, DISABLED_fork_stale)
{
	...
	system1.wallet (0)->insert_adhoc (ysu::dev_genesis_key.prv);
	ysu::system system2 (1);
	auto & node1 (*system1.nodes[0]);
	auto & node2 (*system2.nodes[0]);
	node2.bootstrap_initiator.bootstrap (node1.network.endpoint ());
	std::shared_ptr<ysu::transport::channel> channel (std::make_shared<ysu::transport::channel_udp> (node2.network.udp_channels, node1.network.endpoint (), node2.network_params.protocol.protocol_version));
	auto vote = std::make_shared<ysu::vote> (ysu::dev_genesis_key.pub, ysu::dev_genesis_key.prv, 0, std::vector<ysu::block_hash> ());
	...
	auto send3 = builder.make_block ()
	             ...
	             .balance (ysu::genesis_amount - ysu::Mxrb_ratio)
	             .link (key1.pub)
	             .sign (ysu::dev_genesis_key.prv, ysu::dev_genesis_key.pub)
	             .work (0)
	             .build_shared ();
	...
	auto send1 = builder.make_block ()
	             ...
	             .previous (send3->hash ())
	             .representative (ysu::dev_genesis_key.pub)
	             .balance (ysu::genesis_amount - 2 * ysu::Mxrb_ratio)
	             .link (key1.pub)
	             .sign (ysu::dev_genesis_key.prv, ysu::dev_genesis_key.pub)
	             .work (0)
	             .build_shared ();
	...
	node1.work_generate_blocking (*send2);
	{
		...
		ASSERT_EQ (ysu::process_result::progress, node1.ledger.process (transaction1, *send1).code);
		auto transaction2 (node2.store.tx_begin_write ());
		ASSERT_EQ (ysu::process_result::progress, node2.ledger.process (transaction2, *send2).code);
	}
	node1.process_active (send1);
	node1.process_active (send2);
	node2.process_active (send1);
...
TEST (node, broadcast_elected)
{
	...
	for (auto & type : types)
	{
		...
		auto node2 = system.add_node (node_config, node_flags, type);
		ysu::keypair rep_big;
		ysu::keypair rep_small;
		ysu::keypair rep_other;
		ysu::block_builder builder;
		{
			...
			auto open_other = *builder.open ()
			                   ...
			                   .build ();
			ASSERT_EQ (ysu::process_result::progress, node0->ledger.process (transaction0, fund_big).code);
			ASSERT_EQ (ysu::process_result::progress, node1->ledger.process (transaction1, fund_big).code);
			ASSERT_EQ (ysu::process_result::progress, node2->ledger.process (transaction2, fund_big).code);
			ASSERT_EQ (ysu::process_result::progress, node0->ledger.process (transaction0, open_big).code);
			ASSERT_EQ (ysu::process_result::progress, node1->ledger.process (transaction1, open_big).code);
			ASSERT_EQ (ysu::process_result::progress, node2->ledger.process (transaction2, open_big).code);
			ASSERT_EQ (ysu::process_result::progress, node0->ledger.process (transaction0, fund_small).code);
			ASSERT_EQ (ysu::process_result::progress, node1->ledger.process (transaction1, fund_small).code);
			ASSERT_EQ (ysu::process_result::progress, node2->ledger.process (transaction2, fund_small).code);
			ASSERT_EQ (ysu::process_result::progress, node0->ledger.process (transaction0, open_small).code);
			ASSERT_EQ (ysu::process_result::progress, node1->ledger.process (transaction1, open_small).code);
			ASSERT_EQ (ysu::process_result::progress, node2->ledger.process (transaction2, open_small).code);
			ASSERT_EQ (ysu::process_result::progress, node0->ledger.process (transaction0, fund_other).code);
			ASSERT_EQ (ysu::process_result::progress, node1->ledger.process (transaction1, fund_other).code);
			ASSERT_EQ (ysu::process_result::progress, node2->ledger.process (transaction2, fund_other).code);
			ASSERT_EQ (ysu::process_result::progress, node0->ledger.process (transaction0, open_other).code);
			ASSERT_EQ (ysu::process_result::progress, node1->ledger.process (transaction1, open_other).code);
			ASSERT_EQ (ysu::process_result::progress, node2->ledger.process (transaction2, open_other).code);
		}
		// Confirm blocks to allow voting
		for (auto & node : system.nodes)
		{
			auto block (node->block (node->latest (ysu::dev_genesis_key.pub)));
			ASSERT_NE (nullptr, block);
		...
		auto fork0 = builder.send ()
		             .previous (node2->latest (ysu::dev_genesis_key.pub))
		             .destination (rep_small.pub)
		             .balance (0)
		             .sign (ysu::dev_genesis_key.prv, ysu::dev_genesis_key.pub)
		             .work (*node0->work_generate_blocking (node2->latest (ysu::dev_genesis_key.pub)))
		             .build_shared ();
		// A copy is necessary to avoid data races during ledger processing, which sets the sideband
		auto fork0_copy (std::make_shared<ysu::send_block> (*fork0));
		...
		ASSERT_TIMELY (5s, node1->stats.count (ysu::stat::type::confirmation_observer, ysu::stat::detail::inactive_conf_height, ysu::stat::dir::out) != 0);
	}
}
...
TEST (node, rep_self_vote)
{
	...
	auto fund_big = *builder.send ()
	                 .previous (ysu::genesis_hash)
	                 .destination (rep_big.pub)
	                 .balance (ysu::uint128_t{ "0xb0000000000000000000000000000000" })
	                 .sign (ysu::dev_genesis_key.prv, ysu::dev_genesis_key.pub)
	                 .work (*system.work.generate (ysu::genesis_hash))
	                 .build ();
	auto open_big = *builder.open ()
	                 ...
	                 .work (*system.work.generate (rep_big.pub))
	                 .build ();
	ASSERT_EQ (ysu::process_result::progress, node0->process (fund_big).code);
	ASSERT_EQ (ysu::process_result::progress, node0->process (open_big).code);
	...
	auto block0 = builder.send ()
	              .previous (fund_big.hash ())
	              .destination (rep_big.pub)
	              .balance (ysu::uint128_t ("0x60000000000000000000000000000000"))
	              .sign (ysu::dev_genesis_key.prv, ysu::dev_genesis_key.pub)
	              .work (*system.work.generate (fund_big.hash ()))
	              .build_shared ();
	ASSERT_EQ (ysu::process_result::progress, node0->process (*block0).code);
	...
	ASSERT_NE (rep_votes.end (), rep_votes.find (ysu::dev_genesis_key.pub));
...
{
	ysu::system system0 (1);
	ysu::system system1 (1);
	auto node0 (system0.nodes[0]);
	auto node1 (system1.nodes[0]);
	ysu::keypair key0;
	// node0 knows about send0 but node1 doesn't.
	ysu::send_block send0 (node0->latest (ysu::dev_genesis_key.pub), key0.pub, 500, ysu::dev_genesis_key.prv, ysu::dev_genesis_key.pub, 0);
	{
		auto transaction (node0->store.tx_begin_write ());
		ASSERT_EQ (ysu::process_result::progress, node0->ledger.process (transaction, send0).code);
	}
...
TEST (node, bootstrap_bulk_push)
{
	...
	ysu::keypair key0;
	// node0 knows about send0 but node1 doesn't.
	auto send0 = *ysu::send_block_builder ()
	              .previous (ysu::genesis_hash)
	              .destination (key0.pub)
	              .balance (500)
	              .sign (ysu::dev_genesis_key.prv, ysu::dev_genesis_key.pub)
	              .work (*node0->work_generate_blocking (ysu::genesis_hash))
	              .build ();
	ASSERT_EQ (ysu::process_result::progress, node0->process (send0).code);
...
TEST (node, bootstrap_fork_open)
{
	...
	auto send0 = *builder.send ()
	              .previous (ysu::genesis_hash)
	              .destination (key0.pub)
	              .balance (ysu::genesis_amount - 500)
	              .sign (ysu::dev_genesis_key.prv, ysu::dev_genesis_key.pub)
	              .work (*system.work.generate (ysu::genesis_hash))
	              .build ();
	...
	ASSERT_EQ (ysu::process_result::progress, node0->process (send0).code);
	ASSERT_EQ (ysu::process_result::progress, node1->process (send0).code);
	// Confirm send0 to allow starting and voting on the following blocks
	for (auto node : system.nodes)
	{
		node->block_confirm (node->block (node->latest (ysu::dev_genesis_key.pub)));
	...
	ASSERT_EQ (ysu::process_result::progress, node0->process (open0).code);
...
TEST (node, DISABLED_unconfirmed_send)
{
	...
	wallet0->insert_adhoc (ysu::dev_genesis_key.prv);
	auto send1 (wallet0->send_action (ysu::genesis_account, key0.pub, 2 * ysu::Mxrb_ratio));
	ASSERT_TIMELY (10s, node1.balance (key0.pub) == 2 * ysu::Mxrb_ratio && !node1.bootstrap_initiator.in_progress ());
	auto latest (node1.latest (key0.pub));
	ysu::state_block send2 (key0.pub, latest, ysu::genesis_account, ysu::Mxrb_ratio, ysu::genesis_account, key0.prv, key0.pub, *node0.work_generate_blocking (latest));
	{
		auto transaction (node1.store.tx_begin_write ());
		ASSERT_EQ (ysu::process_result::progress, node1.ledger.process (transaction, send2).code);
	}
	auto send3 (wallet1->send_action (key0.pub, ysu::genesis_account, ysu::Mxrb_ratio));
	ASSERT_TIMELY (10s, node0.balance (ysu::genesis_account) == ysu::genesis_amount);
}
...
TEST (node, rep_list)
{
	...
	wallet0->send_action (ysu::dev_genesis_key.pub, key1.pub, ysu::Mxrb_ratio);
...
TEST (node, rep_weight)
{
	...
	auto & node (*system.nodes[0]);
	ysu::genesis genesis;
	ysu::keypair keypair1;
	ysu::keypair keypair2;
	ysu::block_builder builder;
	auto amount_pr (node.minimum_principal_weight () + 100);
	auto amount_not_pr (node.minimum_principal_weight () - 100);
	std::shared_ptr<ysu::block> block1 = builder
	                                      .state ()
	                                      .account (ysu::dev_genesis_key.pub)
	                                      .previous (genesis.hash ())
	                                      .representative (ysu::dev_genesis_key.pub)
	                                      .balance (ysu::genesis_amount - amount_not_pr)
	                                      .link (keypair1.pub)
	                                      .sign (ysu::dev_genesis_key.prv, ysu::dev_genesis_key.pub)
	                                      .work (*system.work.generate (genesis.hash ()))
	                                      .build ();
	std::shared_ptr<ysu::block> block2 = builder
	                                      .state ()
	                                      ...
	                                      .build ();
	std::shared_ptr<ysu::block> block3 = builder
	                                      .state ()
	                                      .account (ysu::dev_genesis_key.pub)
	                                      .previous (block1->hash ())
	                                      .representative (ysu::dev_genesis_key.pub)
	                                      .balance (ysu::genesis_amount - amount_not_pr - amount_pr)
	                                      .link (keypair2.pub)
	                                      .sign (ysu::dev_genesis_key.prv, ysu::dev_genesis_key.pub)
	                                      .work (*system.work.generate (block1->hash ()))
	                                      .build ();
	std::shared_ptr<ysu::block> block4 = builder
	                                      .state ()
	                                      ...
	                                      .build ();
	{
		...
		ASSERT_EQ (ysu::process_result::progress, node.ledger.process (transaction, *block1).code);
		ASSERT_EQ (ysu::process_result::progress, node.ledger.process (transaction, *block2).code);
		ASSERT_EQ (ysu::process_result::progress, node.ledger.process (transaction, *block3).code);
		ASSERT_EQ (ysu::process_result::progress, node.ledger.process (transaction, *block4).code);
	}
	node.network.udp_channels.insert (ysu::endpoint (boost::asio::ip::address_v6::loopback (), ysu::get_available_port ()), 0);
	ASSERT_TRUE (node.rep_crawler.representatives (1).empty ());
	ysu::endpoint endpoint0 (boost::asio::ip::address_v6::loopback (), ysu::get_available_port ());
	ysu::endpoint endpoint1 (boost::asio::ip::address_v6::loopback (), ysu::get_available_port ());
	ysu::endpoint endpoint2 (boost::asio::ip::address_v6::loopback (), ysu::get_available_port ());
	std::shared_ptr<ysu::transport::channel> channel0 (std::make_shared<ysu::transport::channel_udp> (node.network.udp_channels, endpoint0, node.network_params.protocol.protocol_version));
	std::shared_ptr<ysu::transport::channel> channel1 (std::make_shared<ysu::transport::channel_udp> (node.network.udp_channels, endpoint1, node.network_params.protocol.protocol_version));
	std::shared_ptr<ysu::transport::channel> channel2 (std::make_shared<ysu::transport::channel_udp> (node.network.udp_channels, endpoint2, node.network_params.protocol.protocol_version));
	...
	auto vote0 = std::make_shared<ysu::vote> (ysu::dev_genesis_key.pub, ysu::dev_genesis_key.prv, 0, genesis.open);
	auto vote1 = std::make_shared<ysu::vote> (keypair1.pub, keypair1.prv, 0, genesis.open);
	auto vote2 = std::make_shared<ysu::vote> (keypair2.pub, keypair2.prv, 0, genesis.open);
	...
	ASSERT_EQ (node.balance (ysu::dev_genesis_key.pub), reps[0].weight.number ());
	ASSERT_EQ (ysu::dev_genesis_key.pub, reps[0].account);
...
TEST (node, rep_remove)
{
	...
	std::shared_ptr<ysu::block> block1 = builder
	                                      ...
	                                      .previous (genesis.hash ())
	                                      .representative (ysu::dev_genesis_key.pub)
	                                      .balance (ysu::genesis_amount - node.minimum_principal_weight () * 2)
	                                      .link (keypair1.pub)
	                                      .sign (ysu::dev_genesis_key.prv, ysu::dev_genesis_key.pub)
	                                      .work (*system.work.generate (genesis.hash ()))
	                                      .build ();
	...
	std::shared_ptr<ysu::block> block3 = builder
	                                      ...
	                                      .previous (block1->hash ())
	                                      .representative (ysu::dev_genesis_key.pub)
	                                      .balance (ysu::genesis_amount - node.minimum_principal_weight () * 4)
	                                      .link (keypair2.pub)
	                                      .sign (ysu::dev_genesis_key.prv, ysu::dev_genesis_key.pub)
	                                      .work (*system.work.generate (block1->hash ()))
	                                      .build ();
	...
	node.network.udp_channels.modify (channel_udp, [fake_timepoint](std::shared_ptr<ysu::transport::channel_udp> channel_a) {
	...
	auto node1 = system.add_node (ysu::node_config (ysu::get_available_port (), system.logging));
	system.wallet (1)->insert_adhoc (ysu::dev_genesis_key.prv);
	auto channel1 (node.network.find_channel (node1->network.endpoint ()));
	ASSERT_NE (nullptr, channel1);
	auto vote2 = std::make_shared<ysu::vote> (ysu::dev_genesis_key.pub, ysu::dev_genesis_key.prv, 0, genesis.open);
	...
	auto vote3 = std::make_shared<ysu::vote> (keypair2.pub, keypair2.prv, 0, genesis.open);
	node.network.tcp_channels.start_tcp (node2->network.endpoint (), [node_w, &vote3](std::shared_ptr<ysu::transport::channel> channel2) {
...
TEST (node, no_voting)
{
	...
	// Broadcast a confirm so others should know this is a rep node
	wallet1->send_action (ysu::dev_genesis_key.pub, key1.pub, ysu::Mxrb_ratio);
	ASSERT_TIMELY (10s, node0.active.empty ());
	ASSERT_EQ (0, node0.stats.count (ysu::stat::type::message, ysu::stat::detail::confirm_ack, ysu::stat::dir::in));
}
...
TEST (node, send_callback)
{
	...
	ASSERT_EQ (std::numeric_limits<ysu::uint128_t>::max () - node0.config.receive_minimum.number (), node0.balance (ysu::dev_genesis_key.pub));
}
...
TEST (node, vote_replay)
{
	...
	{
		auto transaction (node1.store.tx_begin_read ());
		auto vote (node1.store.vote_generate (transaction, ysu::dev_genesis_key.pub, ysu::dev_genesis_key.prv, genesis.open));
	}
	auto node2 = system.add_node ();
	{
		auto transaction (node2->store.tx_begin_read ());
		ysu::lock_guard<std::mutex> lock (node2->store.get_cache_mutex ());
		auto vote (node2->store.vote_current (transaction, ysu::dev_genesis_key.pub));
...
TEST (node, balance_observer)
{
	...
	ysu::keypair key;
	node1.observers.account_balance.add ([&key, &balances](ysu::account const & account_a, bool is_pending) {
		if (key.pub == account_a && is_pending)
		{
			balances++;
		}
		else if (ysu::dev_genesis_key.pub == account_a && !is_pending)
		{
			balances++;
		}
	});
...
TEST (node, stat_counting)
{
	ysu::system system (1);
	auto & node1 (*system.nodes[0]);
	node1.stats.add (ysu::stat::type::ledger, ysu::stat::dir::in, 1);
	node1.stats.add (ysu::stat::type::ledger, ysu::stat::dir::in, 5);
	node1.stats.inc (ysu::stat::type::ledger, ysu::stat::dir::in);
	node1.stats.inc (ysu::stat::type::ledger, ysu::stat::detail::send, ysu::stat::dir::in);
	node1.stats.inc (ysu::stat::type::ledger, ysu::stat::detail::send, ysu::stat::dir::in);
	node1.stats.inc (ysu::stat::type::ledger, ysu::stat::detail::receive, ysu::stat::dir::in);
	ASSERT_EQ (10, node1.stats.count (ysu::stat::type::ledger, ysu::stat::dir::in));
	ASSERT_EQ (2, node1.stats.count (ysu::stat::type::ledger, ysu::stat::detail::send, ysu::stat::dir::in));
	ASSERT_EQ (1, node1.stats.count (ysu::stat::type::ledger, ysu::stat::detail::receive, ysu::stat::dir::in));
	node1.stats.add (ysu::stat::type::ledger, ysu::stat::dir::in, 0);
	ASSERT_EQ (10, node1.stats.count (ysu::stat::type::ledger, ysu::stat::dir::in));
}
...
TEST (node, online_reps)
{
	...
	auto vote (std::make_shared<ysu::vote> ());
	node1.online_reps.observe (ysu::dev_genesis_key.pub);
	// 1 minimum, 1 maximum
	node1.online_reps.sample ();
	ASSERT_EQ (ysu::genesis_amount, node1.online_reps.online_stake ());
	// 2 minimum, 1 maximum
	node1.online_reps.sample ();
	ASSERT_EQ (node1.config.online_weight_minimum, node1.online_reps.online_stake ());
}
...
TEST (node, block_confirm)
{
	...
	for (auto & type : types)
	{
		...
		ysu::system system (2, type, node_flags);
		auto & node1 (*system.nodes[0]);
		auto & node2 (*system.nodes[1]);
		ysu::genesis genesis;
		ysu::keypair key;
		...
		auto send1_copy = builder.make_block ()
		                  ...
		                  .build_shared ();
		node1.block_processor.add (send1, ysu::seconds_since_epoch ());
		node2.block_processor.add (send1_copy, ysu::seconds_since_epoch ());
		...
		system.wallet (1)->insert_adhoc (ysu::dev_genesis_key.prv);
		auto send2 (std::make_shared<ysu::state_block> (ysu::dev_genesis_key.pub, send1->hash (), ysu::dev_genesis_key.pub, ysu::genesis_amount - ysu::Gxrb_ratio * 2, key.pub, ysu::dev_genesis_key.prv, ysu::dev_genesis_key.pub, *node1.work_generate_blocking (send1->hash ())));
		{
			auto transaction (node1.store.tx_begin_write ());
			ASSERT_EQ (ysu::process_result::progress, node1.ledger.process (transaction, *send2).code);
		}
		{
			...
			ASSERT_EQ (ysu::process_result::progress, node2.ledger.process (transaction, *send2).code);
		}
...
TEST (node, block_arrival_size)
{
	ysu::system system (1);
	auto & node (*system.nodes[0]);
	auto time (std::chrono::steady_clock::now () - ysu::block_arrival::arrival_time_min - std::chrono::seconds (5));
	ysu::block_hash hash (0);
	for (auto i (0); i < ysu::block_arrival::arrival_size_min * 2; ++i)
	{
		node.block_arrival.arrival.push_back (ysu::block_arrival_info{ time, hash });
		++hash.qwords[0];
	}
	ASSERT_EQ (ysu::block_arrival::arrival_size_min * 2, node.block_arrival.arrival.size ());
	node.block_arrival.recent (0);
	ASSERT_EQ (ysu::block_arrival::arrival_size_min, node.block_arrival.arrival.size ());
}
...
TEST (node, confirm_quorum)
{
	...
	ysu::amount new_balance (node1.config.online_weight_minimum.number () - ysu::Gxrb_ratio);
	auto send1 = ysu::state_block_builder ()
	             .account (ysu::dev_genesis_key.pub)
	             .previous (genesis.hash ())
	             .representative (ysu::dev_genesis_key.pub)
	             .balance (new_balance)
	             ...
	             .build_shared ();
	ASSERT_EQ (ysu::process_result::progress, node1.process (*send1).code);
	system.wallet (0)->send_action (ysu::dev_genesis_key.pub, ysu::dev_genesis_key.pub, new_balance.number ());
	ASSERT_TIMELY (10s, !node1.active.empty ());
	auto election (node1.active.election (ysu::qualified_root (send1->hash (), send1->hash ())));
	ASSERT_NE (nullptr, election);
	ASSERT_FALSE (election->confirmed ());
	ASSERT_EQ (1, election->votes ().size ());
...
TEST (node, local_votes_cache)
{
	...
	ysu::node_config node_config (ysu::get_available_port (), system.logging);
	node_config.frontiers_confirmation = ysu::frontiers_confirmation_mode::disabled;
	node_config.receive_minimum = ysu::genesis_amount;
	auto & node (*system.add_node (node_config));
	ysu::genesis genesis;
	ysu::state_block_builder builder;
	auto send1 = builder.make_block ()
	...
	auto send3 = builder.make_block ()
	             ...
	             .previous (send2->hash ())
	             .representative (ysu::dev_genesis_key.pub)
	             .balance (ysu::genesis_amount - 3 * ysu::Gxrb_ratio)
	             .link (ysu::dev_genesis_key.pub)
	             .sign (ysu::dev_genesis_key.prv, ysu::dev_genesis_key.pub)
	...
	system.wallet (0)->insert_adhoc (ysu::dev_genesis_key.prv);
	ysu::confirm_req message1 (send1);
	ysu::confirm_req message2 (send2);
	auto channel (node.network.udp_channels.create (node.network.endpoint ()));
	node.network.process_message (message1, channel);
	auto wait_vote_sequence = [&node, &system](unsigned sequence) {
		std::shared_ptr<ysu::vote> current_vote;
		system.deadline_set (5s);
		while (current_vote == nullptr || current_vote->sequence < sequence)
		{
			{
				ysu::lock_guard<std::mutex> lock (node.store.get_cache_mutex ());
				auto transaction (node.store.tx_begin_read ());
				current_vote = node.store.vote_current (transaction, ysu::dev_genesis_key.pub);
			}
			ASSERT_NO_ERROR (system.poll ());
		}
	...
	{
		ysu::lock_guard<std::mutex> lock (node.store.get_cache_mutex ());
		ASSERT_EQ (2, node.store.vote_current (node.store.tx_begin_read (), ysu::dev_genesis_key.pub)->sequence);
	}
	// Max cache
	{
		auto transaction (node.store.tx_begin_write ());
	...
	ysu::confirm_req message3 (send3);
	for (auto i (0); i < 100; ++i)
	{
...
TEST (node, local_votes_cache_batch)
{
	...
	auto send1 = ysu::state_block_builder ()
	             ...
	             .work (*node.work_generate_blocking (genesis.hash ()))
	             .build_shared ();
	std::vector<std::pair<ysu::block_hash, ysu::root>> batch{ { genesis.open->hash (), genesis.open->root () }, { send1->hash (), send1->root () } };
	{
		auto transaction (node.store.tx_begin_write ());
		ASSERT_EQ (ysu::process_result::progress, node.ledger.process (transaction, *send1).code);
	}
	ysu::confirm_req message (batch);
	auto channel (node.network.udp_channels.create (node.network.endpoint ()));
	// Generates and sends one vote for both hashes which is then cached
	node.network.process_message (message, channel);
	ASSERT_TIMELY (3s, node.stats.count (ysu::stat::type::message, ysu::stat::detail::confirm_ack, ysu::stat::dir::out) == 1);
	...
	node.network.process_message (message, channel);
	ASSERT_TIMELY (3s, node.stats.count (ysu::stat::type::message, ysu::stat::detail::confirm_ack, ysu::stat::dir::out) == 2);
	ASSERT_EQ (2, node.stats.count (ysu::stat::type::message, ysu::stat::detail::confirm_ack, ysu::stat::dir::out));
	// Test when votes are different
	node.history.erase (genesis.open->root ());
	node.history.erase (send1->root ());
	node.network.process_message (ysu::confirm_req (genesis.open->hash (), genesis.open->root ()), channel);
	ASSERT_TIMELY (3s, node.stats.count (ysu::stat::type::message, ysu::stat::detail::confirm_ack, ysu::stat::dir::out) == 3);
	ASSERT_EQ (3, node.stats.count (ysu::stat::type::message, ysu::stat::detail::confirm_ack, ysu::stat::dir::out));
	node.network.process_message (ysu::confirm_req (send1->hash (), send1->root ()), channel);
	ASSERT_TIMELY (3s, node.stats.count (ysu::stat::type::message, ysu::stat::detail::confirm_ack, ysu::stat::dir::out) == 4);
	ASSERT_EQ (4, node.stats.count (ysu::stat::type::message, ysu::stat::detail::confirm_ack, ysu::stat::dir::out));
	// There are two different votes, so both should be sent in response
	node.network.process_message (message, channel);
	ASSERT_TIMELY (3s, node.stats.count (ysu::stat::type::message, ysu::stat::detail::confirm_ack, ysu::stat::dir::out) == 6);
	ASSERT_EQ (6, node.stats.count (ysu::stat::type::message, ysu::stat::detail::confirm_ack, ysu::stat::dir::out));
}
...
TEST (node, local_votes_cache_generate_new_vote)
{
	...
	ysu::confirm_req message1 (genesis.open);
	auto channel (node.network.udp_channels.create (node.network.endpoint ()));
	node.network.process_message (message1, channel);
	...
	ASSERT_EQ (genesis.open->hash (), boost::get<ysu::block_hash> (votes1[0]->blocks[0]));
	{
		ysu::lock_guard<std::mutex> lock (node.store.get_cache_mutex ());
		auto transaction (node.store.tx_begin_read ());
		auto current_vote (node.store.vote_current (transaction, ysu::dev_genesis_key.pub));
	...
	std::vector<std::pair<ysu::block_hash, ysu::root>> roots_hashes{ std::make_pair (genesis.open->hash (), genesis.open->root ()), std::make_pair (send1->hash (), send1->root ()) };
	ysu::confirm_req message2 (roots_hashes);
	node.network.process_message (message2, channel);
	...
	ASSERT_TIMELY (3s, 3 == node.stats.count (ysu::stat::type::message, ysu::stat::detail::confirm_ack, ysu::stat::dir::out));
}
...
TEST (node, vote_republish)
{
	...
	auto send2 = builder.make_block ()
	             .previous (genesis.hash ())
	             .destination (key2.pub)
	             .balance (std::numeric_limits<ysu::uint128_t>::max () - node1.config.receive_minimum.number () * 2)
	             .sign (ysu::dev_genesis_key.prv, ysu::dev_genesis_key.pub)
	             .work (*system.work.generate (genesis.hash ()))
	             .build_shared ();
	node1.process_active (send1);
	...
	auto vote (std::make_shared<ysu::vote> (ysu::dev_genesis_key.pub, ysu::dev_genesis_key.prv, 0, send2));
	ASSERT_TRUE (node1.active.active (*send1));
	ASSERT_TRUE (node2.active.active (*send1));
	node1.vote_processor.vote (vote, std::make_shared<ysu::transport::channel_loopback> (node1));
...
TEST (node, vote_by_hash_bundle)
{
	...
	system.nodes[0]->observers.vote.add ([&max_hashes](std::shared_ptr<ysu::vote> vote_a, std::shared_ptr<ysu::transport::channel>, ysu::vote_code) {
	...
	for (int i = 1; i <= 200; i++)
	{
		system.nodes[0]->active.generator.add (ysu::genesis_account, ysu::genesis_hash);
	}
...
TEST (node, vote_by_hash_republish)
{
	...
	for (auto & type : types)
	{
		...
		auto vote (std::make_shared<ysu::vote> (ysu::dev_genesis_key.pub, ysu::dev_genesis_key.prv, 0, vote_blocks));
		ASSERT_TRUE (node1.active.active (*send1));
		ASSERT_TRUE (node2.active.active (*send1));
		node1.vote_processor.vote (vote, std::make_shared<ysu::transport::channel_loopback> (node1));
		ASSERT_TIMELY (10s, node1.block (send2->hash ()));
...
TEST (node, vote_by_hash_epoch_block_republish)
{
	...
	auto send1 = ysu::send_block_builder ()
	             ...
	             .work (*system.work.generate (genesis.hash ()))
	             .build_shared ();
	auto epoch1 = ysu::state_block_builder ()
...
TEST (node, epoch_conflict_confirm)
{
	...
	auto epoch_open = builder.make_block ()
	                  ...
	                  .representative (0)
	                  .balance (0)
	                  .link (node0->ledger.epoch_link (ysu::epoch::epoch_1))
	...
	{
		ysu::block_post_events events;
		auto transaction (node0->store.tx_begin_write ());
		ASSERT_EQ (ysu::process_result::progress, node0->block_processor.process_one (transaction, events, send).code);
		ASSERT_EQ (ysu::process_result::progress, node0->block_processor.process_one (transaction, events, send2).code);
		ASSERT_EQ (ysu::process_result::progress, node0->block_processor.process_one (transaction, events, open).code);
	}
	...
	ysu::blocks_confirm (*node0, { change, epoch_open });
	ASSERT_EQ (2, node0->active.size ());
	{
		ysu::lock_guard<std::mutex> lock (node0->active.mutex);
...
TEST (node, fork_invalid_block_signature)
{
	...
	auto send2 = builder.make_block ()
	             ...
	             .work (*system.work.generate (genesis.hash ()))
	             .build_shared ();
	auto send2_corrupt (std::make_shared<ysu::send_block> (*send2));
	send2_corrupt->signature = ysu::signature (123);
	auto vote (std::make_shared<ysu::vote> (ysu::dev_genesis_key.pub, ysu::dev_genesis_key.prv, 0, send2));
	auto vote_corrupt (std::make_shared<ysu::vote> (ysu::dev_genesis_key.pub, ysu::dev_genesis_key.prv, 0, send2_corrupt));
	...
	ASSERT_TIMELY (5s, node1.stats.count (ysu::stat::type::rollback, ysu::stat::detail::all));
...
TEST (node, fork_election_invalid_block_signature)
{
	...
	auto send3 = builder.state ()
	             ...
	             .link (ysu::dev_genesis_key.pub)
	             .work (*system.work.generate (genesis.hash ()))
	             .sign (ysu::dev_genesis_key.prv, 0) // Invalid signature
	             .build_shared ();
	auto channel1 (node1.network.udp_channels.create (node1.network.endpoint ()));
	node1.network.process_message (ysu::publish (send1), channel1);
	ASSERT_TIMELY (5s, node1.active.active (send1->qualified_root ()));
	auto election (node1.active.election (send1->qualified_root ()));
	ASSERT_NE (nullptr, election);
	ASSERT_EQ (1, election->blocks ().size ());
	node1.network.process_message (ysu::publish (send3), channel1);
	node1.network.process_message (ysu::publish (send2), channel1);
...
TEST (node, block_processor_signatures)
{
	...
	auto & node1 (*system0.nodes[0]);
	system0.wallet (0)->insert_adhoc (ysu::dev_genesis_key.prv);
	ysu::block_hash latest (system0.nodes[0]->latest (ysu::dev_genesis_key.pub));
	ysu::state_block_builder builder;
	ysu::keypair key1;
	ysu::keypair key2;
	ysu::keypair key3;
	...
	auto send4 = builder.make_block ()
	             ...
	             .previous (send3->hash ())
	             .representative (ysu::dev_genesis_key.pub)
	             .balance (ysu::genesis_amount - 4 * ysu::Gxrb_ratio)
	             .link (key3.pub)
	             .sign (ysu::dev_genesis_key.prv, ysu::dev_genesis_key.pub)
	...
	auto send5 = builder.make_block ()
	             ...
	             .previous (send3->hash ())
	             .representative (ysu::dev_genesis_key.pub)
	             .balance (ysu::genesis_amount - 5 * ysu::Gxrb_ratio)
	             .link (key3.pub)
	             .sign (ysu::dev_genesis_key.prv, ysu::dev_genesis_key.pub)
	             .work (*node1.work_generate_blocking (send3->hash ()))
	             .build_shared ();
...
TEST (node, block_processor_full)
{
	...
	auto & node = *system.add_node (ysu::node_config (ysu::get_available_port (), system.logging), node_flags);
	ysu::genesis genesis;
	ysu::state_block_builder builder;
	auto send1 = builder.make_block ()
	             .account (ysu::dev_genesis_key.pub)
...
TEST (node, confirm_back)
{
	...
	auto & node (*system.nodes[0]);
	ysu::genesis genesis;
	auto genesis_start_balance (node.balance (ysu::dev_genesis_key.pub));
	auto send1 = ysu::send_block_builder ()
	             .previous (genesis.hash ())
	             .destination (key.pub)
	...
	node.process_active (send2);
	ysu::blocks_confirm (node, { send1, open, send2 });
	ASSERT_EQ (3, node.active.size ());
	std::vector<ysu::block_hash> vote_blocks;
	vote_blocks.push_back (send2->hash ());
	auto vote (std::make_shared<ysu::vote> (ysu::dev_genesis_key.pub, ysu::dev_genesis_key.prv, 0, vote_blocks));
	node.vote_processor.vote_blocking (vote, std::make_shared<ysu::transport::channel_loopback> (node));
	ASSERT_TIMELY (10s, node.active.empty ());
}
...
TEST (node, peers)
{
	...
	ysu::endpoint_key endpoint_key{ endpoint.address ().to_v6 ().to_bytes (), endpoint.port () };
	auto & store = node2->store;
	{
		...
		store.peer_put (transaction, ysu::endpoint_key{ boost::asio::ip::address_v6::any ().to_bytes (), 55555 });
	}
...
TEST (node, peer_cache_restart)
{
	...
	auto path (ysu::unique_path ());
	{
		auto node2 (std::make_shared<ysu::node> (system.io_ctx, ysu::get_available_port (), path, system.alarm, system.logging, system.work));
		system.nodes.push_back (node2);
		auto & store = node2->store;
		{
			// Add a peer to the database
	...
	// Restart node
	{
		...
		auto node3 (std::make_shared<ysu::node> (system.io_ctx, ysu::get_available_port (), path, system.alarm, system.logging, system.work, node_flags));
...
TEST (node, unchecked_cleanup)
{
	...
	ysu::keypair key;
	auto & node (*system.add_node (node_flags));
	auto open = ysu::state_block_builder ()
	            .account (key.pub)
	            .previous (0)
	            .representative (key.pub)
	            .balance (1)
...
TEST (node, dont_write_lock_node)
{
	...
	std::thread ([&path, &write_lock_held_promise, &finished_promise]() {
		...
		auto store = ysu::make_store (logger, path, false, true);
		{
			ysu::genesis genesis;
	...
	ysu::inactive_node node (path, ysu::inactive_node_flag_defaults ());
...
TEST (node, aggressive_flooding)
{
	...
	wallet1.insert_adhoc (ysu::dev_genesis_key.prv);
	std::vector<std::pair<std::shared_ptr<ysu::node>, std::shared_ptr<ysu::wallet>>> nodes_wallets;
	bool const sanitizer_or_valgrind (is_sanitizer_build || ysu::running_within_valgrind ());
	...
	auto large_amount = (ysu::genesis_amount / 2) / nodes_wallets.size ();
	std::vector<std::shared_ptr<ysu::block>> genesis_blocks;
	for (auto & node_wallet : nodes_wallets)
	{
		...
		auto block (wallet1.send_action (ysu::dev_genesis_key.pub, keypair.pub, large_amount));
	...
	for (auto & node_wallet : nodes_wallets)
	{
		...
		ASSERT_EQ (node1.latest (ysu::dev_genesis_key.pub), node_wallet.first->latest (ysu::dev_genesis_key.pub));
	}
	...
	ysu::state_block_builder builder;
	std::shared_ptr<ysu::state_block> block{};
	{
		auto transaction (node1.store.tx_begin_read ());
		block = builder.make_block ()
		        .account (ysu::dev_genesis_key.pub)
		        .representative (ysu::dev_genesis_key.pub)
		        .previous (node1.ledger.latest (transaction, ysu::dev_genesis_key.pub))
		        .balance (node1.ledger.account_balance (transaction, ysu::dev_genesis_key.pub) - 1)
		        .link (ysu::dev_genesis_key.pub)
		        .sign (ysu::dev_genesis_key.prv, ysu::dev_genesis_key.pub)
		        .work (*node1.work_generate_blocking (node1.ledger.latest (transaction, ysu::dev_genesis_key.pub)))
		        .build ();
	}
	// Processing locally goes through the aggressive block flooding path
	node1.process_local (block, false);

	auto all_have_block = [&nodes_wallets](ysu::block_hash const & hash_a) {
	...
	auto wallet_block = wallet1.send_sync (ysu::dev_genesis_key.pub, ysu::dev_genesis_key.pub, 10);
...
TEST (node, max_work_generate_difficulty)
{
	...
	auto & node = *system.add_node (node_config);
	auto initial_difficulty = node.default_difficulty (ysu::work_version::work_1);
	ASSERT_EQ (node.max_work_generate_difficulty (ysu::work_version::work_1), ysu::difficulty::from_multiplier (node.config.max_work_generate_multiplier, initial_difficulty));
	...
	auto final_difficulty = node.default_difficulty (ysu::work_version::work_1);
	ASSERT_NE (final_difficulty, initial_difficulty);
	ASSERT_EQ (node.max_work_generate_difficulty (ysu::work_version::work_1), ysu::difficulty::from_multiplier (node.config.max_work_generate_multiplier, final_difficulty));
}
...
TEST (active_difficulty, recalculate_work)
{
	...
	auto send1 = ysu::send_block_builder ()
	             ...
	             .work (*system.work.generate (genesis.hash ()))
	             .build_shared ();
	auto multiplier1 = ysu::difficulty::to_multiplier (send1->difficulty (), node1.network_params.network.publish_thresholds.epoch_1);
	...
	ASSERT_EQ (node1.active.active_difficulty (), ysu::difficulty::from_multiplier (sum / node1.active.multipliers_cb.size (), node1.network_params.network.publish_thresholds.epoch_1));
...
TEST (node, rollback_vote_self)
{
	...
	auto send1 = builder.make_block ()
	             ...
	             .representative (ysu::dev_genesis_key.pub)
	             .link (key.pub)
	             .balance (ysu::genesis_amount - weight)
	             .sign (ysu::dev_genesis_key.prv, ysu::dev_genesis_key.pub)
	             .work (*system.work.generate (ysu::genesis_hash))
	             .build_shared ();
	auto open = builder.make_block ()
	...
	auto vote (votes.find (ysu::dev_genesis_key.pub));
...
TEST (node, dependency_graph)
{
	...
	auto key3_epoch = builder.make_block ()
	                  ...
	                  .link (node.ledger.epoch_link (ysu::epoch::epoch_1))
	                  .balance (key3_receive->balance ())
	                  .sign (ysu::dev_genesis_key.prv, ysu::dev_genesis_key.pub)
	                  .work (*system.work.generate (key3_receive->hash ()))
	                  .build ();

	ASSERT_EQ (ysu::process_result::progress, node.process (*gen_send1).code);
	ASSERT_EQ (ysu::process_result::progress, node.process (*key1_open).code);
	ASSERT_EQ (ysu::process_result::progress, node.process (*key1_send1).code);
	ASSERT_EQ (ysu::process_result::progress, node.process (*gen_receive).code);
	ASSERT_EQ (ysu::process_result::progress, node.process (*gen_send2).code);
	ASSERT_EQ (ysu::process_result::progress, node.process (*key2_open).code);
	ASSERT_EQ (ysu::process_result::progress, node.process (*key2_send1).code);
	ASSERT_EQ (ysu::process_result::progress, node.process (*key3_open).code);
	ASSERT_EQ (ysu::process_result::progress, node.process (*key2_send2).code);
	ASSERT_EQ (ysu::process_result::progress, node.process (*key1_receive).code);
	ASSERT_EQ (ysu::process_result::progress, node.process (*key1_send2).code);
	ASSERT_EQ (ysu::process_result::progress, node.process (*key3_receive).code);
	ASSERT_EQ (ysu::process_result::progress, node.process (*key3_epoch).code);
	ASSERT_TRUE (node.active.empty ());

	// Hash -> Ancestors
	std::unordered_map<ysu::block_hash, std::vector<ysu::block_hash>> dependency_graph{
...
TEST (node, dependency_graph_frontier)
{
	...
	config.peering_port = ysu::get_available_port ();
	config.frontiers_confirmation = ysu::frontiers_confirmation_mode::always;
	...
	{
		auto transaction (node->store.tx_begin_write ());
		ASSERT_EQ (ysu::process_result::progress, node->ledger.process (transaction, *gen_send1).code);
		ASSERT_EQ (ysu::process_result::progress, node->ledger.process (transaction, *key1_open).code);
		ASSERT_EQ (ysu::process_result::progress, node->ledger.process (transaction, *key1_send1).code);
		ASSERT_EQ (ysu::process_result::progress, node->ledger.process (transaction, *gen_receive).code);
		ASSERT_EQ (ysu::process_result::progress, node->ledger.process (transaction, *gen_send2).code);
		ASSERT_EQ (ysu::process_result::progress, node->ledger.process (transaction, *key2_open).code);
		ASSERT_EQ (ysu::process_result::progress, node->ledger.process (transaction, *key2_send1).code);
		ASSERT_EQ (ysu::process_result::progress, node->ledger.process (transaction, *key3_open).code);
		ASSERT_EQ (ysu::process_result::progress, node->ledger.process (transaction, *key2_send2).code);
		ASSERT_EQ (ysu::process_result::progress, node->ledger.process (transaction, *key1_receive).code);
		ASSERT_EQ (ysu::process_result::progress, node->ledger.process (transaction, *key1_send2).code);
		ASSERT_EQ (ysu::process_result::progress, node->ledger.process (transaction, *key3_receive).code);
		ASSERT_EQ (ysu::process_result::progress, node->ledger.process (transaction, *key3_epoch).code);
	}
...
TEST (node, deferred_dependent_elections)
{
	...
	ASSERT_EQ (ysu::process_result::fork, node.process (*fork).code);
	node.process_active (fork);
	node.block_processor.flush ();
	ASSERT_FALSE (node.active.active (receive->qualified_root ()));
...
TEST (node, default_difficulty)
{
	ysu::system system (1);
	auto & node (*system.nodes[0]);
	auto const & thresholds = ysu::network_params{}.network.publish_thresholds;
	ASSERT_EQ (thresholds.epoch_1, node.default_difficulty (ysu::work_version::work_1));
	ASSERT_EQ (thresholds.epoch_1, node.default_receive_difficulty (ysu::work_version::work_1));
	ysu::upgrade_epoch (system.work, node.ledger, ysu::epoch::epoch_1);
	ASSERT_EQ (thresholds.epoch_1, node.default_difficulty (ysu::work_version::work_1));
	ASSERT_EQ (thresholds.epoch_1, node.default_receive_difficulty (ysu::work_version::work_1));
	ysu::upgrade_epoch (system.work, node.ledger, ysu::epoch::epoch_2);
	ASSERT_EQ (thresholds.epoch_2, node.default_difficulty (ysu::work_version::work_1));
	ASSERT_EQ (thresholds.epoch_2_receive, node.default_receive_difficulty (ysu::work_version::work_1));
}
...
TEST (rep_crawler, recently_confirmed)
{
	...
	auto const block = ysu::genesis ().open;
	{
		ysu::lock_guard<std::mutex> guard (node1.active.mutex);
...
TEST (rep_crawler, local)
{
	...
	auto & node = *system.add_node (flags);
	auto loopback = std::make_shared<ysu::transport::channel_loopback> (node);
	auto vote = std::make_shared<ysu::vote> (ysu::dev_genesis_key.pub, ysu::dev_genesis_key.prv, 0, std::vector{ ysu::genesis_hash });
	{
		ysu::lock_guard<std::mutex> guard (node.rep_crawler.probable_reps_mutex);
		node.rep_crawler.active.insert (ysu::genesis_hash);
...
TEST (online_reps, backup)
{
	...
	ysu::online_reps online_reps (ledger, params, 0);
	ASSERT_EQ (0, online_reps.list ().size ());
	online_reps.observe (ysu::dev_genesis_key.pub);
	...
	ASSERT_EQ (ysu::genesis_amount, online_reps.online_stake ());
	online_reps.sample ();
	ASSERT_EQ (0, online_reps.list ().size ());
}
