...
{
	ysu::daemon_config daemon_config (data_path);
	...
	toml.write (ysu::get_node_toml_config_path (data_path));
	...
	// Write rpc config
	ysu::tomlconfig toml_rpc;
	rpc_config.serialize_toml (toml_rpc);
	toml_rpc.write (ysu::get_rpc_toml_config_path (data_path));
}
...
int main (int argc, char * const * argv)
{
	...
	description.add_options ()
		...
		("node_path", boost::program_options::value<std::string> (), "The path to the ysu_node to test")
		("rpc_path", boost::program_options::value<std::string> (), "The path to the ysu_rpc to test");
	// clang-format on
	...
	{
		auto node_filepath = running_executable_filepath.parent_path () / "ysu_node";
	...
	{
		std::cerr << "ysu_node executable could not be found in " << node_path << std::endl;
		return 1;
	}
	...
	{
		std::cerr << "ysu_rpc executable could not be found in " << rpc_path << std::endl;
		return 1;
	}
	...
	{
		auto data_path = ysu::unique_path ();
	...
	std::thread t ([send_count, &ioc, &primary_node_results, &resolver, &node_count, &destination_count]() {
		...
		wallet_add_rpc (ioc, primary_node_results, wallet, ysu::dev_genesis_key.prv.data.to_string ());
		...
		for (auto i = 0; i < send_count; ++i)
		{
			...
			boost::asio::spawn (ioc, [&ioc, &primary_node_results, &wallet, destination_account, &send_calls_remaining](boost::asio::yield_context yield) {
				send_receive (ioc, wallet, ysu::genesis_account.to_account (), destination_account->as_string, send_calls_remaining, primary_node_results, yield);
			});
		}
	...
	ysu::thread_runner runner (ioc, simultaneous_process_calls);
	t.join ();
	runner.join ();
