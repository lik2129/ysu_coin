...
void run (boost::filesystem::path const & data_path, std::vector<std::string> const & config_overrides)
{
	...
	auto error = ysu::read_rpc_config_toml (data_path, rpc_config, config_overrides);
	if (!error)
	{
		...
		try
		{
			ysu::ipc_rpc_processor ipc_rpc_processor (io_ctx, rpc_config);
			auto rpc = ysu::get_rpc (io_ctx, rpc_config, ipc_rpc_processor);
			rpc->start ();
			...
			runner = std::make_unique<ysu::thread_runner> (io_ctx, rpc_config.rpc_process.io_threads);
			runner->join ();
...
int main (int argc, char * const * argv)
{
	...
	description.add_options ()
		...
		("config", boost::program_options::value<std::vector<ysu::config_key_value_pair>>()->multitoken(), "Pass RPC configuration values. This takes precedence over any values in the configuration file. This option can be repeated multiple times.")
	...
	if (vm.count ("daemon") > 0)
	{
		...
		if (config != vm.end ())
		{
			config_overrides = ysu::config_overrides (config->second.as<std::vector<ysu::config_key_value_pair>> ());
		}
